...
ainsi que \cite{Nagel1990,Miquel-Incompl} pour les théorèmes
d'incomplétude.
...
formalismes de preuves par Gentzen (décrits dans \cite{Gentzen1935}) que sont
le calcul des séquents et la déduction naturelle. L'étude de ces deux
formalismes nous permet d'introduire le $\lambda$-calcul et d'intégrer la
correspondance de Curry-Howard. Enfin, nous étudions le premier problème central
de la théorie de la démonstration~: la cohérence de l'arithmétique, à travers
la méthode de Gentzen développée dans \cite{Gentzen1936} et à travers la
réalisabilité de Kleene \cite{Kleene1945}.
...
\item Le \cref{chp.sequents} introduit le calcul des séquents. Le chapitre
  ...
  l'éliminagion des coupures, en montrant le théorème du séquent du milieu et
  le théorème de Herbrand en conséquence. Le théorème d'élimination des coupures
  est présenté ensuite pour un calcul modal contenant de nombreux constructeurs,
  de sorte que tous les calculs des séquents d'utilité pour notre cours en sont
  des fragments.
  ...
  motiver plus efficacement la sémantique de Kripke, en en donnant la forme
  générale puis en démontrant la complétude de la logique modale $S4$ vis à vis
  des modèles de Kripke sur des pré-ordres. On montre enfin l'équivalence entre
  $\LJ$ et un fragment de $S4$ pour en déduire la complétude de la logique
...
\item Le \cref{chp.reecr} donne une introduction à la théorie abstraite de la
  ...
  le biais de l'algorithme de complétion de Knuth-Bendix.
...
\item Le \cref{chp.deduc.nat} s'intéresse à la déduction naturelle. Si ce
  ...
  On profite de l'introduction de ce modèle de calcul pour motiver la traduction
  de Gödel-Gentzen, permettant de faire un lien entre la logique intuitionniste
  et la logique classique par le biais des doubles négations.
...
\item Le \cref{chp.lambda} décrit l'un des formalismes de calcul centraux dans
  ...
  fonctionnement et des moyens de le formaliser correctement.
  On prouve ensuite que ce formalisme est équivalent à celui des machines de
  Turing, puis on démontre le bon comportement de la réduction des termes en
  montrant la propriété de Church-Rosser et en exhibant une stratégie
  normalisante à l'aide du lemme de standardisation.
...
\item Le \cref{chp.lambda_type} est dédié à la variante typée du
  ...
  On introduit dans ce chapitre différents systèmes de types~: le système de
  types simples, pour la logique minimale puis propositionnelle, un système de
  types dédié à refléter les arbres de la logique du premier ordre, et le
  système T de Gödel permettant d'écrire des fonctions arithmétiques
  efficacement. Enfin, on donne la méthode des témoins de réductibilité /
  ensembles saturés pour prouver la forte normalisation du système dans sa
  version typée.
...
\item le \cref{chp.coh.arith} termine la partie en étudiant différentes méthodes
  ...
  Gentzen de la cohérence de l'arithmétique, qu'on décrit en détails et dans un
  ...
  l'arithmétique~: l'arithmétique de Heyting. On donne alors la méthode de
  Kleene pour prouver la cohérence de l'arithmétique par le biais de la
  réalisabilité. Pour renforcer la pertinence de cette approche, on décrit
  l'astuce de Friedmann permettant de montrer que l'arithmétique de Heyting a la
  même force que l'arithmétique de Peano jusqu'aux formules $\HierArithP 2$.
  ...
  ordre (intuitionniste comme classique, grâce à l'astuce de Friedmann).
\end{itemize}
