...
\begin{definition}[Propriété de Henkin]
  ...
  de Henkin si pour toute formule $F$ telle que $\mathcal T\vdash\exists x, F$,
  il existe un terme $t_F\in\Term(\Sigma)$ tel que $\mathcal T\vdash P[t_F/x]$.
\end{definition}
...
formule $\exists x, F(x)$, un témoin $c_F$ ainsi qu'une formule dans notre
théorie disant que, dans le cas où $\exists x, F(x)$ est vérifiée, il vient
directement que $F(c_F)$ est vérifiée aussi. Cependant, comme on veut permettre
de considérer des formules non closes $F$, on remplace l'idée d'un témoin
constant $c_F$ par une fonction $f_F$ prenant en arguments les variables libres
de $F$. On ajoute aussi dans les paramètres le variable $x$ sur laquelle on
utilise la quantification existentielle.
...
\begin{definition}[Clôture par témoins]
  ...
  \item Si $\Sigma_n$ et $\mathcal T_n$ sont construits, on définit la signature
    ...
    $F \in \Formula(\Sigma_n)$, dont les variables libres seront considérées être
    $x_1,\ldots,x_n$ et chaque variable $x \in \Var$, on ajoute une fonction
    $f_{F,x}$ d'arité $n - 1$ si $x \in \VL(F)$ et $n$ sinon. On construit ensuite
    la théorie $\mathcal T_{n + 1}$ en ajoutant à $\mathcal T_n$, pour chaque
    formule $\exists x, F(x_1,\ldots,x_n,x)$, la formule
    \[\forall x_1,\ldots,x_n, (\exists x, F(x_1,\ldots,x_n,x)) \to
    F(x_1,\ldots,x_n,f_{F,x}(x_1,\ldots,x_n))\]
  \end{itemize}
  On définit alors
...
\begin{proof}
  ...
  qu'il existe $n\in \mathbb N$ tel que $F\in\Formula(\Sigma_n)$, et on pose
  $x_1,\ldots,x_n$ les variables libres de $F$. Cela signifie
  donc que
  \[\forall x_1,\ldots,x_n, (\exists x,F(x_1,\ldots,x_n,x))\to 
  F(x_1,\ldots,x_n,f_{F,x}(x_1,\ldots,x_n)) \in \mathcal T_{n+1}\]
  On en déduit, quitte à appliquer un nombre fini de fois $\forall_\mathrm e$
  pour substituer $\forall x_i, P$ par $P[x_i/x_i]$~:
  \begin{prooftree}
    \AxiomC{}
    \RightLabel{Ax}
    \UnaryInfC{$\hclose{\mathcal T}\vdash 
    (\exists x, F(x))\to F(f_{F,x}(x_1,\ldots,x_n))$}
    \AxiomC{$\hclose{\mathcal T}\vdash \exists x,F(x)$}
    \RightLabel{$\to_\mathrm e$}
    \BinaryInfC{$\hclose{\mathcal T}\vdash F(f_{F,x}(x_1,\ldots,x_n))$}
  \end{prooftree}
\end{proof}
...
\begin{proof}
  Comme $\forall x_1,\ldots,x_n,(\exists x,F)\to F(f_{F,x})$ appartient à
  $\mathcal S$, l'arbre de preuve précédent fonctionne encore en remplaçant
  $\hclose{\mathcal T}$ par $\mathcal S$.
\end{proof}
...
vérifiées par $\mathcal M_{\mathcal T'}$ sont exactement les résultats montrés par
la théorie $\mathcal T'$. Comme l'induction se fera sur la structure de la formule
vérifiée, il nous faut considérer des possibles variables libres. On définit donc
d'abord une notation pour définir un environnement sur $\mathcal M_{\mathcal T'}$.
...
\begin{lemma}
  ...
  $\overline\rho$ telle que $\VL(\varphi)\subseteq \dom(\rho)$, on a l'équivalence
  suivante~:
...
\begin{proof}
  ...
  $t \in \Term(\Sigma')$, on a $\overline{\rho(t)} = \overline \rho(t)$ (l'exercice
  est laissé au lecteur, mais il s'agit d'une induction qui ne présente aucune
  difficulté).
  ...
  \item si $\varphi = \exists x, \psi$, où $x \notin \VL(\rho)$; alors supposons
    que $\mathcal T'\vdash \exists x, \rho(\psi)$~: la propriété de Henkin sur
    $\mathcal T'$ nous donne un terme $t$ tel que
    $\mathcal T'\vdash \rho(\psi[t/x])$
    et donc un élément $\overline \rho(t) \in |\mathcal M_{\mathcal T'}|$ tel que
    $\mathcal M_{\mathcal T'}, \overline\rho[x\mapsto \overline \rho(t)] \models \psi$,
    donc
