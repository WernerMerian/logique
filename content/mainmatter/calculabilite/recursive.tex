...
\begin{definition}[Algorithme d'Earley \cite{EarleyAnalyser}]
  On considère un alphabet $\Sigma$, une grammaire $G = (\Gamma, \to, S)$
  sur $\Sigma$ et $u \in \Sigma^\star$. L'algorithme d'Earley consiste en la
  construction donnée à la \cref{fig.Earley}.
\end{definition}

\begin{figure}[h]
  \centering
  \begin{algorithm}[H]
    \KwData{un vecteur $T[\;]$ de taille $n$ contenant des ensembles d'items
      d'Earley, initialisé par $T[i] = \varnothing$ pour tout $i$\\
      un mot $u \in \Sigma^\star$ \\
      une grammaire $G = (\Gamma, \to, S)$}
    \KwResult{si $u \in \mathcal L_G$, la dérivation permettant de construire
      $u$ dans la grammaire $G$\\
      si $u \notin\mathcal L_G$, la constante $\bot$}
    \For{$S \to u$}{$T[0] \leftarrow (S\to \bullet u, 0)$}
    \For{$j = 1$ \KwTo $n$}{
      \For{A FAIRE}{A FAIRE}
    }
...
\begin{proposition}
  Soit $\Sigma,\Gamma$ deux alphabets et $G$ une grammaire vérifiant
  l'hypothèse de l'\cref{exo.auto.grammaire}. On suppose que pour toute règle
...
\begin{remark}
  ...
  cas pour un langage déterministe). Dans notre cas, cette version suffit pour
  prouver les propriétés essentielles sur les formules puisqu'elles sont
  engendrées par une grammaire dont l'automate associé est déterministe.
\end{remark}
