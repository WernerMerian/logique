...
\begin{theorem}[Point fixe de Kleene]\label{thm.Kleene.fix}
...
\begin{definition}[Hiérarchie arithmétique]\label{def.HA}
...
\begin{remark}
  Comme la situation est symétrique entre $\HierArithS{}$ et $\HierArithP{}$,
  on peut se contenter de considérer à chaque fois des problèmes unaires, et
  des quantifications unaires.
\end{remark}
...
\begin{exercise}
  Montrer que les classes $f$-calculables $f$-récursives sont égales.
\end{exercise}
...
précédents~: il existe une machine universelle pour les fonctions
...
\begin{definition}[Saut Turing]
  Soit $\degD \in \TuringDeg$. On définit le saut Turing
  $\degD' \in \TuringDeg$ par, pour tout $X \in \degD$~:
  \[\degD' \defeq \{Y \subseteq \bN \mid Y \TuringEq K^X\}\]

  On définit pour tout $n \in \bN$ sont saut de Turing itéré par~:
  \begin{itemize}
...
\begin{theorem}[Post]\label{thm.Post}
  ...
  \begin{itemize}
  \item $K^{(n)}$ est $\HierArithS n$-complet pour la réduction many-one.
  \item $\complement K^{(n)}$ est $\HierArithP n$-complet pour la réduction
    many-one.
