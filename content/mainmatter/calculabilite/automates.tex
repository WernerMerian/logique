...
\begin{proof}
  ...
  \item en considérant la fonction proposée précédemment, on voit d'abord que
    $\relev{f}(x) = f(x)$ pour $x\in \ABA$, et que si $\varwd=\varwv\star\varww$
...
\begin{definition}[Automate fini déterministe]
  ...
  \item $\trA : \varQQ\times \ABA \partialto \varQQ$ est une fonction partielle
...
\begin{definition}[Fonction de transition étendue]
  ...
  fonction $\trAS : \varQQ \times \toStA \partialto \varQQ$ définie par
...
\begin{definition}[Interprétation d'une expression rationnelle]
  On définit la fonction $\Val : \Regex(\ABA)\to\Ratio(\ABA)$ par induction~:
  \begin{itemize}
  \item $\Val(\vide) = \vide$
...
\begin{remark}
  ...
  $\Ewd$ des expressions rationnelles. Par exemple, $a + a$ a la même
...
\begin{proof}
  ...
  \mathcal A \models u \lor \mathcal A'\models u\qedhere\]
\end{proof}
