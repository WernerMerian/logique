...
l'essentiel, mais cela donne une vision assez restrictive des modèles de calcul
...
mais fondé sur des notions très proches.

Ce chapitre couvre donc les bases de l'étude des langages rationnels et des
...
automates et les expressions rationnelles, dont on prouve l'équivalence.
Nous étudions après cela les théorèmes de structure sur ces
langages~: le lemme de l'étoile et le théorème de Myhill-Nérode, ainsi que les
conséquences de ce dernier théorème. Nous traitons en dernier le cas de
l'équivalence des langages rationnels avec les langages reconnus par des
monoïdes, pour ainsi revenir sur un aspect plus algébrique, proche du début du
chapitre.
...
qu'est un mot et un langage, avant d'aborder l'aspect théorique de l'étude des
monoïdes.
...
\begin{notation}
  ...
  de $\varla$ dans $\varwd$.
  \index[notation]{Longueur!d'un Mot!$\wul$}
  \index[notation]{Longueur!d'un Mot!$\wul_{\varla}$}
  \index[notation]{$\varwd_i$}
\end{notation}
...
\begin{definition}[Extension sur les langages]
  Soit $\ABA$ un alphabet et $\varOpe$ une opération binaire sur $\toStA$. On
  étend l'opération $\varOpe$ sur les langages sur $\ABA$ en posant, pour tous
...
\begin{definition}[Concaténation]
  ...
  Pour tout $n\in \bN$, on notera $\varwd^n$ l'itération de l'opération
  $\wdPt$~:
  \[\begin{cases}
  \varwd^0 \defeq \Ewd \\
  \forall n \in \bN, \varwd^{n+1} = \varwd^n \wdPt \varwd
  \end{cases}\]
  \index[notion]{Concaténation}
  \index[notation]{$\varwd\wdPt\varwv$}
\end{definition}
...
\begin{definition}[Morphisme de monoïde]
  ...
  \begin{equation*}
    f(\neuMM) = \neuMN
  \end{equation*}
  \index[notion]{Morphisme!de Monoïde}
\end{definition}
...
\begin{theorem}[Propriété universelle du monoïde libre]\label{thm.PU.monoid}
  ...
  \index[notion]{Propriété!Universelle!du Monoïde libre}
\end{theorem}
...
\begin{definition}
  ...
  \index[notion]{Langage!Induit!par un Monoïde}
\end{definition}
...
Le formalisme derrière cette définition est lourd, mais nécessaire, nous donnons
...
\begin{definition}[Automate fini déterministe]
  ...
  \index[notion]{Automate!Fini!Déterministe}
  \index[notion]{Fonction!de Transition}
  \index[notation]{$auto(\ABA)$}
\end{definition}

Le processus de lecture décrit plus haut, en reprenant les notations
introduites, consiste à prendre un mot $\varwd = \varwd_0\ldots \varwd_n$, à
calculer $\trA(\trA(\cdots\trA(\varqZ,\varwd_0)\cdots),\varwd_n)$, puis à
vérifier que cette valeur appartient à $\varAF$.
...
\begin{definition}[Langage accepté par un automate]
  ...
  \index[notion]{Langage!Accepté!par un Automate}
  \index[notation]{$\varAuA\satisf\varwd$}
\end{definition}
...
est de dessiner un graphe dont les sommets sont les états (l'ensemble $\varQQ$),
...
\begin{definition}[Langage reconnaissable]
  Soit $\ABA$ un alphabet. On appelle classe des langages reconnaissables sur
  $\ABA$ l'ensemble des langages reconnus par les automates sur $\ABA$~:
  \[\Reco(\ABA) \defeq \compr{\varLL_{\varAuA}}{\varAuA\in\auto(\ABA)}\]
  \index[notion]{Langage!Reconnaissable}
  \index[notation]{$\Reco(\ABA)$}
\end{definition}
...
\begin{definition}[Langage rationnel]
  ...
  \index[notion]{Langage!Rationnel}
  \index[notation]{$\Ratio(\ABA)$}
\end{definition}
...
\begin{definition}[Expression rationnelle]
  ...
  \index[notion]{Expression!Rationnelle}
\end{definition}
...
plus, on ajoute $\Ewd$ dans nos expressions rationnelles, par souci de
lisibilité.
...
\begin{definition}[Automate union]
  ...
  \begin{itemize}
    ...
    \end{minipage}
  \end{itemize}
  \index[notion]{Automate!Union}
\end{definition}

\begin{proposition}\label{prop.union.ratio}
  Soient deux automates $\varAuA, \varAuB$ sur un alphabet $\ABA$ donné. On a
  l'égalité
...
\begin{exercise}[Automate produit]
  ...
  Montrer que cet automate reconnaît $\varLL_{\varAuA} \cap \varLL_{\varAuB}$, en
...
\begin{definition}[Automate fini non déterministe]
  ...
  \end{itemize}
  \index[notion]{Automate!Fini!non Déterministe}
\end{definition}
...
\begin{definition}[Automate des parties]
  ...
  \begin{itemize}
    ...
    \end{minipage}
  \end{itemize}
  \index[notion]{Automate!des Parties}
\end{definition}
...
\begin{proof}
  ...
  (où le deuxième terme est une image directe).
  \begin{itemize}
...
\begin{definition}[Automate avec $\Ewd$-transitions]
  ...
  \index[notion]{Automate!avec $\Ewd$-Transitions}
\end{definition}
...
\begin{definition}[\'Elimination des $\Ewd$-transitions]
  ...
  \end{itemize}
  \index[notion]{\'Elimination!des $\Ewd$-Transitions}
\end{definition}
...
la génération d'une expression rationnelle à partir d'un automate, nous allons
...
\begin{definition}[Automate normalisé]
  ...
  \index[notion]{Automate!Normalisé}
\end{definition}
...
contient pas $\Ewd$, si l'on veut renforcer le résultat en donnant un automate
...
algorithme qui construit une expression rationnelle pour un automate. On
remarque que les $\Ewd$-transitions sont parfaitement adaptées dans ce contexte,
puisque $\Val(\Ewd e) = \Val(e)$, comme on l'attend d'une $\Ewd$-transition.
...
\begin{definition}[Automate fini généralisé]
  Soit $\ABA$ un alphabet, on appelle automate fini généralisé un quadruplet
  $(\varQQ,\varQZ,\trA,\varAF)$ tel que~:
  \begin{itemize}
  ...
  \item $\varAF\subseteq\varQQ$ est l'ensemble des états terminaux.
  \end{itemize}
  \index[notion]{Automate!Fini!Généralisé}
\end{definition}
...
\begin{proof}
  ...
  On en déduit donc que pour tout mot $\varwd \in \toStA$, on a l'équivalence
  suivante~:
...
\begin{theorem}[Kleene \cite{Kleene1956}]
  Soit $\ABA$ un alphabet. Les classes $\Ratio(\ABA)$ et $\Reco(\ABA)$ sont
  égales.
  \index[notion]{Théorème!de Kleene}
\end{theorem}
...
\begin{lemma}[de l'étoile]
  ...
  \end{cases}\]
  \index[notion]{Lemme!de l'\'Etoile}
\end{lemma}
...
\begin{theorem}[Myhill-Nérode \cite{Nerode1958}]
  ...
  est fini.
  \index[notion]{Théorème!de Myhill-Nérode}
\end{theorem}
...
qu'un langage n'est pas rationnel~: en exhibant une infinité de résiduels, on en
...
\begin{exercise}
  ...
  \[\varLL \defeq \compr{a^nb^mc^m}{(n,m)\in (\bN^*)^2}\cup
  \compr{b^nc^m}{(n,m)\in \bN^2}\]
...
\begin{definition}[Morphisme d'automate]
  Soit $\ABA$ et $\varAuA$, $\varAuB$ deux automates finis déterministes sur
  ...
  \end{itemize}
  \index[notion]{Morphisme!d'Automate}
\end{definition}
...
\begin{definition}[Congruence d'automate]
  ...
  \end{itemize}
  \index[notion]{Congruence!d'Automate}
\end{definition}
...
\begin{definition}[Congruence de Nérode \cite{Nerode1958}]
  ...
  \index[notion]{Congruence!de Nérode}
\end{definition}
...
\begin{proposition}
  Soit $\ABA$ un alphabet, $\varMMo$ un monoïde fini et $\varLL$ un langage
  reconnu par une fonction $\makeFunName{h}{\ABA}{\varMMo}$ et
  $S\subseteq \varMMo$. Alors $\varLL \in \Reco(\ABA)$.
\end{proposition}
...
\begin{definition}[Monoïde des transitions]
  ...
  \end{itemize}
  \index[notion]{Monoïde!des Transitions}
\end{definition}
...
d'importance sont plus précisément les relations telles que pour tout couple
