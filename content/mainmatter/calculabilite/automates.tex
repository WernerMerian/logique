...
automates et les expressions rationnelles, ont on prouve leur équivalence.
...
d'ensemble de mots. Tout d'abord, nous donnons une descriptoin concrète de ce
qu'est un mot et un langage, avant d'aborder l'aspect théoriques de l'étude des
monoïdes.

\subsection{Premières définitions}

Définissons donc la notion d'alphabet. Intuitivement, l'alphabet
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[node distance = 2cm, on grid, auto,
    ...
    \node[state, initial, color = red] (q_0) {$q_0$};
    \node[state,accepting] (q_1) [right = of q_0] {$q_1$};
    \node[state] (q_2) [above right = of q_1] {$q_2$};
    \node[state, accepting] (q_3) [below right = of q_1] {$q_3$};
    \path[->,>=latex]
    ...
    \node (a) [right = of q_1] {};
    \node (b) [right = of a] {};
    \path[|->, >=latex] (a) edge node {$\delta(0,q_0)$} (b);

    \node[state, initial] (p_0) [right = of b] {$q_0$};
    \node[state,accepting, color = red] (p_1) [right = of p_0] {$q_1$};
    \node[state] (p_2) [above right = of p_1] {$q_2$};
    \node[state, accepting] (p_3) [below right = of p_1] {$q_3$};
    \path[->,>=latex]
    ...
    \node (aa) [right = of p_1] {};
    \node (bb) [right = of aa] {};
    \path[|->, >=latex] (aa) edge node {$\delta(0,q_1)$} (bb);

    \node[state, initial] (r_0) [ right = of bb] {$q_0$};
    \node[state,accepting] (r_1) [right = of r_0] {$q_1$};
    \node[state] (r_2) [above right = of r_1] {$q_2$};
    \node[state, accepting, color = red] (r_3) [below right = of r_1] {$q_3$};
    \path[->,>=latex]
    ...
    (r_3) edge node {1} (r_0);
  \end{tikzpicture}
  \caption{Lecture du mot $00$ par l'automate précédent}
  \label{fig.ex.reco}
\end{figure}
...
\begin{definition}[\'Elimination des $\varepsilon$-transitions]
  ...
  \item $\displaystyle\delta_{\mathcal A_{\not\varepsilon}}(q,a) \defeq
    \bigcup_{\genfrac{}{}{0pt}{1}{q' \in Q}{q\to^\star q'}} \delta_{\mathcal A}(q',a)$
...
\begin{proof}
  ...
  \[\sum_{\genfrac{}{}{0pt}{1}{e \in \Regex(\Sigma)}{R_\delta(q,e,q')}} e\]
...
\begin{remark}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance = 3cm, on grid, auto]
      ...
      \node[state] (q_1) [above = of q_0] {$q_0$};
      \node[state] (q_2) [right = of q_0] {$q_1$};
      \node[state,accepting] (q_3) [right = of q_1] {$q_f$};
      \node[state] (q_p) [below left = of q_0] {$q_p$};
      \path[->,>=latex]
      (q_0) edge node {$0$} (q_1)
      (q_0) edge node {$1$} (q_2)
      (q_1) edge node {$0$} (q_3)
