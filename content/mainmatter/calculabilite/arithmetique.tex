...
un arbre de preuve bien formé dont la conclusion est $\Gamma\vdash \varphi$ où
...
\begin{proof}
  ...
  $K \manyOne\Th(\mathcal T)$, donc $\Th(\mathcal T)$ est
...
\begin{proof}
  ...
  Par $\Sigma$-cohérence, on en déduit que $\lnot G$ n'est pas démontrable dans
  $\mathcal T$.
\end{proof}
...
L'énoncé $G'$ est donc une variante de l'énoncé $G$, dans lequel on ajoute une
...
seulement que la formule $\varphi_f$ représentant la fonction RP $f$ est une
...
théorie $\Sigma$-complète $\mathcal T$, mais il n'est pas possible de montrer
\[\begin{cases} \mathcal T \vdash \forall x, x^0 = 1 \\
\mathcal T \vdash \forall x\,y,x^{\Ss y} = x \times x^y\end{cases}\]
...
$f$ représentée par une formule $\varphi$, il est prouvable que $\varphi$ est
...
\item Si $f_1,\ldots,f_k$ sont représentées par des relations fonctionnelles
  ...
  $\varphi_1,\ldots,\varphi_k$ et $\psi$, alors la représentation de
  $g\circ(f_1,\ldots,f_k)$, donnée par la formule
  \[\exists y_1,\ldots,y_k,
  \left(\bigwedge_{i = 1}^k \varphi_i(\vec x, y_i)\right) \land
  \psi(y_1,\ldots,y_k,z)\]
  en $(\vec x, z)$, est aussi fonctionnelle totale. En effet, la totalité des
  $\varphi_i$ donne l'existence de chaque $y_i$, et la totalité de $\psi$
  donne alors l'existence d'un $z$ associé à $\vec x$. De plus, si
  $z$ et $z'$ sont associés à $\vec x$, alors ils vérifient
  $\psi(y_1,\ldots,y_k,z)$ et $\psi(y_1,\ldots,y_k,z')$ avec les mêmes
  $y_i$ (puisque chaque $\varphi_i$ est une relation fonctionnelle), mais
  alors $z = z'$ puisque $\psi$ est elle-même une relation fonctionnelle.
\item De façon peut surprenante, c'est donc le cas récursif qui n'est pas
  automatique à prouver. Rappelons-nous que la représentation de $\rec(f,g)$ en
  fonction des représentations $\varphi, \psi$ respectivement de $f$ et $g$ est
  donnée, pour des variables $\vec x, y$, par le fait qu'il existe une suite
  finie (codée par la fonction $\beta$) telle que sa première valeur est l'image
...
\begin{theorem}[Extension conservative des fonctions RP]
  ...
  Soit $\mathcal T$ une théorie $\Sigma$-complète prouvant les
  \cref{thm.chinois} et \cref{lem.beta} et vérifiant le shéma de récurrence
  $\HierArithS 0$~:
  \[\mathcal T \vdash \varphi(0) \implies (\forall n, \varphi(n) \implies
  \varphi(\Ss n)) \implies \forall n, \varphi(n)\]
  pour toute formule $\varphi$ (potentiellement à paramètres) $\HierArithS 0$.
...
\begin{proof}
  Comme décrit précédemment, on procède par induction sur la structure des
  fonctions RP, et le seul cas problématique est le cas des définitions par
  récursion primitive.
...
\begin{theorem}[Représentation forte des théories arithmétiques]
  Pour toute théorie arithmétique $\mathcal T$ contenant (ou prouvant les
  axiomes de) $\ISigma$, l'ajout pour chaque fonction RP $f : \bN^k \to \bN$
  d'un symbole de fonction $k$-aire correspondant, et de l'axiome
  \[\forall \vec x, \varphi_f(\vec x,f(\vec x))\]
  est une extension conservative de $\mathcal T$.
\end{theorem}

\begin{proof}
  On applique simplement le \cref{prop.ext.def.cons} d'extension par définition
  au cas des formules $\varphi_f$ dont on sait qu'elles sont fonctionnelles
...
une théorie suffisamment puissante pouvant prouver internement le bon
comportement des fonctions calculables, il lui est alors possible de prouver
elle-même que cet argument diagonal fonctionne. On assiste donc à une
...
\begin{proof}
  ...
  représentant les triplets $(\varphi,\psi,\varphi \implies \psi)$, on
  souhaite prouver
...
%ces deux énoncés sont techniques (et peu intéressants) à prouver. Il
...
Si nous évitons de donner la preuve explicite que $\ArithPeano$ prouve les
...
théorème de $\ArithPeano$ sur les codes d'objets syntaxiques. Puisque toutes
les fonctions RP sont représentables par des symboles de fonctions, et qu'il est
...
\item le premier théorème d'incomplétude nous permet d'exhiber une formule
  $G$ dont on sait qu'elle n'est pas prouvable~;
\item en internalisant ce fait, on en déduit que
  $\square G \implies \square \bot$, mais la cohérence signifie que
  $\bot$ n'est pas prouvable (\latinexpr{i.e.}
  $\Coher(\mathcal T) \triangleq \lnot\square \bot$), donc on a
  \[\Coher(\mathcal T) \implies \lnot\square G\]
  par contraposée.
  Or, $\lnot\square G$ correspond à $G$, d'où le fait que
  $\Coher(\mathcal T)$ implique un énoncé indémontrable~: c'est un énoncé
...
$\varphi$ telle que $\square \varphi \implies \varphi$ est démontrable,
$\varphi$ est 
...
%Finalement, plutôt que de démontrer directement le second théorème
%d'incomplétude, on se
%propose d'en donner d'abord une version plus générale (dont la démonstration
%est
%presque identique). Cette version générale énonce que pour prouver qu'un
%énoncé est vrai, on peut supposer cet énoncé prouvable. De façon alternative,
%supposer qu'un énoncé est prouvable ne nous aide pas à le démontrer (puisqu'il
%est toujours possible d'éliminer cette hypothèse par élimination de
%l'implication).
...
\begin{remark}
  Le fait que $\mathcal T$ est r.a. est nécessaire dans la démonstration
  ci-dessus~: l'énoncé $\Th(\encode{\godcod\psi})$ est représentable pour cette
  raison, et la construction du point fixe nécessite la représentation de cet
  énoncé.
\end{remark}
...
%\begin{remark}
%  Essayons de détailler le contenu du théorème précédent~:
%  \begin{itemize}
%  \item on commence par montrer que l'arithmétique peut s'internaliser
%    elle-même, dans le sens où elle vérifie les conditions de Hilbert-Bernays~;
%  \item le premier théorème d'incomplétude nous permet d'exhiber une formule
...
%  \item en internalisant ce fait, on en déduit que
%    $\square G \implies \square \bot$, mais la cohérence signifie que
%    $\square \bot$ n'est pas prouvable, donc on a
%    \[\Coher(\mathcal T) \implies \lnot\square G\]
%    sauf que $\lnot\square G$ correspond à $G$, d'où le fait que
%    $\Coher(\mathcal T)$ implique un énoncé indémontrable~: c'est un énoncé
%    lui-même indémontrable.
%  \end{itemize}
%\end{remark}
...
\begin{definition}[Traduction de signature]
  Soient $\Sigma,\Sigma'$ deux signatures du premier ordre. On appelle
  traduction de $\Sigma$ vers $\Sigma'$ la donnée~:
  \begin{itemize}
  \item d'une formule à une variable libre $\Psi$~;
  \item pour chaque symbole de fonction $f$ d'arité $n$ dans $\Sigma$, d'un
    terme $t_f$ sur la signature $\Sigma'$ contenant (au plus) $n$ variables
    libres~;
  \item pour chaque symbole de relation $R$ d'arité $n$ dans $\Sigma$, d'une
    formule $\varphi_R$ sur la signature $\Sigma'$ contenant (au plus) $n$
    variables libres.
  \end{itemize}

  On étend naturellement une traduction de signature sur tous les termes
  $t \in \Term(\Sigma)$ et toutes les formules $\varphi \in \Formula(\Sigma)$
  pour obtenir $T(t) \in \Term(\Sigma')$ et $T(\varphi)\in\Formula(\Sigma')$~:
  \begin{itemize}
  \item la traduction d'une variable $x$ est $x$ lui-même
  \item la traduction de $f(t_1,\ldots,t_n)$ est $t_f(T(t_1),\ldots,T(t_n))$
    pour tout symbole de fonction $f$ d'arité $n$ dans $\Sigma$
  \item la traduction de $R(t_1,\ldots,t_n)$ est $\varphi(T(t_1),\ldots,T(t_n))$
    pour tout symbole de relation $R$ d'arité $n$ dans $\Sigma$ (l'égalité y
    compris)
  \item la traduction de $\top$ est $\top$ et celle de $\bot$ est $\bot$
  \item la traduction de $\lnot \varphi$ est $\lnot T(\varphi)$
  \item la traduction de $\varphi \oplus \psi$ est
    $T(\varphi)\oplus T(\psi)$ pour les connecteurs binaires
    $\oplus\in \{\lor,\land,\to\}$
  \item la traduction de $\forall x, \varphi$ est
    $\forall x, \Psi(x) \to T(\varphi)$
  \item la traduction de $\exists x, \varphi$ est
    $\exists x, \Psi(x) \land T(\varphi)$
  \end{itemize}
\end{definition}
...
\begin{theorem}
  Soient $\Sigma,\Sigma'$ deux signatures du premier ordre,
  $\Gamma \in \List(\Formula(\Sigma'))$ et $\varphi \in \Formula(\Sigma')$.
  On suppose de plus que pour chaque symbole de fonction $f$ d'arité $n$, on
  peut prouver
  \[\Psi(x_1),\ldots,\Psi(x_n) \vdash \Psi(f(x_1),\ldots,f(x_n))\]
  Alors on a, en notant $\VL(\Gamma,\varphi) = \{x_1,\ldots,x_k\}$~:
  \[\Gamma\vdash \varphi \implies
  \Psi(x_1),\ldots,\Psi(x_k),T(\Gamma)\vdash T(\varphi)\]
\end{theorem}
...
\begin{remark}
  ...
  \begin{itemize}
  \item la stabilité de $\Psi$ par $f$ peut être vérifiée seulement dans un
    contexte logique $\Delta\in\List(\Formula(\Sigma'))$ fixé, auquel cas
    l'implication devient
    \[\Gamma\vdash \varphi \implies
    \Psi(x_1),\ldots,\Psi(x_k),T(\Gamma),\Delta \vdash T(\varphi)\]
...
\begin{notation}[Prouvabilité modulo traduction]
  Soient $\Sigma,\Sigma'$ deux signatures et $T$ une traduction de $\Sigma$
  vers $\Sigma'$. Pour tout $\Gamma \in \List(\Formula(\Sigma'))$ et
  $\varphi \in \Formula(\Sigma)$, on note
  \[\Gamma \vdash_T \varphi\]
  pour $\Gamma\vdash T(\varphi)$. De même, on note
  \[\mathcal T \vdash_T \varphi\qquad \text{et}\qquad
  \mathcal T \vdash_T \mathcal T'\]
  pour $\mathcal T \in \powerset(\Formula(\Sigma'))$ et
  $\mathcal T' \in \powerset(\Formula(\Sigma))$
  pour définir respectivement
  \[\exists \Gamma\subfin \mathcal T, \Gamma\vdash_T \varphi
  \qquad\text{et}\qquad
  \forall \varphi \in \mathcal T', \mathcal T \vdash_T \varphi\]
\end{notation}

\begin{corollary}
  Avec les notations précédentes, si $\mathcal T\vdash_T \mathcal T'$ alors
  pour tout théorème $\varphi$ de $\mathcal T'$, $\mathcal T\vdash_T \varphi$.
\end{corollary}

Avec ces traductions, il est désormais possible d'étendre les
...
\begin{theorem}[Premier théorème d'incomplétude]\label{thm.Godel1.ZFC}
  Soit $\mathcal T$ une théorie cohérente et r.a. sur une signature $\Sigma$ et
  $T$ une traduction de $\mathcal L_{\mathrm{arith}}$ vers $\Sigma$ telle que
  $\mathcal T\vdash_T \ArithRobin^+$ (où $\ArithRobin^+$ est l'arithmétique
  de Robinson augmentée du schéma énonçant que la comparaison avec les
  entiers standards est totale), alors $\mathcal T$ est une théorie incomplète~:
  il existe un énoncé $R \in \HierArithP 1$ tel que
  $\mathcal T \nvdash_T R$ et $\mathcal T\nvdash_T \lnot R$.
\end{theorem}

\begin{theorem}[Second théorème d'incomplétude]\label{thm.Godel2.ZFC}
  Soit $\mathcal T$ une théorie cohérente et r.a. sur une signature $\Sigma$ et
  $T$ une traduction de $\mathcal L_{\mathrm{arith}}$ vers $\Sigma$ telle que
  $\mathcal T \vdash_T \ISigma$, alors $\mathcal T\nvdash_T \Coher(\mathcal T)$.
\end{theorem}

\begin{theorem}[Indéfinissabilité de la vérité]\label{thm.Tarski.ZFC}
  Soit $\mathcal T$ une théorie cohérente sur une signature $\Sigma$ et $T$ une
  traduction de $\mathcal L_{\mathrm{arith}}$ vers $\Sigma$, alors il n'existe
  aucune formule $V(x)$ a une variable libre telle que
  \[\forall \varphi \in \Formula(\Sigma),
  \mathcal T\vdash \varphi \iff V(T(\encode{\godcod{\varphi}}))\]
\end{theorem}

\begin{exercise}
  \'Ecrire une traduction $T$ du langage de l'arithmétique vers le langage de la
  théorie des ensembles. Montrer que
  \[\Zermelo - \AxInf \vdash_T \ArithPeano\]
