...
\begin{proof}
  ...
  en $S$.
  On peut de plus remarquer que $M^\star = \{\varepsilon\} + MM^\star$, ce qui
  motive à
  introduire une nouvelle matrice $Y$ avec les règles $Y \to I + RY$ (où $I$
  ...
  \begin{align*}
    ...
    Y_{i,i} &\to' \varepsilon
  \end{align*}
  Cette grammaire engendre le même langage que $G$. En effet, il nous suffit de
  montrer que chaque dérivation pour $\to$ depuis un $X_i$ et arrivant en un mot
  ...
  des dérivations pour $\to'$. Si $u \to_{i,j} v$, alors on remplace les
  occurrences de $X_i$ dans $u$ par des occurrences de $Y_{i,j}$. On prouve alors
  que la transformation effectuée donne une dérivation pour $\to'$, par
  induction sur la longueur de la dérivation. Supposons que
  $u \to v \to^\star w$ et montrons que $u' {\to'}^\star w'$ (où
  $u'$ et $w'$ respectivement sont les mots $u$ et $w$ après transformation)~:
  \begin{itemize}
  \item si $u \to_i v$, alors $u' = u$ et $u \to' v Y_{i,i} \to' v$, d'où le
    résultat par hypothèse d'induction.
  \item si $u \to_{i,j} v$, alors on peut réécrire $v = X_j v_0$ et on a
    $u' \to' Y_{i,j} v_0$, mais alors $Y_{i,j} (v_0)' = v'$ et, par hypothèse
    d'induction, $v' {\to'}^\star w'$, donc $u' {\to'}^\star w'$.
  \end{itemize}
  On remarque qu'il est possible d'effectuer la transformation réciproque, et
  que la seule différence entre un mot $u$ et sa transformée $u'$ se trouve dans
  les symboles non terminaux~: en particulier, si $u \in \Sigma^\star$, alors
  $X_i \to^\star u$ si et
  seulement si $X_i {\to'}^\star u$. Les deux grammaires sont donc équivalentes.

  Finalement, on crée une nouvelle grammaire $G''$ en faisant la modification
  suivante~: pour chaque règle $Y_{i,j} \to' uY_{j,k}$ où $u \in M_{i,j}$, si $u$
  commence par le non terminal $X_p$, alors on remplace la règle par l'ensemble
  de règles $Y_{i,j} \to' S_p(X_p^{-1}u)Y_{j,k}$. Cela ne change pas la grammaire
  engendrée, puisque chaque règle modifiée ici correspond à une suite de deux
  règles de dérivation $\to' X_pu' \to' S_pu'$ dans l'ancienne grammaire.

  En éliminant les $\varepsilon$-transitions dans la grammaire $G''$, on obtient
...
On définit la hauteur d'un arbre comme la longueur de la plus grande suite
allant de la racine à une feuille de l'arbre (la hauteur d'un arbre limité à une
racine étant $0$). Il est alors possible de définir et raisonner par récurrence
sur la hauteur de l'arbre.
...
\begin{remark}
  ...
  Nous ne détaillons pas la construction formelle qu'il faudrait effectuer
  pour obtenir une telle famille. L'ensemble des arbres de dérivation depuis
  une lettre $A$ est alors $X_A$, et $X_S$ est l'ensemble des arbres de
...
\begin{definition}[Codage de Gödel - version 1 \cite{Godel1931-GDEBFU}]
...
\begin{notation}
  ...
  dont on ne spécifie pas les composantes a pour ensemble d'état $Q$, pour état
...
\begin{definition}[Trace de l'exécution d'un automate à pile]
  Soit $\Sigma$ un alphabet et $\mathcal A$ un automate à pile sur $\Sigma$.
  On appelle trace sur $\mathcal A$ une suite finie $(C_p)$ de
  configurations telle que, pour tout indice $p$ où $C_p$ et $C_{p+1}$ sont
  définies, en notant $C_p = (q,u)$ et $C_{p+1} = (q',v)$, on peut trouver deux
  lettres $\alpha,\beta$ et deux mots $u',v'$ tels que
  \[\begin{cases}
  u = \alpha u'\\
  v = v'u'\\
...
\begin{example}\label{ex.pile.trace}
  ...
  $\Sigma = \btwo$. On donne en exemple une trace d'exécution du mot $011$
  par cet automate, en prenant comme symbole initial $*$~:
  \[(q_0,*) \xrightarrow{0} (q_0,**) \xrightarrow{1} (q_0,*)
  \xrightarrow{1} (q_0,\varepsilon)\]
\end{example}
...
\begin{proposition}
  ...
  il existe un automate à fond de pile testable reconnaissant le même langage
  que $\mathcal A$, que $\mathcal A$ reconnaisse par état final ou par pile
  vide.
\end{proposition}
...
\begin{theorem}
  ...
  à pile par état final si et seulement s'il est reconnu par un automate à pile
  par pile vide.
\end{theorem}
...
\begin{proof}
  ...
  On suppose donc, pour commencer, que le langage $\mathcal L$ est reconnu par
  un automate à pile à fond de pile testable par état final $\mathcal A$. On
  veut maintenant construire un automate à pile par pile vide. Pour cela, on
  ajoute deux états $q_+$ et $q_-$ représentant respectivement le cas où on
  ...
  $q,\alpha\xrightarrow \beta q_+,\varepsilon$ pour chaque transition de la
  forme $q,\alpha\xrightarrow \beta q',\gamma$ où $q' \in F$, ainsi que toutes
  les transitions $q_+,\alpha \xrightarrow\varepsilon q_+,\varepsilon$,
  permettant ainsi de transformer une trace pour le premier automate en une
  trace du nouvel automate identique jusqu'à l'état final, remplacé par $q_+$ où
  la pile se vide alors.
  Pour le cas de $q_-$, on ajoute toutes les transitions
  $q,\alpha \xrightarrow \beta q_-,\alpha$ pour $\alpha$ une lettre de fond de
  pile ne menant pas à un état final. Ainsi les traces sur l'automate initial
  sont associées soit à une trace vidant sa pile dans l'état $q_+$, soit par une
  ...
  pile testable par pile vide, alors on peut ajouter un unique état final $q_f$
  et une transition $q,\alpha\xrightarrow\beta q_f,\alpha$ pour toute lettre
  $\alpha$ de fond de pile.
\end{proof}
...
\begin{definition}[Langage reconnaissable par pile]
  Un langage $\mathcal L \subseteq \Sigma^\star$ est reconnaissable par automates
  à pile lorsqu'il existe un critère parmi ceux cités précédemment et un
  automate à pile $\mathcal A$ tel que pour tout $u \in \Sigma^\star$,
  $u \in L$
  si et seulement s'il existe une trace valide pour $u$ dont la dernière
...
\begin{theorem}\label{thm.reco.alg}
  Soit $\Sigma$ un alphabet. On a égalité entre la classe des langages
...
\begin{proof}
  ...
  Soit une grammaire $G \defeq \{\Gamma, \{A_i \to w_i\}_{i \in I}, S\}$, on
  construit l'automate $\mathcal A_G$ suivant~:
  \begin{itemize}
  \item $Q \defeq q_0 \cup q_f \cup \{q_{i,j} \mid i \in I, w \in
    \{1,\ldots,|w_i|\}\}$
  ...
  \item la relation $R$ est définie par les faits suivants~:
    ...
    \item pour toute règle $A_i \to w_i$, on ajoute les transitions
      (en considérant que $q_{i,0} = q_0$)
  ...
  \item des blocs augmentant la taille de la pile, où toutes les transitions
    sont de la forme $q_0,\varepsilon \xrightarrow\alpha q_0,\alpha$
  \item des blocs diminuant la taille de la pile, où le bloc commence à l'état
    $q_0$ puis dépile toutes les lettres correspondant au mot $w_i$ d'une règle
    $A_i \to w_i$, puis revient à l'état $q_0$ en empilant la lettre $A_i$
  \end{itemize}
  Suite à cette alternance, on obtient une pile constituée uniquement par $S$
  sur l'état $q_0$, permettant d'aller à l'état $q_f$. Pour une dérivation d'un
  mot $u$ depuis $S$, on remarque qu'il est possible d'effectuer le parcourt de
  l'automate à l'envers~: si la première règle utilisée est $S \to w$, alors on
  a un bloc de transitions de $q_0$ à $q_0$ tel que la pile vaut d'abord $w$,
  puis finit par valoir $S$. Ainsi, les dérivations correspondent à des traces
  valides, et inversement puisqu'un bloc va toujours correspondre à une
  dérivation.

  Montrons le sens réciproque. Pour cela, on utilise la méthode des triplets de
  Ginsburg \cite{ginsburg1966mathematical}. Soit un automate à pile $\mathcal A$
  reconnaissant le langage $\mathcal L$ par pile vide. On suppose sans perte de
  ...
  $\mathcal L_{q,q',\alpha}$ l'ensemble des mots $u$ des tels qu'il existe un
  calcul valide pour $u$ entre la configuration $(q,\alpha)$ et la configuration
  ...
  \begin{align*}
    ...
    \mathcal L_{q,q',\alpha} &= \{\beta \mid q,\alpha \xrightarrow \beta q',
    \varepsilon\}
    \cup\displaystyle\bigcup_{q,\alpha\xrightarrow\beta p,\gamma}
    \beta\mathcal L_{p,q',\gamma}
  ...
  $u$ peut se réécrire en l'existence d'un calcul valide pour $u$ entre
  $(q_0,\alpha_0)$ et $(q',\varepsilon)$ pour un état $q'$.

  La seconde égalité, elle, contient trois termes. En effet, si l'on veut faire
  un calcul entre $(q,\alpha)$ et $(q',\varepsilon)$, trois cas sont possibles~:
  \begin{itemize}
...
d'utiliser des $\varepsilon$-transitions dans un automate à pile déterministe,
tant que cette transition n'intervient pas sur un état où d'autres transitions
avec des lettres sont possibles (ceci pour permettre de mieux gérer l'empilement
et le dépilement).

\begin{definition}[Automate à pile déterministe]
  Un automate à pile $\mathcal A = (Q,q_0,\Gamma,\alpha_0,R)$ sur un alphabet
  $\Sigma$ est dit déterministe si pour chaque configuration $(q,\alpha)$, l'une
  des deux conditions suivantes est vérifiée~:
  \begin{itemize}
  \item soit il existe une unique transition
    $q,\alpha\xrightarrow\varepsilon q',\beta$
...
\begin{proposition}[Non ambiguïté des langages algébriques déterministes]
  Un langage $\mathcal L\subseteq \Sigma^\star$ algébrique déterministe est un
  langage non ambigu.
\end{proposition}
...
\begin{proof}
  ...
  possibilité de faire des $\varepsilon$-transitions à la fin du calcul, ou une
  infinité d'$\varepsilon$-transitions au milieu du calcul. Il est possible de
  modifier l'automate pour assurer ce fait, mais nous l'admettons ici (la preuve
...
\begin{exercise}[Un lemme classique sur les langages algébriques]
  Montrer que si $\mathcal L$ est un langage algébrique et $\mathcal L'$ est un
  langage rationnel, alors $\mathcal L \cap \mathcal L'$ est un langage
  algébrique.
