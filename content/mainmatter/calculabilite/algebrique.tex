...
$\compr{a^nb^n}{n\in\bN}$ n'est pas rationnel. Un tel langage paraît pourtant
intéressant, d'où la volonté de construire un formalisme plus riche que celui
des expressions rationnelles.

Les langages algébriques, encore appelés langages hors contexte, sont des
...
grammaires ainsi que les automates associés aux langages algébriques~: les
...
\begin{remark}
  ...
  donnée dans la suite de ce chapitre des grammaires hors contexte. Dans ce
...
n'utiliserons pas d'autres grammaires ici, on parlera directement de grammaire
...
\begin{definition}[Grammaire hors contexte]
  ...
  \index[notion]{Grammaire!Hors contexte}
\end{definition}
...
d'intérêt, d'un symbole de départ donné et de règles permettant de construire
...
\begin{definition}[Dérivation]
  ...
  \index[notion]{Dérivation!dans une Grammaire}
  \index[notation]{$\varwd\to\varwd'$}
\end{definition}
...
\begin{definition}[Langage engendré par une grammaire]
  ...
  $\varLL_{\varGG}$.
  \index[notion]{Langage!Engendré!par une Grammaire}
\end{definition}

\begin{definition}[Langage hors contexte]
  Soit $\ABA$ un alphabet. On appelle classe des langages algébriques (ou hors
  contexte) la classe de langages sur $\ABA$ définie par
  \[\varCC_{\alg}\defeq \compr{\varLL_{\varGG}}{\varGG\in\Gram(\ABA)}\]
  \index[notion]{Langage!Hors contexte}
\end{definition}
...
Un point essentiel avec les grammaires hors contexte est que, comme la seule
...
\cite{Carton2014}, et décidons donc de conserver la mise en avant de la
...
\begin{definition}[Forme réduite]
  ...
  \item pour tout $\nTA \in \ABB$, il existe $\varwd,\varwv\in\toStAB$ tels que
    $\StGr \toSt{\to} \varwd\nTA\varwv$.
  \end{itemize}
  \index[notion]{Grammaire!sous forme réduite}
\end{definition}
...
présentation des grammaires dans laquelle l'alphabet $\ABB$ se déduit des
...
\begin{definition}[Forme propre]
  ...
  $\nTA \to \nTB$ où $\nTA,\nTB\in\ABB$.
  \index[notion]{Grammaire!sous forme propre}
\end{definition}
...
\begin{definition}[Substitution]\label{def.ghc.subst}
  ...
  élément neutre).
  \index[notion]{Substitution!de Langage}
\end{definition}
...
\begin{proof}
  ...
  \item supposons que $\nTA \rtClot{\to}\varwd$ pour tout $\nTA$ et tout
    ...
    \item si $\varla \in \ABD$, alors
      $\subSimul(\varww') = \subSimul(\varww)\{\varla\}$, et on en déduit donc
      que soit $\varwd \in \subSimul(\varww)\setminus\{\Ewd\}$ (auquel cas la
      propriété est vérifiée par hypothèse de récurrence), soit
  ...
  Pour cela, on supprime simplement ces telles règles, et on ajoute toutes les
  règles de la forme $\nTA \to \varww$ où $\nTA \rtClot{\to} \nTB$ et
...
\begin{definition}[Forme normale de Chomsky \cite{Chomsky1959}]
  Soient un alphabet $\ABA$ et une grammaire $\varGG=(\ABB,\to,\StGr)$ sur
  $\ABA$.
  ...
  \begin{itemize}
    ...
    \end{minipage}
  \end{itemize}
  \index[notion]{Grammaire!sous forme normale de Chomsky}
\end{definition}
...
\begin{definition}[Forme normale de Greibach \cite{Greibach1965}]
  ...
  $\nTA \to \varla\nTB$ ou $\nTA \to \varla\nTB\nTC$ avec
  $\varla\in\ABA$ et $\nTB,\nTC \in \ABB$.
  \index[notion]{Grammaire!sous forme normale de Greibach}
\end{definition}

\begin{proposition}[\cite{Rosenkrantz1967}]
...
pour avoir des familles d'ensembles mutuellement inductifs.
...
\begin{definition}[Signature inductive généralisée]
  ...
  appelée arité.
  \index[notion]{Signature!Ensemble inductif!Généralisée}
\end{definition}
...
\begin{definition}[Famille inductivement engendrée]
  ...
  \index[notion]{Famille!Inductivement engendrée}
\end{definition}
...
\begin{theorem}[Récursion, induction]\label{thm.ind.rec.gen}
  ...
  \item pour toute famille d'ensembles $(Y_s)_{s\in\varSup}$ et toute famille
    ...
    \makeFunName{f_c}{\prod_{i = 1}^k Y_{s_k}}{Y_i}\]
    ...
    \end{multline*}
  \item pour toute famille $(\varPt_s)_{s\in \varSup}$ de parties de
    $(X_s)_{s\in\varSup}$, si pour tout $c \in C$ et en prenant la décomposition
    $\arite(c) = ((s_1,\ldots,s_k),s)$ on a
    \[\forall x_1\in \varPt_{s_1}\cdots x_k\in \varPt_{s_k},
    c(x_1,\ldots,x_k) \in \varPt_s\]
    alors pour tout $s \in \varSup$, $\varPt_s = X_s$.
  \end{itemize}
  \index[notion]{Induction!sur une Famille}
  \index[notion]{Récursion!sur une Famille}
\end{theorem}
...
\begin{definition}[Famille des arbres de dérivation]
  ...
  \index[notion]{Famille!des Arbres de dérivation}
\end{definition}
...
\begin{definition}[Frontière d'un arbre de dérivation]
  ...
  \end{itemize}
  \index[notion]{Frontière!d'un Arbre de dérivation}
\end{definition}
...
\begin{lemma}[Itération]
  ...
  \end{cases}\]
  \index[notion]{Lemme!d'Itération}
\end{lemma}
...
\begin{proof}
  ...
  inférieure à $d^{\Card(\ABB)+2}$, donc par contraposée tout arbre de dérivation
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \fill[color=gray!80]
    (-8,0) -- (-11,-4) -- (-9.5,-4) -- (-7.875,-2) -- (-6.5,-4) --
    (-5,-4) -- (-8,0);
    \fill[color=white]
    (-7.875,-2) -- (-9.5,-4) -- (-8.5,-4) -- (-8,-3) -- (-6.5,-4) --
    (-6.5,-4) -- (-7.875,-2);
    \draw (-8,0) -- (-11,-4) -- (-9.5,-4) -- (-7.875,-2) -- (-6.5,-4) --
    (-5,-4) -- (-8,0);
    \draw (-7.875,-2) -- (-9.5,-4) -- (-8.5,-4) -- (-8,-3) -- (-7.5,-4) --
    (-6.5,-4) -- (-7.875,-2);
    ...
    \fill[color=gray!80]
    (0,0) -- (-3,-4) -- (-1.5,-4) -- (0.125,-2) -- (1.5,-4) -- (3,-4) -- (0,0);
    \fill[color=white]
    (0.125,-2) -- (-1.5,-4) -- (-0.5,-4) -- (0,-3) -- (0.5,-4) -- (1.5,-4) --
    (0.125,-2);
    \fill[color=gray!30] (0,-3) -- (-0.5,-4) -- (0.5,-4) -- (0,-3);
    \draw (0,0) -- (-3,-4) -- (-1.5,-4) -- (0.125,-2) -- (1.5,-4) -- (3,-4) --
    (0,0);
    \draw (0.125,-2) -- (-1.5,-4) -- (-0.5,-4) -- (0,-3) -- (0.5,-4) --
    (1.5,-4) -- (0.125,-2);
    ...
    \fill[color=gray!80] (8,0) -- (5,-4) -- (6.5,-4) -- (8.125,-2) -- (9.5,-4)
    -- (11,-4) -- (8,0);
    \fill[color=white] (8.125,-2) -- (6.5,-4) -- (7.5,-4) -- (8,-3) -- (8.5,-4)
    -- (9.5,-4) -- (8.125,-2);
    \fill[color=gray!30] (8,-3) -- (7.5,-4) -- (8.5,-4) -- (8,-3);
    \draw (8,0) -- (5,-4) -- (6.5,-4) -- (8.125,-2) -- (9.5,-4) -- (11,-4) -- (8,0);
    \draw (8.125,-2) -- (6.5,-4) -- (7.5,-4) -- (8,-3) -- (8.5,-4) --
    (9.5,-4) -- (8.125,-2);
    ...
    \fill[color=white] (8,-3) -- (6.375,-5) -- (7.375,-5) -- (7.875,-4) --
    (8.375,-5) -- (9.375,-5) -- (8,-3);
    \fill[color=gray!30] (7.875,-4) -- (7.375,-5) -- (8.375,-5) -- (7.875,-4);
    \draw (8,-3) -- (6.375,-5) -- (7.375,-5) -- (7.875,-4) -- (8.375,-5) --
    (9.375,-5) -- (8,-3);
...
\begin{definition}[Ambiguïté]
  ...
  $\varLL$.
  \index[notion]{Grammaire!Ambiguë}
\end{definition}
...
\begin{proposition}[Propriété universelle d'une grammaire non ambiguë]
  ...
  $\makeFunName{f_{\nTA,\varwd}}{X^k}{X_{\nTA}}$.
  Il existe alors une unique fonction $\makeFunName{f}{\varLL_{\varGG}}{X}$ telle
  que pour toute règle $\nTA \to \varwd_1\nTA_1\cdots \nTA_k\varwd_{k+1}$ et mots
  ...
  \index[notion]{Propriété!Universelle!d'une Grammaire non ambiguë}
\end{proposition}
...
\begin{proof}
  ...
  un mot $\varwd\in\varLL_{\varGG}$, il existe par non ambiguïté un unique arbre
...
\begin{remark}
  ...
  lesquelles chaque non terminal donne une fonction vers un ensemble différent,
...
\begin{definition}[Bijection de Cantor \cite{Cantor1878}]\label{def.bij.Cantor}
  ...
  \index[notion]{Bijection!de Cantor}
\end{definition}
...
\begin{definition}[Codage de Gödel - version 1 \cite{Cori2020}]
  Soit $\ABA$ un alphabet et $\varGG=(\ABB,\to,\StGr)$ une grammaire non ambiguë
...
\begin{example}
  ...
  \begin{align*}
    ...
    \symbR^p(\Term,\ldots,\Term)\mid (\StGr)\\
    \Term &\inddefeq x \mid \symbF^p(\Term,\ldots,\Term)
  \end{align*}
  où $x \in \Var$, $\symbR^p$ décrit l'ensemble des symboles de relation
  d'arité $p$ et $\symbF^p$ les symboles de fonctions d'arité $p$. Les tuples
...
\begin{remark}
  ...
  \cite{Goedel1931} utilisait la décomposition en facteurs premiers, pour
...
\begin{remark}
  Dans notre description des formules, nous avons un ensemble infini de
...
Dans cette section, nous présentons la notion d'automate à pile. Nous avons vu
...
La première sous-section est consacrée aux définitions, et en particulier au cas
de l'acceptation, car plusieurs possibilités existent pour définir l'acceptation
...
\begin{definition}[Configuration]
  ...
  et $\varwd \in \toStB$.
  \index[notion]{Configuration!d'un Automate à pile}
\end{definition}

Comme dans le chapitre précédent, on définit une notion de trace, qui correspond
...
\begin{definition}[Trace de l'exécution d'un automate à pile]
  ...
  le mot de départ.
  \index[notion]{Trace!d'Exécution!d'un Automate à pile}
\end{definition}
...
$(C_p)_{p = 0,\ldots,k}$, nous n'avons pas de condition sur la dernière
...
\begin{remark}
  Dans l'exemple précédent, on peut déduire que $011$ est reconnu par pile
...
\begin{definition}[Langage reconnaissable par pile]
  ...
  \index[notion]{Langage!Reconnaissable!par Automate à pile}
\end{definition}

Nous concluons maintenant la sous-section en montrant l'équivalence entre les
grammaires hors contexte et les automates à pile.

\begin{theorem}\label{thm.reco.alg}
  Soit $\ABA$ un alphabet. On a égalité entre la classe des langages
  algébriques et celle des langages reconnaissables par un automate à pile.
\end{theorem}
...
\begin{proof}
  ...
  Soit une grammaire $\varGG=\{\ABB,\{\nTA_i\to\varww_i\}_{i \in \indSet},\StGr\}$,
  on construit l'automate $\varAuP{\varGG}$ suivant~:
  \begin{itemize}
    \begin{minipage}{0.65\textwidth}
    \item $\varQQ \defeq \{\varqZ\} \cup \{\varqP{f}\} \cup
      \compr{\varqP{i,j}}{i \in \indSet, j \in \intervIntB{\wdl{\varww_i}}}$
    \end{minipage}
  ...
  \item la relation $\varRR$ est définie par les faits suivants~:
    ...
    \item pour toute règle $\nTA_i \to \varww_i$, on ajoute les transitions
      ...
      \[\forall j < \wdl{\varww_i}, \varqP{i,j}, (\varww_{i})_{\wdl{\varww_i}- j}
  ...
  d'effectuer le parcours de l'automate à l'envers~: si la première règle
  ...
  Ginsburg \cite{Ginsburg1966}. Soit un automate à pile $\varAuA$
  ...
  $\varLL_{\varq,\varq',\varla}$ l'ensemble des mots $\varwd$ tels qu'il existe
  ...
  \item les transitions sont celles indiquées par la seconde équation (on
...
\begin{definition}[Automate à pile déterministe]
  ...
  \item soit il existe une unique transition
    ...
    $\varq,\varla\xrightarrow{\varlb} \varq',\varlc$ avec $\varlb \in \ABA$
  ...
  \index[notion]{Automate!à Pile!Déterministe}
\end{definition}
...
\begin{proof}
  ...
  trouver dans \cite{Carton2014} à la proposition 2.77).
\end{proof}
