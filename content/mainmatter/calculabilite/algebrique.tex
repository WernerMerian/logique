...
\begin{definition}[Forme normale de Chomsky]
  ...
  $\Sigma$. On dit que $G$ est sous forme normale de Chomsky (ou forme normale
...
\begin{definition}[Forme normale de Greibach]
  ...
  plus, de la forme $A \to a$, $A \to aB$ ou $A \to aBC$ avec
...
\begin{proof}
  Soit $\Sigma$ un alphabet et $G$ une grammaire. Notons
  \[G \defeq (\{X_1,\ldots,X_n\}, \to, X_1)\]
  On pose $G(X_i)$ l'ensemble des mots directement dérivés de chaque
  $X_i$.

  Un mot engendré par une lettre $X_i$ est dans l'un des deux cas suivants~:
  \begin{itemize}
  \item le mot est de la forme $X_j w$ pour un certain $j$
  \item le mot est dans $\Sigma (\Sigma \cup \Gamma)^\star$
  \end{itemize}
  dans le premier cas, le mot $w$ appartient à $X_j^{-1}G(X_i)$ et dans le
  deuxième cas, le mot appartient à $G(X_i) \cap \Sigma(\Sigma\cup\Gamma)^\star$.

  Pour améliorer la lisibilité de ces manipulations, employons un vocabulaire
  matriciel~: on pose
  \[ M_{i,j} \defeq X_j^{-1}G(X_i) \qquad
  S_{i} \defeq G(X_i) \cap \Sigma(\Sigma\cup\Gamma)^\star\]
  on peut alors réécrire notre grammaire par
  \[X_i \to \sum_{j} X_j M_{i,j} + S_i\]
  ou, matriciellement~:
  \[X \to X M + S\]
  
  Remarquons que le langage engendré par une grammaire de la forme $X \to XM + S$
  (en oubliant que les objets sont ici des matrices) est exactement $SM^\star$.
  Les deux cas dans ce langage sont l'ajout d'un $M$ à droite et la dérivation en $S$.
  On peut de plus remarquer que $M^\star = \{\varepsilon\} + MM^\star$, ce qui motive à
  introduire une nouvelle matrice $Y$ avec les règles $Y \to I + RY$ (où $I$ représente
  une matrice identité).
  On introduit donc une famille $(Y_{i,j})$ de nouveaux symboles non terminaux
  et on considère la grammaire $G'$ suivante (où le symbole
  $S$ est toujours $X_1$)~:
  \begin{align*}
    X &\to' SY \\
    Y &\to' MY \\
    Y_{i,i} &\to' \varepsilon
  \end{align*}
  Cette grammaire engendre le même langage que $G$. En effet, il nous suffit de montrer
  que chaque dérivation pour $\to$ depuis un $X_i$ et arrivant en un mot
  $u \in \Sigma^\star$ correspond à une dérivation pour $\to'$.

  Pour cela, on va commencer par étiqueter les dérivations en un pas
  $u_0 X_i u_1 \to u_0 v' u_1$ de la façon suivante~:
  \begin{itemize}
  \item si $v' \in S_i$, alors on étiquette la dérivation
    $u_0 X_i u_1 \to_i u_0 v' u_1$
  \item si $v' = X_j w$ où $w \in M_{i,j}$, alors on étiquette la dérivation
    $u_0 X_i u_1 \to_{i,j} u_0 v' u_1$
  \end{itemize}
  On transforme maintenant les dérivations pour $\to$ (supposées étiquetées) en des
  dérivations pour $\to'$. Si $u \to_{i,j} v$, alors on remplace les occurrences de
  $X_i$ dans $u$ par des occurrences de $Y_{i,j}$. On prouve alors que la transformation
  effectuée donne une dérivation pour $\to'$, par induction sur la longueur de la
  dérivation. Supposons que $u \to v \to^\star w$ et montrons que $u' {\to'}^\star w'$ (où
  $u'$ et $w'$ respectivement sont les mots $u$ et $w$ après transformation)~:
  \begin{itemize}
  \item si $u \to_i v$, alors $u' = u$ et $u \to' v Y_{i,i} \to' v$, d'où le résultat par
    hypothèse d'induction.
  \item si $u \to_{i,j} v$, alors on peut réécrire $v = X_j v_0$ et on a
    $u' \to' Y_{i,j} v_0$, mais alors $Y_{i,j} (v_0)' = v'$ et, par hypothèse d'induction,
    $v' {\to'}^\star w'$, donc $u' {\to'}^\star w'$.
  \end{itemize}
  On remarque qu'il est possible d'effectuer la transformation réciproque, et que
  la seule différence entre un mot $u$ et sa transformée $u'$ se trouve dans les symboles
  non terminaux~: en particulier, si $u \in \Sigma^\star$, alors $X_i \to^\star u$ si et
  seulement si $X_i {\to'}^\star u$. Les deux grammaires sont donc équivalentes.

  Finalement, on crée une nouvelle grammaire $G''$ en faisant la modification suivante~:
  pour chaque règle $Y_{i,j} \to' uY_{j,k}$ où $u \in M_{i,j}$, si $u$ commence par le
  non terminal $X_p$, alors on remplace la règle par l'ensemble de règles
  $Y_{i,j} \to' S_p(X_p^{-1}u)Y_{j,k}$. Cela ne change pas la grammaire engendrée, puisque
  chaque règle modifiée ici correspond à une suite de deux règles de dérivation
  $\to' X_pu' \to' S_pu'$ dans l'ancienne grammaire.

  En éliminant les $\varepsilon$-transitions dans la grammaire $G''$, on obtient ainsi
  une grammaire sous forme normale de Greibach. De plus, si la grammaire est sous forme
  normale de Chomsky (ce qu'on peut supposer sans perte de généralité d'après la
  \cref{prop.chom}), alors cette grammaire est sous forme de Greibach quadratique.
\end{proof}
...
On définit la hauteur d'un arbre comme la longueur de la plus grande suite allant
de la racine à une feuille de l'arbre (la hauteur d'un arbre limité à une racine
étant $0$). Il est alors possible de définir et raisonner par récurrence sur la
hauteur de l'arbre.
...
\begin{remark}
  ...
  symboles avec $x \in \mathcal X$. Pour correspondre à notre paradigme où les
...
\begin{definition}[Automate à pile]
    ...
    \begin{itemize}
        ...
        \item 
        $R \subseteq Q \times \Gamma\times (\Sigma\cup \{\varepsilon\})\times
...
\begin{remark}
  Avec la preuve précédente, il est direct d'en déduire aussi que la combinaison des
  deux critères d'arrêt donne encore un critère d'arrêt pour les automates à piles, qui
  engendre la même classe de langage.
\end{remark}

\begin{definition}[Langage reconnaissable par pile]
  Un langage $\mathcal L \subseteq \Sigma^\star$ est reconnaissable par automates à pile
  lorsqu'il existe un critère parmi ceux cités précédemment et un automate à pile
  $\mathcal A$ tel que pour tout $u \in \Sigma^\star$, $u \in L$ si et seulement s'il
  existe une trace valide pour $u$ dont la dernière configuration est une configuration
  d'arrêt pour le critère choisi.
\end{definition}
...
\begin{theorem}\label{thm.reco.alg}
    ...
    et celle des langages reconnaissable par un automate à pile.
\end{theorem}

\begin{proof}
  Commençons par montrer qu'un langage algébrique est reconnaissable par un automate
  à pile pour le critère de pile vide et état final.

  Soit une grammaire $G \defeq \{\Gamma, \{A_i \to w_i\}_{i \in I}, S\}$, on construit
  l'automate $\mathcal A_G$ suivant~:
  \begin{itemize}
  \item $Q \defeq q_0 \cup q_f \cup \{q_{i,j} \mid i \in I, w \in \{1,\ldots,|w_i|\}\}$
  \item $\Gamma_{\mathcal A} \defeq \Sigma \cup \Gamma$
  \item le symbole initial est vide (quitte à créer une transition le supprimant au
    début de l'automate)
  \item l'unique état final est $q_f$
  \item la relation $R$ est définie par les faits suivants~:
    \begin{itemize}
    \item on a une transition $q_0,S\xrightarrow\varepsilon q_f,\varepsilon$
    \item pour toute règle $A_i \to w_i$, on ajoute les transitions (en considérant que
      $q_{i,0} = q_0$)
      \[\forall j < |w_i|, q_{i,j}, (w_{i})_{j} \xrightarrow\varepsilon
      q_{i,j+1},\varepsilon\]
      ainsi que $q_{i,|w_i|},\varepsilon\xrightarrow\varepsilon q_0,A_i$
    \item pour toute lettre $\alpha \in \Sigma$, on ajoute la transition
      \[q_0,\varepsilon \xrightarrow\alpha q_0,\alpha\]
      (que l'on peut encoder par, pour tout symbole de pile $\beta$,
      $q_0,\beta\xrightarrow\alpha q_0,\alpha\beta$)
    \end{itemize}
  \end{itemize}
  On ne montrera pas rigoureusement l'équivalence des deux langages, mais on donne une
  esquisse de preuve. D'abord,
  on remarque qu'une trace d'exécution valide est constituée d'alternances de
  deux types de blocs de configurations~:
  \begin{itemize}
  \item des blocs augmentant la taille de la pile, où toutes les transitions sont de
    la forme $q_0,\varepsilon \xrightarrow\alpha q_0,\alpha$
  \item des blocs diminuant la taille de la pile, où le bloc commence à l'état $q_0$
    puis dépile toutes les lettres correspondant au mot $w_i$ d'une règle
    $A_i \to w_i$, puis revient à l'état $q_0$ en empilant la lettre $A_i$
  \end{itemize}
  Suite à cette alternance, on obtient une pile constituée uniquement par $S$ sur l'état
  $q_0$, permettant d'aller à l'état $q_f$. Pour une dérivation d'un mot $u$ depuis
  $S$, on remarque qu'il est possible d'effectuer le parcourt de l'automate à l'envers~:
  si la première règle utilisée est $S \to w$, alors on a un bloc de transitions de
  $q_0$ à $q_0$ tel que la pile vaut d'abord $w$, puis finit par valoir $S$. Ainsi, les
  dérivations correspondent à des traces valides, et inversement puisqu'un bloc va
  toujours correspondre à une dérivation.

  Montrons le sens réciproque. Pour cela, on utilise la
  méthode des triplets de Ginsburg \cite{ginsburg1966mathematical}. Soit un automate
  à pile $\mathcal A$ reconnaissant le langage $\mathcal L$ par pile vide. On suppose
  sans perte de généralité (quitte à ajouter des
  états et des transitions) que toute transition n'empile qu'au plus deux symboles.

  Pour tous états $q,q'$ et symbole de pile $\alpha$, on définit
  $\mathcal L_{q,q',\alpha}$ l'ensemble des mots $u$ des tels qu'il existe un calcul
  valide pour $u$ entre la configuration $(q,\alpha)$ et la configuration
  $(q',\varepsilon)$. On remarque que si $u \in \mathcal L_{q,q',\alpha}$, alors
  pour tout mot $w$, il existe un calcul valide pour $u$ entre
  $(q,\alpha w)$ et $(q',w)$. On donne les égalités~:
  \begin{align*}
    \mathcal L &= \displaystyle\bigcup_{q' \in Q} \mathcal L_{q_0,q',\alpha_0} \\
    \mathcal L_{q,q',\alpha} &= \{\beta \mid q,\alpha \xrightarrow \beta q',\varepsilon\}
    \cup\displaystyle\bigcup_{q,\alpha\xrightarrow\beta p,\gamma} \beta\mathcal L_{p,q',\gamma}
    \cup\bigcup_{q,\alpha\xrightarrow\beta p,\gamma\delta}
    \beta\mathcal L_{p,p',\gamma}\mathcal L_{p',q',\delta}
  \end{align*}
  La première égalité signifie simplement que reconnaître par pile vide un mot $u$
  peut se réécrire en l'existence d'un calcul valide pour $u$ entre $(q_0,\alpha_0)$ et
  $(q',\varepsilon)$ pour un état $q'$.

  La seconde égalité, elle, contient trois termes. En effet, si l'on veut faire un
  calcul entre $(q,\alpha)$ et $(q',\varepsilon)$, trois cas sont possibles~:
  \begin{itemize}
  \item soit on supprime simplement la $\alpha$ dans une transition d'étiquette
    $\beta$~;
  \item soit on a une transition d'étiquette $\beta$ jusqu'à une configuration
    $(p,\gamma)$, puis un calcul de $(p,\gamma)$ à $(q',\varepsilon)$~;
  \item soit on a une transition d'étiquette $\beta$ jusqu'à une configuration
    $(p,\gamma\delta)$, puis un calcul de $(p,\gamma,\delta)$ à
    $(p',\delta)$, puis un calcul de $(p',\delta)$ à $(q',\varepsilon)$.
  \end{itemize}
  On peut ainsi construire naturellement une grammaire correspondant à ces langages~:
  \begin{itemize}
  \item $\Gamma$ est pris comme l'ensemble des triplets $(q,q',\alpha)$
  \item le non terminal de départ est $S$ muni d'une transition vers chaque
    $\mathcal L_{q_0,q',\alpha_0}$ pour $q' \in Q$
  \item les transitions sont celles indiquées par la seconde équations (on remplace
    simplement les unions par des $+$ et le $=$ par une flèche $\to$)
  \end{itemize}
...
Dans le \cref{chp.auto}, nous avons vu que les automates finis déterministes et non
déterministes sont équivalents quant au langage qu'ils reconnaissent. Dans le cas des
automates à pile, on va voir que cela n'est plus le cas.

Pour les automates finis, comme chaque étape de calcul correspond à la lecture d'une
lettre, le déterminisme ou non d'un automate se caractérise simplement par le fait que
sa relation de transition est fonctionnelle. Dans le cas présent, cependant, cette
distinction n'est pas aussi simple~: il est par exemple permis d'utiliser des
$\varepsilon$-transitions dans un automate à pile déterministe, tant que cette
transition n'intervient pas sur un état où d'autres transitions avec des lettres sont
possibles (ceci pour permettre de mieux gérer l'empilement et le dépilement).

\begin{definition}[Automate à pile déterministe]
  Un automate à pile $\mathcal A = (Q,q_0,\Gamma,\alpha_0,R)$ sur un alphabet
  $\Sigma$ est dit déterministe si pour chaque configuration $(q,\alpha)$, l'une des
  deux conditions suivantes est vérifiée~:
  \begin{itemize}
  \item soit il existe une unique transition $q,\alpha\xrightarrow\varepsilon q',\beta$
    et il n'y a aucune transition $q,\alpha\xrightarrow a q',\beta$ pour un
    élément $a \in \Sigma$
  \item soit il n'existe aucune transition de la forme
    $q,\alpha\xrightarrow\varepsilon q',\beta$ et pour chaque $a \in \Sigma$, il
    existe au plus une transition de la forme $q,\alpha\xrightarrow a q',\beta$.
  \end{itemize}

  Un langage algébrique est dit déterministe s'il est reconnaissable par un automate
  à pile déterministe.
\end{definition}

La proposition principale qui justifie l'introduction de la version déterministe des
automates à pile est la suivante~:

\begin{proposition}[Non ambiguïté des langages algébriques déterministes]
  Un langage $\mathcal L\subseteq \Sigma^\star$ algébrique déterministe est un langage
  non ambigu.
\end{proposition}

\begin{proof}
  En utilisant la méthode des triplets de Ginsburg sur l'automate à pile reconnaissant
  le langage, on obtient une correspondance entre les calculs de l'automate et les
  dérivations de la grammaire. Il nous suffit donc d'avoir l'unicité des calculs de
  l'automate~: l'obstacle majeur à cette unicité est la possibilité de faire des
  $\varepsilon$-transitions à la fin du calcul, ou une infinité
  d'$\varepsilon$-transitions au milieu du calcul. Il est possible de modifier l'automate
  pour assurer ce fait, mais nous l'admettons ici (la preuve peut se trouver dans
  \cite{carton2008langages} à la proposition 2.77).
\end{proof}

\begin{exercise}
  Soit un alphabet $\Sigma$ et une grammaire $G = (\Gamma,\to,S)$ sur $\Sigma$. Montrer
  que si l'automate associé à $G$ par la construction du \cref{thm.reco.alg} est un
  automate déterministe, alors $G$ est une grammaire non ambigüe.

  En déduire que l'ensemble des formules sur une signature donnée est effectivement
  une grammaire non ambigüe.
\end{exercise}

On en déduit le corollaire suivant~:

\begin{corollary}
  La classe des langages algébriques déterministes est incluse dans la classe des
  langages algébriques non ambigus, et est donc une sous-classe stricte de la classe
...
\begin{exercise}[Un lemme classique sur les langages algébriques]
  Montrer que si $\mathcal L$ est un langage algébrique et $\mathcal L'$ est un langage
  rationnel, alors $\mathcal L \cap \mathcal L'$ est un langage algébrique.

  En supposant $\mathcal L'$ algébrique, le résultat est-il toujours vrai ?
\end{exercise}
