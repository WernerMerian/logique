...
\begin{definition}[Espace topologique]
  ...
  ouvert de $X$ un élément $U\in\Omega_X$.
\end{definition}
...
\begin{property}\label{prop.carac.cont.fil}
  Soient deux espaces $(X,\Omega_X)$ et $(Y,\Omega_Y)$ et une fonction
...
\begin{proof}
  ...
  Soit $V\in\vois_{f(x)}$ et $U$ un ouvert contenant $f(x)$ contenu dans $V$
  ...
  Supposons maintenant que pour tout $x\in X$, $f\vois_x\supseteq \vois_{f(x)}$
  et montrons que $f$ est continue.

  Soit $U$ un ouvert de $Y$, montrons que $f^{-1}(U)$ est un ouvert de $X$.
  Pour cela, il nous suffit de montrer que pour tout $x \in f^{-1}(U)$,
  $f^{-1}(U)$ est un voisinage de $x$. Soit donc $x \in f^{-1}(U)$, on sait donc
  que $f(x) \in U$, donc $U \in \vois_{f(x)}$, d'où par inclusion
  $U \in f\vois_x$, c'est-à-dire $f^{-1}(U) \in \vois_x$. Ainsi la préimage d'un
  ouvert est voisinage de tous ses points, donc que c'est un ouvert~: $f$ est
  donc continue.
\end{proof}
...
\begin{proof}
  ...
  l'\cref{exo.carac.adhe}, montrons que pour tout ouvert $U$ contenant $x$,
  $U\cap F\neq\varnothing$. Soit donc $U\in \Omega_X$ contenant $x$, c'est donc
  un voisinage de $x$, donc en particulier $U\in\mathcal H$, et
  $F\in\mathcal H$, donc par compatibilité $U\cap F \in\mathcal H$. Comme
  $\mathcal H$ est un filtre propre, $U\cap F \neq \varnothing$, donc
  $x\in\adher F$.
...
Cette propriété exprime que de tout recouvrement de $X$ par des ouverts, on
...
possibles. En particulier, pour tout $i \in I$ et $U$ ouvert de $X_i$, on
demande que $\pi_i^{-1}(U)$ soit ouvert, et cet ensemble est exactement
$\prod_{j\neq i} X_i \times U$.
...
On donnera une définition plus explicite de la topologie produit, mais
...
\begin{definition}[Topologie produit]
  ...
  avec la topologie engendrée par $\Cylind((X_i,\Omega_{X_i})_{i\in I})$.
\end{definition}
...
\begin{remark}
  ...
  chaque $i \in I$, il est possible d'étendre $\mathcal F$ en un utlrafiltre,
...
fonctions soient des inverses \og faibles\fg~: appliquer une fonction puis
...
\begin{proof}
  ...
  \item si $x\neq y$, on peut trouver un ouvert-fermé contenant $x$ et pas $y$
    ...
    ouvert-fermé $K\subseteq U_x$ tel que $x\in K$ et, comme
    $U_x\cap U_y=\varnothing$, on en déduit que $y\notin K$, d'où le résultat.
  ...
  \item Soit $U$ un ouvert de $\Spec(K\Omega_X)$. Par hypothèse, on peut
    ...
    tel que $\varphi^{-1}(\ext(K)) = K'$. L'ensemble $K'$ est donc
    \[K' = \{x \mid K \in \varphi(x)\}\]
...
\begin{exercise}[Sur les espaces ultra-métriques]
  ...
  vérifie les deux premiers axiomes d'une distance et la propriété suivante,
  à la place de l'inégalité triangulaire~:
