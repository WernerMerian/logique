...
\begin{theorem}[Knaster-Tarski (faible)]\label{thm.ind.KT}
...
\begin{exercise}
  ...
  fonction préservant les intersections et telle que
  \[\forall X \in \powerset(E), X\subseteq f(X)\]
  et $A \in \mathcal P(E)$. Montrer que $f$ est croissante, et que l'on peut
  étendre le théorème pour trouver un plus petit point fixe $\alpha$ tel que
  $A\subseteq \alpha$.
\end{exercise}

\begin{exercise}
  Soit $E$ un ensemble quelconque, et $f : \powerset(E) \to \powerset(E)$ une
  fonction croissante pour l'inclusion préservant les intersections. On définit
  \[\begin{array}{ccccc}
    g_f & : & \powerset(E) & \longrightarrow & \powerset(E)\\
    & & X & \longmapsto & X \cup f(X)
  \end{array}\]
  Montrer que $g_f$ est croissante pour l'inclusion, et préserve les
  intersections.
\end{exercise}
