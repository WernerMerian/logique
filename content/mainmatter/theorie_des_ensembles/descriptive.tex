...
présentant les propriétés essentielles des espaces polonais ainsi qu'une étude
...
\begin{proof}
  ...
  $\intFF{0}{\vareps}$ (si $\vareps > 1$, alors trouver une boule incluse dans
  ...
  définit alors la distance $\vardis$ sur $\displaystyle\prod_{i\in\bN} X_i$ par
  ...
  \[\forall m \geq n_0, \forall x,y \in X_m,
  ...
  Enfin, l'espace produit est séparable. Pour tout $n \in \bN$, on choisit
  $D_n$ une partie dense dénombrable de $X_n$ et $x_n$ un élément
  quelconque de $X_n$. On définit alors, pour chaque $F\subfin \bN$~:
  \[D'_F \defeq \compr{(y_i)_{i \in \bN}}{\begin{array}{l}
    \forall i \in F, y_i \in D_i\\
    \forall j \in \bN \setminus F, y_j = x_j
  \end{array}}\]
  Le cardinal de $D'_F$ est donc dénombrable, puisqu'il y a autant d'éléments
  dans $D'_F$ que de tuples dans le produit fini des $D_i$ pour $i \in F$. On
  en déduit, comme l'ensemble des parties finies de $\bN$ est dénombrable,
  que
  \[D' \defeq \bigcup_{F\subfin \bN} D'_F\]
  est dénombrable. Montrons maintenant que $D'$ est dense dans le produit
  ...
  ouvert $\varOp_n \subseteq X_n$. Pour chaque $n$, comme $D_n$ est dense dans
  $X_n$, on peut donc trouver $y_n \in D_n \cap \varOp_n$. Alors la suite
  \[u_n \defeq \begin{cases} y_n\text{ si } n \in F \\
    x_n \text{ sinon}\end{cases}\]
  appartient au cylindre en $(\varOp_n)_{n \in F}$, et à $D'$, donc $D'$
  intersecte tout ouvert, donc $D'$ est dense dans
  $\displaystyle \prod_{n \in \bN} X_n$.
\end{proof}
...
\begin{definition}[Union disjointe d'espaces topologiques]
  Soit $(X_i,\topo_i)_{i\in\indSet}$ une famille d'espaces topologiques. L'union
  disjointe de cette famille est l'espace donné par
  \[\coprod_{i\in \indSet} X_i \defeq \compr{(i,x)}{i\in\indSet, x \in X_i}\]
  et où la topologie est engendrée par la famille de parties
  \[\topoP{\coprod_{i\in \indSet} X_i}\defeq
  \bigcup_{i \in \indSet}\compr{\compr{(i,x)}{x\in \varOp}}{\varOp\in\topo_i}\]
  \index[notion]{Union!Disjointe}
\end{definition}
...
\begin{proof}
  ...
  \begin{itemize}
  \item soit $i\in \indSet, \varOp \in \topo_i$. Comme la topologie de $X_i$
    est engendrée par $\vardis_i$, on peut écrire
    \[\varOp = \bigcup_{j \in \indSetB} \boule{x_j}{\vareps_j}\]
    pour un certain ensemble d'indices $\indSetB$. Mais dans ce cas,
    \[\bigcup_{j \in \indSetB}\boule{(i,x_j)}{\vareps_j} =
    \compr{(i,x)}{x \in \varOp}\]
...
\begin{proof}
  ...
  $\{x_n\}_{n\in\bN}$. On construit alors un ensemble $D'\subseteq \varCl$~: pour
  ...
  \begin{align*}
    ...
    &\leq \vareps
  \end{align*}
  $D'$ est donc dense dans $\varCl$. $\varCl$ est donc un espace polonais.
\end{proof}
...
\begin{example}
  ...
  \begin{itemize}
  \item l'intervalle $\intFF{0}{1}$, les hypercubes $\intFF{0}{1}^n$ pour tout
...
aux boréliens, notion importante entre autres en théorie de la mesure. Nous
...
\begin{definition}[Ensemble des boréliens]
  ...
  \item si $(\varPt_i)_{i\in\bN}$ est une suite d'éléments de $\Borel$, alors
    $\displaystyle\bigcap_{i\in\bN} \varPt_i\in \Borel$ et
    $\displaystyle\bigcup_{i\in\bN} \varPt_i\in\Borel$.
  \item si $\varPt\in\Borel$, alors $X\setminus \varPt \in \Borel$.
  \end{itemize}
  \index[notion]{Borélien}
\end{definition}
...
\begin{definition}[Hiérarchie de Borel]
  ...
  \begin{itemize}
  \item $\HierBorS{1} = \topo$
  \item pour une partie $\varPt\subseteq X$, $\varPt\in\HierBorS{\varOa}$ si et
    seulement s'il existe une suite $(\varPt_i)_{i\in\bN}$ telle que
    \[\begin{cases}
    \forall i \in \bN, \exists \varOb < \varOa, \varPt_i \in \HierBorP{\varOb}\\
    \displaystyle\bigcup_{i\in\bN} \varPt_i = \varPt
    \end{cases}\]
  \item pour tout $\varOa \in \Ord$, on définit
    \[\HierBorP{\varOa} \defeq
    \compr{X\setminus \varPt}{\varPt \in \HierBorS{\varOa}}\]
  \item pour tout $\varOa \in \Ord$, on définit
    \[\HierBorD{\varOa}\defeq \HierBorS{\varOa}\cap\HierBorP{\varOa}\]
  \end{itemize}

  En particulier, on dit qu'une partie $\varPt\subseteq X$ est un $\Fsig$ si
  $\varPt\in\HierBorS{2}$ et un $\Gdel$ si $\varPt\in\HierBorP{2}$, ce sont
  respectivement les unions dénombrables de fermés et les intersections
  dénombrables d'ouverts.
  \index[notion]{Hiérarchie!de Borel}
  \index[notation]{$\HierBorD{n}$}
  \index[notation]{$\HierBorP{n}$}
  \index[notation]{$\HierBorS{n}$}
\end{definition}
...
\begin{proposition}
  Soit $(X,\topo)$ un espace polonais et $\varPt\subseteq X$ un $\Gdel$. Alors
  $\varPt$ muni de la topologie induite est un espace polonais.
\end{proposition}

\begin{proof}
  On se donne une distance complète $\vardis$ sur $X$ engendrant $\topo$. Soit
  $(\varOp_n)_{n\in\bN}$ une suite d'ouverts telle que
  $\bigcap \varOp_n = \varPt$. On définit
  \[\varCl_n \defeq X\setminus \varOp_n\]
  et une nouvelle distance $\vardis'$ sur $\varPt$~:
  ...
  Cauchy de $(\varPt,\vardis')$. $(x_n)$ est aussi une suite de Cauchy dans
  ...
  pour tout $n$, on en déduit que $x\in \bigcap \varOp_n$, donc que
  $x\in \varPt$. Ainsi $(\varPt,\vardis')$ est complet.
...
\begin{definition}[Espace localement compact]
  ...
  \index[notion]{Espace!Localement compact}
\end{definition}
...
\begin{proof}
  ...
  \item réciproquement, si l'espace est localement compact, montrons que pour
    ...
    $\front(\varCK \cap \varOp)$. On a donc trouvé un voisinage fermé $\varCl$
    de $x$ contenu dans $\varOp$.\qedhere
  \end{itemize}
\end{proof}
...
\begin{definition}[Compactification d'Alexandrov \cite{Alexandroff1924}]
  ...
  où $\infty\notin X$.
  \index[notion]{Compactifié d'Alexandrov}
\end{definition}
...
\begin{proof}
  Soit $(\varOp_i)_{i\in \indSet}$ un recouvrement ouvert de $\CompA X$, alors il
  existe $i\in \indSet$ tel que $\infty\in\varOp_i$. Par définition de
  $\CompA\topo$, on trouve $\varCK$ compact tel que
  $\varOp_i=(X\setminus\varCK)\cup\{\infty\}$.
  On en déduit que $(\varOp_j)_{j\in \indSet\setminus\{i\}}$ est un recouvrement de
  $\varCK$, mais comme on a un recouvrement d'un compact, on peut en extraire un
  recouvrement fini $(\varOp_j)_{j\in F}$ où $F\subfin \indSet\setminus\{i\}$.
  Ainsi $(\varOp_j)_{j\in F\cup \{i\}}$ est un recouvrement fini de $\CompA X$,
  donc $(\CompA X,\CompA\topo)$ est compact.
\end{proof}
...
\begin{definition}[Espace à base dénombrable]
  ...
  \index[notion]{Espace!à Base dénombrable}
\end{definition}
...
\begin{proof}
  ...
  $x\in X \setminus \varCl$. Notons $\varOp \defeq X \setminus \varCl$. Comme
...
conditions de la \cref{desc.prop.second} pour avoir un espace localement
compact plutôt que compact.
...
\begin{definition}[Espace de Cantor, espace de Baire]
  ...
  $\bN$.
  \index[notion]{Espace!de Cantor}
  \index[notion]{Espace!de Baire}
\end{definition}
...
\begin{definition}[Schéma de Cantor]
  ...
  \item pour tout $\varwd\in 2^{<\Ow}$,
    $X_{\varwd\wdPt 0}\cap X_{\varwd\wdPt 1} = \vide$.
...
\begin{proposition}\label{desc.prop.schem.C}
  ...
  une fonction $\makeFunName{f}{2^{\bN}}{X}$ et une seule vérifiant
...
\begin{proof}
  ...
  $X_\varwd \subseteq X_{p\wdPt 0}$ et $X_\varwv \subseteq X_{p\wdPt 1}$, donc
...
\begin{definition}[Point d'accumulation]
  Soit $(X,\topo)$ un espace topologique, $\varPt\subseteq X$ une partie et
  $x\in X$. On dit que $x$ est un point d'accumulation de $\varPt$ si pour tout
  ouvert $\varOp\in\topo$ contenant $x$, il existe un point $y\in \varPt$
  différent de $x$ tel que $y\in U$. On dira que $x$ est un point d'accumulation
  sans autre précision si $x$ est un point d'accumulation de $X$.
  \index[notion]{Point!d'Accumulation}
\end{definition}
...
\begin{definition}[Espace parfait]
  ...
  $\varPt\subseteq X$ est un espace parfait (pour la topologie induite), on dit
  alors que $\varPt$ est parfait dans $X$.
  \index[notion]{Espace!Parfait}
\end{definition}
...
\begin{proposition}
  Soit $(X,\topo)$ un espace polonais parfait. Il existe une injection continue
  $2^{\bN} \to X$.
\end{proposition}

\begin{proof}
  On fixe $\vardis$ une distance sur $X$.
  ...
  \end{itemize}
  Comme $X_\varwd$ a un diamètre inférieur à $2^{-wul}$, on en déduit que le
  ...
  trouver une suite $(x_n)_{n\in\bN}$ telle que $x_n\in X_{\varCan\restr n}$~:
...
\begin{theorem}[Cantor-Bendixson]\label{thm.cantor.bend}
  Soit $(X,\topo)$ un espace polonais. Il existe alors un ouvert dénombrable
  $\varOp\in \topo$ et une partie parfaite disjointe $\varPaf\subseteq X$ tels
  que $X = \varOp \cup \varPaf$.
  \index[notion]{Théorème!de Cantor-Berndixson}
\end{theorem}
...
\begin{proof}
  ...
  processus n'a besoin d'être itéré qu'un nombre dénombrable de fois pour
  converger, ce qui signifie qu'un point qui ne sera jamais retiré est un point
  dont les voisinages sont indénombrables.
  ...
  $(\varOp_i)_{i\in \indSet}$. Pour tout $i\in \indSet$, si $\varOp_i$ est
  dénombrable alors tous ses éléments sont hors de $\varPaf$ (par définition
  d'être un point de condensation). Réciproquement, si un point $x$ n'est pas de
  condensation, alors on trouve un ouvert $\varOp$ dénombrable tel que
  $x\in \varOp$, mais alors comme $(\varOp_i)$ est une base, on peut trouver un
  ouvert $\varOp_i\subseteq \varOp$ tel que $x\in \varOp_i$, qui est
  nécessairement dénombrable par inclusion~: $x$ appartient à un certain
  $\varOp_i$ dénombrable.

  On en déduit donc que
  \[\varOp \defeq X\setminus \varPaf =
  \bigcup\compr{\varOp_i}{i\in \indSet, \Card(\varOp_i)\leq\Cal_0}\]
...
\begin{proof}
  ...
  de plus surjective~: pour tout $x \in X$, on peut trouver une suite $\varCan$
  telle que $f(\varCan) = x$. Si $x \in X_0$, alors on définit $\varCan_0 = 0$,
  sinon on définit $\varCan_0 = 1$, et on procède ainsi par induction sur $n$.
  Si la suite n'atteint pas $x$, cela signifie que $x$ est dans un ensemble
  $X_\varwd$ (car il est au moins dans $X_\Ewd = X$) mais dans aucun élément de la
  partition de $X_\varwd$ en $X_1,\ldots,X_n$, ce qui est absurde. Il nous reste
  enfin à vérifier que l'image d'un ouvert de l'espace de Cantor est ouvert.
  Cela se voit directement, puisque l'image de $\ext(\varwd)$ pour
  $\varwd\in 2^{<\omega}$ est choisie comme un ouvert-fermé.
...
\begin{definition}[Espace de Lindelöf]
  ...
  \index[notion]{Espace!de Lindelöf}
\end{definition}
...
\begin{proof}
  ...
  $(\varOpB_i)_{i \in \indSet}$ de $X$. Pour tout $x \in X$, on sait qu'on peut
  trouver $i_x \in \indSet$ tel que $x \in \varOpB_{i_x}$. Comme $(\varOp_n)$ est
  une base de l'espace et que $\varOpB_i$ est un ouvert, on peut donc trouver
  $\indSetB_x \subseteq \bN$ tel que
  \[\varOpB_{i_x} = \bigcup_{n \in \indSetB_x} \varOp_n\]
  On considère maintenant
  \[\indSetB \defeq \bigcup_{x \in X} \indSetB_x\]
  On sait donc que l'union des $\varOp_j$ pour $j \in \indSetB$ vaut $X$,
  puisque ces indices donnent un recouvrement de $X$. De plus, pour tout
  $\varOp_j$, il existe un $\varOpB_i$ contenant $\varOp_j$ (par construction),
  donc on peut considérer l'ensemble $\compr{\varOpB_{i_j}}{j \in \indSetB}$ des
  tels $\varOpB_i$. Cet ensemble est dénombrable et recouvre $X$ puisqu'il
  contient l'union des $\varOp_j$.
\end{proof}
...
\noindent
Cela nous permet enfin de donner la caractérisation de l'espace de Baire.
...
\begin{proof}
  ...
  jusqu'à une taille $n$ donnée. Soit $\varwd \in \bN^{<\omega}$, on veut
...
\begin{exercise}
  Soit $X$ un espace compact et à base dénombrable. Montrer que l'espace
  métrique
  \[C(X) \defeq \compr{\makeFunName{f}{X}{\bR}}{f\text{ est continue}}\qquad
  \vardis(f,g) \defeq \sup_{x \in X} \left\vert f(x) - g(x) \right\vert\]
  est un espace polonais. Déduire que si $X$ est localement compact à base
  dénombrable, alors $C(X)$ est aussi un espace polonais.
  \textit{Indication~: On admet le théorème de Stone-Weierstrass, disant qu'une
    sous-algèbre de $C(X)$ est dense si et seulement si elle contient pour
    chaque $x \in X$ une fonction ne s'annulant pas en $x$, et pour tous
    $x,y \in X$ il existe une fonction prenant des valeurs différentes en
    $x$ et en $y$.}
\end{exercise}

\begin{exercise}[L'ensemble de Cantor]
  On définit l'ensemble de cantor $K \subseteq \intFF{0}{1}$ par la procédure
  suivante~:
  \[\begin{cases}
  K_0 \defeq \intFF{0}{1}\\
  \forall n \in \bN, K_{n+1} \defeq
  \compr{x \in K_n}{3x \in K_n}\cup \compr{x \in K_n}{3x - 2 \in K_n}\\
  K \defeq \displaystyle\bigcap_{n \in \bN} K_n
  \end{cases}\]
  Montrer que $K$ est un fermé, et que la famille
  $\left(\compr{x\in K_n}{3x\in K_n}\right)_{n \in \bN}$ et la famille
  $\left(\compr{x\in K_n}{3x - 2 \in K_n}\right)_{n \in \bN}$ forment une base
  d'ouverts-fermés. En déduire que $K$ est homéomorphe à l'espace de Cantor.
\end{exercise}

\begin{exercise}[Les irrationnels]
  Montrer que l'ensemble $\bR\setminus \bQ$ des irrationnels est homéomorphe à
  l'espace de Baire.
\end{exercise}

\begin{exercise}[Des $\Gdel$ et rien d'autre]
  Montrer que si $X$ est un espace polonais et $\varPt\subseteq X$ est aussi un
  espace polonais pour la topologie induite, alors $\varPt$ est un $\Gdel$ de
  $X$.
\end{exercise}

\begin{exercise}
  Soit $(X,\topo)$ un espace polonais. Montrer qu'il existe un homéomorphisme
  entre $X$ et un $\Gdel$ de l'espace de Cantor. \textit{Indication~: On pourra
    considérer une base dénombrable $(\varOp_n){n\in\bN}$ de $X$ et associer à
    un élément $x$ la suite $(\indicP{\varOp_n}(x))_{n\in\bN}$.}
\end{exercise}

\begin{exercise}[Image polonaise]
  Soit $X$ un espace polonais et $Y$ un espace métrisable. Montrer que si
  $\makeFunName{f}{X}{Y}$ est une fonction surjective, continue et ouverte,
  alors $Y$ est un espace polonais.
\end{exercise}

\begin{exercise}[Théorème de Baire]\label{exo.baire}
  Soit $(X,\topo)$ un espace polonais et $(\varOp_n)_{n \in \bN}$ une famille
  dénombrable d'ouverts denses dans $X$. Montrer que
  $\displaystyle \bigcap_{n \in \bN} \varOp_n$ est un $\Gdel$ dense dans $X$.
\end{exercise}
...
\begin{problem}[Rang de Cantor-Bendixson]
  Le théorème de Cantor-Bendixson nous dit qu'un ensemble polonais est l'union
  disjointe d'un ensemble parfait et d'une partie dénombrable isolée. On a
  proposé, dans la preuve de ce théorème, de supprimer les points isolés les uns
  après les autres jusqu'à obtenir un ensemble parfait~: on va étudier plus
  finement ce procédé dans ce problème.

  \pbPart{Dériver des espaces}

  Soit $(X,\topo)$ un espace topologique. On définit l'espace $\DerivS(X,\topo)$
  (ou juste $\DerivS(X)$) comme étant l'ensemble des points d'accumulation de
  $(X,\topo)$.
  \begin{pbQuest}
  \item Montrer que $\DerivS(X)$ est un fermé de $X$.
  \item Pour une famille d'espaces topologiques $(X_i)_{i \in \indSet}$, montrer
    que
    $\displaystyle\DerivS\left(\coprod_{i \in I} X_i\right) =
    \coprod_{i \in I}\DerivS(X_i)$.
  \item Si $X$ est localement compact, séparé mais non compact, montrer que
    le compactifié d'Alexandrov de l'espace dérivé de $X$ est l'espace dérivé du
    compactifié d'Alexandrox de $X$~: $\DerivS(\CompA{X})=\CompAB{\DerivS(X)}$.
  \end{pbQuest}

  Pour tout espace $X$ et ordinal $\varOa$, on définit la dérivée itérée de $X$,
  notée $X^{\varOa}$, par induction sur $\varOa$~:
  \[\begin{cases}
  X^0 \defeq X \\
  X^{\varOa+1} \defeq \DerivS(X^{\varOa})\\
  \displaystyle
  \forall \varOl\in\Lim, X^{\varOl} \defeq \bigcap_{\varOb < \varOl} X^{\varOb}
  \end{cases}\]
  \begin{pbQuest}[resume]
  \item Montrer que pour tout espace polonais $(X,\topo)$, il existe un ordinal
    dénombrable $\varOa < \Ow_1$ tel que pour tout $\varOb > \varOa$,
    $X^{\varOa} = X^{\varOb}$. Montrer qu'alors $X^{\varOa}$ est parfait.
  \item En déduire le \cref{thm.cantor.bend}.
  \end{pbQuest}

  On appelle rang de Cantor-Bendixson d'un espace $X$ le plus petit $\varOa$
  tel que la famille $(X_{\varOb})_{\varOb \geq \varOa}$ est constante. On note ce
  rang $\rangCB(X)$.
  \begin{pbQuest}[resume]
  \item Donner un exemple d'espace (non polonais) dont le rang de
    Cantor-Bendixson n'est pas dénombrable.
  \end{pbQuest}

  \pbPart{Les compacts dénombrables de $\bR$}

  On a vu que les espaces polonais ont un rang dénombrable. On cherche
  maintenant à inverser le processus~: étant donné un ordinal dénombrable
  $\varOa < \Ow_1$, peut-on construire un espace $X$ tel que
  $\rangCB(X) = \varOa$~? On va d'abord chercher de tels ordinaux. Tout
  d'abord, comme les espaces polonais sont union d'une partie dénombrable et
  d'une partie parfaite (de cardinal $2^{\Cal_0}$), nos candidats d'espaces
  d'un rang donné peuvent se limiter aux espaces dénombrables.
  \begin{pbQuest}
  \item Montrer que pour tout ordinal dénombrable $\varOa < \Ow_1$, l'espace
    $\Ow^{\varOa} + 1$ est de rang $\varOa + 1$.
  \item Montrer que si $X$ est dénombrable et compact, alors $\rangCB(X)$ est
    un ordinal successeur.
  \end{pbQuest}
  Concentrons-nous sur les compacts dénombrables. Dans ce cadre, on peut se
  restreindre encore plus en n'étudiant que les compacts dénombrables de $\bR$.
  \begin{pbQuest}[resume]
  \item Montrer que tout espace compact dénombrable est homéomorphe à un compact
    de $\bR$. \textit{Indication~: Pour $X$ l'espace considéré, on peut étudier
      l'espace de Banach $C(X)$ et utiliser le théorème de Baire
      (\cref{exo.baire}).}
  \end{pbQuest}
  On en arrive donc à classifier les compacts dénombrables de $\bR$. En gardant
  en tête le fait qu'une telle partie $X$ a comme rang un ordinal successeur, on
  peut considérer la dernière dérivée de $X$ qui est non vide, qu'on appelle
  résidu de $X$~:
  \begin{quotation}
    \noindent
    Si $X \subseteq \bR$ est un compact dénombrable de rang $\varOa + 1$, alors
    on appelle résidu l'ensemble $X^{\varOa}$.
  \end{quotation}
  \begin{pbQuest}[resume]
  \item Montrer que le résidu d'un compact dénombrable est un ensemble fini.
  \item Montrer que si $X$ est un compact dénombrable de $\bR$ de rang $1$ et de
    résidu $1$, alors $X \iso \Ow + 1$.
  \item Soit $\varOa$ un ordinal dénombrable fixé. Montrer que si tout compact
    dénombrable de $\bR$ de rang $\varOa$ et de résidu $1$ est homéomorphe à
    $\Ow^{\varOa} + 1$, alors tout compact dénombrable de $\bR$ de rang $\varOa$
    et de résidu $n\in\bN$ est homéomorphe à $\Ow^{\varOa}\times n + 1$.
  \item Soit $\varOb$ un ordinal dénombrable fixé. Montrer que si, pour tout
    $\varOb < \varOa$ et $n < \Ow$, les compacts dénombrables de $\bR$ de rang
    $\varOb$ et de résidu $n$ sont homéomorphes à $\Ow^{\varOb} \times n + 1$,
    alors tout compact dénombrable de $\bR$ de rang $\varOa$ et de résidu $1$
    est homéomorphe à $\Ow^{\varOa} + 1$.
  \item En déduire que pour tout compact dénombrable $X$ de $\bR$, en notant
    $\varOa$ son rang et $n$ son résidu, on a
    $X \iso \Ow^{\varOa} \times n + 1$.
  \item Pour conclure, montrer que pour tout $\varOa < \Ow_1$ il existe une
    partie $X\subseteq \bR$ telle que $\rangCB(X) = \varOa$.
  \item Donner le cardinal des parties compactes dénombrables de $\bR$ à
    homéomorphisme près.
  \end{pbQuest}
\end{problem}
