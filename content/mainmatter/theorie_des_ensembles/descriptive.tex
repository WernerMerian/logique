...
\begin{exercise}[Image polonaise]
  Soit $X$ un espace polonais et $Y$ un espace métrisable. Si
  $\makeFunName{f}{X}{Y}$ est une fonction surjective, continue et ouverte,
  alors $Y$ est un espace polonais.
\end{exercise}

\begin{exercise}[Dériver des espaces]
  Soit $(X,\topo)$ un espace topologique. On appelle dérivée de $X$ l'ensemble
  $\DerivS(X,\topo)$ des points d'accumulation de $X$, c'est-à-dire l'ensemble
  $X \setminus Y$ où $Y$ est l'ensemble des points isolés. On définit les
  dérivées successives $X^{\varOa}$, pour $\varOa \in \Ord$, par
  \begin{itemize}
  \item $X^0 \defeq X$~;
  \item $X^{\alpha+1}\defeq \DerivS(X)$~;
  \item pour $\varOl$ limite,
    $\displaystyle X^{\varOl}\defeq \bigcap_{\varOb<\varOl}X^{\varOb}$.
  \end{itemize}
  Montrer que $\DerivS(X,\topo)$ est un fermé de $(X,\topo)$. Dans le cas d'un
  espace polonais, montrer qu'il existe $\varOa < \Ow_1$ tel que pour tout
  $\varOb > \varOa$, $X^{\varOa} = X^{\varOb}$. Donner un exemple d'espace non
  polonais dans lequel $X^{\varOa} \neq X^{\varOb}$ pour tout $\varOa < \Ow_1$.
\end{exercise}
