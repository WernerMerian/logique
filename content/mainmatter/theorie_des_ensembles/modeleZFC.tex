...
\begin{definition}[L'univers bien fondé]
  On définit la hiérarchie d'ensembles $\bV_{\varOa}$, pour $\varOa \in \Ord$,
  par induction transfinie~:
  \begin{itemize}
  \item $\bV_0 \defeq \vide$.
  \item $\bV_{\varOa + 1} = \powerset(\bV_{\varOa})$
  ...
  \[\bV \defeq \bigcup_{\varOa \in \Ord} \bV_\varOa\]
\end{definition}

\begin{remark}
  Par une union indicée par $\varOa \in \Ord$, il faut en fait comprendre que le
  prédicat $\bV(x)$ est défini comme
  \[\bV(x) \defeq \exists \varOa \in \Ord, x \in \bV_{\varOa}\]
  On préfère une approche sémantique que syntaxique ici, car celle-ci est bien
  plus facile à appréhender.
\end{remark}

Un premier résultat important~: chaque $\bV_\varOa$ est transitif, et
$\bV$ tout entier l'est donc aussi.

\begin{property}
  Pour chaque $\varOa\in\Ord$, $\bV_\varOa$ est un ensemble transitif. De
  plus, $\bV$ est une classe transitive.
\end{property}
...
\begin{proof}
  ...
  \item supposons que $\bV_\varOa$ est transitif. Soit $x\in\bV_{\varOa+1}$. Par
    définition, cela signifie que $x\subseteq \bV_\varOa$. Soit $y\in x$~: par
    inclusion, $y\in \bV_\varOa$, donc (comme $\bV_\varOa$ est transitif)
    $y\subseteq \bV_\varOa$, c'est-à-dire $y\in \bV_{\varOa + 1}$,
    donc $x\subseteq \bV_{\varOa + 1}$.
  \item si pour tous $\varOb < \varOl$, $\bV_\varOb$ est transitif, alors
    pour $x\in \bV_\varOl$, on trouve $\varOa < \varOl$ tel que $x\in \bV_\varOa$,
    donc $x\subseteq \bV_\varOa$. De plus, $\bV_\varOa \subseteq \bV_\varOl$ donc
    $x\subseteq \bV_\varOl$.
  \end{itemize}
  Ainsi tous les $\bV_\varOa$ sont transitifs. Le fait que $\bV$ est une classe
...
\begin{property}
  Si $\varOa \leq \varOb$, alors $\bV_\varOa\subseteq \bV_\varOb$.
\end{property}
...
\begin{proof}
  ...
  $\varOa + \varOc = \varOb$~:
  \begin{itemize}
  \item pour tout $\varOa$, $\bV_\varOa \subseteq \bV_\varOa$.
  \item supposons que $\bV_\varOa \subseteq \bV_\varOb$, alors montrons que
    $\bV_\varOa \subseteq \bV_{\varOb+1}$~: il nous suffit pour cela de montrer que
  ...
  \item supposons que pour tout $\varOd < \varOc$ et $\varOc$ ordinal limite,
    $\bV_\varOa \subseteq \bV_{\varOa + \varOd}$. Alors
    comme
    \[\bV_{\varOa + \varOc} =
    \bigcup_{\varOd < \varOc} \bV_{\varOa + \varOd}\]
    on en déduit que $\bV_\varOa \subseteq \bV_{\varOa + \varOc}$.\qedhere
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  \item vérifions l'axiome de l'ensemble des parties~: soit $x\in \bV$,
    on trouve $\varOa$ tel que $x \in\bV_\varOa$. Alors pour tout $y\subseteq x$,
    $y\subseteq \bV_\varOa$ donc $y\in\bV_{\varOa +1}$. On en déduit que pour tout
    $y\in \powerset(x)$, $y\in \bV$, donc $\powerset(x)\in\bV$.
  \item vérifions l'axiome de remplacement~: soit $f$ une fonction partielle
    dont le domaine est un ensemble $x \in \bV_\varOa$, et dont l'image
    est $y \subseteq \bV$. On peut alors considérer pour chaque $z \in y$
    un $\varOb_z$ tel que $z \in \bV_{\varOb_z}$. En prenant
  ...
  \item vérifions l'axiome du choix~: soit un ensemble $X$ non vide, dont les
    ...
    $X \in \bV_\varOa$, on sait donc que tous les éléments de $X$ sont
    des éléments de $\bV_\varOa$. Par l'axiome du choix, on trouve
    ...
    ensemble dont les éléments sont des éléments de $\bV_\varOa$, donc
...
\begin{proof}
  ...
  \[\rank(x) \defeq \min\compr{\varOa \in \Ord}{x \subseteq\bV_\varOa}\]
  puis
  \[\varOa \defeq \sup_{x \in X} \rank(x)\]
  on sait que $x\in \bV_{\varOa+1}$ pour tout $x \in X$, donc
  $X\subseteq \bV_\varOa$, donc $X \in \bV_{\varOa + 2}$.
