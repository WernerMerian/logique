...
une théorie riche en termes d'expressivité, au point que l'ensemble des
...
se définir en termes d'ensembles, il semble que tout ce dont nous avons parlé
...
alors se dire que, comme on peut formaliser nos résultats de logique en termes
...
approche formelle de la logique~: les théorèmes d'incomplétude de Gödel
...
abordons dans le \cref{chp.arith}. Pour comprendre au mieux le domaine
d'application de ce dernier théorème, appelons propositions (respectivement
formules) les objets usuels que nous utilisons pour donner des énoncés
mathématiques, et $\ZFC$-propositions (respectivement $\ZFC$-formules) les
objets à l'intérieur de $\ZFC$ représentant les propositions.
Il existe un codage $\godcod{\tok}$ qui, à une proposition $\varFF$, associe la
$\ZFC$-proposition $\godcod{\varFF}$, et il existe dans $\ZFC$ une relation
$\satisf$ reliant un ensemble et une $\ZFC$-proposition, de telle sorte que
$X\satisf \varFF$ est une proposition, pour $X$ un ensemble et $\varFF$ une
$\ZFC$-proposition. Néanmoins, pour une proposition $\varFF$, sa valeur est
considérée sur tout l'univers de discours (moralement, sur $\unZF$), tandis que
$X\satisf \varFF$ se limite à une vérité \og locale\fg, au sein d'un ensemble
$X$. Le \cref{thm.Tarski.ZFC} énonce qu'il n'est pas possible de construire une
formule $\satisf\varFF$ permettant d'énoncer la vérité globale d'une
$\ZFC$-proposition. Plus formellement, cela signifie qu'il n'existe pas de
prédicat $\TarV(x)$ telle que pour toute proposition $\varFF$, on a
\[\ZFC\vdP \TarV(\godcod{\varFF}) \iffS \varFF\]

Cette limitation implique que l'on ne peut pas, à strictement parler, écrire une
formule comme $\unZF\satisf \ZFC$ pour notre univers ensembliste $\unZF$. Par
contre, il est possible d'écrire par exemple $\ZFC\vdP \varFF$, qui en utilisant
le théorème de complétude permet de dire que $\unZF\satisf \varFF$~: ainsi,
l'écriture de $\unZF\satisf\ZFC$ se traduit en fait par une infinité de
théorèmes de la forme $\ZFC\vD\varFF$ pour chaque axiome $\varFF \in \ZFC$.

S'il est assez clair que $\ZFC\vdP\ZFC$ est un constat assez vide, cela nous
...
\begin{proof}
  ...
  \item[$\RforaE$~:] si
    ...
    pour une variable $y \in \VL(\varCG)\cup \VL(A)$ (puisque tout terme dans
  ...
  \item[$\RexE$~:] si
    ...
    $\varFP(x)\land\varFF^\varFP$ et $\varFP(x),\varFF^\varFP$ sont des
...
il semble (d'après \cite{Moore1982}) que celui-ci date des travaux de 
...
\begin{definition}[L'univers bien fondé]
  ...
  \index[notion]{Univers!de Von Neumann}
  \index[notation]{Univers!Von Neumann@$\bV$}
\end{definition}

\begin{remark}
  Par une union indexée par $\varOa \in \Ord$, il faut en fait comprendre que le
...
\begin{proof}
  ...
  \item supposons que $\varCM$ est clos par sous-ensemble. Soient
    ...
    donc il existe bien dans $\varCM$ un ensemble $\varPt$ tel que
    $\forall y, y \in \varPt \iff y \in b \land \varFP^{\varCM}(\vec x,y)$.
  ...
  \item de même, si $\varCM$ est stable par réunion, on a directement un
    candidat pour l'union d'un ensemble $x$, donné par $\bigcup x$
  ...
  \item soit une formule fonctionnelle $\varFP^\varCM(\vec z,x,y)$,
    ...
    par $\varFP^\varCM(\vec a)$, et une fonction $\makeFunNamePartial{f}{b}{c}$.
    ...
    Ainsi, $\makeFunNamePartial{f}{b}{c}$, dont le domaine est $a \in \varCM$, a
    une image $c \subseteq \varCM$. Par notre hypothèse, $c \in \varCM$, donc
    l'axiome de remplacement est vérifié.\qedhere
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  \item vérifions l'axiome du choix~: soit un ensemble $X$ non vide, dont les
    ...
    $\varCh\in\bV_{\varOa + 1}$, d'où le résultat.\qedhere
  \end{itemize}
\end{proof}
...
Le rang, introduit pour la démonstration précédente, indique à quelle étape de
la construction des $\bV_\varOa$ notre ensemble a été construit.

\begin{definition}[Rang]
  On définit le rang d'un ensemble $x\in\bV$ par~:
  \[\rk(x) \defeq \min \compr{\varOa \in \Ord}{x\subseteq \bVa}\]
  \index[notion]{Rang}
  \index[notation]{Rang!V@$\rk(x)$}
\end{definition}
...
définition par récursion sur $\in$ (récursion bien fondée dans le cas présent
...
pourraient aussi bien être présents plus tôt dans la hiérarchie des $\bVa$.
...
\begin{property}\label{prop.rg.oper}
  ...
  \item $\displaystyle\forall x\in\bV,\rk\left(\bigcup x\right)\leq\rk(x)
    \leq \rk\left(\bigcup x\right) + 1$
...
\begin{proof}
  ...
  \item en utilisant la \cref{prop.cara.rk}, puisque les éléments de $\bigcup x$
    sont exactement les éléments d'éléments de $x$, on a
    \begin{equation}\label{eq.rk.union}
      \rk\left(\bigcup x\right) = \sup_{y \in \bigcup x} (\rk(y) + 1)
      = \sup_{y \in x}\sup_{z \in y}(\rk(z) + 1)
    \end{equation}
    On montre maintenant les deux inégalités~:
    \begin{itemize}
    \item on vérifie que $\rk(x)$ est un majorant de $\rk(z) + 1$ pour
      $z \in y \in x$, ce qui se vérifie directement en considérant la première
      inégalité qu'on a démontrée. Ainsi, puisque $\rk\left(\bigcup x\right)$
      est une borne supérieure, elle est inférieure à $\rk(x)$.
    \item on peut écrire $\displaystyle\rk(x) = \sup_{y \in x} (\rk(y) + 1)$, qui
      est inférieur à $\displaystyle(\sup_{y \in x} \rk(y)) + 1$ (car ce dernier
      est un majorant de tous les $\rk(y) + 1$). Ce dernier sup est
      (par la \cref{prop.cara.rk} appliquée à l'\cref{eq.rk.union})
      $\rk\left(\bigcup x\right)$, d'où le résultat.
    \end{itemize}
  ...
  \item on prouve cela par induction transfinie~:
    ...
    \item si $\varOl$ est un ordinal limite, alors
      $\rk(\varOl) = \sup_{\varOb < \varOl} (\rk(\varOb) + 1)$ et par hypothèse
      d'induction, $\rk(\varOb) = \varOb$, donc $\rk(\varOl) = \varOl$.
    \end{itemize}
    Ainsi pour tout $\varOa \in \Ord, \rk(\varOa) = \varOa$.\qedhere
  \end{itemize}
\end{proof}
...
puissance pour de la théorie des ensembles~: le schéma d'axiomes de remplacement
...
\begin{definition}[Ensemble héréditaire]
  ...
  \index[notion]{Ensemble!Héréditaire}
  \index[notation]{$\bH$}
\end{definition}
...
On veut maintenant étudier les cas d'égalité entre $\mathbb H$ et $\mathbb V$.
...
\begin{proof}
  ...
  \item axiome de la réunion~: si $X\in \bH_\varCk$, alors
    ...
    puisque $\trcl(X)$ contient déjà $\bigcup X$ et est transitif. Il vient
    alors l'inégalité souhaitée~:
    $\displaystyle\Card\left(\trcl\left(\bigcup X\right)\right) < \varCk$.
  \item axiome de remplacement~: soit $f$ une fonction partielle d'un
    ...
    $\Card(\trcl(y)) < \varCk$, donc $y \in \bH_\varCk$.
...
\begin{definition}[Cardinal fortement inaccessible]
  ...
  simplement, inaccessible) si $\varCCl > \Ow$, $\varCCl$ est régulier et
  \[\forall \varCk \in \varCCl, 2^\varCk \in \varCCl\]
  \index[notion]{Cardinal!Inaccessible!Fortement}
\end{definition}
...
\begin{definition}[Partie définissable]\label{def.definissable.L}
  Soit un ensemble $X$, on dit qu'une partie $\varPt\subseteq X$ est
  définissable dans $X$ s'il existe une formule $\varFF(\vec x,y)$ à $n+1$
  variables libres et des ensembles $\vec a \in X^n$ tels que
  \[\forall b \in X, b \in \varPt \iff X\satisf \varFF(\vec a,b)\]

  \noindent
  On appelle ensemble des parties définissables d'un ensemble $X$ l'ensemble
  \[\powerdef(X) \defeq \compr{\varPt \subseteq X}{\exists
    \varFF(\vec x,y)\in\Formula_{n+1}(\SignP{\ZF}),
    \exists \vec a \in X^n, \forall b \in X, b \in \varPt \iff X \satisf
    \varFF(\vec a,b)}\]
  \index[notion]{Partie!Définissable!Théorie des ensembles}
  \index[notation]{$\powerdef(X)$}
\end{definition}
...
L'univers constructible est alors l'itération de cette opération à la place de
l'ensemble des parties.

\begin{definition}[Univers constructible de Gödel\cite{Goedel1940b}]\label{def.L}
  On définit par induction transfinie la hiérarchie $\bLa$~:
  \begin{itemize}
  \item $\bL_0 = \vide$
  \item $\bL_{\varOa + 1} = \powerdef (\bLa)$
  ...
  \index[notion]{Univers!Constructible}
  \index[notation]{Univers!Gödel@$\bL$}
\end{definition}
...
$\varCM$ garde-t-elle la même valeur de vérité selon qu'on l'étudie dans
$\varCM$ ou dans $\varCM'$~?
...
\begin{definition}[Formule absolue]\label{def.absoluite}
  ...
  \index[notion]{Formule!Absolue}
  \index[notation]{$\absolF$}
\end{definition}
...
\begin{definition}[Hiérarchie de Lévy \cite{Levy1965}]\label{def.Levy}
  ...
  \index[notion]{Hiérarchie!de Lévy}
  \index[notation]{$\HierLevD{n}$}
  \index[notation]{$\HierLevP{n}$}
  \index[notation]{$\HierLevS{n}$}
\end{definition}
...
\begin{proof}
  On prouve le résultat par induction sur la formule $\varFF$ qu'on considère~:
  \begin{itemize}
  ...
  \item dans le cas de $\exists$, supposons donc que
    $\varFF(\vec x,z) = \exists y \in z,\varFP(\vec x,y,z)$
    (les termes dans $\SignP{\ZF}$ sont uniquement des variables). Par hypothèse
    d'induction, on suppose donc que $\varFP$ est absolue pour $\varCM$. Soient
    des paramètres $\vec a$ et $c$ dans $\varCM$. On veut donc prouver que
    \[\exists b^{\varCM}, b \in c \land \varFP^{\varCM}(\vec a,b,c)
    \iff \exists b \in c, \varFP(\vec a,b,c)\]
    ...
    Alors, par absoluité de $\varFP$ pour $\varCM$, on en déduit que
    $\varFP(\vec a,b,c)$.
  \item on peut prouver le cas de $\forall$ en passant par les lois de De
    Morgan.
  \end{itemize}
  Ainsi, par induction, on sait que $\varFF \in \DeltaZL$ est absolue pour
  $\varCM$.
\end{proof}
...
\begin{example}
  ...
  \item pour un couple $\alpha$, la formule $x = \projP{1}{}(\alpha)$
    (respectivement $x = \projP{2}{}(\alpha)$)
  \item $f$ est une fonction, car cela s'écrit
    \[\left(\forall \gamma \in f, \gamma\text{ est un couple }\right)\land
    \left(\forall \gamma,\delta \in f, \projP{2}{}(\gamma) = \projP{2}{}(\delta)
    \implies \projP{1}{}(\gamma) = \projP{1}{}(\delta)\right) \]
...
\begin{theorem}[Réflexion]\label{thm.reflexion}
  Soit une fonction $\makeFunName{\varCW}{\Ord}{\bV}$ vérifiant~:
  \begin{itemize}
  ...
  \index[notation]{Théorème!de Réflexion}
\end{theorem}
...
\begin{proof}
  ...
  \item La première inclusion, $\powerdef(X)\subseteq\powerset(X)$, est directe.
    Supposons que $X$ est fini et soit $\varPt\subseteq X$. Décomposons
    $\varPt = \{y_1,\ldots,y_p\}$. On peut alors définir la proposition
    \[\varFF(x) \defeq \bigvee_{i = 1}^p (x = y_i)\]
    dont il est évident que $x \in \varPt \iff X \satisf \varFF(x)$. Ainsi
    $\varPt\in \powerdef(X)$, d'où l'égalité.
  \item Supposons que $X$ est infini. Pour énumérer $\powerdef(X)$, il nous
    ...
    une partie définissable de $X$, donc $\Card(\powerdef(X))\leq \Card(X)$.
    De plus, on peut injecter $X$ dans $\powerdef(X)$ par $x\mapsto \{x\}$, dont
    la représentation est la formule
    \[\varFP(y) \defeq x = y \]
...
\begin{proof}
  ...
  \item supposons que $\bLa$ est transitif. Soit alors
    ...
    à paramètre (ici, $y$) dans $\bLa$. Ainsi $y \in \bL_{\varOa + 1}$, donc
...
Par cardinalité, on remarque que $\powerdef(X)\neq \powerset(X)$ lorsque
$X$ est infini. On peut ainsi étudier les cas où $\bL$ et $\bV$ coïncident.
...
\begin{proof}
  ...
  \item comme on sait que $\Card(\powerdef(X)) = \Card(X)$ pour $X$ infini, on
    ...
    l'inclusion stricte par contraposée.\qedhere
  \end{itemize}
\end{proof}
...
Il y a donc toujours plus d'éléments dans $\bVa$ que dans $\bLa$, excepté aux
ordinaux fortement inaccessibles. Cependant, on pourrait imaginer qu'en prenant
par exemple $\varOa' = \Cbe_\varOa$, on puisse rattraper $\bVa$ en considérant
$\bL_{\varOa'}$. Le fait que l'on puisse toujours rattraper la hiérarchie
$(\bVa)_{\varOa\in\Ord}$ avec $\bL$ est une assertion indépendante de $\ZFC$, qu'on
introduit sous la forme de l'axiome suivant~:

\begin{axiom}[Constructibilité]\label{ax.VeqL}
  On définit l'axiome de constructibilité $\VeqL$ par la formule suivante~:
  \[\VeqL \defeq \forall x, \exists \varOa \in \Ord, x \in \bLa\]
  \index[notion]{Axiome!de Constructibilité}
\end{axiom}
...
\begin{property}
  ...
  $\varOa$ et toute fonction $\makeFunName{f}{\varOa}{\bV}$, le prédicat
  $\ind_H(f)$ est $\DeltaZL$. La relation fonctionnelle définie par récursion
  transfinie par $H$ est une relation $\HierLevD{1}$.
\end{property}

\begin{proof}
  En effet, dire que $\makeFunName{f}{\varOa}{\bV}$ est $H$-inductive signifie
  que~:
  \begin{itemize}
  ...
  \end{itemize}
  On sait donc que, pour une fonction $\makeFunName{f}{\varOa}{\bV}$, savoir si
  $f$ est $H$-inductive est $\DeltaZL$.

  \'Etant donné $H$, on considère la relation fonctionnelle
  \[F(\varOa,y) \defeq \exists \makeFunName{f}{\Ss \varOa}{\bV},
  ...
  \[F(\varOa,y) \defeq \forall \makeFunName{f}{\Ss \varOa}{\bV},
  \ind_H(f) \implies (\varOa,y) \in f\]
  donc la fonction $\Ord \to \bV$ définie par $H$ est, elle, $\HierLevD{1}$.
\end{proof}

\begin{remark}
  Si $H$ est $\HierLevD{1}$, alors $\ind_H(f)$ est aussi $\HierLevD{1}$ et la
  relation fonctionnelle est une relation $\HierLevD{1}$.
\end{remark}
...
que $\HierLevD{0}$, qui est une formule absolue.

\begin{property}\label{prop.Form.Delta}
  Le prédicat $X = \Formula(\SignP{\ZF})$ est $\DeltaZL$.
\end{property}

\begin{proof}
  On peut définir $\Formula$ comme étant un certain ensemble de suites finies
  sur un alphabet fixé (\latinexpr{c.f.} \cref{chp.alg}). Toutes les définitions
  qui en suivent se placent dans des ensembles fixés, qui définissent donc des
  objets $\DeltaZL$.
\end{proof}

\begin{property}
  Le prédicat $Y = \powerdef(X)$ est $\HierLevD{1}$.
\end{property}
...
\begin{proof}
  ...
  une induction sur $\Formula(\SignP{\ZF})$. La quantification sur les éléments
  de $X$ est $\DeltaZL$. Ainsi la formule en $\varPt$ suivante
  \[\exists \varFF(\vec x,y)\in\Formula_{n+1}(\SignP{\ZF}),
  \exists \vec a\in X^n, \forall b\in X, \\
  b\in \varPt \iff X\satisf \varFF(\vec a,b)\]
  est bien $\HierLevD{1}$, donc $\varPt\in \powerdef(X)$ est un prédicat
  $\HierLevD{1}$. Maintenant, dire que $Y = \powerdef(X)$ signifie
  \begin{multline*}
  ...
  \end{multline*}
  donc $Y = \powerdef(X)$ est un prédicat $\HierLevD{1}$.
\end{proof}
...
Cela paraît assez évident, mais il faut rappeler qu'\latinexpr{a priori}, on
...
\begin{proof}
  ...
  \item axiome de l'infini~: comme $\varCM$ est clos par parties finies grâce à
    ...
    trouver $z$ contenant $\bV_\Ow$ et donc $\Ow$, puis on utilise la
    compréhension.
  \end{itemize}
...
\begin{axiom}[Axiome du choix global]
  ...
  \index[notion]{Axiome!du Choix!Global}
\end{axiom}
...
\item si $\bLa$ peut être bien ordonné, alors l'ensemble des suites finies
  ${\bLa}^{<\Ow}$ à valeurs dans $\bLa$ peut l'être aussi.
\end{itemize}
...
ordonner $\bL_{\varOa + 1}$. Remarquons cependant que plusieurs tuples peuvent
...
\begin{theorem}
  Il existe une fonction $\makeFunName{\mathord{\leL}}{\Ord}{\bV}$ telle que
  $\mathbin{\leL}(\varOa)$ est un bon ordre sur $\bL_\varOa$ pour tout
  $\varOa\in \Ord$ et telle que pour tous $\varOa < \varOb$,
  $\mathbin{\leL}(\varOa)$ est un segment initial de $\mathbin{\leL}(\varOb)$.
\end{theorem}
...
\begin{proof}
  ...
  \item supposons que $\mathbin{\leL}(\varOa)$ existe. On utilise ce qu'on a dit
    ...
    $\Formula(\SignP{\ZF})\times {\bLa}^{<\Ow}$, soit
  ...
  \end{itemize}
  On a donc construit notre fonction $\makeFunName{\mathord{\leL}}{\Ord}{\bV}$
  telle que $\mathbin{\leL}(\varOa)$ est un bon ordre sur $\bLa$ pour tout
  $\varOa\in\Ord$.
\end{proof}
...
par donner le lemme de condensation. Celui-ci permet de dire que les seuls
ensembles dans un $\bLa$ qui vérifient les mêmes propriétés que $\bLa$ sont
eux-mêmes de la forme $\bL_{\varOb}$.
...
\begin{proof}
  ...
  \item[\fbox{$\subseteq$}] On sait que $X\satisf\VeqL$, d'où
    ...
    et, comme on a pris $\varOb \defeq X \cap \Ord$, on en déduit que
    $\forall x \in X, \exists \varOc < \varOb, x \in \bL_{\varOc}$, donc
    $X \subseteq \bL_{\varOb}$.
...
\begin{proof}
  ...
  $x \in y$ et $\Card(y) \leq \varCCl < \SsC{\varCCl}$. On définit
  $\rk_{\bL}(y)$ comme étant le rang minimal tel que
  $y\in\bL_{\rk_{\bL}(y)}$.
  Dans $\bV$, on peut montrer que pour tout ordinal $\varOa$ il existe un
  cardinal régulier indénombrable $\varCk_{\varOa}$ supérieur strictement à
  $\varOa$. Comme on suppose $\VeqL$, on en déduit ici qu'il existe $\varCk_y$
  régulier indénombrable tel que $\rk_{\bL}(y) < \varCk_y$.
  ...
  $\Card(\varMM) \leq \varCCl$ (ce qui est possible car $\Card(y)\leq \varCCl$).

  En utilisant alors le \cref{lem.most}, on trouve un ensemble $\varMN$ et un
  isomorphisme $\makeFunNameIso{\varCol}{(\varMM,\in)}{(\varMN,\in)}$. Comme
  $y\in \varMM$ est transitif, il est un point fixe par $\varCol$, donc pour
  tout $z\in y, \varCol(z) = z$. En particulier, $\varCol(x) = x$ et
  $x\in \varMN$.

  Par le lemme précédent, on sait que $\varMN = \bL_{\varOb}$ pour
  $\varOb = \varMN\cap\Ord \leq \Card(\varMM) < \varCk$, donc (comme
  $x \in \varMN$) $x \in \bL_{\varCk}$.
  \index[notation]{$\rk_{\bL}(x)$}
\end{proof}
...
\begin{proof}
  ...
  $\bH$ et $\bL$, et le lemme précédent nous donne une inclusion pour le cas
  successeur. Il nous reste donc à prouver que $\bL_\varCk\subseteq\bH_\varCk$
  pour $\varCk$ successeur.
...
\begin{proof}
  ...
  donc $\Card(\powerset(\varCCl)) \leq \SsC{\lambda}$.
\end{proof}

Nous concluons donc ce chapitre sur ce résultat important~:
\[\Coher(\ZF) \implies \Coher(\ZFC + \HCG)\]

$\HCG$ ne peut donc pas être prouvé faux dans $\ZFC$. Pour montrer que ce
...
forcing, introduite par Cohen dans
\cite{Cohen1963,Cohen1964}.
...
\begin{exercise}
  Soit $\varFF$ une formule arithmétique, c'est-à-dire une formule du premier
  ordre sur la signature $\SignArith$. Construire une formule $\varFF^{\Ow}$,
  relativisation de $\varFF$ au modèle standard de l'arithmétique interne à
  $\ZF$. Placer $\varFF^{\Ow}$ dans la hiérarchie de Lévy. En déduire que
  $\varFF^{\Ow}$ est absolue entre deux modèles classe transitifs de $\ZF$.
\end{exercise}

\begin{exercise}
  On enrichit la signature $\SignP{\ZFC}$ en
  \[\SignP{\ZFC^{+}} \defeq \SignP{\ZFC} \cup\{\varMM^0\}\]
  où $\varMM$ est une constante. On considère la théorie
  \[\ZFC^+ \defeq \ZFC \cup \compr{\varFF^{\varMM}}{\varFF\in\ZFC}\]
  \begin{enumerate}[label=(\roman*)]
  \item Montrer que $\ZFC$ est cohérente si et seulement si $\ZFC^+$ est
    cohérente.
  \item En supposant le second théorème d'incomplétude, en déduire l'implication
    \[\Coher(\ZFC + \ogM \exists \varMM, \varMM\satisf\ZFC\fgM)\implies
    \Coher(\ZFC^+)\]
    mais que la réciproque est fausse.
  \item Montrer l'implication
    \[\Coher(\ZFC^+ + \ogM \omega^{\varMM} = \omega\fgM)\implies
    \Coher(\ZFC + \ogM \exists \varMM, \varMM\satisf\ZFC\fgM)\]
    mais que la réciproque est fausse. \textit{Indication~: On conseille au
      lecteur de lire d'abord le \cref{chp.arith} avant de traiter cette
      question.}
  \end{enumerate}
\end{exercise}

\begin{exercise}
  Montrer que $\VeqL\implies\diamondsuit$, où $\diamondsuit$ est le principe
  du diamant introduit dans le \cref{pb.souslin} du
  \cref{chp.combi}.\footnote{On en déduit qu'il n'est pas possible de donner
  dans $\ZFC$ une preuve positive au problème de Souslin~: il peut exister des
  droites de Souslin et les conditions données par Souslin ne suffisent pas
  forcément à caractériser $\bR$.}
\end{exercise}

\begin{exercise}[Donner des rangs]
  On rappelle que le $\bL$-rang d'un ensemble $x$, qu'on note ici $\rk_{\bL}$,
  est le plus petit ordinal $\varOa$ tel que $x \in \bLa$.\footnote{Il est
  important de considérer l'appartenance et non l'inclusion, car un ensemble
  peut être une partie de $\bLa$ et pour autant ne pas être dans
  $\bL_{\varOa+1}$.} Pour $x,y \in \bL$, calculer le $\bL$-rang de
  $\bigcup x$, $\{x\}$, $x\times y$, $x \cup y$, $\{x,y\}$ et $(x,y)$ en
  fonction de $\rk_{\bL}(x)$ et $\rk_{\bL}(y)$.

  Montrer que $\bZ,\bQ \in \bL_{\Ow+\Ow}$. Montrer que $\bR$ (défini par les
  coupures de Dedekind tel qu'on l'a construit dans le \cref{chp.axiomes})
  vérifie $\bR^{\bL} = \bR\cap\bL$ et $\rk_{\bL}(\bR) = \Ow_1^{\bL}$.
\end{exercise}

\begin{exercise}
  Montrer qu'il n'est pas possible de montrer dans $\ZFC$ l'existence d'un
  cardinal faiblement inaccessible. \textit{Indication~: On pourra supposer
  $\VeqL$.}
\end{exercise}
...
\begin{problem}[Nier la fondation]
  On a vu, grâce au modèle classe $\bV$, que l'axiome de fondation est
  compatible avec $\ZFC$. On va ici montrer que la négation de $\AxF$ est elle
  aussi compatible avec $\ZFC$, en construisant un modèle contenant des
  atomes~: des ensembles $x$ tels que $x = \{x\}$.
  On va construire ce modèle en construisant une nouvelle relation $\in'$. Pour
  cela, on commence par fixer un ensemble d'atomes, judicieusement noté $\Atom$.

  \pbPart{Construire un nouveau modèle}

  Plutôt que des ensembles $x = \{y_1,\ldots,\}$, on veut désormais considérer
  des \og ensembles étiquetés\fg{} qui sont de la forme
  $x = (i, \{y_1,\ldots,\})$ où $i$ est un certain ensemble, apportant une
  information supplémentaire aux éléments de $x$. De cette façon, on peut
  distinguer par exemple $a \in \Atom$ considéré en tant qu'atome et
  $a \in \Atom$ considéré en tant qu'ensemble. On définit l'opération d'ensemble
  des parties étiquetées par
  \[\powerset_{\Atom}(x) \defeq
  \compr{(0,y)}{y \subseteq x}\cup \compr{(1,a)}{a \in \Atom}\]
  et la famille d'ensembles
  \[\bV_{\Atom,0} \defeq \vide \qquad
  \bV_{\Atom,\varOa+1}\defeq \powerset_{\Atom}(\bV_{\Atom,\varOa})\]
  puis $\displaystyle\bV_{\Atom} \defeq \bigcup_{\varOa\in\Ord}\bV_{\Atom,\varOa}$.
  \begin{pbQuest}
  \item Montrer que pour tous $\varOa,\varOb\in\Ord$ tels que
    $\varOa \leq \varOb$, $\bV_{\Atom,\varOa}\subseteq\bV_{\Atom,\varOb}$.
  \item La classe $\bV_{\Atom}$ est-elle transitive~?
  \end{pbQuest}

  On définit maintenant sur $\bV_{\Atom}$ la nouvelle relation $\in_{\Atom}$
  suivante~:
  \[x \in_{\Atom} y \defeq (\exists z\in\Atom, x = y = (1,z))\lor
  (\exists y_0, y = (0,y_0) \land x \in y_0)\]
  \begin{pbQuest}[resume]
  \item Montrer que $(\bV_{\Atom},\in_{\Atom})$ est un modèle classe de $\ZF$, et
    un modèle classe de $\ZFC$ si on suppose $\AxC$.
  \item En déduire $\Coher(\ZF)\implies \Coher(\ZFC + \lnot\AxF)$.
  \end{pbQuest}

  \pbPart{\'Etendre l'indépendance}

  On a vu ici que, si l'on suppose la cohérence de $\ZF$, alors on peut se
  permettre d'ajouter des éléments $x$ tels que $x \in x$ en nombre aussi grand
  que l'on veut. Cependant, ce cas est plutôt simple~: on ne peut pas construire
  par exemple de suite $x_0 \in x_1 \in \cdots \in x_n \in \cdots$ avec tous les
  $x_i$ deux à deux disjoints. On souhaite donc prouver la cohérence relative
  d'un axiome plus fort, appelé axiome d'anti-fondation d'Aczel. Pour donner cet
  axiome, on commence par introduire la notion de graphe.
  \begin{quotation}
    \noindent
    Un graphe est un couple $(S,A)$ où $S$ est un ensemble et $A$ est une
    relation binaire sur $S$.
  \end{quotation}
  \begin{pbQuest}
  \item Construire un modèle classe $\bV_{(S,A)}$ et une relation $\in_{(S,A)}$
    inspirée par la partie précédente, pour laquelle $(1,x)\in_{(S,A)} (1,y)$
    signifie que $x,y \in S$ et $(x,y) \in A$.
  \item En déduire que pour tout graphe $(S,A)$, l'axiome
    \begin{quotation}
      \noindent
      Il existe un ensemble $X$ et un isomorphisme
      $\makeFunNameIso{f}{(S,A)}{(X,\in)}$.
    \end{quotation}
  \end{pbQuest}

  L'axiome d'anti-fondation est, en quelque sorte, la version globale de
  l'axiome dont nous venons de prouver l'indépendance relative à $\ZFC$~:
  \begin{quotation}
    \noindent
    Pour toute paire $(S,A)$ où $S$ est un ensemble et $A\subseteq S \times S$,
    il existe un ensemble $X$ et un isomorphisme
    $\makeFunNameIso{f}{(S,A)}{(X,\in)}$.
  \end{quotation}

  \begin{pbQuest}[resume]
  \item Prouver que l'axiome d'anti-fondation est incompatible avec $\AxF$ et
    qu'il implique le schéma d'axiomes précédents.
  \end{pbQuest}

  Une façon naïve de construire un modèle $\bV'$ vérifiant l'axiome
  d'anti-fondation est de considérer l'ensemble des parties étiquetées par des
  graphes~:
  \[\powerset'(x) \defeq \compr{(0,y)}{y\subseteq x}\cup
  \compr{((S,A),y)}
        {\begin{array}{l} (S,A)\text{ un graphe} \\ y \in S \end{array}}\]
  Remarquons que, comme $0 = \vide$ et $(S,A)$ est un couple, on a forcément
  $0 \neq (S,A)$.
  \begin{pbQuest}[resume]
  \item Montrer que pour tout ensemble $x$, $\powerset'(x)$ n'est pas un
    ensemble.
  \end{pbQuest}
  On peut néanmoins construire une opération légèrement plus faible, qui ne
  considère que les graphes existant dans $x$~:
  \[\powerset'(x) \defeq \compr{(0,y)}{y\subseteq x}\cup
  \compr{((S,A),y)}
        {\begin{array}{l}
            S\subseteq x\\
  ...
  \]
  \begin{pbQuest}[resume]
  \item Construire un modèle classe vérifiant l'axiome d'anti-fondation. En
    déduire que $\ZFC$ est compatible avec l'axiome d'anti-fondation.
  \end{pbQuest}
\end{problem}
