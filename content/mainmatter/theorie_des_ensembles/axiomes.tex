...
utilisation pour construire les objets mathématiques que nous connaissons
habituellement.
...
d'habitude ensembles, et si un objet $X$ apparaît dans une collection $\varCC$,
...
distinguer des objets intuitifs associés. Aussi nous appellerons
\og ensemble\fg{} un objet de la théorie $\ZFC$, et \og collection\fg{} un
ensemble au sens intuitif, dans $\unZF$. De même, les objets formels seront
associés à la notion d'appartenance, et les collections à la notion
d'occurrence.
...
plus pratique d'énoncer certaines constructions en termes de classes. Une classe
...
Ce chapitre ne contient pas de problèmes dans les pages d'exercices, car ce
chapitre s'y prête assez mal.
...
\begin{axiom}[Extensionalité]\label{ax.ZF.ext}
  ...
  \index[notion]{Axiome!d'Extensionalité}
\end{axiom}
...
\begin{axiom}[Paire]\label{ax.ZF.pair}
  ...
  a = x \lor a = y\]
  \index[notion]{Axiome!de la Paire}
  \index[notation]{Paire@$\{x,y\}$}
\end{axiom}
...
\begin{notation}
  ...
  \index[notation]{Intersection@$\bigcap$}
\end{notation}
...
\begin{definition}[Relation fonctionnelle]
  ...
  \index[notion]{Relation!Fonctionnelle}
\end{definition}
...
\begin{axiom}[Schéma de remplacement]\label{ax.ZF.repl}
  ...
  \end{multline*}
  \index[notion]{Axiome!de Remplacement}
\end{axiom}
...
une définition simple de la fonction $\Ss$~:
\[\Ss x\defeq x\cup \{x\}\]
...
\begin{axiom}[Infini]\label{ax.ZF.infini}
  ...
  \index[notion]{Axiome!de l'Infini}
\end{axiom}
...
\begin{remark}
  ...
  \begin{multline*}
    ...
    z \in x))
  \end{multline*}
  On reconnaît les deux parties du $\land$~: à gauche, on dit que tout ensemble
...
\begin{definition}[Entiers naturels]
  ...
  \[\bN \defeq \compr{x\in X}{\forall \varPt, \varPt\subseteq X\land
    ...
    \implies x\in \varPt}\]
  \index[notion]{Ensemble!des Entiers Naturels}
  \index[notation]{$\bN$}
\end{definition}
...
\begin{definition}[Fonction]
  ...
  une partie $\funGraph{f}\subseteq X \times Y$ (appelée graphe de $f$) telle
  que le prédicat $(x,y)\in \funGraph{f}$, défini pour $x\in X$ et $y\in Y$, est
  une relation fonctionnelle. On désigne par $\dom(f)$ et $\im(f)$ les ensembles
  ...
  (totales) de $X$ dans $Y$~:
  \[\Funct(X,Y) \defeq
  \compr{f \subseteq X \times Y}{\FunTot(\ogM (x,y)\in f\fgM)}\]
  \index[notion]{Fonction}
  \index[notation]{$\Funct(X,Y)$}
\end{definition}
...
\begin{remark}
  ...
  $(X,Y,\funGraph{f})$. \'Etant donné que nous étudierons presque toujours des
...
\begin{notation}
  ...
  suivante~:
  \[\App(X,Y,f,x,y) \defeq x \in X \land y \in Y \land (x,y) \in \funGraph{f}\]
...
\begin{notation}
  ...
  donner une description fiable du graphe $\funGraph{f}$, ainsi que des
  ensembles $X,Y$ respectivement de départ et d'arrivée. Dans le cas où on
  souhaite uniquement désigner le graphe de la fonction, on pourra écrire
  $x \mapsto t$ directement.
\end{notation}
...
\begin{definition}[Injectivité, surjectivité, bijectivité]
  ...
  \index[notion]{Fonction!Injective}
  \index[notion]{Fonction!Surjective}
  \index[notion]{Fonction!Bijective}
\end{definition}
...
\begin{definition}[Produit cartésien quelconque]
  ...
  \index[notion]{Produit!Cartésien}
\end{definition}
...
\begin{axiom}[Fondation]\label{ax.ZF.AF}
  ...
  \index[notion]{Axiome!de Fondation}
\end{axiom}
...
\begin{definition}[Ensemble transitif]
  ...
  \index[notion]{Ensemble!Transitif}
\end{definition}
...
\begin{proposition}
  ...
  \index[notation]{Clôture!transitive@$\trcl(x)$}
\end{proposition}
...
\begin{lemma}[Effondrement de Mostowski \cite{Mostowski1949}]\label{lem.most}
  ...
  $\makeFunNameIso{\varCol}{(\varCC,\varRR)}{(\varCM,\in)}$, isomorphisme entre
  les deux classes munies d'une relation.
  \index[notion]{Lemme!d'Effondrement de Mostowski}
\end{lemma}
...
Ernst \name{Zermelo} dans \cite{Zermelo1908}, est la
...
\begin{proof}
  ...
  \item soient $n,m\in \bN$, supposons que $n \leq m$ et $m \leq n$. On
    ...
    la deuxième inégalité dans la première, on obtient alors
    \begin{align*}
      n &= (n + k') + k\\
      &= n + (k' + k)\\
      n + 0 &= n + (k' + k)\\
...
\begin{definition}[Congruence additive]
  ...
  \index[notion]{Congruence!dans $\bN\times\bN$}
\end{definition}
...
\begin{definition}[Congruence]
  ...
  \index[notion]{Congruence!dans un Magma}
\end{definition}
...
Ainsi, en notant $\bZ \defeq \quot{(\bN \times \bN)}{\sim}$, on a
...
\og couples\fg{} de la forme $\dfrac{p}{q}$ où plusieurs règles nous permettent
de transformer un couple en un autre. Ces règles peuvent se synthétiser en
...
\begin{proof}
  ...
  $r \times s' = r' \times s$, ce qui nous donne directement le résultat en
...
\cite{Dedekind1872}, nous adoptons une construction adaptée de la complétion
idéale, présentée par exemple dans \cite{AmadioCurien1998}.
...
suivante de paramètre $\varAA$~:
\[\forall k\in \varAA, \exists n \in\bN, n \geq k\]
...
\begin{definition}[Ordre total complet]
  ...
  \index[notion]{Ensemble!Ordonné!Totalement ordonné!Complet}
\end{definition}
...
voulu définir un idéal comme une partie fermée, on aurait au contraire demandé
...
\begin{definition}[Complétion idéale]
  ...
  \index[notion]{Complétion!Idéale}
\end{definition}
...
\begin{remark}
  ...
  l'union fonctionne sans heurt, tandis que l'intersection arbitraire demande
...
\begin{theorem}[Propriété universelle de la complétion idéale]
  ...
  \index[notion]{Propriété!Universelle!de la Complétion Idéale}
\end{theorem}
...
\begin{remark}
  ...
  donne un ensemble dénombrable \cite{BauerHanson2026}).
\end{remark}
...
\begin{exercise}[Versions faibles de l'axiome du choix]
  On considère ici des versions de l'axiome du choix strictement plus faibles,
  qui sont le choix dénombrable (noté $\AxC_{\omega}$) et le choix dépendant
  (noté $\DC$) et dont on donne les énoncés~:
  \begin{quotation}
    $\AxC_{\omega}$ stipule que pour toute famille dénombrable $(X_n)_{n \in \bN}$
    d'ensembles non vides, il existe une suite $(x_n)_{n \in \bN}$ telle que
    pour tout $n \in \bN$, $x_n \in X_n$.

    Soit un ensemble $X$ muni d'une relation binaire $\varRR$ totale,
    c'est-à-dire telle que pour tout $x \in X$ il existe $y \in X$ tel que
    $x\varRR y$. Soit de plus $a \in X$, $\DC$ stipule qu'il existe une
    suite $(x_n)_{n \in \bN} \in X^\bN$ telle que $x_0 = a$ et pour tout
    $n \in \bN$, $x_n \varRR x_{n+1}$.
  \end{quotation}

  Montrer que $\AxC\implies \DC$, puis que $\DC\implies \AxC$.
  \textit{Indication~: On peut considérer, pour une famille dénombrable
    $(X_n)$ d'ensembles, l'ensemble des couples $(n,x)$ avec $x \in X_n$.}
\end{exercise}

\begin{exercise}[Deux notions d'infini]
  Soit $X$ un ensemble. On dit
  \begin{itemize}
  \item que $X$ est $\bN$-infini s'il existe une injection de $\bN$ dans $X$~;
  \item que $X$ est Dedekind-infini s'il existe une partie stricte
    $\varPt\subsetneq X$ telle que $X$ s'injecte dans $\varPt$.
  \end{itemize}
  Montrer que tout ensemble fini n'est infini pour aucune des deux définitions.
  Montrer qu'un ensemble $\bN$-infini est Dedekind-infini. Montrer qu'en
  supposant $\DC$, un ensemble Dedekind-infini est $\bN$-infini.
\end{exercise}

\begin{exercise}[Le schéma de collection]
  On appelle schéma d'axiomes de collection le schéma suivant, pour toute
  formule $\varFF(\vec x,a,b)$~:
  \[\forall \vec x \forall a,\exists b,\qquad
  \forall y\in a,(\exists z, \varFF(\vec x,y,z))
  \implies \exists z, z \in b \land \varFF(\vec x, y,z)\]
  Montrer que ce schéma est équivalent au schéma d'axiomes de remplacement en
  présence des autres axiomes de $\ZF$.
\end{exercise}

\begin{exercise}[Un autre axiome de l'infini]
  On propose le schéma d'axiomes suivant, pour toute formule
  $\varFF(\vec x,a,b)$~:
  \[\FunTot(\varFF(\vec x)) \implies \forall a, \exists b,\qquad
  a\in b \land (\forall c \in b,\exists d \in b, \varFF(\vec x,c,d))\]
  Montrer que ce schéma d'axiomes est équivalent à l'axiome de l'infini en
  présence des autres axiomes de $\ZF$.
\end{exercise}

\begin{exercise}[Affaiblir les axiomes]
  Montrer qu'en présence du schéma d'axiomes de compréhension, on peut affaiblir
  les axiomes de la paire, de l'ensemble des parties et de l'union en remplaçant
  le symbole $\iff$ par un symbole $\Leftarrow$.
\end{exercise}
