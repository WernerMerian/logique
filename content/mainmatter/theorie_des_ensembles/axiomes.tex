...
\begin{remark}
  ...
  utilisons abondamment l'extension par définition (\cref{def.ext.def.rel} et
  ...
  de $\mathrm{ZF}$. Ainsi, dans le \cref{chp.modZFC}, nous considérons des
  sous-modèles (en un sens large) de l'univers $\mathcal U$ mais uniquement vis
...
\begin{remark}
  ...
  \item le schéma de remplacement total, énonçant la même chose que le schéma
    ...
    \[\Funct_{\mathrm{tot}}(R) \defeq \forall x \exists! y, R(x,y)\]
    plutôt que $\Funct$.
  \end{itemize}
...
\item le principe de récurrence, que l'on peut encoder dans les ensembles par le
  ...
  $\forall n, n\in F \implies \Ss n \in F$ alors $F = \mathbb N$.
\end{itemize}
...
nous dit que la collection $\{S^n\;0\mid n \in \mathbb N\}$ est bien elle-même
...
\begin{axiom}[Infini]\label{ax.ZF.infini}
  Il existe un ensemble contenant $\varnothing$ et stable par $\Ss$~:
...
\begin{remark}
  ...
  \begin{multline*}
    \exists x, \quad
    (\forall y, (\forall z, \lnot z \in y) \implies y \in x) \land\\
...
\begin{definition}[Entiers naturels]
  ...
  \varnothing \in Y \land (\forall a, a \in Y \implies \Ss a\in Y) \implies
  x\in Y\}\]
\end{definition}
...
\begin{remark}
  ...
  $f : \Ss X \to Y$~: la fonction est la même mais cette propriété change. Pour
...
\begin{notation}
  ...
  par exemple ajouter une opération agissant sur tout $\mathcal U$ de la façon
  suivante~:
  \[\mathrm{App}(f,x,y) \defeq (x,y) \in \Gamma_f\]
  Cette formule $\mathrm{App}$ définit une fonction partielle qu'on peut rendre
  totale \latinexpr{e.g.} en ajoutant que s'il n'existe aucun $y$ tel que
  $(x,y) \in \Gamma_f$, alors $\mathrm{App}(f,x,z)$ est vraie pour un $z$
  quelconque. On peut ainsi ajouter un symbole de fonction $\mathrm{App}$ et
  considérer que $f(x)$ est une réécriture plus lisible du symboles de
  fonction associé à $\mathrm{App}(f,x)$.
\end{notation}
...
\begin{notation}
  ...
  donner une description fiable du graphe $\Gamma_f$, ainsi que des ensembles
...
\begin{definition}[Injectivité, surjectivité, bijectivité]
  ...
  \[\forall a_0,a_1\in A, f(a_0) = f(a_1)\implies a_0 = a_1\]
...
\begin{exercise}
  ...
  une fonction $f_{\Ss} : \mathbb N \times X \to X$ alors il existe une unique
  fonction $f : \mathbb N \to X$ telle que
  \[\begin{cases}
  f(0) = x_0\\
  \forall n \in \mathbb N, f(\Ss n) = f_{\Ss}(n,f(n))
  \end{cases}\]
\end{exercise}
...
formulations, plutôt que d'isoler un ensemble $C$ intersectant chaque élément
d'un ensemble $X$ en des singletons, on considère directement une fonction qui
va, à un ensemble $X$, associer à chaque $x \in X$ un élément de $x$. On appelle
une telle fonction une fonction de choix, et l'axiome du choix peut se formuler
en un principe d'existence de fonctions de choix.
...
\begin{proof}
  ...
  $f(x) \in x$, $C\cap x$ contient au moins un élément. Donc $C\cap x$ est un
  singleton.
\end{proof}
...
\begin{proof}
  ...
  $\powerset(x)\setminus\{\varnothing\}$ ne contient que des éléments non vides.
...
ensembliste $\mathcal U$ est bien fondée au sens de la \cref{def.wf.ord}, à ceci
...
\begin{remark}
  ...
  $z \in x$ tel que $z \in y$, ce qui revient à dire que
  $x \cap y = \varnothing$. Ainsi, il existe $y$ tel que
  $x \cap y = \varnothing$, mais cette proposition est à considérer comme
  portant avant tout sur la relation $\in$ elle-même.
\end{remark}
...
\begin{lemma}[Effondrement de Mostowski \cite{Mostovski}]\label{lem.most}
  ...
  Alors il existe une unique classe $\mathcal N$ transitive et un unique
  isomorphisme $\phi : \mathcal M\to \mathcal N$ tels que
...
\begin{proof}
  ...
  $\theta : M_1\to M_2$ est un isomorphisme pour la relation $\in$, alors
  $\theta$ est l'identité.

  Par l'absurde, supposons que $\theta$ n'est pas l'identité. Soit alors
  $C = \{x\mid \theta(x)\neq x\}$, non vide par hypothèse. Soit alors
  $x_0 \in C$~: $\trcl(\{x_0\}) \cap C$ est un ensemble non vide (il contient au
  moins $x_0$) et, par
  fondation, il est possible de choisir $x$ minimal, \latinexpr{i.e.}
  $x \in \trcl(\{x_0\})\cap C$ tel que
  $x\cap \trcl(\{x_0\})\cap C = \varnothing$. Alors,
  pour tout $y \in x$, on sait (par minimalité de $x$) que $y\notin C$, donc
  par définition que $\theta(y) = y$. Mais alors, comme $\theta$ est un
  isomorphisme pour $\in$~:
  \begin{align*}
    \theta(x) &= \{ z \in \theta(x)\} \\
    &= \{ \theta(y) \mid \theta(y) \in \theta(x)\} &
    \text{en posant}\; y \defeq \theta^{-1}(z) \\
    &= \{ \theta(y) \mid y \in x \} \\
    &= \{ y\mid y \in x \} & \text{par la remarque précédente}\\
    \theta(x) &= x & \text{par extensionalité}
  \end{align*}
  donc $\theta(x) = x$, ce qui contredit le fait que $x$ soit dans $C$.

%  La classe
%  n'est donc pas vide. On peut donc considérer
%  $x_0 \in C$, puis par l'axiome de
%  fondation, $x \in C$ minimal pour $\in$, en prenant $x$ un élément minimal de
%  $\trcl(x_0)$.
%  on trouve un élément minimal $x\in C$, et soit $y = \theta(x)$. On
...
%  on en déduit que $a\in x$, et puisque $x$ est minimal dans $C$,
%  $\theta(a)=a$,
%  c'est-à-dire $z = a$. Donc $y\subseteq x$. On a donc une absurdité étant
%  donné
%  que $x = \theta(x)$ : ainsi $C$ est vide, donc $\theta$ est l'identité.
  ...
  \[\pi(x,z) \defeq z = \{\pi(y)\mid y \in R^{-1}[x]\}\]
  ...
  par la proposition
  \[z \in \mathcal N \defeq \exists x\in\mathcal M, \pi(x,z)\]
...
\begin{remark}
  ...
  $f : X \to \mathcal U$, qui peut là encore sembler litigieuse. Une fonction
  $X \to \mathcal U$ est en fait un ensemble décrivant une relation
  fonctionnelle totale dont le domaine est $X$, sans restriction sur l'ensemble
  d'arrivée. Ces fonctions $X \to \mathcal U$ constituent une classe, mais
  cela n'est pas un problème lorsqu'on effectue une quantification.
\end{remark}

\begin{remark}
  Dans la conclusion de la démonstration, dire que $\mathcal N = \mathcal N'$
  signifie que $x \in \mathcal N \iff x \in \mathcal N'$, \latinexpr{i.e.} que
  les formules $\varphi$ et $\varphi'$ définissant respectivement $\mathcal N$
  et $\mathcal N'$ sont équivalentes.
\end{remark}
...
Dans la suite du chapitre, nous n'aurons besoin que de $\mathrm Z - \AxF$.
...
\begin{proof}
  ...
  \item $+$ est commutatif~: on montre d'abord par récurrence sur $m$ que pour
    tous $n,m\in\mathbb N$, on a $\Ss n+m = \Ss (n+m)$~:
    \begin{itemize}
...
\begin{definition}[Inégalité]
  On définit la relation $\mathord{\leq}\subseteq \mathbb N \times \mathbb N$
  par~:
...
\begin{proof}
  ...
  et $n + m + 0 = 0 + m + n$, donc $\overline{(n+m,m+n)}=\overline{(0,0)}$,
  d'où le résultat.
\end{proof}
