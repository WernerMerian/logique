...
\begin{definition}[Relation fonctionnelle]
  ...
  qu'une relation $\varRR(x,y)$ est fonctionnelle si pour chaque $x$, il existe
  au plus un $y$ tel que $\varRR(x,y)$. On écrira pour raccourcir
  \[\Funct(\varRR) \defeq \forall x\forall y\forall z,
  \varRR(x,y)\land \varRR(x,z)\implies y = z\]

  Pour une relation fonctionnelle $\varRR(x,y)$, on définit la collection du
  domaine de $\varRR$~:
  \[\dom(\varRR)(x)\defeq \exists y, \varRR(x,y)\]
  et la collection de l'image de $\varRR$~:
  \[\im(\varRR)(y)\defeq \exists x, \varRR(x,y)\]
\end{definition}
...
\begin{remark}
  ...
  \item le schéma de remplacement total, énonçant la même chose que le schéma
    ...
    \[\FunTot(\varRR) \defeq \forall x \exists! y, \varRR(x,y)\]
    plutôt que $\Funct$.
  \end{itemize}
...
%Comme nous l'avons fait remarquer dans le \cref{chp.logpred}, certaines
%conventions considèrent qu'il existe un élément dans l'univers de discours.
%Comme nous ne souhaitons pas invoquer explicitement cette convention, nous
%ajoutons en tant qu'axiome qu'il existe bien un objet dans notre univers.
...
\item le principe de récurrence, que l'on peut encoder dans les ensembles par le
  fait que si $F\subseteq\bN$, $0 \in F$ et
  $\forall n, n\in F \implies \Ss n \in F$ alors $F = \bN$.
\end{itemize}
...
\begin{definition}[Entiers naturels]
  ...
  $\bN$ comme
  \[\bN \defeq \compr{x\in X}{\forall Y, Y\subseteq X\land
  \vide \in Y \land (\forall a, a \in Y \implies \Ss a\in Y) \implies
  x\in Y}\]
\end{definition}
...
\begin{theorem}[Récurrence]
  Soit $F$ une partie de $\bN$ telle que $\vide \in F$ et telle que
  $\forall n, n \in F \implies \Ss n \in F$, alors $F = \bN$.
\end{theorem}

\begin{proof}
  Comme $F$ est une partie de $\bN$, il nous suffit de montrer que
  $\bN \subseteq F$. Par transitivité de l'inclusion,
  $F\subseteq X$ pour $X$ l'ensemble à partir duquel $\bN$ a été défini.
  On sait de plus que
  \[x \in \bN \implies \forall Y, Y\subseteq X \land
  \vide \in Y \land (\forall a, a \in Y \implies \Ss a \in Y) \implies
  x\in Y\]
  d'où, en spécialisant $Y$ en $F$, et sachant que $\vide\in F$ et
  $\forall n, n \in F \implies \Ss n \in F$, il vient que
  \[x\in \bN \implies x \in F\] ce qui est exactement
  $\bN\subseteq F$, d'où le résultat.
\end{proof}
...
\begin{definition}[Fonction]
  Soient $X,Y$ deux ensembles. Une fonction partielle $f : X \partialto Y$ est
  ...
  $f$, on notera $f : X \to Y$, et on appellera simplement \og fonction\fg
...
\begin{remark}
  ...
  formalisme considérant qu'une fonction $f : X \to Y$ est un triplet
...
%  fonction dans notre langage, dont l'argument est un élément de $X$. Cela
%  n'est
...
%  \begin{itemize}
%  \item si $\varphi = R(f(x))$ où $R$ est une proposition atomique (une
%  relation
...
%  On peut en particulier voir nos ajouts de notations tels que $\varnothing$,
%  $\bigcup$ ou $\{ x \in X \mid \varphi(x)\}$ comme de tels abus de notation.
...
\begin{notation}
  Dans le cas d'une fonction $f : X \to Y$, il est naturel d'ajouter un
  symbole de fonction correspondant, pour externaliser l'objet ensembliste
  interne (et ainsi parler dans notre langage de la fonction $f$ associée).
  Il existe plusieurs façons de traiter formallement cet ajout. On peut
  ...
  totale \latinexpr{e.g.} en ajoutant que s'il n'existe aucun $y$ correspodnant
...
\begin{definition}[Injectivité, surjectivité, bijectivité]
  Une fonction $f : X \to Y$ est dite injective si chaque élément $y\in Y$ a au
  plus un antécédent, c'est-à-dire si la proposition suivante est vérifiée :
  \[\forall x_0,x_1\in X, f(x_0) = f(x_1)\implies x_0 = x_1\]

  Une fonction $f : X \to Y$ est dite surjective si tout élément $y\in Y$ a au
  moins un antécédent, c'est-à-dire si la proposition suivante est vérifiée :
  \[\forall y\in Y, \exists x \in X, f(x) = y\]

  Une fonction $f : X \to Y$ est dite bijective si elle est à la fois injective
  et surjective.
\end{definition}
...
\begin{exercise}
  ...
  une fonction $f_{\Ss} : \bN \times X \to X$ alors il existe une unique
  fonction $f : \bN \to X$ telle que
  \[\begin{cases}
  f(0) = x_0\\
...
\begin{proof}
  ...
  pour tout $x\in X$. Soit alors $\varCh = \im(f)$. Comme $f(x)\in x$ et tous
...
\begin{proposition}
  ...
  \exists f : \powerset (x) \setminus \{\vide\} \to x,
...
\begin{definition}[Produit cartésien quelconque]
  ...
  \[\prod X \defeq \compr{f : X \to \bigcup X}{\forall x \in X, f(x)\in x}\]

  Si $\{X_i\}_{i\in I}$ est une famille d'ensembles (c'est-à-dire une fonction
  $g : i \mapsto \unZF$, \textit{i.e.} une relation fonctionnelle entre un
  ensemble $I$ et des ensembles quelconques) alors on considère que
  \[\prod_{i\in I} X_i \defeq
  \compr{ f : I \to \bigcup_{i\in I} X_i}{\forall i \in I, f(i) \in X_i}\]
\end{definition}
...
\begin{lemma}[Effondrement de Mostowski \cite{Mostovski}]\label{lem.most}
  ...
  isomorphisme $\varCol : \varCC\to \varCM$ tels que
  $(\varCC,\varRR) \overset{\varCol}{\cong} (\varCM,\in)$.
\end{lemma}
...
\begin{remark}
  ...
  isomorphisme $\varFunCl: (\varCC,\varRR)\cong (\varCC',\varRR')$ est une
  formule $\varFunCl(x,y)$ à deux variables libres telle que, pour une
  ...
  \item $\forall x,\forall x',\forall y, \forall y', \varFP(x)\land\varFP(x')\land
...
\begin{proof}
  ...
  et que $\varFunCl : \varCC_1\to \varCC_2$ est un isomorphisme pour la relation
  $\in$, alors $\varFunCl$ est l'identité.
...
\begin{remark}
  ...
  \begin{multline*}
    ...
    \land \exists f : X \to \unZF, \\(\forall z\in X,
  ...
  $f : X \to \unZF$, qui peut là encore sembler litigieuse. Une fonction
  $X \to \unZF$ est en fait un ensemble décrivant une relation
...
$\ZF - \AxF$ par exemple. De plus, la théorie $\Zermelo$, de Zermelo, est la
...
\begin{definition}[Addition, multiplication]
  On définit la fonction $+ : \bN \times \bN \to \bN$ et la
  fonction $\times : \bN \times \bN \to \bN$ par récursion
...
\begin{definition}
  Soit $(\varMon,\star,e)$ un monoïde commutatif. On définit sur
  $\varMon\times\varMon$ muni des opérations coordonnée par coordonnée la
  congruence suivante~:
  \[(m,m') \sim (a,a') \defeq \exists k \in M,
  m \star a' \star k = m' \star a \star k\]

  On appelle symétrisé de $(\varMon,\star,e)$ le groupe abélien obtenu à partir
  de $\varMon\times \varMon$ par quotient par $\sim$.
\end{definition}
...
\begin{proof}
  ...
  \[\clas{(m,m')}\star \clas{(m',m)} =
  \clas{(m\star m',m'\star m)}\]
...
\begin{exercise}
  Soit $\varMon$ un monoïde commutatif et $\varMon^\dagger$ son symétrisé.
  Montrer que la fonction
  \[
  \makeFun{\iota}{\varMon}{\varMon^\dagger}{m}{\clas{(m,e)}}
  \]
...
\begin{exercise}
  ...
  $f : \varMon \to \varGrp$ il existe un unique
  morphisme de groupe $\relev{f} : \varMon^\dagger \to \varGrp$ tel que
  ...
  \begin{center}
    \begin{tikzcd}
      \varMon \ar[r,"f"]\ar[d,"\iota"] & \varGrp\\
      \varMon^\dagger \ar[ur,"\hat f",dashed]
    \end{tikzcd}
  \end{center}
\end{exercise}
...
\begin{definition}[Complétion idéale]
  ...
  \item $Y\neq \vide$
  \item $Y\neq X$
  \item si $\opJn Y$ existe, alors $\opJn Y \in Y$.
  \end{itemize}
...
\begin{remark}
  ...
  les intervalles ouverts, c'est-à-dire de la forme $\compr{x \in X}{y < x < z}$
  pour $y,z\in X$.
\end{remark}
...
\begin{proof}
  ...
  considère comme borne supérieure de $A$ l'ensemble $B \cup\{\opJn B\}$,
  sinon on considère simplement $B$. On note cet ensemble $C$. On vérifie que
  $C\in \ideal(X)$~:
  \begin{itemize}
  \item soient $x,y\in X$ tels que $x < y$ et $y\in C$. Si $y = \opJn B$,
    alors comme $x < \opJn B$, on en déduit qu'il existe $z \in B$ tel que
  ...
  \item comme $A$ est majorée, on trouve $Y \in \ideal(X)$ tel que pour tout
    ...
    $C\neq X$.
  \item si $\opJn C$ existe, alors par construction c'est $\opJn B$ qui est
    un élément de $C$.
  \end{itemize}
  Donc $C\in\ideal(X)$.
  ...
  \item soit $Z\in\ideal(X)$ tel que pour tout $Y\in A, Y\subseteq Z$. Alors
    $B\subseteq Z$, il nous faut donc montrer que si $\opJn B$ existe alors
    $\opJn B \in Z$. Supposons que cet élément existe et qu'il n'appartient
    pas à $Z$. Alors il est un majorant de $Z$. Soit $z$ un autre majorant de
    $Z$, alors si $z < \opJn B$, on en déduit qu'il existe $Y\in A$ tel que
    $z\in Y$, donc $z \in Z$, donc $z$ est le maximum de $Z$, qui est une borne
    supérieure~: on en déduit que $\opJn B = z$, donc que $\opJn B \in Z$,
    ce qui est absurde. Ainsi $\opJn B \in Z$, donc $C\subseteq Z$.
  \end{itemize}
...
\begin{proposition}
  ...
  \[\makeFun{\iota}{X}{\ideal(X)}{x}{\compr{y \in X}{y \leq x}}\]
...
\begin{proof}
  ...
  \item $\opJn\iota(x) = x$ et $x\in\iota(x)$ donc la borne supérieure  de
...
\begin{theorem}[Propriété universelle de la complétion idéale]
  ...
  unique fonction $\relev{f} : \ideal(X) \to Y$ qui préserve les bornes
  supérieures et les bornes inférieures et telle que
  $f = \relev{f} \circ \iota$.
\end{theorem}
...
\begin{proof}
  ...
  \item Montrons que notre candidat $\relev{f}$ est une fonction croissante qui
    ...
    \item Tout d'abord, si
      ...
      on en déduit que $\opJn f(Y) \leq \opJn f(Z)$, d'où
      $\relev{f}(Y)\leq \relev{f}(Z)$.
    ...
    \item Soit $A\subseteq \ideal(X)$ admettant une borne supérieure, qu'on note
      ...
      donne donc
      \[\relev{f}(a) = \opJn_{x \in a} f(x)\]
      on veut montrer que cette valeur est la borne supérieure de
      $\relev{f}(A)$. Comme la borne supérieure est croissante vis à vis de
      l'inclusion, on sait déjà que $\relev{f}(a)$ est un majorant de
      $\relev{f}(A)$.
      Soit $b$ un majorant de $\relev{f}(A)$, cela signifie que pour tout
      $Y\in A, \opJn_{x \in Y} f(x) \leq b$, donc pour tout
      $x \in \bigcup A, f(x) \leq b$. Il nous reste donc, si $\opJn a$ existe,
      à montrer que $f(\opJn a) \leq b$. Mais cela est dû au fait que $f$
...
On peut donc construire l'ensemble ordonné $(\bR,\leq)$, mais nous
...
$\bQ$. Cependant, il nous est ici nécessaire de supposer un résultat
...
\begin{proof}
  ...
  \item soient $x,x'\in X$ tels que $x\leq x'$ et $x\in Y + Z$. Par définition,
    ...
    inférieur à un élément de $Y + Z$. En effet, si $x \leq \opJn(Y + Z)$ et
    $x > a$ pour tout $a \in Y + Z$, alors $x$ est un majorant de $Y + Z$ donc
    $\opJn(Y + Z) \leq x$, donc $x = \opJn (Y + Z)$.
  ...
  \item pour tous $Y,Z,A\in\ideal(X)$, on a
    \begin{align*}
      Y + (Z + A) &= \compr{y + (z + a)}{y \in Y, z \in Z, a \in A}\\
      &= \compr{(y + z) + a}{y \in Y, z \in Z, a \in A}\\
      &= (Y + Z) + A
    \end{align*}
  \item en notant $0$ le neutre de $X$, on a pour tout $Y \in \ideal(X)$~:
    \begin{align*}
      Y + \iota(0) &= \compr{ y + 0}{y \in Y}\\
      &= \compr{y}{y \in Y}\\
      &= Y
    \end{align*}
  \item pour tout $Y\in \ideal(X)$, on définit
    \[-Y \defeq X \setminus \compr{- y}{y \in Y}\]
    ...
    \item comme $Y \neq X$, on trouve $y\notin Y$, et alors $-y \in -Y$, donc
      $-Y\neq \vide$.
    \item comme $Y \neq \vide$, on trouve $y\in Y$, et alors
      $-y \notin -Y$, donc $-Y\neq X$.
  ...
  \item enfin, $+$ est commutatif~: pour tout $Y,Z \in \ideal(X)$, on a~:
    \begin{align*}
      Y + Z &= \compr{y + z}{y \in Y, z \in Z}\\
      &= \compr{z + y}{z \in Z, y \in Y}\\
      &= Z + Y
    \end{align*}
  \end{itemize}
...
On peut donc utiliser ce fait pour munir $\bR$ de la loi $+$. Cependant,
pour la loi $\times$, cela pose un problème~: $\times$ n'est pas compatible avec
$\leq$ dans $\bQ$. Pour contourner ce problème, on peut définir d'abord
$\times$ sur $\bR_+^\star$~: la loi $\times$ est en effet compatible
avec $\leq$ sur $\bQ_+^\star$ et donne un groupe.

\begin{definition}[Ensemble des réels]
  On définit $(\bR,+,\leq)$ comme la complétion idéale de $\bQ$.
  On définit de plus $(\bR_+^\star,\times, \leq)$ comme la complétion
  idéale de $(\bQ_+^\star,\times)$.
\end{definition}

\begin{remark}
  Ces constructions peuvent être définies grâce au fait que $\bQ$ et
  $\bQ_+^\star$ sont non majorés.
\end{remark}

\begin{definition}[Produit de réels]
  On définit maintenant le produit de nombres réels par
  \[x \times y \defeq \begin{cases}
  0 \text{ si } x = 0 \lor y = 0 \\
  x \times_{\bQ_+^\star} y \text{ si } x > 0 \land y > 0\\
  - (x \times_{\bQ_+^\star} (-y)) \text{ si } x > 0 \land y < 0\\
  - ((-x) \times_{\bQ_+^\star} y) \text{ si } x < 0 \land y > 0\\
  - ((-x) \times_{\bQ_+^\star} (-y)) \text{ si } x < 0 \land y < 0
  \end{cases}\]
...
\begin{remark}
  ...
  inversant $\times$ dans $\bN^\star$), on peut directement considérer le
  demi-anneau $(\bR,+,\times,\leq)$ complet, puis le symétriser pour la
  ...
  le cas d'un monoïde régulier, puisque $(\bQ_+,+)$ n'a pas une structure
  de groupe.
\end{remark}
