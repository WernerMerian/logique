...
\begin{proof}
  ...
  \item On sait que pour tout $\alpha \in Y$, $\beta(\alpha) = \beta$, et
    que $X_\alpha^{\beta(\alpha)}$ est stationnaire, donc la famille
...
$\lambda$ de cofinalité indénombrable, alors celle-ci est vraie aussi en ce
cardinal.
...
\begin{definition}[Suite cofinale continue]
  ...
  tout $\gamma \in\Lim_\alpha$, $\sup_{\beta < \gamma} \alpha_\beta = \alpha_\gamma$.
\end{definition}
...
\begin{proof}
  ...
  donc à valeur dans $\aleph_{\alpha_\beta + 1} < \lambda$. Montrons maintenant que
...
\begin{proof}
  ...
  \[A_\beta \defeq \{\gamma \mid f(\beta) < \aleph_{\alpha_\gamma}\}\]
...
\begin{proof}
  ...
  supposons que chacun de ces ensembles $h^{-1}(\{(T,\overline\gamma)\})$ est
