...
d'abord les clubs, qui sont une notion liée à la topologie des ordinaux, que
nous devons donc aborder en premier lieu.
...
\begin{definition}[Topologie de l'ordre]
  ...
  $a,b\in X\cup\{\mInf,\pInf\}$, le segment ouvert
  ...
  ouvert de la forme $\intOO{\mInf}{a}$ signifie $\compr{x}{x < a}$ (les
  autres cas sont définis de façon analogue).
  \index[notion]{Topologie!de l'Ordre}
\end{definition}
...
\begin{example}
  ...
  puisqu'elle peut s'écrire $\intOO{\mInf}{\Ow}$. De plus, $\Ow + 1$ est aussi
  un ouvert~: il vaut $\intOO{\mInf}{\Ow+1}$ mais il est aussi un fermé,
  puisque son complémentaire est $\intOO{\Ow + 1}{\pInf}$.
\end{example}
...
\begin{proof}
  $\varOb$ est simplement $\intOO{\mInf}{\varOb}$.
\end{proof}
...
par le fait qu'il est clos par borne supérieure~: c'est un sens très proche du
...
\begin{proposition}
  ...
  et seulement si pour toute partie $\varPt\subseteq \varCl$, si
  $\sup \varPt \in \varOa$ alors $\sup \varPt \in \varCl$.
\end{proposition}

\begin{proof}
  Supposons que $\varCl$ est un fermé et soit $\varPt\subseteq \varCl$ tel que
  $\sup \varPt \in \varOa$. On veut montrer que $\sup \varPt$ est dans
  l'adhérence de $\varCl$ (qui est $\varCl$ lui-même par hypothèse). Soit
  $\varOp$ un ouvert contenant $\sup \varPt$, sans perte de généralité (quitte à
  prendre une plus petite partie), on peut considérer que
  $\varOp = \intOO{\varOb}{\varOc}$ où
  $\varOb < \sup \varPt < \varOc$, mais alors par définition de $\sup \varPt$,
  on trouve $\varOd \in \varPt$ tel que $\varOb < \varOd < \sup \varPt$, donc
  $\varOp \cap \varPt \neq\vide$, ce qui signifie que
  $\sup \varPt \in \adher{\varPt}$, donc que $\sup\varPt \in \varCl$.

  Réciproquement, supposons que pour tout $\varPt\subseteq \varCl$, si
  $\sup \varPt \in \varOa$ alors $\sup \varPt \in \varCl$. On veut montrer que
  $\varCl$ est fermé. Pour cela, on va montrer que tout point adhérent de
  $\varPt$ est dans $\varPt$. Soit $x \in \adher \varPt$~: si $x$ est un
  successeur ou $0$ alors il est un point isolé, donc il appartient directement
  à $\varPt$ en considérant le fait que $\{x\}\cap \varPt$ est non vide. On
  suppose donc que $x$ est une limite, et on construit une famille $(x_\varOb)$
  d'éléments de $\varPt$ dont le sup est $x$~:
  \begin{itemize}
  \item on définit $x_0$ comme un élément de $\intOO{\mInf}{\Ss x}\cap \varPt$,
    qui est non vide car $x$ est adhérent à $\varPt$.
  \item si on a défini $x_\varOa$, alors $x_{\varOa + 1}$ est un élément de
    $\intOO{x_\varOa}{\Ss x}\cap \varPt$.
  \item si $\varOl$ est limite, alors
    $x_{\varOl}=\sup_{\varOb < \varOl}x_\varOb$, qui est un élément de $\varPt$ car
    tous les $x_\varOb$ sont inférieurs à $x$, donc $\sup x_\varOb\in \varOa$,
    et en utilisant notre hypothèse sur $\varPt$.
  \end{itemize}
  On a donc créé une suite strictement croissante de $\Ord$ dans
  $\intOO{\mInf}{\Ss x}\cap \varPt$, ce qui est impossible. Notre famille finit
  donc à un certain $\varOc$, et $\sup_{\varOb < \varOc} x_\varOb = x$, donc en
  utilisant notre hypothèse sur $\varPt$ on en déduit que $x\in \varPt$.
\end{proof}
...
\begin{proof}
  ...
  le recouvrement
  \[\varRecouv \defeq \{\intOO{\mInf}{\varOb}\}_{\varOb \in \varOa}\]
  Pour toute partie $F\subfin\varRecouv$, on trouve un plus grand $\varOb$ tel
  que $\bigcup F = \intOO{\mInf}{\varOb}$, donc $\varOb + 1 \notin\bigcup F$.
...
l'occurrence, ce n'est pas le cas dans le cas général, mais c'est le cas à
...
\begin{proof}
  ...
  images $\varPt$ et $\varPtB$, respectivement, de $(\varOa_{2n})$ et
  $(\varOa_{2n+1})$, sont cofinales dans $\varOa$ et disjointes. De plus,
  $\varPt$ et $\varPtB$ sont des parties fermées de $\varOa$. On a donc trouvé
  deux parties disjointes de $\varOa$ qui sont des clubs.
...
\begin{definition}[Filtre des clubs]
  ...
  \index[notion]{Filtre!des Clubs}
\end{definition}

\begin{proof}
  On a vérifié que l'intersection de deux clubs est encore un club, donc
...
\begin{definition}[Parties stationnaires, non stationnaires]
  ...
  \index[notion]{Partie!Stationnaire}
\end{definition}
...
\begin{definition}[Point de fermeture]
  Soit $\varOa$ un ordinal et $\makeFunName{f}{\varOa}{\varOa}$. On dit qu'un
  point $\varOb \in \varOa$ est un point de fermeture de $f$ lorsque
  \[\forall \varOc < \varOb, f(\varOc) < \varOb\]
  ou, de façon équivalente, lorsque $f(\varOb) \subseteq \varOb$ en considérant
  l'image directe pour $f(\varOb)$ (et non l'image de $\varOb$). On notera
  $\Clos(f)$ l'ensemble des points de fermeture de $f$.
  \index[notion]{Point!de Fermeture}
\end{definition}

\begin{proposition}
  Soit $\varCk$ un ordinal régulier indénombrable et
  $\makeFunName{f}{\varCk}{\varCk}$, alors $\Clos(f)$ est un club de $\varCk$.
\end{proposition}

\begin{proof}
  On vérifie que $\Clos(f)$ est un club~:
  \begin{itemize}
  \item soit $\varPt\subseteq \Clos(f)$, montrons que
    $\sup \varPt \in \Clos(f)$. Si $\sup \varPt \in \varPt$ le résultat est
    évident. On suppose donc que $\sup \varPt$ est un ordinal limite. Soit
    $\varOb \in \sup \varPt$, comme $\sup \varPt$ est limite, on trouve
    $\varOa \in \varPt$ tel que $\varOb + 1 \leq \varOa$, et comme
    $\varOb < \varOa$ et $\varOa \in \Clos(f)$, on en déduit que
    $f(\varOb) < \varOa < \sup \varPt$, donc pour tout
    $\varOb \in \sup \varPt, f(\varOb) < \sup \varPt$, d'où
    $\sup\varPt\in \Clos(f)$.
...
en prenant $\makeFunName{f}{\Cal_{\Ow_1}}{\Cal_{\Ow_1}}$ tel que
...
\begin{definition}[Ensemble des limites]
  ...
  \[\Lim_\varOa \defeq
  \compr{\varOb \in \varOa}{\bigcup \varOb = \varOb}\]
\end{definition}
...
\begin{proof}
  ...
  \begin{itemize}
  \item Soit $\varPt\subseteq \Lim_\varCk$ tel que $\sup \varPt \in \varCk$,
    alors
    \begin{align*}
      \bigcup (\sup \varPt) &= \bigcup_{\varOa \in \sup \varPt} \varOa \\
      &= \bigcup_{\varOa \in \bigcup \varPt} \varOa \\
      &= \bigcup_{\varOa \in \varPt} \left(\bigcup \varOa\right)\\
      &= \bigcup_{\varOa \in \varPt} \varOa\\
      &= \sup \varPt
    \end{align*}
    donc $\sup \varPt \in \Lim_\varCk$, donc $\Lim_\varCk$ est fermé.
...
\begin{proof}
  ...
  $\varClub$ un club de $\varCCl$. On définit
  $\makeFunName{f}{\varCCl}{\varClub}$ par récursion transfinie~:
  \begin{itemize}
...
\begin{proof}
  ...
  un club. Prenons $\varCCl$ minimal, et soit $(\varPt_\varOa)$ une énumération
  de $\varFam$~:
  \[\varFam \defeq \compr{\varPt_\varOa}{\varOa < \varCCl}\]
  On souhaite montrer que $\bigcap \varFam$ est un club. Pour cela, on
  définit d'abord la famille suivante~:
  \[\varPtB_\varOa \defeq \bigcup_{\varOb < \varOa} \varPt_\varOb\]
  On remarque que
  $\displaystyle\bigcap_{\varOa < \varCCl} \varPtB_\varOa = \bigcap\varFam$, donc il
  nous suffit de montrer que cette intersection est un club. De plus, on sait
  que chaque $\varPtB_\varOa$ est un club, car il est l'intersection
  d'une famille de cardinal strictement inférieur à $\varCCl$ de clubs (et
  $\varCCl$ est supposé minimal). Comme chaque $\varPtB_\varOa$ est un fermé,
  $\bigcap \varPtB_\varOa$ aussi. Il nous reste donc à montrer que
  $\bigcap \varPtB_\varOa$ est cofinal.
  ...
  \begin{itemize}
  \item on sait que $\varPtB_0 = \varPt_0$ est cofinal, donc on peut trouver
    $\varOb_0 \geq \varOb$ un élément de $\varPtB_0$.
  \item si on a défini $\varOb_\varOc$, alors on peut utiliser le fait que
    $\varPtB_{\varOc + 1}$ est un club pour trouver
    $\varOb_{\varOc + 1}\in \varPtB_{\varOc+ 1}$ supérieur à $\varOb_\varOc$.
  \item si on a défini $\varOb_\varOd$ pour tout $\varOd < \varOc$ et $\varOc$
    ...
    sait que $\sup_{\varOd < \varOc} \varOb_\varOd < \varCk$, donc on peut prendre
    $\varOb_\varOc \in \varPtB_\varOc$ supérieur à ce supremum.
  \end{itemize}
  On considère maintenant
  \[\varOc \defeq \sup_{\varOd < \varCCl} \varOb_\varOd\]
  Comme $\varCk$ est régulier et $\varCCl < \varCk$, on sait que
  $\varOc < \varCk$, et cet élément est dans tous les $\varPtB_\varOa$. En effet,
  il est la borne supérieure d'une famille d'éléments de $\varPtB_\varOa$, donc
  comme $\varPtB_\varOa$ est fermé, $\varOc \in \varPtB_\varOa$.

  Ainsi $\displaystyle\bigcap_{\varOa < \varCCl} \varPtB_\varOa$ est un club, ce qui
  est absurde. On en déduit que l'intersection d'une famille de cardinal
...
car deux ensembles stationnaires disjoints dans $\Cal_2$ peuvent sembler peu. On
va ici simplement donner un résultat légèrement plus fort.
...
\begin{proof}
  Pour commencer, on pose $\makeFunName{f}{\Ow_1}{\intFF{0}{1}\setminus \bQ}$
  une injection (qui existe par un simple argument de cardinalité). On va
  ...
  \[\begin{cases}
  \varSt_{\varwd\wdPt 0} \defeq \compr{\varOa \in \varSt_{\varwd}}{f(\varOa) < I}\\
  ...
  suite $\makeFunName{g}{\Ow}{\intB}$ de telle sorte que $\varSt_{g\restr n}$
  contienne toujours un club. Ce faisant,
  $\displaystyle\bigcap_{n\in \bN} \varSt_{g\restr n}$
...
\begin{definition}[Fonction régressive]
  ...
  $\makeFunName{f}{\varSt}{\varCk}$. On dit que $f$ est régressive si la
  propriété suivante est vérifiée~:
  \[\forall \varOa \in \varSt\setminus\{0\}, f(\varOa) < \varOa\]
  \index[notion]{Fonction!Régressive}
\end{definition}
...
\begin{lemma}[Fodor, version faible]\label{lem.fodor.faible}
  ...
  un ensemble stationnaire. Soit $\makeFunName{f}{\varSt}{\varCCl}$ régressive,
  alors $f$ est constante sur un ensemble de taille $\varCCl$, c'est-à-dire
  qu'il existe $\varStT\subseteq \varSt$ et $\varOa \in \varCCl$ tels que
  \[\begin{cases}
...
matrices d'Ulam. Celles-ci vont nous servir pour prouver le théorème sur les
...
\begin{definition}[Matrices d'Ulam \cite{Ulam1930}]
  ...
  $\varOa < \varCCl$, on se donne une injection
  $\makeFunName{f_{\varOa}}{\varOa}{\varCk}$.
...
\begin{proof}
  ...
  \item Soit $\varOb < \varCk$ et $\varOa, \varOc \in\varCCl$. Si
    ...
    $\varOa = \varOc$.
  \item Soit $\varOx \in \varCCl \setminus (\varOa + 1)$.
    $\varOx > \varOa$, donc $f_\varOx$ est définie en $\varOa$, et
    $\makeFunName{f_\varOx}{\varOx}{\varCk}$, ce qui signifie que
  ...
  \item On utilise maintenant le \cref{lem.fodor.faible}~: la fonction
    $g : \varOa \longmapsto \varOb(\varOa)$ est régressive sur
...
\begin{definition}[Intersection diagonale]
  ...
  \index[notion]{Intersection!Diagonale}
\end{definition}
...
\begin{proof}
  ...
  \begin{itemize}
  \item Soit $\varPt\subseteq D$ borné, montrons que $\sup \varPt \in D$. Soit
    $\varOb < \sup \varPt$, par définition pour tout $\varOd$ tel que
    $\varOb<\varOd<\sup \varPt$ et $\varOd \in \varPt$,
    $\varOd \in \varClub_\varOb$. On en déduit, comme $\sup \varPt$ est une
    limite d'éléments de $\varPt$ supérieurs à $\varOb$, que $\sup \varPt$ est
    une limite d'éléments de $\varClub_\varOb$, donc un élément de
    $\varClub_\varOb$ puisque $\varClub_\varOb$ est un club (en particulier un
    fermé). Donc $\sup \varPt\in \varClub_\varOb$ pour tout
    $\varOb < \sup \varPt$, donc $\sup \varPt \in D$.
  \item On utilise un argument développé dans la preuve de la
    ...
    $\varOa \in D$ et $\varOa > \varOb$.\qedhere
  \end{itemize}
\end{proof}
...
\begin{lemma}[Fodor \cite{Fodor1956}]
  Soit $\varCk$ un cardinal régulier indénombrable, $\varSt \subseteq \varCk$
  une partie stationnaire et $\makeFunName{f}{\varSt}{\varCk}$ une fonction
  régressive. Alors $f$ est constante sur un ensemble stationnaire.
  \index[notion]{Lemme!de Fodor}
\end{lemma}
...
\begin{definition}[Suite cofinale continue]
  ...
  \index[notion]{Suite!Cofinale continue}
\end{definition}
...
On aura aussi besoin de la notion de famille éventuellement différente, qui
peut se voir comme une généralisation de celle de famille presque disjointe pour
un cardinal autre que $\Ow$.

\begin{definition}[Famille éventuellement différente]
  ...
  \index[notion]{Famille!\'Eventuellement différente}
\end{definition}

\begin{notation}
  Dans la suite de cette sous-section, on pose $\varCCl$ un cardinal singulier
...
\begin{proof}
  ...
  Pour chaque $\varPt\subseteq \varCCl$, on définit la fonction
  \[\makeFun{f_{\varPt}}{\varCk}{\varCCl}{\varOb}
            {\iota_\varOb(\varPt\cap\Cal_{\varOa_\varOb})}\]
  Remarquons que cette fonction est bien définie, car
  $\varPt\cap \Cal_{\varOa_\varOb}$ est une partie de $\Cal_{\varOa_\varOb}$ et elle est
  donc à valeur dans $\Cal_{\varOa_\varOb + 1} < \varCCl$. Montrons maintenant que
  $\{f_{\varPt}\}_{\varPt\subseteq \varCCl}$ est une famille éventuellement
  différente~:

  Soient $\varPt,\varPtB\subseteq \varCCl$ tels que $\varPt\neq \varPtB$. On
  veut montrer que $\compr{\varOx}{f_{\varPt}(\varOx) = f_{\varPtB}(\varOx)}$ est
  de cardinal strictement inférieur à $\varCk$. Comme $\varPt\neq \varPtB$, on
  trouve $\varOd = \min \varPt \diffsym \varPtB$. La suite
  $(\varOa_\varOb)$ est cofinale dans $\varOa$, donc on peut trouver
  $\varOc\in \varCk$ tel que $\Cal_{\varOa_\varOc} > \varOd$. Comme $(\varOa_\varOb)$
  est strictement croissante, on en déduit que pour tout
  $\varOc' > \varOc, \Cal_{\varOc'} > \varOd$.

  En remarquant que
  $(\varPt\diffsym \varPtB) \cap \Cal_{\varOa_\varOb} =
  (\varPt\cap \Cal_{\varOa_\varOb})\diffsym (\varPtB\cap \Cal_{\varOa_\varOb})$,
  si $\varOb \in \varCk$ est tel que $\varOd \in \Cal_{\varOa_\varOb}$, alors
  $\varOd\in(\varPt\cap \Cal_{\varOa_\varOb})\diffsym(\varPtB\cap \Cal_{\varOa_\varOb})$
  donc $\varPt\cap \Cal_{\varOa_\varOb}\neq \varPtB\cap \Cal_{\varOa_\varOb}$,
  d'où par injectivité de $\iota_\varOb$,
  $f_{\varPt}(\varOb) \neq f_{\varPtB}(\varOb)$.

  On voit donc que l'ensemble
  $\compr{\varOx}{f_{\varPt}(\varOx) = f_{\varPtB}(\varOx)}$ est borné par
  $\varOc$ tel que $\varOd \in \Cal_\varOc$, c'est donc un ensemble de cardinal
  inférieur à $\varOc$ et $\varOc < \varCk$, donc $\{f_{\varPt}\}$ est une
...
\begin{definition}
  Pour deux parties $\varPt,\varPtB\subseteq \varCCl$, on définit la relation
  $\varRR$ par
  \[\varPt \varRR \varPtB\defeq
  \compr{\varOc}{f_{\varPt}(\varOc) < f_{\varPtB}(\varOc)}
  \text{ est stationnaire dans }\varCk\]
\end{definition}

\begin{property}
  Soient $\varPt\neq \varPtB$ deux parties de $\varCCl$. Alors
  $\varPt\varRR \varPtB$ ou $\varPtB\varRR \varPt$.
\end{property}

\begin{proof}
  Comme $\{f_{\varPt}\}$ est une famille éventuellement différente, on peut
  trouver $\varCk' < \varCk$ tel que
  $\compr{\varOx}{f_{\varPt}(\varOx) =f_{\varPtB}(\varOx)}\subseteq\varCk'$
  donc $\compr{\varOx}{f_{\varPt}(\varOx)\neq f_{\varPtB}(\varOx)}$ contient un
  club (la partie $\varCk\setminus \varCk'$). De plus on a l'égalité
  \[\compr{\varOx}{f_{\varPt}(\varOx) \neq f_{\varPtB}(\varOx)} =
  \compr{\varOx}{f_{\varPt}(\varOx) < f_{\varPtB}(\varOx)}
  \cup \compr{\varOx}{f_{\varPt}(\varOx) > f_{\varPtB}(\varOx)}\]
...
\begin{proposition}
  Il existe une partie $\varPt\subseteq \varCCl$ telle que
  $\Card(\LclR{\varPt}) \geq \SsC{\varCCl}$.
\end{proposition}

\begin{proof}
  Soit $\varFam\subseteq \powerset(\varCCl)$ tel que
  $\Card(\varFam) = \SsC{\varCCl}$. On définit, pour tout $\varPt\in \varFam$~:
  \[\varFam_{\varPt}\defeq \LclR{\varPt}\cup \{\varPt\}\]
  Il y a alors deux possibilités à traiter~:
  \begin{itemize}
  \item s'il existe $\varPt\in \varFam$ tel que
    $\Card(\varFam_{\varPt})\geq \SsC{\varCCl}$,
    alors en prenant ce tel $\varPt$, on a bien prouvé l'affirmation souhaitée.
  \item sinon, alors on sait que pour tout $\varPt\in \varFam$,
    $\Card(\varFam_{\varPt})< \SsC{\varCCl}$,
    donc en utilisant le \cref{thm.Konig}, on en déduit que~:
    \[\Card\left(\bigcup_{\varPt\in \varFam}\varFam_{\varPt}\right) \leq
    \SsC{\varCCl}\]
    donc, comme on a supposé que $2^\varCCl > \SsC{\varCCl}$, on peut trouver une
    partie
    \[\varPt\in \powerset(\varCCl)\setminus
    \left(\bigcup_{\varPt\in \varFam}\varFam_{\varPt}\right)\]
    Comme on sait que pour tous $\varPt\neq \varPtB$, $\varPt\varRR \varPtB$ ou
    $\varPtB\varRR \varPt$, le fait que $\varPt\notin \varFam_{\varPtB}$ pour
    tout $\varPtB\in \varFam$ signifie que $\LclR{\varPt}\supseteq A$, donc que
    $\Card(\LclR{\varPt}) \geq\SsC{\varCCl}$.\qedhere
  \end{itemize}
\end{proof}

\begin{notation}
  On fixe maintenant $\varPt$ tel que défini dans la proposition précédente. De
  plus, pour tout $\varOc \in \varCk$, comme
  $f_{\varPt}(\varOc) \in \Cal_{\varOa_\varOc + 1}$, on se donne une injection
  $\makeFunName{g_\varOc}{f_{\varPt}(\varOc)}{\Cal_{\varOa_\varOc}}$.

  Pour tout $\varPtB\in\LclR{\varPt}$, on définit
  $\varSt_{\varPtB} = \compr{\varOc}{f_{\varPtB}(\varOc) < f_{\varPt}(\varOc)}$.
\end{notation}
...
\begin{lemma}[Fodor, variante]
  ...
  $\makeFunName{f}{\varSt}{\Cal_\varOa}$ telle que
...
\begin{lemma}\label{lem.Silver.F}
  Pour tout $\varPtB\in\LclR{\varPt}$, on peut trouver
  $\varStT_{\varPtB}\subseteq \varSt_{\varPtB}$
  stationnaire et $\overline\varOc_{\varPtB}$ tels que
  \[\forall \varOc \in \varStT_{\varPtB},
  g_\varOc(f_{\varPtB}(\varOc)) < \Cal_{\overline\varOc_Y}\]
\end{lemma}

\begin{proof}
  Soit $\varPtB\in\LclR{\varPt}$. On applique la variante du lemme de Fodor en
  considérant la suite cofinale continue $(\varOa_\varOb)$, l'ensemble
  stationnaire $\varSt_{\varPtB}$ et la fonction
  $\varOc\mapsto g_\varOc(f_{\varPtB}(\varOc))$.

  La fonction
  \[\makeFun{F_{\varPtB}}{\varSt_{\varPtB}}{\varCCl}
            {\varOc}{g_\varOc(f_{\varPtB}(\varOc))}\]
  est bien définie car sur $\varSt_{\varPtB}, f_{\varPtB} < f_{\varPt}$. De plus,
  $\makeFunName{g_\varOc}{f_{\varPt}(\varOc)}{\Cal_{\varOa_\varOc}}$ donc
  $F_{\varPtB}$ vérifie bien les conditions de la variante du lemme de Fodor.

  On trouve donc $\overline\varOc_Y$ et
  $\varStT_{\varPtB}\subseteq \varSt_{\varPtB}$ tels que
  \[\forall \varOc \in \varStT_{\varPtB},
  g_\varOc(f_{\varPtB}(\varOc)) < \Cal_{\overline\varOc_{\varPtB}}\qedhere\]
\end{proof}

On généralise ce lemme pour avoir un $\varStT$ et un $\overline\varOc$
indépendants de $\varPtB$.

\begin{lemma}
  Il existe $\varStT\subseteq \varCk$ stationnaire et un ordinal
  $\overline\varOc$ tels que
  \[\Card\left(
  \compr{\varPtB}{\varStT_{\varPtB} = \varStT \land
    \overline\varOc_{\varPtB} = \overline\varOc}\right) \geq\SsC{\varCCl}\]
\end{lemma}

\begin{proof}
  Soit la fonction
  \[\makeFun{h}
            {\LclR{\varPt}}{\powerset(\varCk)\times\varOa}
            {\varPtB}{(\varStT_{\varPtB},\overline\varOc_{\varPtB})}\]
  On peut donc écrire $\LclR{\varPt}$ comme l'union des
  ...
  ce qui contredit le fait que cette union vaut $\LclR{\varPt}$, qui est
...
\begin{proof}
  ...
  Soit
  \[\varFamB \defeq \compr{\varPtB\subseteq \varCCl}
          {\varStT_{\varPtB}=\varStT\land
            \overline\varOc_{\varPtB}=\overline\varOc}\]

  On commence par trouver $\varPtB_1\neq \varPtB_2$ deux éléments de $\varFamB$
  tels que
  \[\forall \varOc \in \varStT,
  g_\varOc(f_{\varPtB_1}(\varOc)) = g_\varOc(f_{\varPtB_2}(\varOc))\]

  Un argument de dénombrement suffit. En effet, la condition précédente revient
  à dire que, pour la famille de fonctions $F_{\varPtB}$ définies dans le
  \cref{lem.Silver.F}, on a l'égalité $F_{\varPtB_1} = F_{\varPtB_2}$, et ces deux
  fonctions sont des éléments de
  $\Funct(\varStT,\Cal_{\overline\varOc})$. On sait que
  $\Card(\varStT) < \Cal_{\overline\varOc}$, donc
  \begin{align*}
    \Card(\Funct(\varStT,\Cal_{\overline\varOc})) &
    \leq \Cal_{\overline\varOc}^{\Cal_{\overline\varOc}}\\
    &\leq 2^{\Cal_{\overline\varOc}} \\
    &\leq \Cal_{\overline\varOc + 1}
  \end{align*}
  Comme $\Card(\varFamB) = \SsC{\varCCl}$, on en déduit qu'il existe au moins
  deux parties $\varPtB_1,\varPtB_2$ telles que
  $F_{\varPtB_1} = F_{\varPtB_2}$ et $\varPtB_1\neq \varPtB_2$.

  Maintenant, comme on sait que chaque $g_\varOc$ est injective, et qu'on sait
  que
  \[\forall \varOc\in \varStT,
  g_\varOc(f_{\varPtB_1}(\varOc)) = g_\varOc(f_{\varPtB_2}(\varOc))\]
  on en déduit que $f_{\varPtB_1}$ et $f_{\varPtB_2}$ coïncident sur $\varStT$.
  Pourtant, on sait que
  $\compr{\varOx}{f_{\varPtB_1}(\varOx) = f_{\varPtB_2}(\varOx)}$ est de cardinal
...
\begin{exercise}
  Soit $(\varPt_{\varOa})_{\varOa < \Ow_1}$ une famille d'ensembles finis. Montrer
  qu'il existe une partie $W \subseteq \Ow_1$ de cardinal $\Cal_1$ et un
  ensemble fini $F$ tels que
  \[\forall i,j \in W, i \neq j \implies \varPt_i \cap \varPt_j = F\]
\end{exercise}

\begin{exercise}
  Soit $\makeFunName{f}{\Ord}{\Ord}$. On dit que $f$ est continue si pour tout
  $\varOa\in\Ord$, pour tous $\varOb,\varOc > \varOa$ tels que
  $f(\varOa) < \varOc$, $f$ est continue en $\varOa$ en tant que fonction de
  $\varOb$ dans $\varOc$.
  \begin{enumerate}[label=(\roman*)]
  \item Montrer que pour tous $\varOa,\varOb\in\Ord$ et
    $\makeFunName{f}{\varOa}{\varOb}$, $f$ est continue si et seulement si
    pour toute partie $\varPt\subseteq\varOa$ telle que $\sup\varPt \in\varOa$,
    $f(\sup\varPt) = \sup f(\varPt)$.
  \item Montrer qu'une fonction $\makeFunName{f}{\Ord}{\Ord}$ est continue si et
    seulement si pour tout ordinal $\varOl$ limite,
    $f(\varOl) = \displaystyle\bigcup_{\varOb < \varOl} f(\varOb)$.
  \item En déduire que pour tout $\varOa$, les fonctions
    $(\varOb\longmapsto \varOa + \varOb)$,
    $(\varOb \longmapsto \varOa\times \varOb)$ et
    $(\varOb\longmapsto \varOa^{\varOb})$ sont continues.
  \item Les fonctions
    $(\varOa\longmapsto \varOa + \varOb)$,
    $(\varOa \longmapsto \varOa\times \varOb)$ et
    $(\varOa\longmapsto \varOa^{\varOb})$ sont-elles continues pour tout
    $\varOb \in \Ord$~? La fonction successeur est-elle continue~?
  \end{enumerate}
\end{exercise}
...
\begin{problem}[Souslin a un problème]\label{pb.souslin}
  On va étudier un problème combinatoire important de la théorie des ensembles~:
  le problème de Souslin. Pour le présenter au mieux, parlons d'abord d'un
  théorème de Cantor~: pour tout ensemble totalement ordonné $(X,<)$ qui est non
  borné et dense, il existe un isomorphisme $X \iso \bQ$. Si l'on considère
  maintenant un ensemble totalement ordonné $(X,<)$ non borné, possédant pour
  chaque partie bornée une borne supérieure et inférieure, et qui possède une
  partie dense dénombrable, alors (on peut se référer à la construction de
  $\bR$) on a un isomorphisme $X\iso \bR$. On a ainsi une caractérisation de la
  structure ordonnée de $\bR$ (au second ordre, puisqu'on considère des bornes
  supérieures).

  Le problème de Souslin s'attarde sur une version affaiblie de ces conditions,
  et en particulier de la condition d'avoir une partie dense dénombrable.

  \pbPart{Condition de chaîne dénombrable}

  La condition mentionnée plus tôt est nommée \og condition de chaîne
  dénombrable\fg{}. Comme son nom l'indique (ou pas), elle porte sur les
  antichaînes d'un ensemble ordonné. Pour un ensemble ordonné $(X,<)$, on dit
  que $X$ a la condition de chaîne dénombrable si toute antichaîne forte de
  $X$\footnote{Rappelons qu'une antichaîne forte est une partie
  $\varPt\subseteq X$ telle que tous deux éléments sont incompatibles (vers le
  bas~: ils n'ont pas de borne inférieure dans $X$).} est
  dénombrable.

  \begin{pbQuest}
  \item Donner un exemple d'ensemble ordonné vérifiant la condition de chaîne
    dénombrable, et un exemple d'ensemble ordonné ne la vérifiant pas.
  \item On pose $X$ l'ensemble des intervalles ouverts non vide de $\bR$,
    c'est-à-dire de la forme $\intOO{a}{b}$ où $a\in \bR\cup\{\mInf\}$,
    $b \in \bR\cup\{\pInf\}$ et $a < b$. Montrer que $(X,\subseteq)$ est un
    ensemble indénombrable vérifiant la condition de chaîne dénombrable.
  \end{pbQuest}

  Le problème de Souslin est donc le suivant~:
  \begin{quotation}
    \noindent
    Soit $(X,<)$ un ensemble ordonné non borné, dense, tel que toute partie
    bornée possède une borne supérieure et une borne inférieure, et tel que
    toute famille d'intervalles ouverts non vides disjoints de $X$ est
    dénombrable. Est-ce que $X\iso\bR$~?
  \end{quotation}
  On va montrer qu'un certain principe combinatoire suffit à prouver que tout
  tel ensemble $X$ n'est pas isomorphe à $\bR$. On appelle dans la suite du
  problème \og droite de Souslin\fg{} un ensemble ordonné $X$ tel que plus haut,
  qui n'est pas isomorphe à $\bR$.

  \pbPart{Arbre de Souslin}

  Avant de montrer qu'il existe une droite de Souslin différente de $\bR$, on
  reformule le problème sous une autre forme, utilisant des arbres. Il nous faut
  donc introduire d'abord la notion d'arbre du point de vue de la théorie des
  ensembles.

  On a déjà pu présenter un arbre bien fondé comme un certain ensemble inductif
  et un arbre potentiellement infini comme un ensemble de listes (ou de suites
  finies). Ici, on veut s'autoriser des arbres dont les chemins ont une
  longueur transfinie, ce pourquoi on prend un arbre comme un ensemble ordonné.

  \begin{quotation}
    \noindent
    Un arbre est un ensemble ordonné $(\varTree,<)$ possédant un minimum et tel
    que, pour tout $x \in \varTree$, l'ensemble $\compr{y \in \varTree}{y < x}$
    (qu'on notera $\downcl{x}$) est bien ordonné par $<$.
  \end{quotation}

  \begin{pbQuest}
  \item Montrer que l'ensemble $\Tree(n)$ défini dans l'\cref{exo.ind.arbres}
    est un arbre au sens précédent.
  \item Définir un arbre infini qui n'est pas un élément de $\Tree(n)$ pour un
    certain $n \in \bN$.
  \end{pbQuest}

  Dans un arbre $\varTree$, pour tout $x \in \varTree$, on peut définir la
  hauteur respectivement de $x$ et de $\varTree$~:
  \[\height(x) \defeq \type(\downcl{x})\qquad
  \height(\varTree) \defeq \sup_{x \in \varTree} \height(x)\]
  \begin{pbQuest}[resume]
  \item Définir un arbre de hauteur $\Ow_1$.
  \item Dans un arbre $\varTree$ de hauteur $\Ow+1$, pour deux éléments
    $x,y \in \varTree$, montrer qu'il peut ne pas exister de borne inférieure
    $x\opmt y \in \varTree$.
  \end{pbQuest}
  On peut partitionner les éléments d'un arbre $\varTree$ par hauteur, donnant
  lieu aux niveaux de $\varTree$~:
  \[\level(\varTree,\varOa) \defeq \compr{x \in \varTree}{\height(x) = \varOa}\]
  \begin{pbQuest}[resume]
  \item Soit $\varOa$ un ordinal, $(N_{\varOb})_{\varOb<\varOa}$ une famille
    d'ensembles et
    \[\makeFunName{f_{\varOb}}
                  {N_{\varOb}}
                  {\powerset\left(\bigcup_{\varOc < \varOb} N_{\varOc}\right)}\]
    une famille de fonctions pour $\varOb < \varOa$ telle que pour tous
    $\varOc < \varOb < \varOb$, $x \in N_{\varOb}$ et
    $y\in f_{\varOb}(x)\cap N_{\varOc}$, $f_{\varOb}(x)\cap N_{\varOc} = f_{\varOc}(x)$.
    Pour tous $\varOc < \varOb < \varOa$, $x\in N_{\varOb}$, $y\in N_{\varOc}$,
    on définit l'ordre $y < x \defeq y \in f_{\varOb}(x)$.
    Montrer que l'ensemble
    \[(\varTree,<) \defeq \left(\bigcup_{\varOb < \varOa} N_{\varOb}, <\right)\]
    est un arbre, et que l'association
    $\varTree \mapsto (\level(\varTree),\varOb))_{\varOb < \height(\varTree)}$
    établit une bijection entre les deux classes.
  \item Donner un exemple d'instance de $(f_{\varOb})$ pour laquelle au moins
    une fonction $f_{\varOb}$ n'est pas injective. Montrer que si toutes les
    fonctions $(f_{\varOb})$ sont injectives, alors l'arbre construit est un
    inf demi-treillis.
  \end{pbQuest}

  On appelle branche dans un arbre $\varTree$ une chaîne de $\varTree$ maximale
  pour l'inclusion.
  \begin{pbQuest}[resume]
  \item Construire une branche dans chacun des arbres définis précédemment.
  \item Montrer que pour une branche $B \subseteq \varTree$, l'ensemble
    \[\compr{\height(x)}{x \in B}\]
    est un segment initial de $\height(\varTree)$. On appelle cet ordinal la
    hauteur de $B$, qu'on note $\height(B)$.
  \end{pbQuest}
  On dit qu'une branche $B$ de $\varTree$ est cofinale si
  $\height(B) = \height(\varTree)$.
  \begin{pbQuest}[resume]
  \item Montrer le lemme de König sur les arbres~: pour tout arbre
    $(\varTree,<)$ de hauteur $\Ow$ et tel que $\level(\varTree,n) < \Ow$ pour
    tout $n \in \Ow$, il existe une branche cofinale dans $\varTree$.
  \end{pbQuest}
  Pour le cas d'un arbre de Souslin, les conditions que l'on va chercher forcent
  l'arbre à ne pas vérifier le lemme de König dans un sens fort~:
  \begin{quotation}
    \noindent
    Un arbre de Souslin est un arbre $(\varTree,<)$ de hauteur $\Ow_1$ dans
    lequel toute branche est dénombrable et toute antichaîne est dénombrable.
  \end{quotation}
  \begin{pbQuest}[resume]
  \item Montrer que si $(X,<)$ est une droite de Souslin, alors l'ensemble des
    intervalles ouverts non vides de $X$ est un arbre de Souslin.
    \textit{Indication~: Une droite de Souslin étant non isomorphe à $\bR$, elle
      n'a pas de partie dénombrable dense.}
  \end{pbQuest}
  On veut montrer la réciproque~: à partir d'un arbre de Souslin, il est
  possible de construire une droite de Souslin. Pour cela, on veut d'abord
  normaliser l'arbre qu'on va construire. On dit qu'un arbre $(\varTree,<)$ est
  normal si les conditions suivantes sont réunies~:
  \begin{itemize}
  \item si $\varOa < \varOb < \height(\varTree)$ et
    $x \in \level(\varTree,\varOa)$ alors il existe
    $y \in \level(\varTree,\varOb)$ tel que $x < y$~;
  \item pour tout $x \in \varTree$, l'ensemble
    \[\Ss_{\varTree}(x) \defeq
    \compr{y \in \varTree}{x < y \land \height(y) = \Ss(\height(x))}\]
    est infini~;
  \item pour tout $\varOa < \height(\varTree)$, la fonction
    \[\makeFun{\downcl{\tok}}
              {\level(\varTree,\varOa)}
              {\bigcup_{\varOb < \varOa}\level(\varTree,\varOb)}
              {x}{\downcl{x}}\]
    est injective.
  \end{itemize}
  \begin{pbQuest}[resume]
  \item Montrer que s'il existe un arbre de Souslin, alors il existe un arbre
    de Souslin normal.
  \end{pbQuest}
  \'Etant donné un arbre de Souslin $(\varTree,<)$ normal, pour tout
  $x \in \varTree$, on fixe un ordre total $<_x$ sur $\Ss_{\varTree}(x)$ de type
  $\bQ$\footnote{Cet ordre existe~: $\Ss_{\varTree}(x)$ est infini et
  au plus dénombrable car $(\varTree,<)$ est un arbre de Souslin normal.}. On
  construit la droite de Souslin associée par l'ordre $<'$ sur $\varTree$
  suivant~:
  \begin{itemize}
  \item si $x < y$ alors $x <' y$~;
  \item si $x$ et $y$ sont incomparables, on pose $z \defeq x \opmt y$ leur
    borne inférieure. On considère $x_0$ et $y_0$ respectivement l'élément
    tel que $z < x_0 < x$ et $\height(x_0) = \height(z) + 1$, et l'élément tel
    que $z < y_0 < y$ et $\height(y_0) = \height(z) + 1$. On définit
    $x < y$ par le fait que $x_0 <_z y_0$.
  \end{itemize}
  \begin{pbQuest}[resume]
  \item Montrer que pour $(\varTree,<)$ un arbre de Souslin normal,
    $(\varTree,<')$ est une droite de Souslin.
  \end{pbQuest}

  \pbPart{Le principe du diamant}

  On sait maintenant qu'un cas de droite de Souslin se ramène à un arbre de
  Souslin, ce que l'on va maintenant construire en utilisant l'axiome
  combinatoire appelé principe du diamant (en anglais \foreignexpr{diamond
    principle}) et noté $\diamondsuit$~:
  \begin{quotation}
    \noindent
    Il existe une famille $(\varPt_{\varOa})_{\varOa < \Ow_1}$ telle que pour tout
    $\varOa < \Ow_1$, $\varPt_{\varOa}\subseteq \varOa$ et telle que pour toute
    partie $\varPt\subseteq \Ow_1$, l'ensemble
    $\compr{\varOa < \Ow_1}{\varPt\cap \varOa = \varPt_{\varOa}}$ est
    stationnaire dans $\Ow_1$.
  \end{quotation}
  \begin{pbQuest}
  \item Prouver que $\diamondsuit\implies\HC$.
  \end{pbQuest}
  On cherche à construire un arbre de Souslin sous l'hypothèse $\diamondsuit$.
  On suppose donc vrai $\diamondsuit$ pour la suite du problème.
  \begin{pbQuest}[resume]
  \item Soit $\varTree$ un arbre tel que pour tout $x \in \varTree$, il existe
    deux éléments $y,z \in \varTree$ incompatibles vers le haut tels que $x < y$
    et $x < z$. Montrer que si toute antichaîne de $\varTree$ est au plus
    dénombrable, alors toute chaîne de $\varTree$ est au plus dénombrable.
  \item Construire un arbre de Souslin. \textit{Indication~: On peut construire
