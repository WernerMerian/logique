...
\begin{exercise}
  Montrer que $\leq$ est une relation d'ordre compatible avec l'addition,
...
\begin{exercise}
  Montrer que $(\mathbb Q,0,+,1,\times,\leq)$ est un aussi un anneau ordonné.
  On dit alors que c'est un corps ordonné.
\end{exercise}
...
filtre (on exprimera différemment ces paires dans le contexte des coupures de
Dedekind).

\begin{definition}[Coupure de Dedekind]\label{def.ax.cut}
  On appelle coupure de Dedekind un couple $(L,U)\in (\powerset(\mathbb Q))^2$
  tel que~:
  \begin{itemize}
  \item $L \neq \varnothing$ et $U\neq\varnothing$
  \item $L\cup U = \mathbb Q$
  \item $\forall x \in L, \forall y \in B, x < y$
  \item si $L$ possède une borne supérieure (dans $\mathbb Q$), alors cette
    borne inférieure est dans $L$
  \end{itemize}

  On note $\Dedekind(\mathbb Q)$ l'ensemble des coupures de Dedekind.
\end{definition}

Les trois premières conditions expriment que $L$ est un enemble clos par le bas,
$U$ un ensemble clos par le haut, que les deux forment une partition de
$\mathbb Q$ et que $L$ est strictement en-dessous de $U$ (d'où la notation de
$L$ pour \textit{lower} et $U$ pour \textit{upper}).

L'idée est alors de représenter un réel $x\in\mathbb R$ par la coupure de
Dedekind des points inférieurs et supérieurs~:
\[(\{r\in \mathbb Q \mid r \leq x\},\{r\in \mathbb Q \mid r > x\})\]
On voit qu'avec seulement les trois premières conditions, on aurait pu choisir
de dire que $r < x$ et $r \geq x$ dans les deux conditions précédentes. La
dernière condition de la \cref{def.ax.cut} fixe notre choix en disant que si
le point identifié par la coupure est déjà rationnel, alors il est placé dans
la coupure du bas.

Les définitions sur les coupures de Dedekind souffrent d'un problème important~:
il faut faire des disjonctions de cas suivant le signe des éléments. Pour
éviter ce problème, on va à la place construire $\mathbb R_+$, que l'on peut
étendre à $\mathbb R$ par exemple en reprenant le processus de symétrisation.
On notera donc notre ensemble de coupures $\Dedekind(\mathbb Q_+)$. Remarquons
que notre choix de mettre le point rationnel potentiel dans la partie du bas
permet à la partie du bas de rester non vide, même pour représenter $0$.

\begin{definition}
  On définit sur $\Dedekind(\mathbb Q_+)$ l'opération $+$ par~:
  \[(L,U)+(L',U') \defeq
  (\{a + b \mid a \in L, b \in L'\},\{a + b \mid a \in U, b \in U'\})\]
  et l'opération $\times$ par~:
  \[(L,U)\times(L',U')\defeq (\{a \times b \mid a \in L, b \in L'\},
  \{a \times b \mid a \in U, b \in U'\})\]
\end{definition}

\begin{exercise}
  Montrer que $(\Dedekind(\mathbb Q_+),0_\Dedekind,+,1_\Dedekind,\times)$ est un
  demi-anneau, où
  \[0_\Dedekind = (\{0\},\mathbb Q\setminus \{0\})\qquad
  1_\Dedekind = (\{r \in \mathbb Q \mid 0 \leq r \leq 1\},
  \mathbb Q \setminus [0,1]\})\]
\end{exercise}

On peut aussi définir l'ordre sur les coupures.

\begin{definition}
  On définit sur $\Dedekind(\mathbb Q_+)$ l'ordre $\leq$ par~:
  \[(L,U)\leq (L',U')\defeq L \subseteq L'\]
\end{definition}

\begin{exercise}
  Montrer que $+$ est compatible avec $\leq$.
\end{exercise}

On revient maintenant à $\Dedekind(\mathbb Q)$~: cet ensemble possède la
propriété de la borne supérieure. Nous allons montrer la version de la borne
inférieure à la place, car nos choix rendent plus simple cette démonstration.
Il est direct, dans un deuxième temps, de voir que la borne supérieure est la
borne inférieure de l'ensemble opposé.

\begin{proposition}
  Toute partie $S\subseteq \Dedekind(\mathbb Q)$ non vide minorée admet une
  borne inférieure.
\end{proposition}

\begin{proof}
  Soit une telle partie $S$. On définit la borne inférieure de $S$ comme
  \[(L_S,U_S) \defeq (\bigcap_{(L,U)\in S} L, \bigcup_{(L,U)\in S} U)\]
  Montrons que cela définit bien une coupure~:
  \begin{itemize}
  \item $S$ est non vide, donc on trouve $(L,U)\in S$~: $U$ est non vide donc
    on trouve $r\in U$, et $U\subseteq U_S$ donc $r\in U_S$. $U_S$ est donc
    non vide. Comme $S$ est minorée, on trouve $(L,U)$ minorant $S$~: il existe
    donc $r \in L$ et pour tout $(L',U')\in S$, comme $(L',U')\leq(L,U)$, on
    en déduit que $r\in L'$, donc $r\in L_S$~: $L_S$ est non vide.
  \item soit $r\in \mathbb Q$. S'il existe $(L,U) \in S$ tel que $r\in U$,
    alors $r\in U_S$ par définition. Sinon, alors pour tout $(L,U)\in S$,
    $r\notin U$, donc $r\in L$~: on en déduit donc que $r\in L_S$. Donc
    $L_S\cup U_S = \mathbb Q$.
  \item si $x \in L_S$ et $y\in U_S$ alors on trouve $(L,U)\in S$ tel que
    $y\in U$. Comme $x \in U$ et $(L,U)\in\Dedekind(\mathbb Q)$, on en
    déduit que $x < y$.
  \item si $L_S$ possède une borne supérieure $x$, alors soit $(L,U) \in S$.
    Comme $L_S\subseteq L$, tout majorant de $L$ est un majorant de $L_S$
    donc $x$ est plus petit que tout majorant de $L$. Si $L$ n'a pas de borne
    supérieure, alors $x$ ne peut pas être un majorant de $L$ et est donc
    un élément de $L$. Si $L$ a une borne supérieure, alors $x\in L$ puisque
    c'est le plus petit majorant. Dans les deux cas, $x \in L$, donc $x$ est
    dans $L$ pour tout $(L,U)\in S$, donc $x\in L_S$.
  \end{itemize}

  On veut maintenant montrer que $(L_S,U_S)$ est bien la borne inférieure
  de $S$. On sait que $L_S\subseteq L$ pour tout $(L,U)\in S$ par propriété de
  l'intersection. Soit $(L',U')$ tel que
  \[\forall (L,U)\in S, (L',U')\leq (L,U)\]
  alors cela signifie que $\displaystyle L'\subseteq \bigcap_{(L,U)\in S} L$
  ce qui signifie exactement que $(L',U')\leq (L_S,U_S)$. D'où le résultat.
\end{proof}
