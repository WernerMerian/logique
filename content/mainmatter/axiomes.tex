...
\begin{proof}
  ...
  \item pour tout $Y\in \ideal(X)$, on définit
    ...
    $Y + (-Y) \subseteq \iota(0)$. Réciproquement, soit $x \leq 0$, montrons
    qu'il existe $y \in Y, y' \in (-Y)$ tels que $x = y + y'$. Pour commencer,
    montrons que soit $0 \in Y$, soit $0 \in -Y$. En effet, si $0 \notin Y$,
    alors $-0 = 0 \in -Y$, donc que $0 \in Y$ ou non, on a $0 \in Y \cup (-Y)$.
    Disons sans perte de généralité que $0 \in Y$, alors comme $x \leq 0$, on
    trouve $y \in Y$ (car $Y \neq \varnothing$) et on sait que $y + x \leq y$,
    donc par clôture vers le bas $y + x \in Y$. De plus, A FAIRE
