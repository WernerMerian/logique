...
\begin{exercise}[Mise en jambes]
  Prouver les formules suivantes~:
  \[\begin{array}{cc}
...
\begin{exercise}[Des formules utiles]
  ...
  \item Soit $x_1,\ldots,x_n$ des variables. Construire une formule
    $\mathrm{Ineq_{x_1,\ldots,x_n}}$ exprimant que toutes les variables
  ...
  \item Pour un cardinal $\kappa$ infini, peut-on trouver une formule
    similaire ? \textit{Indication~: on peut utiliser le théorème de compacité.}
  \end{enumerate}
\end{exercise}

\begin{exercise}[Le connecteur d'équivalence]
...
\begin{exercise}[Formes prénexes]
  ...
  On définit inductivement l'ensemble des formules sous forme prénexe
  $\Formula_{\mathrm{pren}}(\Sigma)\subseteq \Formula(\Sigma)$ par
...
\begin{exercise}[La skolémisation]
  ...
  \item Plus généralement, montrer que la formule
    ...
    formule
    $\forall x_1,\ldots,\forall x_n,(\varphi[f(x_1,\ldots,x_n)/x])$ est
...
\begin{exercise}[Extension par définition]
  ...
  \item si $\psi = Q x, \psi'$ où $Q$ est un quantificateur, alors
    $\psi_{-R} = Q x, \psi'_{-R}$.
  \end{itemize}
  Montrer que, pour tout formule $\psi \in \Formula(\Sigma')$~:
  \[\mathcal T' \vdash \psi \iff \mathcal T \vdash \psi_{-R}\]
  ...
  On définit de plus, pour tout terme $t$ sur $\Sigma'$, un ensemble
  $\mathcal V_t$ des associations de variables nécessaires à la traduction de
  $t$ et un terme $t_{-f}$~:
  \begin{itemize}
  \item si $t = x$, alors $\mathcal V_t = \varnothing$ et $t_{-f} = x$~;
  \item si $t = g(t_1,\ldots,t_n)$ où $g \neq f$, alors
    $\mathcal V_t = \bigcup_{i = 1}^n \mathcal V_{t_i}$ et
    $t_{-f} = g(t_{1,-f}, \ldots, t_{n,-f})$~;
  \item si $t = f(t_1,\ldots,t_n)$, alors
    \[\mathcal V_t = \left(\bigcup_{i = 1}^n \mathcal V_{t_i}\right)\cup
    \{(t_{1,-f},\ldots,t_{n,-f},y)\}\]
    où $y$ est une variable fraîche (non apparue dans aucun $t_i$ ni
    $t_{i,-f}$) et $t_{-f} = y$.
  \end{itemize}
  Pour chaque association $a = (t_1,\ldots,t_n,x)$, on définit
  $\varphi_a \defeq \varphi(t_1,\ldots,t_n,x)$, et on définit la traduction
  $\varphi_{-f}$ d'une formule par A FAIRE
\end{exercise}
...
\begin{exercise}[La théorie des relations d'équivalence]
  ...
  \item Donner un ensemble d'axiomes $\mathrm{nb}_{\infty}$ tel que
    $\mathcal T_{\mathrm{equiv}}\cup \mathrm{nb}_\infty$ est la classe des
  ...
  \item Donner un ensemble d'axiomes $\mathrm{cl}_\infty$ tel que
    $\mathcal T_{\mathrm{equiv}}\cup \mathrm{cl}_\infty$ est la classe des
...
\begin{exercise}[Une application du théorème de compacité]
  ...
  \item L'objectif de cette question est de prouver qu'il n'existe pas de
    ...
    Construire pour tout $n$ une formule $\varphi_n$ exprimant que $v$ et $w$
