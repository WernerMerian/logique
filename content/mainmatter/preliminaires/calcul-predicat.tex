...
\begin{example}
  ...
  triviale. On fixe la signature $\Sigma \defeq \{f^1, R^1\}$ où $f$ est un
  symbole de fonction et $R$ est un symbole de relation. Posons
  \[\Gamma \defeq \forall x, R(x) \to R(f(x))\]
  et prouvons que $\Gamma\vdash \forall x, R(x) \to R(f(f(x)))$~:
  \begin{center}
  \resizebox{\textwidth}{!}{\(
    \AxiomC{}
    \RightLabel{Ax}
    \UnaryInfC{$\Gamma,R(x)\vdash \forall x, R(x) \to R(f(x))$}
    \RightLabel{$\forall_\mathrm e$}
    \UnaryInfC{$\Gamma, R(x)\vdash R(f(x)) \to R(f(f(x)))$}
    \AxiomC{}
    \RightLabel{Ax}
    \UnaryInfC{$\Gamma,R(x)\vdash \forall x, R(x) \to R(f(x))$}
    \RightLabel{$\forall_\mathrm e$}
    \UnaryInfC{$\Gamma, R(x)\vdash R(x) \to R(f(x))$}
    \AxiomC{}
    \RightLabel{Ax}
    \UnaryInfC{$\Gamma, R(x)\vdash R(x)$}
    \RightLabel{$\to_\mathrm e$}
    \BinaryInfC{$\Gamma, R(x)\vdash R(f(x))$}
    \RightLabel{$\to_\mathrm e$}
    \BinaryInfC{$\Gamma, R(x)\vdash R(f(f(x)))$}
    \RightLabel{$\to_\mathrm i$}
    \UnaryInfC{$\Gamma\vdash R(x) \to R(f(f(x)))$}
    \RightLabel{$\forall_\mathrm i$}
    \UnaryInfC{$\Gamma\vdash \forall x, R(x) \to R(f(f(x)))$}
    \DisplayProof
    \)}
  \end{center}
\end{example}
...
\begin{proof}
  ...
  finie de $\mathcal T'$, les autres peuvent être associés au même élément).
  Ainsi on a un modèle de $\mathcal T'$.
\end{proof}
