...
\begin{exercise}[Formes prénexes]
...
\begin{problem}[Extension par définition]
  On fixe une signature $\Sigma$ et une théorie $\mathcal T$. Cet exercice vise
  à justifier une pratique courante~: donner des noms à des objets définis par
  une définition.

  Pour cela, on introduit d'abord la notion d'extension conservative~: une théorie
  $\mathcal T'$ est une extension conservative de $\mathcal T$ si cette théorie ne
  prouve aucun nouveau théorème possible à exprimer dans $\mathcal T$.

  \begin{definition}[Extension conservative]\label{def.ext.cons}
      Soient $\Sigma,\Sigma'$ deux signatures telles que $\Sigma\subseteq\Sigma'$
      et deux théories $\mathcal T, \mathcal T'$ respectivement sur
      $\Sigma$ et $\Sigma'$ telles que $\mathcal T \subseteq \mathcal T'$.
      On dit que $\mathcal T'$ est une extension conservative si~:
      \[\forall \varphi \in \Formula(\Sigma),
      \mathcal T \vdash \varphi \iff \mathcal T' \vdash \varphi\]
  \end{definition}
  
  Pour justifier l'utilisation de définitions, nous allons montrer le théorème de
  conservation des extensions par définitions~:
  \begin{theorem}[Conservation des extensions par définition]\label{thm.ext.def}
      Soit $\Sigma$ une signature du premier ordre et $\mathcal T$ une théorie
      sur $\Sigma$, alors~:
      \begin{itemize}
          \item pour toute formule $\varphi(x_1,\ldots,x_n)$, la théorie
          sur la signature $\Sigma\cup \{R\}$ où $R$ est un nouveau symbole
          de relation $n$-aire, définie par
          \[\mathcal T' \defeq\mathcal T \cup
          \{`` \forall x_1,\ldots,x_n, R(x_1,\ldots,x_n) \leftrightarrow
          \varphi(x_1,\ldots,x_n)"\}\]
          est une extension conservative de $\mathcal T$.
          \item pour toute formule $\varphi(x_1,\ldots,x_n,y)$ telle que
          \[\mathcal T \vdash \forall x_1,\ldots,x_n, \exists! y,
          \varphi(x_1,\ldots,x_n,y)\]
          la théorie sur la signature $\Sigma\cup\{f\}$ où $f$ est un
          nouveau symbole de fonction $n$-aire, définie par
          \[\mathcal T' \defeq \mathcal T \cup \{`` \forall x_1,\ldots,x_n,
          \varphi(x_1,\ldots,x_n,f(x_1,\ldots,x_n))"\}\]
          est une extension conservative de $\mathcal T$.
      \end{itemize}
  \end{theorem}

  \paragraph{Définition d'une relation}
  Soit une formule $\varphi(x_1,\ldots,x_n)$. On définit un nouveau symbole de
  relation $R(x_1,\ldots,x_n)$ et on note $\Sigma' \defeq \Sigma\cup\{R\}$. De
  plus, on augmente la théorie $\mathcal T$ en
  \[\mathcal T' \defeq \mathcal T \cup
  \{`` \forall x_1,\ldots,x_n, R(x_1,\ldots,x_n) \leftrightarrow
  \varphi(x_1,\ldots,x_n)"\}\]
  \'Etant donnée une formule $\psi$ sur $\Sigma'$, on définit sa traduction
  $\psi_{-R}\in\Formula(\Sigma)$ récursivement~:
  \begin{itemize}
  \item si $\psi = \top, \bot, t = u, S(t_1,\ldots,t_k)$ où $S \neq R$, alors
    $\psi_{-R} = \psi$~;
  \item si $\psi = R(t_1,\ldots,t_n)$, alors
    $\psi_{-R} = \varphi(t_1,\ldots,t_n)$~;
  \item si $\psi = \lnot \psi'$, alors $\psi_{-R} = \lnot \psi'_{-R}$~;
  \item si $\psi = \psi' \oplus \psi''$ où $\oplus \in \{\land,\lor,\to\}$,
    alors $\psi_{-R} = \psi'_{-R} \oplus \psi''_{-R}$~;
  \item si $\psi = Q x, \psi'$ où $Q$ est un quantificateur, alors
    $\psi_{-R} = Q x, \psi'_{-R}$.
  \end{itemize}
  \begin{enumerate}[label=(\roman*)]
      \item Montrer que, pour toute formule $\psi\in\Formula(\Sigma')$~:
      \[\mathcal T' \vdash \psi \leftrightarrow \psi_{-R}\]
      \item En déduire que $\mathcal T'$ est une extension conservative de
      $\mathcal T$.
  \end{enumerate}

  \paragraph{Définition d'une fonction}
  Soit désormais une formule $\varphi(x_1,\ldots,x_n,y)$. On suppose prouvable
  l'hypothèse
  \begin{equation*}\label{eq.phi_uniq}
    \mathcal T \vdash \forall x_1,\ldots,x_n, \exists! y,
    \varphi(x_1,\ldots,x_n,y)
  \end{equation*}
  On ajoute maintenant un symbole $f$ d'arité $n$, donnant
  $\Sigma' \defeq \Sigma\cup \{f\}$, et on définit la nouvelle théorie
  \[\mathcal T' \defeq \mathcal T \cup \{`` \forall x_1,\ldots,x_n,
  \varphi(x_1,\ldots,x_n,f(x_1,\ldots,x_n))"\}\]
  Comme pour le cas des définitions pour des relations, on cherche maintenant
  à transformer une formule $\varphi \in \Formula(\Sigma')$ en une formule
  $\varphi_{-f}\in \Formula(\Sigma)$, mais il est nécessaire de faire une
  transformation plus subtile. Tout d'abord, par induction sur la structure
  des termes, on définit pour un terme $t$ la liste $\mathcal V_t$ et le
  terme $t_{-f}$~:
  \begin{itemize}
  \item si $t = x$, alors $\mathcal V_t \defeq \nil$ et $t_{-f} \defeq x$~;
  \item si $t = g(t_1,\ldots,t_n)$ où $g \neq f$, alors
    $\mathcal V_t \defeq \bigoplus_{i = 1}^n \mathcal V_{t_i}$ et
    $t_{-f} \defeq g(t_{1,-f}, \ldots, t_{n,-f})$~;
  \item si $t = f(t_1,\ldots,t_n)$, alors
    \[\mathcal V_t \defeq
    \left(\bigoplus_{i = 1}^n \mathcal V_{t_i}\right)\oplus
    \{(t_{1,-f},\ldots,t_{n,-f},y)\}\]
    où $y$ est une variable fraîche (non apparue dans aucun $t_i$ ni
    $t_{i,-f}$) et $t_{-f} \defeq y$.
  \end{itemize}
  \'Etant donnée une formule atomique $\psi = R(t_1,\ldots,t_n)$, on pose
  $\mathcal V \defeq \bigoplus_{i = 1}^n \mathcal V_{t_i}$ et, en notant
  $\mathcal V = \{(x_{1,i},\ldots,x_{n,i},y_i)\mid i \in \{0,\ldots,k\}\}$ pour
  un certain entier $k$, on définit la formule $\psi_{-f} \in \Formula(\Sigma)$
  par
  \[\psi_{-f} \defeq \forall y_1,\ldots,y_k, R(t_{1,-f},\ldots,t_{n,-f})\]
  On définit ensuite, pour toute formule $\psi \in \Formula(\Sigma')$, la formule
  $\psi_{-f}\in\Formula(\Sigma)$
  par induction sur la structure des formules~:
  \begin{itemize}
      \item le cas d'une formule atomique a été traité~;
      \item si $\psi = \top$ ou $\psi = \bot$ alors
      $\psi_{-f} = \psi$~;
      \item si $\psi = \lnot \psi'$ alors $\psi_{-f} = \lnot \psi'_{-f}$~;
      \item si $\psi = \psi' \oplus \psi''$ où $\oplus \in \{\land, \lor,\to\}$
      alors $\psi_{-f} = \psi'_{-f} \oplus \psi''_{-f}$~;
      \item si $\psi = Qx, \psi'$ où $Q$ est un quantificateur, alors
      $\psi_{-f} = Qx, \psi'_{-f}$.
  \end{itemize}
  \begin{enumerate}[label=(\roman*)]
      \item Montrer que, pour toute formule $\psi \in \Formula(\Sigma')$~:
      \[\mathcal T' \vdash \psi \leftrightarrow \psi_{-f}\]
      \item En déduire que $\mathcal T'$ est une extension conservative de $\mathcal T$.
  \end{enumerate}

  En conclure le \cref{thm.ext.def}. Ainsi, lorsqu'on introduit une nouvelle
  notation de relation ou de fonction à partir d'une formule, on ne change pas
  l'ensemble des théorèmes de notre langage d'origine~: on se contente simplement
  d'ajouter des écritures plus courtes pour nos théorèmes.
\end{problem}