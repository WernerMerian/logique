...
\begin{theorem}[Induction sur une relation]
  ...
  $(X\mapsto P)\models \ruleR$. Alors $R_{\ruleR}\subseteq P$.
\end{theorem}

\begin{proof}
  Il suffit de remarquer que $P$ est un point fixe de $\ruleR$ puisque
  $(X\mapsto P)\models \ruleR$. Ainsi, comme $P$ est un point fixe de $\ruleR$
...
\begin{definition}[Règle admissible]
  ...
  $R\models \ruleR$. La règle est admissible pour un ensemble de règles
...
\begin{exercise}[Différents arbres]
  ...
  \[\PreTree(A) \defeq \{T \subseteq \List(A) \mid \forall \varlist,\varlist',
...
\begin{problem}[La coinduction]
  ...
  \[\ideal(X_\bot) \defeq \{I \subseteq X_\bot \mid I \text{ est un idéal de }
  X_\bot\}\]
  
  \begin{pbQuest}[resume]
  \item Montrer que la fonction
    \[\makeFun{\iota}{X_\bot}{\ideal(X_\bot)}{x}
              {\{ y \in X_\bot \mid y < x\}\cup\{x\}}\]
  ...
  \[X_\infty \defeq \{I \in \ideal(X) \mid \forall J \in \ideal(X),
  ...
  \{(c,\vec x) \mid \vec x \in X^{\arite(c)}\}\]
...
%      \[h(y) = c(h(y_{f,1}),\ldots,h(y_{f,\arite(c)}))\]
  ...
  notera
  $(X\mapsto R, x_1 \mapsto a_1,\ldots, x_n \mapsto a_n)\comodels \ruleR$.
  ...
  \item Soit $E$ un ensemble et
    ...
    Si $(X \mapsto R, x_1\mapsto a_1,\ldots x_n \mapsto a_n) \comodels \ruleR$
  ...
  \item En déduire le théorème de coinduction suivant~:
    \begin{quotation}
      ...
      $(X \mapsto R, x_1\mapsto a_1,\ldots,x_n\mapsto a_n)\comodels \ruleR_i$
