...
l'ensemble $C_0 = \compr{c \in C}{\alpha(c) = 0}$, puis l'ensemble $C_1$ est
...
\begin{definition}[Ensemble inductif sur une signature]
  ...
  \begin{itemize}
  \item $X_0 = \vide$
  \item $X_{n+1} = \compr{(c,x_1,\ldots,x_p)}{c\in C, (x_1,\ldots,x_p)\in (X_n)^p,
    \alpha(c) = p }$
  \end{itemize}
...
\begin{proof}
  ...
  \begin{multline*}
    \forall n\in \mathbb N, \exists ! f_n : X_n \to Y, \forall c \in C,\\
    \forall (x_1,\ldots,x_p)\in (X_n)^{\alpha(c)}, f_n((c,x_1,\ldots,x_p))
  ...
  \item Soit $n \in \mathbb N$. Supposons qu'il existe une unique fonction
    ...
    On définit alors
    \[\makeFun{f_{n+1}}{X_{n+1}}{Y}{(c,x_1,\ldots,x_p)}{f_c(f_n(x_1),\ldots,f_n(x_p))}\]
    On remarque que cette fonction vérifie bien la propriété. De plus, si une
    autre fonction $g$ vérifie la propriété, alors pour $x\in X_{n+1}$, on trouve par
...
un ensemble inductif en utilisant sa structure. Lorsqu'on définira une fonction en
utilisant le \cref{thm.PU.ind} sur un ensemble inductif $X$, on utilisera
généralement la formulation \og par induction sur la structure de $X$\fg voire
directement \og par induction sur $x$\fg si le contexte rend clair que $x$ est un
élément de $X$.
...
\begin{exercise}
  ...
  Définir une fonction $|-| : \mathrm{BinTree}(A) \to \bN$ donnant le
  nombre d'étiquettes d'un arbre.
\end{exercise}

\begin{exercise}
  En utilisant la structure inductive de $\bN$, définir la fonction
  $+ : \bN \times \bN \to \bN$. On pourra pour cela définir
...
\begin{proof}
  ...
  \[\bigcup_{n \in \bN} X_n \subseteq P\]
...
que dans le cas de $\bN$, engendré par $0$ et $S$, ce principe nous dit
qu'une partie $P$ contenant $0$ et telle que
$\forall n\in\bN, n\in P \implies n+1\in P$ est exactement $\bN$~:
...
\begin{exercise}
  ...
  définie dans l'\cref{ex.double}. Montrer que pour tout $n \in \bN$,
...
\begin{exercise}
  ...
  $\append : \List(A) \times \List(A)\to \List(A)$ par
...
\begin{definition}[Règle d'inférence]
  ...
  \og la règle $\mathrm r$ est valide pour $X = R$\fg{}, dans le cas où l'on
...
$R$ vérifie $\mathrm r$, alors $R$ est un point fixe de la fonction $\mathrm r$
...
\begin{theorem}[Knaster-Tarski (faible)\cite{Knaster}]\label{thm.ind.KT}
  ...
  que pour tout $B\in\powerset(E)$ tel que $f(B)=B$, $A\subseteq B$.
\end{theorem}
\begin{proof}
  Pour commencer, définissons l'ensemble des pré-points fixes de $f$, qui est
  \[\prefix(f) = \{ A \in \powerset(E)\mid f(A)\subseteq A\}\]
...
\begin{corollary}[Définition d'une relation inductive]
  ...
  $R = \vide$, on dira seulement que la relation est définie par $r$.
\end{corollary}
...
\begin{exercise}
  ...
  relation inductive définie par la règle $\mathrm r$ et
  \begin{prooftree}
...
\begin{exercise}
  ...
  \begin{center}
    ...
    \qquad
    \AxiomC{$\pair(\varlist)$}
    \UnaryInfC{$\pair(\cons(a,\cons(b,\varlist)))$}
    \DisplayProof
  \end{center}
...
\begin{exercise}\label{exo.pair_list}
  ...
  \begin{prooftree}
    \AxiomC{$\pair(\cons(a,\cons(b,\varlist)))$}
...
\begin{exercise}
  Soient des règles $\mathrm r_1,\ldots,\mathrm r_n$ et une règle $\mathrm r$.
  Montrer que toute règle $\mathrm r'$ dérivable dans
  $\mathrm r_1,\ldots,\mathrm r_n$ est encore dérivable dans
  $\mathrm r_1,\ldots,\mathrm r_n,\mathrm r$. En utilisant l'\cref{exo.pair_list},
  est-ce le cas en remplaçant l'hypothèse par $\mathrm r'$ admissible ?
\end{exercise}
...
il permet de dire à partir de $\pair(\ell)$ que $\ell$ est soit la liste
vide, soit $\cons(a,\cons(b,\ell))$ pour deux éléments
