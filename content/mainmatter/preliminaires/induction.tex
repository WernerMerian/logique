...
inductives, que nous présenterons. \`A ces deux objets correspondent deux
formalismes~: les grammaires de Backus-Naur pour les ensembles inductifs, les
points fixes et les treillis pour les relations inductives.
Comme la théorie des treillis sera étudiée plus tard dans cette partie
préliminaire, nous ne traiterons qu'un cas restreint suffisant pour le travail
sur l'induction.
...
donné dans ce chapitre aux multiples variantes qui seront utilisées par la
suite sans être systématiquement explicitées.

On peut trouver plus de détails sur les définitions par induction dans
\cite{Winskel1993}, au chapitre $3$, dans une forme plus orientée vers
l'application aux langages de programmation. On peut aussi citer le livre
\cite{Jacobs1999} qui permet d'avoir une version beaucoup plus abstraite des
structures inductives par le biais de la théorie des catégories, ainsi que
\cite{UnivalentFoundationsProgram2013} pour une présentation qui s'oriente
autour de la théorie des types. Ces deux derniers ouvrages dépassent cependant
...
à être plus facilement accepté qu'un ensemble tel que $\quot{\bR}{\bQ}$.
...
\begin{definition}[Signature]
  ...
  \index[notion]{Signature!Ensemble inductif}
\end{definition}
...
on peut par exemple se référer à \cite{Jacobs1999}.

Une signature sera généralement donnée sous forme dite de
Backus-Naur \cite{Backus1959}, abrégée en BNF
(\foreignexpr{Backus-Naur Form}). Cette
...
argument extérieur à la signature, on définit formellement une famille de
...
l'ensemble $C_1 \defeq \compr{c \in C}{\arite(c) = 0}$, puis l'ensemble $C_2$
est construit par
$C_2 \defeq C_1 \cup \compr{c(x_1,\ldots,x_n)}{c \in C, x_1,\ldots,
...
\begin{definition}[Ensemble inductif sur une signature]
  ...
  L'ensemble inductif engendré par $(C,\arite)$ est alors l'ensemble
  \[X \defeq \bigcup_{n\in \bN} X_n\]
  \index[notion]{Ensemble!Inductif}
\end{definition}
...
\begin{proof}
  ...
  \item supposons que $X_n \subseteq X_m$ pour tout $m \geq n$. Alors
    ...
    $X_{n+1}\subseteq X_m$ pour tout $m \geq n+1$.\qedhere
  \end{itemize}
\end{proof}
...
\begin{proof}
  Soit $(X_n)$ la suite d'ensembles définie précédemment telle que $X$ en est
...
\begin{remark}
  ...
  dans \cite{Cori2003}, a le mérite d'être la plus syntaxique possible
  ...
  voir présenté dans \cite{Jacobs1999} et qui donne un sens plus profond au
  \cref{thm.PU.ind}.
\end{remark}
...
une autre façon de penser ce fait est qu'une fonction partant d'un ensemble
...
\begin{theorem}[Propriété universelle des ensembles inductifs]\label{thm.PU.ind}
  ...
  \index[notion]{Propriété!Universelle!des Ensembles Inductifs}
\end{theorem}
...
\begin{proof}
  ...
  \begin{multline*}
    \forall n\in \bN, \exists ! \makeFunName{f_n}{X_n}{Y}, \forall c \in C,\\
    \forall (x_1,\ldots,x_p)\in (X_n)^{\arite(c)}, f_n((c,x_1,\ldots,x_p))
    = f_c(f_n(x_1),\ldots,f_n(x_p))
  \end{multline*}
  \begin{itemize}
  \item Si $n = 0$, il existe une unique fonction $\makeFunName{f_0}{\vide}{Y}$
    et elle vérifie la propriété par vacuité.
  \item Soit $n \in \bN$. Supposons qu'il existe une unique fonction
    $\makeFunName{f_n}{X_n}{Y}$ telle que
    ...
    \begin{align*}
      ...
      &= f_c(f_n(x_1),\ldots,f_n(x_p)) & \text{par définition }f_{n+1}\\
      &= g((c,x_1,\ldots,x_p))& \text{par hypothèse sur }g\\
      &= g(x) & \text{par l'égalité sur }x
    \end{align*}
    Donc pour tout $x\in X_{n+1}$, $f_{n+1}(x) = g(x)$, ce qui signifie que
...
\begin{example}%\label{ex.double}
  Donnons un premier exemple de fonction que l'on définit récursivement~: la
  fonction $\double : n\mapsto 2n$, définie de $\bN$ dans $\bN$. En effet, on
  peut la décrire par
  \begin{itemize}
  \item $\double(0) = 0$
...
\begin{exercise}
  ...
  racine de l'arbre (la première étiquette) et une feuille (un arbre vide qui
  est sous-arbre). On prend comme convention que $h(\nil) = 0$.
...
\begin{proof}
  ...
  effectuant une récurrence sur $n$~:
  \begin{itemize}
...
\begin{exercise}
  ...
  $\makeFunName{\tok \append \tok}{\List(A) \times \List(A)}{\List(A)}$ par
...
\begin{definition}[Règle d'inférence]
  ...
  \index[notion]{Règle d'inférence}
\end{definition}
...
\begin{example}
  ...
  \begin{prooftree}
    ...
    \UnaryInfC{$x\varRR x$}
  \end{prooftree}
  exprimant que $\varRR$ est réflexive. La fonction associée à $\refl$ est
...
\begin{example}
  ...
  \begin{prooftree}
    ...
    \AxiomC{$y\varRR z$}
    \RightLabel{$\trans$}
    \BinaryInfC{$x\varRR z$}
  \end{prooftree}
  Dans ce cas, l'image de $\varRR$ par $\trans$ est l'ensemble
...
\begin{theorem}[Knaster-Tarski (faible)\cite{KnasterTarski1928}]
  \label{thm.ind.KT}
...
\begin{exercise}
  ...
  $\varPt\subseteq f(\varPt)$. Montrer que l'on peut étendre le
  \cref{thm.ind.KT} pour trouver un plus petit point fixe $\alpha$ tel que
...
\begin{theorem}[Induction sur une relation]
  ...
  \index[notion]{Induction!sur une Relation}
\end{theorem}
...
\varFF(x_1,\ldots,x_n)\]
pour un certain prédicat $\varFF$, il suffit de montrer que ce prédicat est
stable par la règle définissant $\varRR$.

\begin{exercise}
  Soient désormais $n$ règles $\ruleR_1,\ldots,\ruleR_n$ incluant toutes
  un paramètre $\varRR$ d'arité $p$. Montrer qu'on peut leur associer une
  fonction $\makeFunName{\ruleR_{1,\ldots,n}}{\powerset(X^p)}{\powerset(X^p)}$
  croissante. En déduire un analogue des propositions précédentes pour un nombre
  fini de règles.
\end{exercise}
...
Une autre utilité de ce formalisme des règles d'inférence est de permettre de
...
\begin{definition}[Dérivation]
  ...
  \index[notion]{Règle d'inférence!Dérivation}
\end{definition}
...
\begin{definition}[Règle admissible]
  ...
  règles $\ruleR_1,\ldots,\ruleR_n$ si la relation définie par ces règles
  vérifie la règle $\ruleR$.
\end{definition}
...
\begin{definition}[Règle dérivable]
  ...
  $\ruleR$.
  \index[notion]{Règle d'inférence!Dérivable}
\end{definition}

\begin{exercise}
  Montrer qu'une règle dérivable est admissible et que si une règle est
  admissible pour une relation $\varRR$, alors la relation définie par cette
  règle sur $\varRR$ est exactement $\varRR$.
\end{exercise}
...
\begin{exercise}\label{exo.pair_list}
  ...
  \begin{center}
    ...
    \DisplayProof
  \end{center}
  Montrer que la première règle est valide pour $\varRR$, la seconde
  l'est-elle~?
\end{exercise}
...
\begin{exercise}[Clôtures symétriques]\label{exo.sClot}
  Soit $X$ un ensemble et $\varRR$ une relation binaire sur $X$. On définit les
  deux relations $\sClotMin{\varRR}$ et $\sClotMax{\varRR}$ respectivement par
  les règles
  \begin{center}
    \bottomAlignProof
    \AxiomC{$x \varRR y$}
    ...
    \DisplayProof
  \end{center}
  Montrer que $\sClotMax{\varRR}$ est une relation symétrique contenant
  $\varRR$, et que pour toute relation $\varRR'$ symétrique contenant $\varRR$,
...
\begin{exercise}[Relation d'équivalence]\label{exo.rstClot}
  ...
  \noindent
  Montrer que
  $\mathord{\rtsClot{\varRR}} = \mathord{\rtClot{(\sClotMax{\varRR})}}$.
...
\begin{problem}[\'Etude des arbres binaires de recherche]
  ...
  gauche et fils droit. Pour un arbre $\varTree$, les n\oe uds de $\varTree$
  désignent l'ensemble des sous-arbres de $\varTree$ qui ne sont pas $\nil$.

  \begin{pbQuest}
  \item Construire inductivement un prédicat $\isABR \subseteq \BinTree(A)$
    exprimant la propriété suivante sur $\varTree$~:
    \begin{quotation}
      Pour tout n\oe ud $\node(a,g,d)$ de $\varTree$, toutes les étiquettes
      apparaissant dans $g$ sont inférieures à $a$, et toutes les étiquettes
  ...
  appelée un invariant) d'être un $\ABR$.
  ...
  étiquettes, et d'en récupérer le minimum (respectivement le maximum). \`A
  l'aide de ces fonctions, il est possible de trier une liste.
  \begin{pbQuest}
...
\begin{problem}[La coinduction]
  ...
  dualee. C'est cette version que nous utiliserons pour la coinduction.
  ...
  \item Soit $X$ un ensemble et $(C,\arite)$ une signature. Construire une
    ...
    et des familles $\{f_c\}_{c \in C}$ telles que
    $\makeFunName{f_c}{X^{\arite(c)}}{X}$.
  \end{pbQuest}
  ...
  fonction $\makeFunName{h}{X}{Y}$ vérifiant
  ...
  peut se voir comme une notion duale, c'est-à-dire une notion dans laquelle
  on inverse le sens de lecture. Ainsi, \og le plus grand possible\fg signifie
  que toute autre structure est contenue dans l'ensemble co-inductif, qui est
  alors un objet dit terminal. De même, plutôt que de considérer des algèbres,
  on va considérer des co-algèbres.
  ...
  la donnée d'une fonction $\makeFunName{h}{X}{Y}$ telle que, en notant la
  fonction
  ...
  on travaille \latinexpr{a priori} dans un ensemble de parties, et le théorème
  ...
  notations précédentes, $\ruleR$ est dite co-satisfaite pour
  l'instanciation
  ...
  \item Soit $E$ un ensemble et
    ...
    $(X_0 \mapsto \varRR, x_1\mapsto a_1,\ldots x_n \mapsto a_n) \cosatisf
    \ruleR$
    alors la relation inductive engendrée par $\ruleR$ sur la relation
    $\varRR$ est contenue dans la relation coinductivement engendrée par
    $\ruleR$.
