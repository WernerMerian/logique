...
\lettrine{U}{n} principe particulièrement fréquent, en mathématiques, consiste
à reformuler des problème en termes topologiques.
Cette démarche intervient en algèbre, en analyse ou en géométrie. En logique,
des liens existent aussi avec la topologie, et nous avons donc besoin de bases
en topologie pour les explorer.
...
étudions des espaces topologiques, que l'on peut décrire comme des ensembles
(dont les éléments sont en général appelés \og points\fg{}) munis d'une notion
de proximité. La notion la plus générale de proximité peut être donnée au choix
...
\emphexpr{suffisamment proches}, et non proches de façon absolue.
...
\begin{definition}[Espace topologique]
  ...
  \item $\topoP{X}$ est stable par union quelconque~: si
    $\varPt\subseteq \topoP{X}$ alors $\bigcup \varPt \in \topoP{X}$.
  \end{itemize}

  En particulier, $X\in \topoP{X}$ et $\vide\in \topoP{X}$. On appelle
  ouvert de $X$ un élément $\varOp\in\topoP{X}$.
  \index[notion]{Espace!topologique}
  \index[notion]{Topologie}
  \index[notion]{Ouvert}
  \index[notation]{Topologie@$\topo$}
\end{definition}
...
\begin{remark}
  ...
  $\varPt\subseteq\topoP{X}$ n'est pas forcément l'intersection~: elle est le
  plus grand ouvert contenu dans $\bigcap \varPt$.

  On peut établir une version plus générale de la topologie en étudiant les
  algèbres de Heyting complètes, c'est ce que l'on appelle la topologie
  sans points, ou la théorie des locales
...
$\topo \mapsto \compr{X\setminus \varOp}{\varOp \in \topo}$. Voyons enfin
...
\begin{definition}[Base d'une topologie, topologie engendrée]
  ...
  \index[notion]{Base de topologie}
  \index[notion]{Topologie!Engendrée}
\end{definition}
...
ces exemples~: les espaces métriques.

\'Etant donnée une partie $\varPt\subseteq X$, qui n'est ni un ouvert ni un
fermé, disons par exemple $\intFO{1}{2}$, peut-on lui associer
...
\begin{definition}[Intérieur]
  Soit $(X,\topoP{X})$ un espace topologique et $\varPt\subseteq X$. On appelle
  intérieur de $\varPt$, que l'on note $\inter{\varPt}$, le plus grand ouvert
  contenu dans $\varPt$.
  \index[notion]{Ouvert!Intérieur}
  \index[notation]{Ouvert!Intérieure@$\inter{\varPt}$}
\end{definition}

\begin{definition}[Adhérence]
  Soit $(X,\topoP{X})$ un espace topologique et $\varPt\subseteq X$. On appelle
  adhérence de $\varPt$, que l'on note $\adher{\varPt}$, le plus petit fermé
  contenant $\varPt$.
  \index[notion]{Fermé!Adhérence}
  \index[notation]{Fermé!Adhérence@$\adher{\varPt}$}
\end{definition}
...
\begin{definition}[Point intérieur]
  Soit $(X,\topoP{X})$ un espace topologique et $\varPt\subseteq X$. On dit
  qu'un point $x$ est intérieur à $\varPt$ si $\varPt\in\voisP{x}$.
  \index[notion]{Point!Intérieur}
\end{definition}

\begin{definition}[Point adhérent]
  Soit $(X,\topoP{X})$ un espace topologique et $\varPt\subseteq X$. On dit
  qu'un point $x$ est adhérent à $\varPt$ si pour tout ouvert $\varOp$
  contenant $x$, $\varOp\cap \varPt\neq \vide$.
  \index[notion]{Point!Adhérence}
\end{definition}
...
\begin{definition}[Fonction continue]
  ...
  fonction $\makeFunName{f}{X}{Y}$ est dite continue si pour tout
  $\varOp\in\topoP{Y}$, l'ensemble $f^{-1}(\varOp)$ est un ouvert de $X$.
  \index[notion]{Fonction!Continue}
\end{definition}

\begin{remark}
  Si l'on considère un espace topologique comme l'algèbre de Heyting
  complète de son ensemble d'ouverts, alors la définition que nous venons de
  donner signifie que $f$ induit grâce à $f^{-1}$ un morphisme d'algèbres de
  Heyting complètes.
  Cependant, il est important de voir que là où $\makeFunName{f}{X}{Y}$,
  $\makeFunName{f^{-1}}{\powerset(Y)}{\powerset(X)}$, on a donc une inversion du
  sens entre le point de vue algébrique par un ensemble ordonné et le point de
  vue analytique comme un ensemble de points muni d'une topologie.
\end{remark}
...
\begin{definition}[Fonction ouverte]
  Soient $(X,\topoP{X})$ et $(Y,\topoP{Y})$ deux espaces topologiques. Une
  fonction $\makeFunName{f}{X}{Y}$ est dite ouverte si pour tout
  $\varOp \in \topoP{X}$, l'ensemble $f(\varOp)$ est un ouvert de $Y$.
  \index[notion]{Fonction!Ouverte}
\end{definition}
...
topologiques est, comme attendu, une fonction continue admettant un inverse
continu.

\begin{definition}[Homéomorphisme]
  Soient $(X,\topoP{X})$ et $(Y,\topoP{Y})$ deux espaces topologiques, une
  fonction $\makeFunName{f}{X}{Y}$ est appelée un homéomorphisme entre $X$ et
  $Y$ s'il existe une fonction continue $\makeFunName{g}{Y}{X}$ telle que
  $g\circ f = \id_X$ et $f\circ g = \id_Y$.
  \index[notion]{Isomorphisme!Homéomorphisme}
\end{definition}
...
\begin{definition}[Convergence d'un filtre]
  ...
  \index[notion]{Convergence!d'un Filtre}
\end{definition}
...
\begin{definition}[Convergence d'une fonction]
  ...
  $\makeFunName{f}{X}{Y}$. On définit le fait que $f$ converge vers $b\in Y$ en
  $a\in X$ (au sens des filtres) par
  \[\fillim_{x\to a}f(x) = b\defeq f\voisP{a} \supseteq\voisP{b}\]
  \index[notion]{Convergence!d'une Fonction}
\end{definition}
...
l'axiome $T_2$ (du nom de Félix \name{Hausdorff}, grand topologue des
...
\begin{property}\label{prop.carac.cont.fil}
  Soient deux espaces $(X,\topoP{X}), (Y,\topoP{Y})$ et une fonction
  $\makeFunName{f}{X}{Y}$. Alors $f$ est continue si et seulement si
...
\begin{proof}
  ...
  Comme $f$ est continue, cela signifie que $f^{-1}(\varOp)$ est aussi un
  ouvert.
  ...
  $X$. Pour cela, il nous suffit de montrer que pour tout
  $x \in f^{-1}(\varOp)$,
...
\begin{exercise}\label{exo.conv.suite}
  On dit qu'une suite $\makeFunName{u}{\bN}{X}$ converge lorsque le filtre image
  du filtre de Fréchet sur $\bN$ converge vers un certain élément
  $x\in X$. Montrer que cela revient à vérifier la formule usuelle~:
...
\begin{definition}[Valeur d'adhérence d'un filtre]
  ...
  \index[notion]{Valeur d'adhérence!d'un Filtre}
\end{definition}
...
par exemple $\bN$ avec la topologie discrète, le filtre de Fréchet n'a
pas de valeur d'adhérence, puisqu'un filtre de voisinage serait un ultrafiltre
...
d'éléments, mais le $X$ construit \og à l'infini\fg{} ne contient plus
d'élément, puisque tous auront été retiré de $X$.
...
\begin{definition}[Recouvrement, compacité]
  ...
  \index[notion]{Compacité!Topologique}
  \index[notion]{Recouvrement}
\end{definition}
...
\begin{proof}
  ...
  que l'ensemble des valeurs d'adhérence de $\varFil$ est non vide.
...
\begin{definition}[Topologie induite]
  ...
  \index[notion]{Topologie!Induite}
\end{definition}
...
$\makeFunName{\projj}{\prod X_i}{X_j}$ continues (les projections sont en
quelque sorte l'objet le plus primitif pour un produit). On veut donc une
topologie qui rende ces fonctions continues, mais beaucoup de choix de
topologies sont possibles. En particulier, pour tout $i \in I$ et $\varOp$
ouvert de $X_i$, on demande que $(\projj)^{-1}(\varOp)$ soit ouvert, et cet
ensemble est exactement $\prod_{i\neq j} X_i \times \varOp$.
...
l'intuition de la topologie produit reste celle que nous venons de donner.

\begin{definition}[Cylindre]
  Soit $(X_i)_{i\in I}$ une famille d'ensembles. Soit $F\subfin I$ et
  $\displaystyle (\varPt_i)_{i\in F} \in \prod_{i \in F} \powerset(X_i)$,
  on appelle cylindre en $(\varPt_i)_{i \in F}$ l'ensemble
  \[\cylin{\varPt} \defeq
  \compr{(x_i) \in \prod_{i \in I} X_i}{\forall j \in F, x_j \in \varPt_j}\]
  que l'on peut aussi écrire
  $\displaystyle\left(\prod_{j\notin F} X_j\right)\times
  \left(\prod_{i\in F} \varPt_i\right)$.
  \index[notion]{Cylindre}
\end{definition}
...
\begin{property}
  ...
  \compr{\cylin{\varPt}}
        {F\subfin I, (\varPt_i)_{i\in F}\in \prod_{i\in F}\topoP{X_i}}\]
...
\begin{proof}
  ...
  \item Soient $\cylin{\varPt}$ et $\cylin{\varPtB}$ deux éléments de la
    famille. Soient $F$ et $F'$ les parties finies de $I$ apparaissant dans la
    définition de $\varPt$ et de $\varPtB$. Remarquons d'abord que
    \[\cylin \varPt \cap \cylin \varPtB =
    \compr{(x_i)\in \prod_{i \in I} X_i}{ \forall j \in F, x_j \in \varPt_j,
      \forall k \in F', x_k \in \varPtB_j}
    \]

    \noindent
    On définit la famille $\varPtC$ sur $F\cup F'$ par
    \[\varPtC_i =
    \begin{cases}
      \varPt_i\cap \varPt_i\text{ si } i\in F\cap F'\\
      \varPt_i\text{ si } i \in F \setminus F'\\
      \varPtB_i\text{ si } i \in F'\setminus F
    \end{cases}\]
    alors $\cylin{\varPtC}$ appartient à la famille ci-dessus, car
    $\varPt_i\cap \varPtB_i$ est une intersection d'ouverts, donc un ouvert, et
    $\varPt_i$ (respectivement $\varPtB_i$) est un ouvert, donc on a bien une
    famille finie d'ouverts. De plus,
    $\cylin{\varPtC} = \cylin{\varPt} \cap \cylin{\varPtB}$ en déroulant les
    définitions.
  \end{itemize}
...
\noindent
La topologie produit est donc la topologie engendrée par les cylindres
d'ouverts.
...
\begin{definition}[Topologie produit]
  ...
  \index[notion]{Topologie!Produit}
\end{definition}
...
\begin{theorem}[Tychonov \cite{Tikhonov1935}]\label{thm.tychonov}
  ...
  \index[notion]{Théorème!de Tychonov}
\end{theorem}
...
\begin{proof}
  ...
  tout ouvert est une union de cylindre, donc si $(x_i)$ appartient à une union
...
\begin{remark}
  ...
  chaque $i \in I$, il est possible d'étendre $\varFil$ en un ultrafiltre,
  ...
  l'axiome du choix (\cite{Krivine2007} en donne par exemple une
  démonstration).
\end{remark}
...
Stone, remontant aux travaux du mathématicien Marshall Harvey \name{Stone}
(en particulier à \cite{Stone1936}), qui relie fortement la logique booléenne
et la topologie de certains espaces. Dans son énoncé le plus théorique, il
établit une équivalence de catégories, mais comme nous ne présentons pas les
éléments suffisants de théorie des catégories pour l'énoncer, nous nous
contenterons de montrer la construction des espaces à partir d'algèbres de Boole
et réciproquement, et le fait qu'elles sont presque inverses l'une de l'autre.
...
\begin{exercise}
  Soit $\makeFunName{f}{X}{Y}$ une fonction continue entre deux espaces
  topologiques $(X,\topoP{X})$ et $(Y,\topoP{Y})$. Montrer que $f^{-1}$ induit
  un morphisme d'algèbres de Boole entre $(\Comp{\topoP{X}},\subseteq)$ et
...
\begin{definition}[Espace de dimension zéro]
  ...
  \index[notion]{Espace!de Dimension Zéro}
\end{definition}
...
\begin{definition}[Spectre d'une algèbre de Boole]
  ...
  \index[notion]{Spectre d'une algèbre de Boole}
\end{definition}
...
\begin{proof}
  ...
  \item L'espace est compact~: on veut montrer qu'une famille de fermés
    ...
    intersection non vide. Comme la topologie est engendrée par les $\ext(x)$,
...
\begin{proof}
  ...
  \item Comme précédemment, on peut se ramener à montrer que l'image d'un
    ...
    à comprendre comme l'image directe de $\varCK$ par $\isoSt$).
...
\begin{theorem}[Dualité de Stone \cite{Stone1936}]
...
\begin{definition}[Distance]
  ...
  $\makeFunName{\vardis}{X \times X}{\bR_+}$ vérifiant les axiomes suivants~:
  \begin{itemize}
...
\begin{definition}[Boule, topologie métrique]
  ...
  \index[notion]{Topologie!Métrique}
  \index[notion]{Boule}
\end{definition}
...
\begin{definition}[Distance entre parties]
  Soit $(X,\vardis)$ un espace métrique, $\varPt\subseteq X$ et $x\in X$. On
  définit la distance de $x$ à $\varPt$ par
  \[\vardis(x,\varPt)\defeq \inf_{y\in \varPt} \vardis(x,y)\]
  et la distance d'une partie $\varPt\subseteq X$ à une partie
  $\varPtB\subseteq X$ par
  \[\vardis(\varPt,\varPtB)\defeq \inf_{y\in \varPt, z \in \varPtB} \vardis(y,z)\]
  \index[notion]{Distance!entre Parties}
\end{definition}
...
\begin{proposition}
  Soit $(X,d)$ un espace métrique et $\varPt\subseteq X$. Alors pour tout
  $x\in X$, on a l'équivalence suivante~:
  \[x\in \adher \varPt \iff \vardis(x,\varPt) = 0\]
\end{proposition}

\begin{proof}
  Supposons que $x\in \adher{\varPt}$ et montrons que $d(x,\varPt) = 0$. Soit
  $\vareps > 0$, on va montrer qu'il existe $y\in \varPt$ tel que
  $\vardis(x,y)< \vareps$~: par définition de $x\in \adher{\varPt}$, on peut
  trouver un élément dans $\boule{x}{\vareps}\cap \varPt$ (puisque
  l'intersection avec un ouvert contenant $x$ est non vide), mais cet élément
  $y$ vérifie justement $\vardis(x,y) < \vareps$, d'où le résultat.

  Réciproquement, supposons que $\vardis(x,\varPt) = 0$. Soit $\vareps > 0$, on
  veut montrer qu'il existe $y\in \boule{x}{\vareps}\cap \varPt$. Comme on sait
  que $\inf \vardis(x,\varPt) = 0$, on peut trouver $y\in \varPt$ tel que
  $\vardis(x,y) <\vareps$, et cet élément vérifie
  $y\in \boule{x}{\vareps}\cap \varPt$, d'où le fait que $x\in \adher \varPt$.
\end{proof}
...
en jambe (qui découle directement de la proposition précédente).

\begin{exercise}
  Soit un espace métrique $(X,\vardis)$, $\varPt\subseteq X$ et $x\in X$.
  Montrer que $x\in \adher{\varPt}$ si et seulement s'il existe une suite
  $(x_n)\in \varPt^{\bN}$ telle que $\lim x_n = x$.
\end{exercise}
...
\begin{definition}[Sous-suite]
  ...
  $\makeFunName{\varextr}{\bN}{\bN}$ une fonction strictement croissante telle
  que
...
suites. Notre analogie entre les suites et les filtres nous motive à donner une
définition séquentielle des compacts utilisant la notion de valeur d'adhérence,
c'est ce que nous faisons.

\begin{definition}[Compact séquentiel]
  Soit un espace métrique $(X,\vardis)$. On dit qu'une partie
  $\varPt\subseteq X$ est une partie séquentiellement compacte si toute suite
  $(x_n)\in Y^{\bN}$ admet une valeur d'adhérence.
  \index[notion]{Compacité!Séquentielle}
\end{definition}
...
Lebesgue\footnote{Nommé en l'honneur d'Henri \name{Lebesgue}} et la
précompacité.
...
\begin{definition}[Nombre de Lebesgue]
  ...
  \index[notion]{Nombre de Lebesgue}
\end{definition}
...
\begin{proof}
  ...
  séquentielle extraire une sous-suite convergente, dont on note $\varextr$
...
plus petits, ces ouverts trop petits ne sont pas importants à considérer~: tous
...
\begin{definition}[Précompacité]
  ...
  \index[notion]{Espace!Précompact}
\end{definition}
...
\begin{theorem}[Bolzano-Weierstrass généralisé]
  ...
  compacte.
  \index[notion]{Théorème!de Bolzano-Weierstrass généralisé}
\end{theorem}
...
\begin{proof}
  ...
  \[\bigcap_{n \in F}\adher{\compr{x_k}{k > n}} \supseteq
  \adher{\compr{x_k}{k > p}}\]
  ...
  notons $F \subfin X$ l'ensemble des points tels que
  $\compr{\boule{x}{\LebNb}}{x \in F}$ soit le recouvrement fini obtenu par
  ...
  recouvrement fini est alors $\compr{\varOp_{i_x}}{x \in F}$, puisque
  \[\varCK\subseteq \bigcup_{x \in F} \boule{x}{\LebNb}\subseteq
  \bigcup_{x \in F} \varOp_{i_x}\]
...
\begin{definition}[Densité]
  ...
  $\varPt\subseteq X$ est dense si $\adher{\varPt} = X$.
  \index[notion]{Partie!Dense}
\end{definition}
...
\begin{property}
  Soit $(X,\vardis)$ un espace métrique. Une partie $\varPt\subseteq X$ est
  dense si et seulement si pour tout $x\in X$ il existe une suite
  $(y_n)\in \varPt^{\bN}$ telle que $\lim y_n = x$.
\end{property}
...
\begin{exercise}\label{exo.carac.cont.suit}
  ...
  toute fonction $\makeFunName{f}{X}{Y}$ est continue si et seulement si, en
  notant $\ell(X)$ l'ensemble des suites convergentes à valeurs dans $X$, on a
  \[\forall (x_n) \in \ell(X),
...
\begin{definition}[Espace séparable]
  ...
  séparable s'il existe une partie $\varPt\subseteq X$ dense et dénombrable.
  \index[notion]{Espace!Séparable}
\end{definition}
...
notion est celle des suites de Cauchy\footnote{Nommé en l'honneur
d'Augustin-Louis \name{Cauchy}}, dont l'écart entre les différentes
valeurs tend vers $0$.
...
\begin{proof}
  ...
  trouve donc une fonction $\makeFunName{\varextr}{\bN}{\bN}$ strictement
...
\begin{definition}[Espace complètement métrisable]
  ...
  \index[notion]{Espace!Complètement métrisable}
\end{definition}
...
\begin{definition}[Isométrie]
  ...
  fonction $\makeFunName{f}{X}{Y}$ est une isométrie si elle vérifie la
  propriété suivante~:
  \[\forall x,y \in X, \vardis(x,y) = \vardis'(f(x),f(y))\]
  \index[notion]{Fonction!Isométrie}
\end{definition}
...
\begin{proposition}
  ...
  une partie dense.
  \index[notion]{Complétion!Métrique}
\end{proposition}
...
\begin{proof}
  ...
  \item la fonction $\compld$ induite par $\vardis'$ est bien définie. En
    ...
    constante sur les classes d'équivalence.
  ...
  \item si $x_n$ est une suite de Cauchy dans $\compl X$, alors par définition
    ...
    triangulaire, si $p,q > n_1$. Alors, on pose $n_3 = \max(n_0,n_1)$, et on
...
\begin{proof}
  On sait qu'il existe une partie dense dénombrable $\varPt\subseteq X$, et que
  $X$ est dense dans $\compl X$, donc $\varPt$ est dense (et dénombrable) dans
