...
ordonnés. Celle-ci est fortement liée à la logique, et permet entre autres de
...
similaires à ce qu'il connaît en théorie des catégories.
...
supérieure, inférieure, ainsi que les ordres bien fondés (dont la définition
reposae la notion d'élément minimal).
...
une algèbre de Boole est fortement simplifié, et nous aurons alors
...
\begin{definition}[Ensemble ordonné]
  ...
  \begin{itemize}
  \item $\leq$ est réflexive~: pour tout $x\in X$, $x\leq x$.
  ...
  \index[notion]{Ensemble!Ordonné}
  \index[notation]{Ordre!Large@$\leq$}
\end{definition}
...
\begin{remark}
  ...
  introduite dans \cite{Tarski1956}). Nous la verrons plus en détail plus
  tard.
  \index[notion]{Algèbre de Lindenbaum-Tarski}
\end{remark}
...
\begin{definition}[Ordre strict]
  ...
  \item $<$ est transitive.
  \end{itemize}
  \index[notion]{Ensemble!Ordonné!Strict}
  \index[notation]{Ordre!Stricte@$<$}
\end{definition}
...
\begin{definition}[Comparabilité, compatibilité]
  ...
  \index[notion]{\'Elément!Compatible!Vers le haut}
  \index[notion]{\'Elément!Compatible!Vers le bas}
  \index[notion]{\'Elément!Comparable}
  \index[notion]{Ensemble!Ordonné!Totalement ordonné}
\end{definition}
...
\begin{definition}[Majorant, minorant]
  ...
  Par abus de notation, on écrira $\varPt\leq x$ (respectivement $x\leq \varPt$)
  pour dire que $x$ est un majorant (respectivement un minorant) de $\varPt$.
  \index[notion]{Majorant}
  \index[notion]{Minorant}
\end{definition}
...
\begin{definition}[Borne supérieure, borne inférieure]
  ...
  \index[notion]{Borne!Supérieure}
  \index[notion]{Borne!Inférieure}
\end{definition}
...
\begin{definition}[Maximum, minimum]
  ...
  \index[notion]{Maximum}
  \index[notion]{Minimum}
\end{definition}
...
\begin{definition}[\'Elément maximal, minimal]
  ...
  que $y < x$).
  \index[notion]{\'Elément!Maximal}
  \index[notion]{\'Elément!Minimal}
\end{definition}
...
\begin{definition}[Chaîne, antichaîne]
  ...
  \index[notion]{Chaîne}
  \index[notion]{Antichaîne}
  \index[notion]{Antichaîne!Forte!Vers le bas}
  \index[notion]{Antichaîne!Forte!Vers le haut}
\end{definition}
...
\begin{definition}[Ensemble inductif]
  ...
  majorant.
  \index[notion]{Ensemble!Ordonné!Inductif}
\end{definition}
...
\begin{remark}
  ...
  ce nom reste le plus évocateur pour décrire les ensembles définis dans le
  \cref{chp.induction}.
\end{remark}

\begin{theorem}[Lemme de Zorn \cite{Zorn1935}]\label{thm.Zorn}
  Si $(X,\leq)$ est un ensemble inductif et $x\in X$, alors il existe un élément
  maximal $y \in X$ qui est supérieur à $x$.
  \index[notion]{Lemme!de Zorn}
\end{theorem}
...
Si le nom de lemme de Zorn a été donné, par exemple dans \cite{Tukey1940}, ce
...
L'article \cite{Zorn1935}, lui, présente avant tout l'efficacité de ce
...
\begin{definition}[Ordre bien fondé]\label{def.wf.ord}
  ...
  \index[notion]{Ensemble!Ordonné!Bien fondé}
\end{definition}
...
\begin{definition}[Bon ordre]
  ...
  \index[notion]{Ensemble!Ordonné!Bien ordonné}
\end{definition}
...
\begin{notation}
  ...
  \index[notation]{Clôture vers le bas@$\downcl{x}$}
\end{notation}
...
\begin{definition}[Demi-treillis]
  ...
  \index[notion]{Treillis!Demi-treillis}
\end{definition}
...
\begin{property}
  ...
  de deux éléments.
  \index[notation]{Demi-treillis!meet@$\opmt$}
  \index[notation]{Demi-treillis!join@$\opjn$}
\end{property}
...
\begin{definition}[Monoïde commutatif idempotent]\label{def.monoid.ci}
  ...
  \end{align*}
  \index[notion]{Monoïde!Commutatif idempotent}
\end{definition}
...
\begin{exercise}
  ...
  \index[notation]{Algèbre de Lindenbaum-Tarski@$\LinTar(\Sign)$}
\end{exercise}
...
\begin{definition}[Morphisme de demi-treillis]
  ...
  \index[notion]{Morphisme!de Demi-treillis}
\end{definition}
...
\begin{definition}[Morphisme de treillis]
  ...
  \index[notion]{Morphisme!de Treillis}
\end{definition}
...
\begin{definition}[Treillis distributif]
  ...
  \end{enumerate}
  \index[notion]{Treillis!Distributif}
\end{definition}
...
fondateur de l'approche algébrique de la logique \cite{Boole1854}) peut ainsi se
...
\begin{definition}[Algèbre de Boole]
  ...
  complément.
  \index[notion]{Treillis!Algèbre de Boole}
\end{definition}
...
\begin{remark}
  Chez certains auteurs, comme dans \cite{Halmos1974}, une algèbre de
...
algèbres de Boole, mais dans laquelle la logique appliquée est la logique
...
\begin{definition}[Pseudo-complément relatif]
  ...
  \index[notion]{Pseudo-complément}
\end{definition}

\begin{definition}[Algèbre de Heyting \cite{Heyting1930}]
  ...
  maximum.
  \index[notion]{Treillis!Algèbre de Heyting}
\end{definition}
...
artificielle que sa caractérisation algébrique. En effet, dans la version
...
\begin{definition}[Treillis complet]
  ...
  $\varPt\subseteq X$ possède à la fois une borne supérieure et une borne
  inférieure.
  \index[notion]{Treillis!Complet}
\end{definition}
...
\begin{theorem}[Knaster-Tarski \cite{Tarski1955kt}]\label{thm.KT}
  ...
  \index[notion]{Théorème!du Point fixe!de Knaster-Tarski}
\end{theorem}
...
Enfin, donnons deux mots à propos des algèbres de Heyting complètes.
...
\begin{definition}[Algèbre de Heyting complète]
  ...
  \index[notion]{Treillis!Algèbre de Heyting!Complète}
\end{definition}

Un point important pour ces structures est que l'existence des
pseudo-compléments apporte une nouvelle loi de distributivité, infinie. En
effet, étant donnée une algèbre de Heyting complète $\varBH$, une partie
$\varPt\subseteq \varBH$ et un élément $y\in \varBH$, on a l'identité~:
...
pseudo-complément relatif (\latinexpr{c.f.} \cref{exo.galois.con}). On peut
trouver des détails sur les liens entre ces propriétés dans \cite{Gratzer2011}.

\section{Filtres}

Enfin, présentons la notion de filtre, et la notion duale d'idéal. Nous allons
détailler comment se comportent les filtres dans les structures de plus en plus
...
\cite{Bourbaki2007Topologie1}, dans un but topologique (nous verrons dans le
...
Nous ne travaillerons pas sur les idéaux, mais donnons la définition car
celle-ci reste importante.

\begin{example}[Filtre principal]
...
\begin{definition}[Filtre engendré par une partie]
  ...
  \index[notion]{Filtre!Engendré}
  \index[notation]{Filtre!Engendré@$\filclose{\varPt}$}
\end{definition}
...
\begin{proof}
  ...
  \[\opMt (F\cup F') = \left(\opMt F\right)\opmt\left(\opMt F\right)
  \leq x\opmt y\]
...
\begin{definition}[Propriété de l'intersection finie]
  ...
  \index[notion]{Propriété!de l'Intersection finie}
\end{definition}
...
\begin{definition}[Ultrafiltre]
  ...
  \index[notion]{Filtre!Ultrafiltre}
\end{definition}

\begin{remark}
  L'inclusion entre filtres est aussi appelée \og finesse\fg~: un filtre
  $\varFil$ est dit plus fin qu'un filtre $\varFilB$ si
  $\varFil\supseteq\varFilB$. L'idée derrière cette appellation est que, si on
  considère la donnée de tous les \og gros\fg ensembles, ajouter des ensembles
  à notre liste va en ajouter des plus petits. En voyant un filtre comme la
  donnée des parties qui sont filtrées, les parties qu'on laisse passer sont de
  plus en plus fines à mesure qu'on augmente le nombre d'éléments dans le
  filtre.
\end{remark}
...
\begin{theorem}[Lemme de l'ultrafiltre]\label{thm.ultrafilter.lemma}
  ...
  \index[notion]{Lemme!de l'Ultrafiltre}
\end{theorem}
...
\begin{proof}
  ...
  formule $\varFF$, on sait que $\varFF\lor\lnot\varFF \in \varT'$, donc
...
\begin{exercise}[Quotient d'algèbres de Heyting]
  Dans le cas des algèbres de Boole, le complémentaire d'un filtre étant
...
\begin{exercise}[Connexion de Galois]\label{exo.galois.con}
  Soient $(X,\leq)$ et $(Y,\leq)$ deux ensembles ordonnés et
  $\makeFunName{f}{X}{Y}$ une fonction croissante. On appelle connexion
  de Galois à droite de $f$ une fonction $\makeFunName{g}{X}{Y}$ vérifiant
  \[\forall x \in X, \forall y \in Y, f(x) \leq y \iff x \leq g(y)\]
  et connexion de Galois à gauche de $f$ une fonction $\makeFunName{g}{X}{Y}$
  vérifiant
  \[\forall x \in X, \forall y \in Y, y \leq f(x) \iff g(y) \leq f(x)\]
  On note $f\dashv g$ pour dire \og $g$ est une connexion de Galois à droite de
  $f$\fg{} et $g\dashv f$ pour dire \og $g$ est une connexion de Galois à gauche
  de $f$\fg{}.
  \begin{enumerate}[label=(\roman*)]
  \item Montrer qu'une connexion de Galois est unique~: à $f$ fixée, si on a
    $f\dashv g$ et $f\dashv h$ alors $g = h$ (de même avec des connexions de
    Galois à gauche de $f$).
  \item Montrer que si $g$ est une connexion de Galois à droite (respectivement
    à gauche) de $f$, alors $g$ est une fonction croissante.
  \end{enumerate}
  Avec ces deux propriétés, on peut définir en général une connexion de Galois
  comme étant deux fonctions $f\dashv g$, où $f$ peut être définie à partie de
  $g$ et réciproquement.
  \begin{enumerate}[resume]
  \item Donner un exemple de fonction croissante qui n'admet pas de connexion
    de Galois à gauche (respectivement à droite).
  \end{enumerate}
  On peut se demander, dans le cas des treillis, si les connexions de Galois
  permettent de conserver leurs morphismes. On peut en fait donner un résultat
  partiel.
  \begin{enumerate}[resume]
  \item Soit $f\dashv g$ une connexion de Galois. Soit $\varPt\subseteq X$ une
    partie admettant une borne supérieure. Montrer que
    \[f\left(\opJn \varPt\right) = \opJn \compr{f(a)}{a\in\varPt}\]
    où la seconde borne supérieure existe. Montrer que pour $\varPt\subseteq Y$
    une partie admettant une borne inférieure, on a
    \[g\left(\opMt \varPt\right) = \opMt \compr{g(a)}{a\in\varPt}\]
  \item En déduire que, dans la définition d'une algèbre de Heyting, on peut
    limiter les conditions au fait d'être un treillis tel que toute paire
    d'éléments admet un pseudo-complément relatif.
  \end{enumerate}
  Enfin, une connexion de Galois permet aussi de construire ce qu'on appelle
  un opérateur de clôture.
  \begin{enumerate}[resume]
  \item Soit $f\dashv g$ une connexion de Galois. Montrer que $g\circ f$ est
    un opérateur de clôture, c'est-à-dire que les conditions suivantes sont
    vérifiées~:
    \begin{itemize}
    \item pour tout $x \in X$, $x \leq (g\circ f)(x)$~;
    \item $g\circ f$ est une fonction croissante~;
    \item pour tout $x \in X$, $(g\circ f)(x) = (g\circ f)\circ(g\circ f)(x)$.
    \end{itemize}
  \item Réciproquement, soit $(X,\leq)$ un ensemble ordonné et
    $\makeFunName{c}{X}{X}$ un opérateur de clôture. On définit
    \[Y \defeq \compr{x \in X}{c(x) \leq x}\]
    Montrer qu'on a une connexion de Galois $c\dashv \iota$ où $\iota$ est
    l'inclusion $Y\subseteq X$.
  \end{enumerate}
\end{exercise}
...
\begin{problem}[Algébrisation du calcul des prédicats
    \cite{Lawvere1969,Lawvere1970}]\label{pb.quasi.tripos}
  ...
  l'égalité. Dans tout ce problème, on considère une signature $\Sign$ fixée.

  \pbPart{Les renommages}

  L'ensemble de toutes les formules peut en fait être considéré comme une
  suite d'extensions~:
  \[\Prop(\Sign) \subseteq \Formula_1(\Sign)\subseteq\Formula_2(\Sign)\subseteq
  \cdots\]
  où, pour $\Var = \compr{x_n}{n \in \bN}$, on désigne par $\Formula_n(\Sign)$
  l'ensemble
  \[\Formula_n(\Sign) \defeq
  \compr{\varFF\in\Formula(\Sign)}{\VL(\varFF)\subseteq\{x_0,\ldots,x_{n-1}\}}\]
  \begin{pbQuest}
  \item Montrer que pour tout $n \in \bN$, l'algèbre de Lindenbaum-Tarski
    \[\LinTar_n(\Sign)\defeq \quot{\Formula_n(\Sign)}{\dvvd}\]
    est une algèbre de Boole. Montrer que, dans le cas de l'algèbre de
    Lindenbaum-Tarski intuitionniste, on a une algèbre de Heyting.
  \item Montrer que l'inclusion $\LinTar_n(\Sign)\subseteq\LinTar_{n+1}(\Sign)$
    est un morphisme d'algèbres de Boole.
  \end{pbQuest}
  \index[notation]{$\Formula_n$}

  Au sein d'un même ensemble $\LinTar_n(\Sign)$, les connecteurs logiques
  agissent sans souci. Cependant, pour ce qui est de la gestion des variables,
  on a besoin de donner une structure plus claire. Pour cela, on va introduire
  la notion de renommage. Pour $n,m \in \bN$, on appelle $(n,m)$-renommage
  une fonction
  \[\makeFunName{\varRen}{\intervInt{n}}{\intervInt{m}}\]
  L'intérêt de ces renommages est de définir une notion plus
  élémentaire de substitution, n'agissant qu'entre des variables.
  \begin{pbQuest}[resume]
  \item Définir, pour un $(n,m)$-renommage $\varRen$ et une formule
    $\varFF(x_0,\ldots,x_{n-1})\in\Formula_n(\Sign)$, l'action de $\varRen$
    sur $\varFF$ donnée par
    \[(\varRen\actRen\varFF)(x_0,\ldots,x_{m-1}) \defeq
    \varFF(x_{\varRen(0)},\ldots,x_{\varRen(n-1)})\]
  \item Montrer que pour tout $(n,m)$-renommage, l'action de $\varRen$ induit
    un morphisme d'algèbres de Boole
    \[\makeFun{\varRen\actRen\tok}{\LinTar_n(\Sign)}{\LinTar_m(\Sign)}
              {\clas{\varFF}}{\clas{\varRen\actRen\varFF}}\]
  \item Dans le cas du $(n,n+1)$-renommage donné par l'inclusion
    $\Formula_n(\Sign)\subseteq\Formula_{n+1}(\Sign)$, quelle action
    obtient-on~?
  \end{pbQuest}

  Remarquons qu'il est possible de construire ainsi des automorphismes
  d'algèbres de Boole, en considérant l'action par une permutation
  $\varPerm\in\Permut{n}$.
  \begin{pbQuest}[resume]
  \item Montrer que $\varPerm\actRen\tok$ est bien un automorphisme d'algèbres
    de Boole sur $\LinTar_n(\Sign)$, pour tout $n \in \bN$.
  \item Tout automorphisme de $\LinTar_n(\Sign)$ est-il de la forme
    $\varPerm\actRen\tok$ pour une certaine permutation
    $\varPerm\in\Permut{n}$~? \textit{Indication~: On pourra considérer le cas
    de la signature constituée d'un symbole de constante $\Cc$.}
  \end{pbQuest}

  \pbPart{Les quantificateurs et l'égalité}

  On cherche maintenant à définir les quantificateurs. Une première manière
  d'appréhender les quantificateurs au sein des algèbres $\LinTar_n(\Sign)$
  serait de considérer qu'ils constituent des fonctions
  \[
  \makeFun
      {\mathord{\forall},\mathord{\exists}}
      {\LinTar_{n+1}(\Sign)}{\LinTar_n(\Sign)}
      {\clas{\varFF(x_0,\ldots,x_n)}}
      {\clas{\quantif x_n,\varFF(x_0,\ldots,x_{n-1})}}
      \]
  mais on se propose de donner une construction plus fine, dans laquelle chaque
  quantification $\quantif x_i$ donne lieu à une fonction de quantification.

  Soit $n \in \bN$ et $i \in \intervInt{n}$. On définit la fonction de
  renommage
  \[
  \makeFunBis{\delta_{i}}{\intervInt{n}}{\intervP{0}{n}}
             {k}{k\text{ si } k \leq i}
             {k}{k+1\text{ si } k > i}
  \]
  \begin{pbQuest}
  \item Décrire, pour une formule $\varFF\in\Formula_{n}(\Sign)$, l'action
    $\delta_{i}\actRen\varFF$. Dans le cas où $i = n$, que retrouve-t-on~?
  \item Montrer que, pour tout entier $n \in \bN$ et entier $i\in\intervInt{n}$,
    toute formule $\varFF\in\Formula_n(\Sign)$ et toute formule
    $\varFP\in\Formula_{n+1}(\Sign)$, on a les deux équivalences
    \[\delta_i\actRen\varFF\vdP\varFP \iff \varFF \vdP \forall x_i,\varFP
    \qquad
    \varFP\vdP\delta_i\actRen\varFF\iff \exists x_i,\varFP\vdP\varFF\]
  \item Montrer que pour tout $n \in \bN$ et $i \in \intervInt{n}$, il existe
    une unique fonction
    $\makeFunName{(\forall x_i)}{\LinTar_{n+1}(\Sign)}{\LinTar_n(\Sign)}$
    et une unique fonction
    $\makeFunName{(\exists x_i)}{\LinTar_{n+1}(\Sign)}{\LinTar_n(\Sign)}$
    vérifiant la construction précédente.
  \end{pbQuest}

  On peut faire de même pour l'égalité, pour laquelle on introduit la fonction
  de renommage suivante, avec $n \in \bN$ et $i\in\intervInt{n}$~:
  \[
  \makeFunBis{\sigma_{i}}{\intervP{0}{n}}{\intervInt{n}}
             {k}{k\text{ si } k < n}
             {n}{i}
  \]
  \begin{pbQuest}[resume]
  \item Montrer que, pour tout entier $n \in \bN$ et entier $i\in\intervInt{n}$,
    toute formule $\varFF\in\Formula_{n}(\Sign)$ et toute formule
    $\varFP\in\Formula_{n+1}(\Sign)$, on a l'équivalence
    \[\varFF\vdP\sigma_{i}\actRen\varFP \iff \varFF\land(x_i = x_n)\vdP\varFP\]
  \item Montrer que la fonction d'égalité est définie de façon unique de cette
    façon.
  \end{pbQuest}

  On peut utiliser ces caractérisations algébriques pour déduire directement
  certains résultats.
  \begin{pbQuest}[resume]
  \item En utilisant l'\cref{exo.galois.con} et les questions précédentes,
    montrer qu'on a l'égalité suivante dans les algèbres de Lindenbaum-Tarski~:
    \begin{align*}
      \forall x, (\varFF(x)\land\varFP(x)) &\equiv
      (\forall x, \varFF(x))\land(\forall x, \varFP(x))\quad\\ \\
      \exists x, (\varFF(x)\lor\varFP(x)) &\equiv
      (\exists x, \varFF(x))\lor(\exists x, \varFP(x))\\ \\
      (\varFF(x) \land (x = y))\lor(\varFP(x) \land (x = y)) &\equiv
      (\varFF(x)\lor \varFP(x))\land (x = y)
    \end{align*}
  \item Les fonctions définies par les quantificateurs et l'égalité sont-elles
    des morphismes d'algèbres de Boole entre les algèbres de Lindenbaum-Tarski~?
  \end{pbQuest}

  \pbPart{Les conditions de cohérence}

  On peut désormais construire une définition d'une structure algébrique qui
  modélise la logique du premier ordre. On va appeler une telle structure
  une \emphexpr{hyperdoctrine}. Pour l'instant, on connait les composantes de
  ces structures~:
  \begin{itemize}
  \item pour tout $n \in \bN$, une algèbre de Boole $\varBB_n$~;
  \item pour tout $(n,m)$-renommage $\varRen$, un morphisme d'algèbre de Boole
    $\makeFunName{\relev{\varRen}}{\varBB_n}{\varBB_m}$ vérifiant les deux
    conditions de fonctorialité
    \[\relevB{\varRenB\circ\varRen} = \relev{\varRenB}\circ\relev{\varRen}
    \qquad \relevB{\id_n} = \id_{\varBB_n}\]
  \item chaque renommage $\delta_i$ a une connexion de Galois à gauche et à
    droite, c'est-à-dire deux fonctions
    $\makeFunName{E_i,F_i}{\varBB_{n+1}}{\varBB_n}$ telles que pour tous
    $x \in \varBB_n$, $y \in \varBB_{n+1}$~:
    \[\relevB{\delta_i}(x)\leq y \iff x \leq F_i(y)\qquad
    y\leq \relevB{\delta_i}(x) \iff E_i(y) \leq x\]
  \item chaque renommage $\sigma_i$ a une connexion de Galois à gauche,
    c'est-à-dire une fonction $\makeFunName{I_i}{\varBB_n}{\varBB_{n+1}}$ telle
    que pour tous $x \in \varBB_n$, $y \in \varBB_{n+1}$~:
    \[y \leq \relevB{\sigma_i}(x) \iff I_i(y) \leq x\]
  \end{itemize}
  Une telle structure ne suffit pas, car il nous faut aussi des conditions
  permettant d'avoir une cohérence entre l'action du renommage d'un côté et
  la quantification de l'autre.
  \begin{pbQuest}
  \item Soient deux entiers $n,m\in\bN$, deux renommages
    $\makeFunName{\varRen}{\intervP{0}{n}}{\intervP{0}{m}}$ et
    $\makeFunName{\varRenB}{\intervInt{n}}{\intervInt{m}}$ faisant
    commuter le diagramme
    \begin{center}
      \begin{tikzcd}
        \intervInt{n} \ar[dd,"\delta_i"]\ar[rr,"\varRenB"] & &
        \intervInt{m} \ar[dd,"\delta_j"] \\ \\
        \intervP{0}{n}\ar[rr,"\varRen"] & &\intervP{0}{m}
      \end{tikzcd}
    \end{center}
    c'est-à-dire tels que pour tout $k \in \intervInt{n}$ on a l'égalité
    $\varRen(\delta_i(k)) = \delta_j(\varRenB(k))$.
    Montrer que pour toute formule $\varFF\in\Formula_{n+1}(\Sign)$, on a les
    deux égalités
    \[\relevB{\varRenB}(\exists x_i, \varFF) \equiv
    \exists x_j,(\varRen\actRen(\varFF))\qquad
    \relevB{\varRenB}(\forall x_i, \varFF) \equiv
    \forall x_j,(\varRen\actRen(\varFF))\]
  \item Soient deux entiers $n,m\in\bN$, deux renommages
    $\makeFunName{\varRen}{\intervP{0}{n}}{\intervP{0}{m}}$ et
    $\makeFunName{\varRenB}{\intervInt{n}}{\intervInt{m}}$ faisant
    commuter le diagramme
    \begin{center}
      \begin{tikzcd}
        \intervP{0}{n} \ar[dd,"\sigma_i"]\ar[rr,"\varRen"] & &
        \intervP{0}{m} \ar[dd,"\sigma_j"] \\ \\
        \intervInt{n}\ar[rr,"\varRenB"] & &\intervInt{m}
      \end{tikzcd}
    \end{center}
    c'est-à-dire tels que pour tout $k \in \intervP{0}{n}$ on a l'égalité
    $\varRenB(\sigma_i(k)) = \sigma_j(\varRen(k))$.
    Montrer que pour toute formule $\varFF\in\Formula_{n}(\Sign)$, on a
    l'égalité
    \[(\varRenB\actRen(\varFF))\land (x_j = x_m) \equiv
    \varRen\actRen(\varFF\land (x_i = x_n))\]
  \end{pbQuest}
  On appelle ces égalités les conditions de Beck-Chevalley (respectivement pour
  les quantificateurs et pour l'égalité).

  Une autre condition, intervenant formellement dans les preuves et qui n'est
  pas présente dans les conditions énoncées, est la condition dite de Frobenius.
  \begin{pbQuest}[resume]
  \item Montrer que pour tout entier $n \in \bN$ et tout $i\in\intervInt{n}$,
    on a les deux égalités suivantes pour toute formule
    $\varFF\in\Formula_n(\Sign)$ et formule $\varFP\in\Formula_{n+1}(\Sign)$~:
    \[\exists x_i, ((\delta_i\actRen\varFF)\land \varFP) \equiv
    \varFF\land (\exists x_i, \varFP)\qquad
    \forall x_i, ((\delta_i\actRen\varFF) \lor \varFP) \equiv
    \varFF\lor (\forall x_i, \varFP)\]
  \end{pbQuest}
  On donne enfin la substitutivité de l'égalité.
  \begin{pbQuest}[resume]
  \item Montrer que pour tout entier $n \in \bN$ et tout $i\in\intervInt{n}$,
    pour toute formule $\varFF\in\Formula_{n}(\Sign)$, on a l'égalité
    \[(x_i = x_n) \land (\sigma_i\actRen\varFF) \equiv
    (x_i = x_n) \land \varFF\]
  \item Montrer aussi que l'égalité vérifie la condition de Frobenius,
    c'est-à-dire l'égalité
    \[(\varFF\land (\sigma_i\actRen\varFP))\land (x_i = x_n)\equiv
    (\varFF\land (x_i = x_n))\land \varFP\]
  \end{pbQuest}

  On peut ainsi définir une \emphexpr{hyperdoctrine} comme les données
  précédemment énumérées et des propriétés suivantes~:
  \begin{itemize}
  \item Pour deux renommages $\varRen,\varRenB$ et entiers $i,j$ tels que les
    expressions suivantes ont du sens, et tels que
    $\varRen\circ \delta_i = \delta_j\circ \varRenB$, on a les deux égalités
    \[\relevB{\varRenB}\circ E_i = E_j\circ\relevB{\varRen}\qquad
    \relevB{\varRenB}\circ F_i = F_j\circ\relevB{\varRen}\]
  \item Pour deux renommages $\varRen,\varRenB$ et entiers $i,j$ tels que les
    expressions suivantes ont du sens, et tels que
    $\varRenB\circ \sigma_i = \sigma_j\circ \varRen$, on a l'égalité
    \[\relevB{\varRen}\circ I_i = I_j \circ\relevB{\varRenB}\]
  \item Pour tout $n \in \bN$ et $i\in\intervInt{n}$, on a les égalités
    \begin{align*}
      E_i\circ ((x,y) \longmapsto \relevB{\delta_i}(x) \opmt y) &=
      ((x,y) \longmapsto x \opmt E_i(y))\\ \\
      F_i\circ ((x,y) \longmapsto \relevB{\delta_i}(x) \opjn y) &=
      ((x,y) \longmapsto x \opjn F_i(y))
    \end{align*}
  \item Pour tout $n \in \bN$ et $i\in\intervInt{n}$, on a l'égalité
    \[I_i\circ ((x,y)\longmapsto x\opmt \relevB{\sigma_i}(y)) =
    ((x,y) \longmapsto I_i(x)\opmt y)\]
  \end{itemize}
  
  On peut désormais montrer le théorème de correction de $\vdP$ vis-à-vis des
  \emphexpr{hyperdoctrines}.
  \begin{pbQuest}[resume]
  \item Soit $(\varBB_n)$ une hyperdoctrine. Construire pour chaque $n\in\bN$
    une fonction
    \[\makeFunName{f_n}{\Formula_n}{\varBB_n}\]
    associant à une formule à $n$ variables libres sur la signature vide, un
    élément de $\varBB_n$, de telle sorte que $f_n$ se relève en un morphisme
    d'algèbres de Boole $\makeFunName{\relev{f}_n}{\LinTar_n}{\varBB_n}$.
  \item Montrer que pour tout séquent dérivable
    $\varFF_1,\ldots,\varFF_k\vdP\varFP$ dans $\Formula_n$, on a l'inégalité
    \[\opMt_{i = 1}^{k} f_n\left(\clas{\varFF_i}\right) \leq
    f_n\left(\clas{\varFP}\right)\]
  \end{pbQuest}
\end{problem}

\clearpage

\begin{problem}[Logique temporelle linéaire \cite{Pnueli1977}]
  ...
  $\makeFunNamePartial{\env}{\Var}{\powerset(\infWd)}$ et une proposition
  ...
  \item Montrer les équivalences suivantes pour toutes propositions
    ...
    \begin{center}
      \begin{tabular}{cc}
        ...
        (\varFF\modU\varFP)\lor(\varFF\modU\varFC)$
        \\
        \\
        \multicolumn{2}{c}{
  ...
  \item Construire une proposition $\varFF$ telle que $\semFOL{\varFF}$ est
    l'ensemble des suites commençant par $0$.
  ...
  \item Montrer que
    $\semFOL{\varFF\lor\varFP} = \semFOL{\varFF}\cup\semFOL{\varFP}$,
    $\semFOL{\varFF\land\varFP} = \semFOL{\varFF}\cap\semFOL{\varFP}$ et
