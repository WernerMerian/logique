...
\begin{theorem}[Récursion bien fondée]\label{thm.recur.bf}
  ...
  \[\forall x \in X, g(x) = f(\{g(y)\mid y < x\})\]
\end{theorem}
...
ensembles bien ordonnés. On aurait pu renforcer ce théorème en utilisant comme
...
\begin{proposition}
  Soit $(X,\leq)$ un inf demi-treillis et $Y\subseteq X$ une partie non vide.
...
tous différents de l'ensemble vide. Evidemment, cette propriété n'a d'intérêt
...
\begin{proof}
  ...
  majorant de cette chaîne. Il nous suffit pour cela de vérifier que
  $\mathcal F_0$ est bien un filtre propre :
  \begin{itemize}
...
\begin{proof}
  ...
  particulier encore le cas pour $\vdclose{\mathcal T'} = \mathcal T'$).
