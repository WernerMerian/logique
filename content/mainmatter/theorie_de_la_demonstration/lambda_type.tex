...
$\semT{\varTypA}\subseteq\STLCSet$, de sorte qu'un typage comme plus haut soit associé
à la fonction
\[\makeFun{\varCG\tyR t : \varTypB}{\prod_{i = 1}^n\semT{\varTypA_i}}{\semT{\varTypB}}
...
$t$. Il suffit alors de construire une interprétation $\semT{\varTypA}$ adaptée pour
...
\begin{definition}[Substitution simultanée]
  ...
  $\makeFunNamePartial{\subSimul}{\lamVar}{\lamSet}$. La substitution $\subSimul(t)$
  est définie par induction sur $t$~:
  \begin{itemize}
...
$\subSimul(x_i) \in \semT{\varTypA_i}$ pour tout $i$, on veut montrer par induction
que
\[(x_i : \varTypA_i)_i\tyR t : \varTypB \implies \subSimul(t) \in \semT{\varTypB}\]
...
\begin{theorem}[Standardisation faible]
  ...
  \item pour tout contexte $E[\;]\in\Elim$ et termes $t,t_1,t_2,u \in \STLCSet$,
    si on a la réduction $E[\deltaSTLC{\coprojP{i}t}x{t_1}x{t_2}] \STLCred u$ alors
    \begin{itemize}
    \item soit $u = E[t_i[t/x]]$,
    \item soit $u = E'[\deltaSTLC{\coprojP{i}t}x{t_1}x{t_2}]$ où
      $E[\;]\STLCred E'[\;]$,
    \item soit $u = E[\deltaSTLC{\coprojP{i}t'}x{t_1}x{t_2}]$ où $t \STLCred t'$,
    \item soit $u = E[\deltaSTLC{\coprojP{i}t}x{t'}x{t_2}]$ où $t_1 \STLCred t'$,
    \item soit $u = E[\deltaSTLC{\coprojP{i}t}x{t'}x{t_2}]$ où $t_1 \STLCred t'$,
    \item soit $E = E'[[\;]\;v]$ et
...
\item de vérifier que $\semT{\varTypA}$ est stable par extension de tête faible,
  ...
  aussi.
\end{itemize}
La définition de $\semT{\varTypA\to\varTypB}$ ne pose pas de problème pour la première
étape, mais la définition de $\semT{\varTypA + \varTypB}$ en pose un.
...
construire un ensemble $\mathscr A$ de parties $A\subseteq \STLCSet$ dont on sait
qu'elles contiendront au moins les interprétations de types, et dont les propriétés
...
\begin{proof}
  ...
  chaque $\varSATB \in \varFam$ est clos par l'extension de tête faible telle que
  décrite dans la condition de saturation, on en déduit que $\varSATA$ est aussi
  clos pour cette extension. On peut montrer de même que $\bigcup \varFam$ est
  saturé.

  Montrons que $\bot_{\satSN}$ est saturé~:
  \begin{itemize}
  \item si $t \in \bot_{\satSN}$, alors $t \in \SN$, par définition~;
  \item pour tout $E[x]$, $E[x]\rtClot{\WHred} E[x]$, donc $E[x]\in\bot_{\satSN}$~;
...
\begin{lemma}
  Pour tous $\varSATA,\varSATB \in \satSN$, $\varSATA\satTO \varSATB \in \satSN$,
...
On peut finalement définir $\semT{\varTypA}$ de sorte à ce que la preuve se déroule
sans problème.
...
\begin{definition}[Interprétation des types]
  ...
  \begin{itemize}
  \item pour $\varTypBa$ un type de base, $\semT{\varTypBa} \defeq \top_{\satSN}$~;
...
\begin{center}
  \begin{tabular}{rcl}
    $\forall \varSATA$ &$\defeq$& $\compr{t \in \STLCSet}{\forall \btermt \in \Term,
    t\;\btermt \in \varSATA}$
    \\
    \\
    ...
    \\
    $\varSATA_=$ & $\defeq$ &
    $\{ t \in \STLCSet\mid \forall \varSATB \in \satSN,\forall \btermt \in \Term,
...
\begin{multline*}
  ...
  (\forall w \in \semT{\intT}, \forall a \in \varSATA, v\;w\;a \in \varSATA)\implies 
