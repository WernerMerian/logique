\chapter{Lambda-calcul}
\label{chp.lambda}

\minitoc

\lettrine{D}{ans} le \cref{chp.recur}, nous avons abordé deux systèmes de
calcul~: les fonctions récursives et les machines de Turing. Le formalisme des
fonctions récursives a été introduit par Kleene dans \cite{Kleene1936} sur la
base des fonctions récursives primitives mises en avant par Skolem
\cite{Skolem1923}. Cependant, Kleene utilisait aussi de son propre
aveu des intuitions issues d'un autre formalisme, celui de son professeur~:
Alonzo Church.

Le formalisme du $\lambda$-calcul a en effet été introduit par Church dans
\cite{Church1932,Church1933}, en tant que formalisme équationnel et logique~:
les termes
construits par les abstractions et les applications permettent de construire des
objets dans une certaine théorie logique (contrairement à la vision plus
moderne dans laquelle on considère un simple ensemble de $\lambda$-termes avec
des règles de réécriture). Relevons que Kleene et Rosser (ses élèves) ont
participé au développement du $\lambda$-calcul, en en prouvant d'abord
l'incohérence dans \cite{KleeneRosser1935}, mais aussi en participant à la
preuve que les fonctions codées par le $\lambda$-calcul sont exactement les
fonctions calculables au sens de Herbrand-Gödel.

Ce formalisme est donc plus ancien que ceux que nous avons déjà présentés.
Pourtant, le $\lambda$-calcul est encore au c\oe ur des recherches modernes en
informatique théorique et en logique, pour son aspect combinatoire et,
comme nous le verrons dans le \cref{chp.lambda_type}, pour la richesse des
systèmes de types.

Dans ce chapitre, nous nous concentrons sur l'aspect non typé du formalisme.
Les ressources ne manquent pas sur le sujet, et vont plus loin que ce chapitre
introductif~: nous conseillons \cite{Barendregt1984,Krivine1990,Selinger2018},
qui abordent aussi la version typée du calcul.

Nous abordons d'abord la définition des $\lambda$-termes, de la
$\beta$-réduction et la gestion de l'$\alpha$-équivalence. Nous donnons ensuite
le théorème d'équivalence entre le $\lambda$-calcul et les fonctions récursives.
Enfin, nous étudions les propriétés des réductions, en montrant la
propriété de Church-Rosser et le lemme de standardisation.

\section{Premiers pas en lambda-calcul}

Pour introduire le $\lambda$-calcul, commençons par présenter un cas simple
d'expression arithmétique~:
\[e\defeq x + 2y\]
l'expression $e$ comporte des variables libres, ce qui signifie qu'elle peut se
considérer comme une fonction qui, à une valeur fixée de $x$ et de $y$,
retourne la valeur de l'expression. On pourrait cependant souhaiter que nos
expressions permettent aussi de parler de fonctions, de la même façon qu'elles
parlent d'entiers.

Pour travailler avec des fonctions, deux points sont essentiels~:
\begin{itemize}
\item avoir un moyen de construire une fonction (un constructeur)~;
\item avoir un moyen d'appliquer une fonction à un argument (un destructeur).
\end{itemize}
La façon naturelle de construire une fonction est alors de considérer une
expression, telle que $e$, et d'abstraire une de ses variables pour en faire
un argument de fonction, ce que l'on notera
\[e' \defeq \lambda x.e\]
pour définir la fonction qui, à $a$ associe l'évaluation de $e$ où $x$ prend la
valeur $a$. L'expression résultante, $e'$, est alors une fonction pour chaque
paramètre $y$ fixé, et on pourrait donc définir encore
\[e'' \defeq \lambda y.e'\]
pour obtenir une fonction, retournant elle-même une fonction.

On obtient donc assez naturellement un formalisme dans lequel on se permet de
construire à partir d'expressions de nouvelles expressions représentant des
fonctions. L'autre étape, l'application d'une fonction à un argument, est alors
naturelle~: si $e$ et $e'$ sont des expressions, alors $e(e')$ est une nouvelle
expression qui s'évalue en l'application de la fonction représentée par $e$
en la valeur représentée par $e'$.

\subsection{Termes, substitutions, réductions}

Les deux principes d'abstraction et d'application nous suffisent en fait, comme
nous allons le voir, pour définir l'ensemble des fonctions calculables, dans
un formalisme où toutes les expressions sont des fonctions sur toutes les
expressions possibles. Au lieu d'évaluer des expressions, on décide alors de
travailler de façon purement syntaxique, en créant une relation représentant
l'évaluation d'une expression, encore appelée $\lambda$-terme.

\begin{definition}[$\lambda$-terme - version 1]
  On fixe un ensemble $\lamVar$ dénombrable de $\lambda$-variables, dont on
  notera les éléments $x,y,z,\ldots$ On définit l'ensemble $\lamSet_0$ des
  $\lambda$-termes par la BNF suivante~:
  \[t,u\inddefeq x\mid \lambda x.t\mid t\;u\]
  Un terme de la forme $x \in \lamVar$ est appelé une variable, un terme de la
  forme $\lambda x.t$ est appelé une abstraction, et un terme de la forme
  $t\;u$ est appelé une application.
  \index[notion]{$\lambda$-terme}
  \index[notation]{$\lambda x.t$}
  \index[notation]{$\lamVar$}
  \index[notation]{$\lamSet$}
\end{definition}

\begin{remark}
  Plusieurs conventions existent pour parenthéser des $\lambda$-termes. La
  convention à la Krivine, par exemple, est d'écrire $(t)u$ pour le terme
  $t\;u$ précédent. Dans notre cas, nous appliquons un parenthésage uniquement
  pour supprimer des ambiguïtés, et définissons l'application comme associative
  à gauche, ce qui signifie que $t\;u\;v$ doit se lire $(t\;u)\;v$.
\end{remark}

Les $\lambda$-termes représentent donc des fonctions lorsqu'ils sont de la
forme $\lambda x.t$, mais tout terme peut s'appliquer à tout autre terme, donc
même un terme de la forme $x$ ou $t\;u$ peut s'appliquer à un autre terme.

Cependant, c'est bien pour un terme de la forme $\lambda x.t$ que l'idée de
fonction prend tout son sens. On peut alors, naturellement, imaginer comment
réduire (de façon purement syntaxique) une fonction appliquée à un argument~:
si je considère $f(a)$ et que $f$ est de la forme $x \mapsto e$, alors
$f(a)$ correspond exactement à l'expression $e$ dans laquelle $x$ a été
substitué par $a$.

La même idée permet alors de définir une réduction sur les $\lambda$-termes,
en réduisant $(\lambda x.t)u$ en l'expression $t$ où $u$ remplace $x$. On a donc
besoin de la notion de substitution.

\begin{definition}[Substitution - version 1]
  Soient $t,u\in\lamSet_0$ et $x \in \lamVar$. On définit la substitution de
  $x$ par $u$ dans $t$, notée $t[u/x]$, par induction sur $t$~:
  \begin{itemize}
  \item si $t = x$, alors $t[u/x] \defeq u$.
  \item si $t = y$ où $y \in \lamVar$ et $y \neq x$, alors $t[u/x] \defeq t$.
  \item si $t = \lambda x.t'$, alors $t[u/x] \defeq t$.
  \item si $t = \lambda y.t'$ où $y \neq x$, alors
    $t[u/x] \defeq \lambda y.t'[u/x]$.
  \item si $t = t'\;t''$, alors $t[u/x] \defeq (t'[u/x])\;(t''[u/x])$.
  \end{itemize}
  \index[notion]{Substitution!de $\lambda$-termes}
  \index[notation]{Substitution!$t[u/x]$}
\end{definition}

En réalité, cette définition n'est pas bonne. Prenons par exemple le terme
$t \defeq \lambda x.\lambda y.z$ et le terme $u \defeq x$. On a alors l'égalité
\[t[u/z] = \lambda x.\lambda y.x\]
qui correspond moralement à prendre un argument, puis à retourner la fonction
constante en cet argument. Pourtant, si on considère
$t' \defeq \lambda a.\lambda y.z$, alors $t'[u/z]$ est un terme qui se
comporte bien différemment, car la variable $x$ de $u$ n'est alors pas capturée.

Néanmoins, il est clair que la fonction $x \mapsto e(x)$ et la fonction
$y \mapsto e(y)$ doivent être les mêmes, et la substitution ne devrait donc pas
donner deux résultats différents. Il nous faut donc d'abord gérer les variables
libres d'un terme.

\begin{definition}[Variable libre]
  Soit $t \in \lamSet_0$~; on définit par induction sur $t$ les ensembles
  $\VLie$ de ses variables liées et $\VL(t)$ de ses variables libres~:
  \begin{itemize}
  \item si $t = x$ alors $\VLie(x) \defeq \vide$ et $\VL(t) \defeq \{x\}$.
  \item si $t = \lambda x.t'$ alors $\VLie(t) \defeq \{x\} \cup \VLie(t')$ et
    $\VL(t) \defeq \VL(t')\setminus\{x\}$.
  \item si $t = t'\;t''$ alors $\VLie(t) \defeq \VLie(t') \cup \VLie(t'')$ et
    $\VL(t) \defeq \VL(t') \cup \VL(t'')$.
  \end{itemize}

  Si $\VL(t) = \vide$, alors on dit que $t$ est un terme clos.
  \index[notion]{Variable libre!dans un $\lambda$-terme}
  \index[notation]{$\VL$}
  \index[notation]{$\VLie$}
\end{definition}

Avec la notion de variable libre, on peut procéder à une correction importante~:
si $t,u \in \lamSet_0$, pour effectuer la substitution $t[u/x]$ et éviter toute
capture de variable, il nous suffit de demander que
$\VLie(t)\cap\VL(u)=\vide$. Cette condition, comme on l'a vu précédemment,
n'est pas pour autant toujours respectée, mais on remarque qu'il nous suffit
de modifier les variables liées de $t$ pour l'assurer. Intuitivement, puisque
$x$ est une variable muette dans $\lambda x.t$, il est parfaitement légitime
de considérer égaux les termes $\lambda x.t$ et $\lambda y.t[y/x]$.

\begin{definition}[$\alpha$-équivalence]
  On définit la relation d'$\alpha$-équivalence $\GaEq$ comme la plus petite
  congruence sur $\lamSet_0$ contenant les paires
  \[\lambda x.t \GaEq \lambda y.t[y/x]\]
  où $y \notin \VL(t)$.
  \index[notion]{\'Equivalence!$\alpha$}
\end{definition}

\begin{definition}[$\lambda$-terme - version 2]
  L'ensemble $\lamSet$ est défini comme le quotient par l'$\alpha$-équivalence~:
  \[\lamSet \defeq \quot{\lamSet_0}{\GaEq}\]
  \index[notion]{$\lambda$-terme}
  \index[notation]{$\lamSet$}
\end{definition}

\begin{remark}
  Dans $\lamSet$, l'ensemble $\VL$ est bien défini mais l'ensemble $\VLie$ ne
  l'est pas.
\end{remark}

On peut maintenant vérifier le lemme permettant d'éviter les captures de
variables.

\begin{lemma}
  Soient $t,u \in \lamSet_0$, alors il existe $t' \in \lamSet_0$ tel que
  $t' \GaEq t$ et tel que $\VLie(t') \cap \VL(u) =\vide$.
\end{lemma}

\begin{proof}
  Il suffit, pour chaque abstraction dans $t$ liant une variable $x$ libre dans
  $u$, de remplacer $x$ par une variable $y \notin\VL(u)$ (comme $\lamVar$
  est infini, il existe un tel $y$) grâce à l'$\alpha$-équivalence.
\end{proof}

On peut maintenant définir la substitution sans capture de variable.

\begin{definition}[Substitution - version 2]
  Soient $t,u\in\lamSet_0$ et $x \in \lamVar$. On définit la substitution sans
  capture, de $x$ par $u$ dans $t$, notée $t[u/x]$, par induction sur $t$~:
  \begin{itemize}
  \item si $t = x$, alors $t[u/x] \defeq u$.
  \item si $t = y$ où $y \in \lamVar$ et $y \neq x$, alors $t[u/x] \defeq t$.
  \item si $t = \lambda x.t'$, alors $t[u/x] \defeq t$.
  \item si $t = \lambda y.t'$ où $y \notin \VL(u)$, alors
    $t[u/x] \defeq \lambda y.t'[u/x]$.
  \item si $t = t'\;t''$, alors $t[u/x] \defeq (t'[u/x])\;(t''[u/x])$.
  \end{itemize}
  \index[notion]{Substitution!de $\lambda$-termes}
  \index[notation]{Substitution!$t[u/x]$}
\end{definition}

\begin{proposition}
  Pour tous $t,u \in \lamSet$ et $x \in \lamVar$, il existe au moins un
  représentant $t_0 \in t$ et un représentant $u_0 \in u$ tels que
  $t_0[u_0/x]$ est bien défini, et pour tous $t_1\in t, u_1\in u$,
  $t_0[u_0/x] \GaEq t_1[u_1/x]$ si cette dernière valeur est définie.
\end{proposition}

\begin{notation}
  On note désormais $t[u/x]$ le résultat de cette substitution relevé en tant
  que fonction $\lamSet\times\lamSet\times\lamVar \to \lamSet$.
\end{notation}

\begin{exercise}
  Montrer la proposition précédente.
\end{exercise}

On peut maintenant définir correctement la notion de réduction sur des
$\lambda$-termes.

\begin{definition}[$\beta$-réduction]
  On définit la $\beta$-réduction, notée $\bred$, comme la plus petite relation
  $\mathord{\bred}\subseteq\lamSet\times\lamSet$ compatible et contenant les
  paires
  \[(\lambda x.t)u\bred t[u/x]\]
  On appelle redex (\foreignexpr{reducible expression}) les sous-termes d'un
  terme $t$ de la forme $(\lambda x.u)v$.
  \index[notion]{Réduction!$\beta$}
  \index[notation]{$\bred$}
\end{definition}

\begin{remark}
  Notre définition de relation compatible ne peut techniquement plus s'appliquer
  puisqu'on travaille ici avec des classe d'équivalences et non directement
  avec l'ensemble inductif. Cependant, il est clair qu'on peut définir
  $\bred$ sur l'ensemble $\lamSet_0$ puis le relever à l'ensemble $\lamSet$.
  Ce type de procédure étant purement technique et peu profonde, on continuera
  de travailler de la même façon sur $\lamSet$ et sur $\lamSet_0$, en
  considérant que toutes les opérations et relations qu'on définit se relèvent
  en pratique en des opérations et relations sur $\lamSet$.
\end{remark}

Si on considère un $\lambda$-terme comme un programme, la réduction $\bred$
consiste à exécuter le programme, en obtenant potentiellement une valeur.

\begin{exercise}
  On définit les $\lambda$-termes~:
  \[\Ltom \defeq \lambda x.x\;x \qquad \LtOm\defeq \Ltom\;\Ltom\]
  Vérifier que $\LtOm\bred \LtOm$. En déduire que la relation $\bred$ n'est
  pas normalisante.
\end{exercise}

La $\beta$-réduction permet de définir un processus de calcul qui est dirigé~:
il cherche à contracter les redexes et poursuit ainsi autant que possible. On
peut cependant être intéressé par une autre relation à partir de la
$\beta$-réduction, en considérant que le processus de calcul nous donne une
notion d'égalité de programmes.

\begin{definition}[$\beta$-équivalence]
  On définit la relation $\Beq$ comme la plus petite congruence contenant
  $\bred$.
  \index[notion]{\'Equivalence!$\beta$}
  \index[notation]{$\Beq$}
\end{definition}

\begin{exercise}\label{exo.subst.subst}
  Soient $t,u,v \in \lamSet$, $x,y\in \lamVar$. Montrer l'égalité
  \[t[u/x][v/y] = t[v[u/x]/y][u/x]\]
\end{exercise}

\begin{exercise}\label{exo.subst.red}
  Soient $t,t',u\in\lamSet, x \in \lamVar$. Montrer que si $t\bred t'$ alors
  $t[u/x] \bred t'[u/x]$. Pour $t,u,u'\in\lamSet$ tels que $u \bred u'$,
  montrer que $t[u/x] \rtClot{\bred} t'[u/x]$. Montrer que, suivant la forme de
  $t$, la réduction précédente peut se faire en $0$ étapes ou, au contraire,
  en plus de $2$ étapes.
\end{exercise}

\paragraph{La curryfication}
Un point important est la gestion de fonctions à plusieurs variables. Pour
l'instant, le constructeur $\lambda$ ne permet d'abstraire qu'une variable.
Cependant, il est possible d'accumuler les abstractions, pour écrire par exemple
\[\lambda x.\lambda y.x\]
Comment se comporte un tel terme ? Il prend en argument un terme $t$, pour
retourner une fonction prenant elle-même en argument un terme $u$, puis
retourne finalement $t$. La fonction totale a donc deux arguments, en retournant
chaque fois une fonction tant que tous les arguments ne sont pas donnés. On
appelle ce procédé la curryfication, et on peut le décrire par l'isomorphisme
\[((X\times Y) \to Z)\cong X \to (Y \to Z)\]
On comprend alors l'intérêt de la notation $t\;u\;v$ pour écrire $(t\;u)\;v$,
puisqu'alors en voyant $t$ comme une fonction à deux arguments curryfiée,
$t\;u\;v$ représente bien la fonction à laquelle on donne les deux arguments
$u$ et $v$.
\index[notion]{Curryfication}

\subsection{La relation êta}

Continuons notre exploration du $\lambda$-calcul. Le lecteur catégoricien
peut trouver une ressemblance entre ce calcul et le langage catégorique, en ce
qu'il se concentre sur un langage portant sur les fonctions. Dans le cadre
catégorique, une catégorie possède des fonctions identités $\id_X$ et une
opération de composition $\circ$. Ces fonctions peuvent s'écrire dans le
$\lambda$-calcul.

\begin{definition}[Identité et composition]
  On définit les deux $\lambda$-termes suivants~:
  \[\id \defeq \lambda x.x \qquad \mathord{\circ} \defeq
  \lambda f.\lambda g.\lambda x.f\;(g\;x)\]
  On notera naturellement $f\circ g$ pour $\mathbin{\circ}f\;g$.
\end{definition}

Comme avec les catégories, on peut s'attendre à une théorie équationnelle
exprimant que $\id$ est neutre pour la composition, et que la composition est
associative.

Pour la composition, il nous suffit de faire le calcul~:
\begin{align*}
  f\circ (g\circ h) &\bred^2 \lambda x. f\;((g\circ h) x)\\
  &\bred^2 \lambda x. f\;(g\;(h\;x))\\
  (f\circ g)\circ h &\bred^2 \lambda x. (f\circ g)\;(h\;x)\\
  &\bred^2 \lambda x. f\;(g\;(h\;x))\\
  (f\circ g)\circ h &\Beq f\circ (g\circ h)
\end{align*}

On veut maintenant montrer que $\id\circ f \Beq f$, mais la réduction de
l'expression de gauche donne
\[\lambda x.f\;x\]
Cette fonction, représentant $x \mapsto f(x)$, semble parfaitement identique à
$f$~: elle associe les mêmes valeurs aux mêmes arguments. Cette égalité n'est
pourtant pas vérifiée dans $\Beq$ (on pourra le prouver avec la propriété de
Church-Rosser), mais elle peut s'axiomatiser.

\begin{definition}[\'Egalité extensionnelle]
  On définit l'égalité extensionnelle $\cong$ comme la plus petite congruence
  contenant $\Beq$ et vérifiant la règle
  \begin{prooftree}
    \AxiomC{$\forall u \in \lamSet, t\;u \cong t'\;u$}
    \UnaryInfC{$t \cong t'$}
  \end{prooftree}
  \index[notion]{\'Egalité!Extensionnelle}
\end{definition}

On peut alors vérifier que $\id\circ f \cong f$. Cependant, la relation $\cong$
est définie de façon sémantique, et quantifie sur un ensemble infini qu'est
$\lamSet$. On remarque pourtant que, dans le cas précédent, il nous suffit
de réduire $\lambda x.f\;x$ pour en déduire le résultat. On introduit alors
la règle $\eta$.

\begin{definition}[$\beta\eta$-réduction]
  On définit la $\beta\eta$-réduction comme la plus petite relation compatible
  contenant les paires
  \[(\lambda x.t)u \bered t[u/x] \qquad\qquad
  \lambda x.t\;x \bered t \quad x\notin \VL(t)\]
  On définit $\BEeq$ comme la plus petite congruence contenant ces paires.
  \index[notion]{Réduction!$\beta\eta$}
  \index[notation]{$\bered$}
  \index[notation]{$\BEeq$}
\end{definition}

\begin{exercise}
  Montrer que $\BEeq$ et $\cong$ sont des relations équivalentes.
\end{exercise}

On a donc une nouvelle relation qui égalise les termes définissant la même
interprétation en tant que fonction $\lamSet \to \lamSet$, et qui est
définissable de façon purement syntaxique.

Cependant, la relation $\Beq$ nous suffit pour l'usage de ce cours, et c'est
donc celle que nous privilégions.

\subsection{S'affranchir des alpha-équivalences}

La définition de $\lamSet$ en tant que quotient est insatisfaisante si on veut,
par exemple, formaliser des résultats de $\lambda$-calcul dans un assistant de
preuves. Devoir définir un système syntaxique par une relation d'équivalence et
avoir une substitution partiellement définie (bien que totale sur les classes
d'équivalence) donne lieu à des technicités si l'on souhaite raisonner de façon
purement formelle.

Heureusement, autoriser un discours légèrement informel n'est pas limitant dans
le cadre d'un cours, mais nous souhaitons néanmoins proposer une façon de
formaliser plus rigoureusement nos définitions, en utilisant les indices de
De Bruijn.

Cette sous-section est donc à part. En particulier, on redéfinit les opérations
et les objets en les notant de la même façon qu'ailleurs. Hors de cette
sous-section, toutes les définitions sont à considérer comme celles précédemment
données.

\begin{definition}[$\lambda$-termes \cite{DeBruijn1972}]
  On définit par induction l'ensemble $\lamSet$ des $\lambda$-termes par
  la BNF suivante~:
  \[t,u\inddefeq n\mid \lambda t\mid t\;u\]
  où $n \in \bN$.
\end{definition}

Cette définition peut paraître surprenante~: au lieu de variables, on introduit
des entiers, et on n'ajoute aucune variable liante aux abstractions. Le choix
des entiers n'est donc pas simplement une réécriture des variables en les
énumérant par $x_0,x_1,\ldots$ mais bien une façon alternative de lire des
variables.

La façon de lire un terme dans cette syntaxe est d'imaginer que les termes sont
interprétés dans un contexte, qui associe un objet aux variables libres. On
pourrait par exemple imaginer le terme $x\;y$ comme s'interprétant dans le
contexte $x \mapsto a, y \mapsto b$. Si on oublie le nom des variables, mais
qu'on garde en mémoire le contexte, on peut encore référer à l'objet $a$ ou à
l'objet $b$ associé, en donnant l'indice de l'objet dans le contexte.
Précisons que cette idée de contexte n'est pas formelle~: on n'y fait référence
que pour motiver la notation.

Ainsi, le terme $0$ représente par exemple la première valeur du contexte, et
le terme $1\;0$ représente l'application de la deuxième valeur du contexte à la
première (on fait commencer la numérotation à $0$).

Pour interpréter un terme $\lambda t$, on a alors besoin d'un contexte plus
grand pour interpréter $t$. Imaginons une liste $\varlist$ interprétant le
contexte de $\lambda t$, on va alors interpréter $t$ par la liste $a,\varlist$,
où $a$ est en première position. On peut alors voir une variable liée comme
indiquant de combien de $\lambda$ on remonte pour trouver son lieur.

\begin{figure}[ht!]
  \centering
  \hrule

  \vspace{0.2cm}
  \begin{tikzpicture}[node distance = 2cm, on grid, auto]
    \node (a0) at (0,0) {Contexte~: $[a,$};
      \node (b0) at (1.3,0){$b$};
      \node (c0) at (1.6,0) {$,c]$};
    \node (a) at (0,-1) {$\lambda$};
    \node (b) [below = of a] {$\lambda$};
    \node (c) [below = of b] {$@$};
    \node (d) [below left = of c] {$0$};
    \node (e) [below right = of c] {$3$};
    \draw (a) -- (b);
    \draw (b) -- (c);
    \draw (c) -- (d);
    \draw (c) -- (e);
    \draw[->,>=latex, color = red] (d) to[bend right] (c);
    \draw[->,>=latex, color = red] (e) to[bend right] (b0);
  \end{tikzpicture}

  \vspace{0.2cm}
  \hrule

  \vspace{0.4cm}
  \caption{Exemple de terme en indices de De Bruijn~: $\lambda\;\lambda\;0\;3$}
  \label{fig.deBruijn.ex}
  \hrule
\end{figure}

Dans la \cref{fig.deBruijn.ex}, on voit la différence entre une variable liée,
dont l'indice ramène à l'intérieur du terme, et une variable libre dont l'indice
amène à l'extérieur du terme. Remarquons aussi que la valeur d'une variable
libre dépend du nombre de lieurs qui la précède~: si on considère le terme
$\lambda\;\lambda\;0\;3$, la variable libre $3$ est en réalité la variable
$1$ au sens du contexte extérieur. L'ensemble des variables libres peut donc
être surévalué par l'indice maximal d'une variable apparaissant dans le terme,
une fois retranché le nombre de lieurs la précédant. On peut donc définir
$\VL$ non pas comme un ensemble, mais comme un nombre.

\begin{definition}[Variable libre maximale]
  On définit pour tout $t \in \lamSet$ le nombre $\VL(t)$ par induction sur
  $t$~:
  \begin{itemize}
  \item si $t = n$, alors $\VL(t) \defeq n + 1$~;
  \item si $t = \lambda u$, alors $\VL(t) \defeq \VL(u) - 1$~;
  \item si $t = u\;v$, alors $\VL(t) \defeq \max(\VL(u),\VL(v))$.
  \end{itemize}
  Le terme $t$ est clos lorsque $\VL(t) = 0$.
\end{definition}

La valeur de $\VL(t)$ peut s'interpréter par le fait que l'ensemble des
variables à l'intérieur de $t$ est strictement inférieur à $\VL(t)$ (modulo
l'ajout des lieurs).

On peut maintenant définir le relèvement d'un terme, qui correspond à une forme
d'affaiblissement sur les contextes.

\begin{definition}[Relèvement d'un terme]
  Soient $n,k \in \bN$. On définit par induction sur $t$ le relèvement de $t$
  par $k$ en $n$, par~:
  \begin{itemize}
  \item si $t = p$ et $p < k$, alors $\lift t k n \defeq p$.
  \item si $t = p$ et $p \leq k$, alors $\lift t k n \defeq p + n$.
  \item si $t = \lambda\;u$, alors
    $\lift t k n \defeq \lambda (\lift u {k+1} n)$.
  \item si $t = u\;v$, alors $\lift t k n \defeq (\lift u k n)\;(\lift v k n)$.
  \end{itemize}
\end{definition}

Le relèvement $\lift t k n$ correspond à l'ajout de $n$ objets au contexte
d'interprétation, à partir de l'indice $k$ du contexte. Son intérêt se joue dans
la définition de la substitution~: lorsqu'on définit $(\lambda\;t)[u/n]$, on
souhaite que la substitution de $u$ à l'intérieur de $t$ ne capture pas la
variable déclarée par le $\lambda$, ce qui se fait en relevant $u$ de $1$
pour que, dans le contexte de $u$, la variable du $\lambda$ soit prise en
compte.

\begin{definition}[Substitution]
  Soient $t,u \in \lamSet$ et $n \in \bN$. On définit inductivement sur $t$
  la substitution $t[u/n]$~:
  \begin{itemize}
  \item si $t = k < n$ alors $t[u/n] \defeq t$
  \item si $t = n$ alors $t[u/n] \defeq u$.
  \item si $t = k > n$ alors $t[u/n] \defeq k - 1$.
  \item si $t = \lambda\;u$ alors
    $t[u/n] \defeq \lambda(t[\lift u 0 1/n+1])$.
  \item si $t = t'\;t''$ alors $t[u/n] \defeq (t'[u/n])\;(t''[u/n])$.
  \end{itemize}
\end{definition}

\begin{definition}[$\beta$-réduction]
  La relation $\mathord{\bred} \subseteq \lamSet\times\lamSet$ est la plus
  petite relation compatible contenant les couples
  \[(\lambda t)u\bred t[u/0]\]
\end{definition}

\section{Calculabilité dans le lambda-calcul}

Comme dit auparavant, le $\lambda$-calcul permet de représenter toutes les
fonctions calculables (au sens des fonctions $\bN^k \partialto \bN$). On va
donc prouver ce théorème de représentation.

\subsection{Représenter des fonctions}

Tout d'abord, comment représenter des entiers ? C'est, en effet, la première
étape pour parler de fonctions $\bN^k\partialto\bN$. Pour cela, on introduit
les entiers de Church, qui sont une représentation des entiers en
$\lambda$-calcul, fondée sur l'idée qu'un entier $n$ est l'application d'une
action $n$ fois.

\begin{definition}[Entiers de Church]
  Soit $n\in\bN$~; on définit l'entier de Church $\encode n$ par
  \[\encode n \defeq \lambda f.\lambda x.f^n\;x\]
  \index[notion]{Entier!de Church}
\end{definition}

\begin{remark}
  Dans la notation $f^n\;x$, il faut lire $f\;(f\;(f\cdots(f\;x)\cdots))$, et
  non $f\;f\;f\cdots f\;x$.
\end{remark}

\begin{exercise}
  Montrer que pour tout $n \in \bN$, $\encode n$ est une forme normale
  (sous-entendu pour la relation $\bred$).
\end{exercise}

Un entier $n$ est donc représenté comme un terme prenant une fonction et
l'appliquant $n$ fois à son argument. On peut désormais définir la
représentation des fonctions d'entiers.

\begin{definition}[Représentation d'une fonction]
  Soit $\makeFunName{f}{\bN^k}{\bN}$. On dit que $t \in \lamSet$
  représente $f$ si
  \[\forall (n_1,\ldots,n_k) \in \bN^k,
  t\;\encode{n_1}\cdots\encode{n_k} \Beq \encode{f(n_1,\ldots,n_k)}\]
\end{definition}

\begin{notation}
  On utilisera, pour alléger les écritures, la notation
  $t\;\encode{\vec n}$ pour signifier $t\;\encode{n_1}\cdots\encode{n_k}$.
\end{notation}

Les fonctions récursives étant potentiellement partielles, on définit aussi la
représentation (au sens fort) des fonctions partielles.

\begin{definition}[Représentation d'une fonction partielle]
  Soit $\makeFunNamePartial{f}{\bN^k}{\bN}$. On dit que $t \in \lamSet$
  représente $f$ si pour tout $\vec n \in \bN^k$, si
  $f(\vec n) \convcal$ alors $t\;\encode{\vec n}$ se réduit en
  $\encode{f(\vec n)}$ et si $f(\vec n)\divcal$ alors
  $t\;\encode{\vec n}$ ne se réduit en aucune forme normale.
  \index[notion]{Fonction!Représentée!par un $\lambda$-terme}
\end{definition}

\begin{exercise}
  Montrer que la fonction nulle part définie est représentable par un
  $\lambda$-terme.
\end{exercise}

On donne aussi un encodage des tuples, de sorte à pouvoir définir des fonctions
calculables $\bN^k\partialto \bN^p$.

\begin{definition}[Représentation des paires]
  On définit les termes $\langle\tok,\tok\rangle$, $\projP{1}{}$ et $\proj{2}{}$
  comme suit~:
  \begin{align*}
    \langle\tok,\tok\rangle &\defeq \lambda t\;u.\lambda p. p\;t\;u\\
    \projP{1}{} &\defeq \lambda p.p\;(\lambda x\;y.x)\\
    \projP{2}{} &\defeq \lambda p.p\;(\lambda x\;y.y)
  \end{align*}
  On note $\langle t,u\rangle$ pour $\langle\tok,\tok\rangle\;t\;u$.
\end{definition}

On vérifie par un calcul que $\projP{1}{}\langle t,u\rangle \bred t$ et que
$\projP{2}{}\langle t,u\rangle \bred u$~:
\begin{align*}
  \projP{1}{}\langle t,u\rangle &\bred \langle t,u\rangle (\lambda x\;y.x)\\
  &\bred^3 (\lambda x\;y.x)\;t\;u\\
  &\bred^2 t \\
  \projP{2}{}\langle t,u\rangle &\bred \langle t,u\rangle (\lambda x\;y.y)\\
  &\bred^3 (\lambda x\;y.y)\;t\;u\\
  &\bred^2 u
\end{align*}
On peut ainsi travailler avec des paires de termes, et représenter ainsi des
tuples d'entiers.

\begin{exercise}
  Donner un encodage des $k$-uplets de termes, avec une fonction construisant
  ces $k$-uplets et des fonctions de projection correspondantes.
\end{exercise}

\noindent
On peut aussi encoder les booléens.

\begin{definition}[Représentation des booléens]
  On définit les termes $\encode{\top}$ et $\encode{\bot}$ par
  \begin{align*}
    \encode{\top} &\defeq \lambda x\;y.x\\
    \encode{\bot} &\defeq \lambda x\;y.y
  \end{align*}
  Une fonction $\makeFunName{f}{\intB^k}{\intB}$ est représentée par
  $t \in \lamSet$ si $t\;\encode{\vec b} \Beq \encode{f(\vec b)}$ en posant
  $\encode 1 \defeq \encode{\top}$ et $\encode 0 \defeq \encode{\bot}$.
\end{definition}

\begin{exercise}
  Montrer que les opérations booléennes $\lor,\land,\lnot$ sont
  représentables par des $\lambda$-termes.
\end{exercise}

\begin{definition}[Représentation des ensembles d'entiers]
  Une partie $X\subseteq \bN^k$ est représentable par un terme $t$ si
  \[\forall \vec n \in \bN^k, \begin{cases}
    \vec n \in X \iff t\;\encode{\vec n} \Beq \encode{\top}\\
    \vec n \notin X \iff t \;\encode{\vec n} \Beq \encode{\bot}
  \end{cases}\]
\end{definition}

\begin{proposition}
  La partie $\{0\}\subseteq \bN$ est représentable.
\end{proposition}

\begin{proof}
  On donne le terme représentant cette partie~:
  \[\eqZ\defeq \lambda n.n\;(\lambda x.\encode{\bot})\;\encode{\top}\]
  On vérifie par récurrence sur $n$ que $\eqZ\;\encode{n}$ se réduit à $\top$
  si $n = 0$, et à $\bot$ sinon~:
  \begin{itemize}
  \item
    $\eqZ\;\encode 0 = (\lambda f\;x.x)\;(\lambda x.\encode{\bot})\;
    \encode{\top}\bred^2\encode{\top}$.
  \item
    $\eqZ\;\encode{n+1} \bred^2 (\lambda x.\encode{\bot})\;u$ où $u$ est un
    terme sans importance, puisque ce terme se réduit dans tous les cas vers
    $\encode{\bot}$.
  \end{itemize}
\end{proof}

\noindent
On donne aussi la représentation du branchement conditionnel
\foreignexpr{if/then/else}.

\begin{proposition}
  Soient $\makeFunName{f,g}{\bN^k}{\bN}$ des fonctions représentées
  respectivement par $t$ et $u$, et $X \subseteq \bN^k$ une partie
  représentée par $v$. Alors la fonction qui, si $\vec n \in X$, renvoie
  $f(\vec n)$, et sinon renvoie $g(\vec n)$, est représentée par
  \[\ifrm v \thenrm t \elserm u \defeq
  \lambda \vec n.v\;\vec n\;(t\;\vec n)\;(u\;\vec n)\]
\end{proposition}

\begin{proof}
  Si $\vec n \in X$, alors $v\;\vec n \rtClot{\bred} \encode{\top}$, et
  $\encode{\top}\;a\;b \bred a$, donc
  $\ifrm v \thenrm t \elserm u \rtClot{\bred} t\;\vec n$, d'où le résultat
  par représentation de $f$ par $t$.

  Si $\vec n \notin X$, alors $v\;\vec n \rtClot{\bred} \encode{\bot}$, et
  $\encode{\bot}\;a\;b\bred b$, d'où le résultat par représentation
  de $g$ par $u$.
\end{proof}

\subsection{Fonctions primitives récursives}

On commence par encoder les fonctions RP. Les projections se représentent
facilement en définissant
$\projP{i}{k} \defeq \lambda x_1\cdots x_k.x_i$,
la fonction nulle est simplement
$\lambda x.\encode 0$
et la fonction successeur peut s'écrire par
$\Ss \defeq \lambda n.\lambda f.\lambda x.f\;(n\;f\;x)$.

\begin{exercise}
  Montrer que les termes donnés représentent bien les fonctions annoncées.
\end{exercise}

La composition ayant déjà été écrite dans le cas de fonctions unaires, il est
direct de généraliser la construction. On souhaite donc encoder la récursion
primitive. Remarquons que, naturellement, un entier $n$ représente un itérateur.
On peut donc simplement appliquer $n$ pour obtenir le résultat attendu, en
faisant attention à garder à l'esprit les paramètres de la récursion primitive.

\begin{proposition}
  Si $\makeFunName{f}{\bN^{k+2}}{\bN}$ est représentée par $t$, que
  $\makeFunName{g}{\bN^k}{\bN}$ est représentée par $u$, alors la fonction
  $\rec(f,g)$ est représentable, de représentation donnée par
  \[\rec(t,u)\defeq \lambda \vec n. \lambda n. \projP{2}{}(n\;
  (\lambda x. \langle \Ss\;(\projP{1}{}x),
  t\;\vec n\;(\projP{2}{}x)\;(\projP{1}{}x)\rangle)\;
  \langle \encode 0, u\;\vec n\rangle)\]
\end{proposition}

\begin{proof}
  Notons $\rec_0(t,u)$ le terme obtenu en enlevant la projection $\projP{2}{}$
  précédant le $n$. Montrons par récurrence sur $n$ que pour tout
  $\vec n \in \bN^k$, $\rec_0(t,u)\;\encode{\vec n}\;\encode n$ se
  réduit en $\langle \encode n,\encode{\rec(f,g)(\vec n,n)}\rangle$~:
  \begin{itemize}
  \item si $n = 0$, alors le terme se réduit en
    $\langle \encode 0,u\;\vec n\rangle$ et, comme $u$ représente
    $g$, on en déduit le résultat.
  \item si $n = p + 1$, alors le terme se réduit en
    $(\lambda x.\langle \Ss\;(\projP{1}{}x), t \;\vec n\;(\projP{2}{}x)\;
    (\projP{1}{}x)\rangle)\;u'$
    où $u'$, par hypothèse de récurrence, se réduit en
    $\langle \encode p, \encode{\rec(f,g)(\vec n, p)}$ donc,
    comme $t$ représente $f$, le terme total se réduit en
    $\langle \encode n,\encode{\rec(f,g)(\vec n,n)}$, d'où le résultat.
  \end{itemize}
  On en déduit par récurrence, et en composant par $\projP{2}{}$, que
  $\rec(t,u)$ représente $\rec(f,g)$.
\end{proof}

\begin{remark}
  Si on prend $f = \lambda n\;p.n$, on obtient pour $\rec_0$ une fonction qui
  retourne $\langle 0,0\rangle$, puis $\langle n,n-1\rangle$, pour finalement
  retourner $n-1$ une fois la projection effectuée. On peut donc obtenir
  l'opération de prédécesseur de cette manière, puis l'opération $-$ en
  itérant le prédécesseur.
\end{remark}

On en déduit un premier théorème de représentation.

\begin{theorem}[Représentation des fonctions RP]
  Pour toute fonction $\makeFunName{f}{\bN^k}{\bN}$ RP, il existe un terme
  $t \in \lamSet$ représentant $f$.
\end{theorem}

\subsection{Cas de la minimisation}

L'opérateur $\muC$ de minimisation, permettant de passer des fonctions RP aux
fonctions récursives, demande de construire un itérateur potentiellement
infini (c'est la cause du passage de fonction totale à fonction partielle).
Un moyen simple de créer une telle fonction est d'utiliser un point fixe, de
façon analogue au \cref{thm.Kleene.fix}. On peut alors définir $\muC(f)$
comme un point fixe~: on construit une fonction qui appelle toutes les
valeurs de $f$ successivement, et retourne l'indice de la première valeur
d'annulation de $f$. Puisqu'en $\lambda$-calcul, tous les objets sont des
fonctions, le théorème de point fixe peut se construire bien plus facilement
en donnant un terme construisant explicitement un point fixe.

\begin{definition}[Combinateur de point fixe]
  On définit le point fixe de Curry par~:
  \[\Lty \defeq \lambda x.f\;(x\;x)
  \LtY \defeq \lambda f.\Lty\;\Lty\]
  On définit le point fixe de Turing par~:
  \[\Ltt \defeq \lambda x\;y.y\;(x\;x\;y) \qquad
  \LtT \defeq \Ltt\;\Ltt\]
  Pour chacun des deux termes, que l'on notera $T$, et pour tout
  $t \in \lamSet$, on a l'égalité
  \[T\;t \Beq t\;(T\;t)\]
  \index[notion]{Combinateur de point fixe}
  \index[notation]{$\LtY$}
  \index[notation]{$\LtT$}
\end{definition}

\begin{proof}
  Il suffit d'effectuer la réduction~:
  \begin{align*}
    \LtY\;t &\bred (\lambda x.t\;(x\;x))\;(\lambda x.t\;(x\;x))\\
    &\bred t\;(\Lty[t/f]\;\Lty[t/f]) \\
    t(\LtY\;t) &\bred t\;(\Lty[t/f]\;\Lty[t/f])\\
    \LtT\;t &\bred (\lambda y.y\;(\Ltt\;\Ltt\;y))\;t \\
    &\bred t\;(\Ltt\;\Ltt\;t) \\
    &= t\;(\LtT\;t)
  \end{align*}
  D'où le résultat.
\end{proof}

Le combinateur $\LtY$ est le plus naturel à considérer quand on adapte
l'argument diagonal~: la forme du point fixe construit dans le
\cref{thm.Kleene.fix} est exactement la même, mais où le théorème $\Smn{m}{n}$
est nécessaire pour construire des abstractions. Dans le $\lambda$-calcul,
le terme en devient donc d'autant plus simple.

Le terme $\LtT$, lui, a l'avantage de donner un point fixe uniquement par
réduction. Au contraire, $\LtY\;t$ n'est égal à $t\;(\LtY\;t)$ qu'en effectuant
une antiréduction à la fin.

On peut maintenant représenter l'opérateur $\muC$.

\begin{proposition}
  Soit $\makeFunNamePartial{f}{\bN^{k+1}}{\bN}$ représentée par le terme
  $t$, alors $\muC(f)$ est représentée par le terme
  \[\muC(t) \defeq \lambda
  \vec n.\LtT(\lambda f.\lambda m.\ifrm \eqZ t\;\vec n\;m
  \thenrm m \elserm f\;(\Ss m))\;\encode 0\]
\end{proposition}

\begin{proof}
  Fixons le paramètre $\vec n$.
  Supposons que $f$ admette un point d'annulation, que son plus petit
  point d'annulation est $k \in \bN$ et que $f$ est définie pour toute valeur
  inférieure à $k$. Notons $u$ la fonction définie par
  le point fixe $\LtT$, et prouvons par récurrence sur $n$ que
  $u\;\encode{k-n}$ retourne $k$~:
  \begin{itemize}
  \item pour $n = 0$, on considère donc $u\;\encode k$, qui se réduit en
    $\ifrm\eqZ t\;\encode{\vec n}\;\encode k\thenrm \encode k\elserm
    u\;(S\;\encode k)$, qui se réduit lui-même directement en
    $\encode k$.
  \item pour $n = p + 1$, on sait que $f(k-n)\neq 0$, donc
    le terme $u\;\encode{k-n}$ se réduit en
    $u\;(\Ss \encode{k-n})$, mais $\Ss \encode{k-n} \bred \encode{k-p}$, d'où
    le résultat par récurrence.
  \end{itemize}

  Si $f$ n'est pas définie en un point $i$ précédent son plus petit point
  d'annulation, alors une récurrence similaire nous permet de montrer que
  $u\;p \bred \ifrm\eqZ t\;\encode{\vec n}\;\encode i\thenrm \encode i \elserm
  u\;(\Ss \encode i)$, et comme le terme $t\;\encode{\vec n}\;\encode i$ n'a
  pas de forme normale et est en tête du terme, on en déduit que le terme
  total n'a pas de forme normale.

  Si $f$ n'a pas de point d'annulation, alors on peut montrer que
  $\muC(t)$ s'exécute en au moins $n$ étapes.
\end{proof}

On en déduit la représentation des fonctions calculables.

\begin{theorem}[Représentation des fonctions récursives]
  Toute fonction récursive $\makeFunNamePartial{f}{\bN^k}{\bN}$ est
  représentable au sens fort par un $\lambda$-terme.
\end{theorem}

\noindent
Le sens réciproque de ce théorème de représentation est laissé en exercice.

\begin{exercise}
  Soit $t\in\lamSet$. On définit son codage de Gödel par induction sur $t$
  (pris comme écrit en indices de De Bruijn)~:
  \begin{itemize}
  \item si $t = n$, alors $\godcod t \defeq 3n$
  \item si $t = \lambda t'$, alors $\godcod t \defeq 3\godcod{t'} + 1$
  \item si $t = t'\;t''$, alors
    $\godcod t \defeq 3\langle \godcod{t'},\godcod{t''}\rangle + 2$.
  \end{itemize}

  Montrer que la relation $\bred$ est calculable sur les codes de termes.
  En déduire que toute fonction $\bN^k\partialto \bN$ représentée par un
  terme $t$ est calculable.
\end{exercise}

\subsection{Conséquence de la représentation}

Puisque le $\lambda$-calcul est un formalisme équivalent aux autres formalismes
de calculabilité, on déduit les faits suivants~:

\begin{proposition}
  Il existe une fonction $\makeFunName{\godcod\tok}{\lamSet}{\bN}$ et un terme
  $\LtU$ tels que
  \[\forall t \in \lamSet, \LtU\;\encode{\godcod t}
  \Beq \encode{\godcod{\NF(t)}}\]
  et tel que $\LtU\;\encode{\godcod{t}}$ n'a pas de forme normale si $t$ n'en a
  pas.
\end{proposition}

\begin{remark}
  Si on parle de \og la\fg{} forme normale si elle existe, c'est parce qu'on
  prouvera que $\bred$ est confluente.
\end{remark}

\begin{proposition}
  La partie
  \[\compr{\godcod t}{t \in \lamSet, t\text{ est normalisant}}\]
  est r.e.-complète pour la réduction many-one.
\end{proposition}

On en déduit qu'il n'existe pas de terme $\HALT$ tel que $\HALT\;t$ se réduit
en $\encode{\top}$ si $t$ a une forme normale et en $\encode{\bot}$ sinon.

On peut aussi en déduire, par le théorème de Rice~:

\begin{proposition}
  Il n'existe aucun terme $e$ tel que $e\;t\;u$ se réduit en $\encode{\top}$ si
  $t \BEeq u$ et en $\encode{\bot}$ sinon.
\end{proposition}

\begin{proof}
  Si un tel terme existait, on pourrait calculer l'ensemble
  \[\compr{(\godcod t,\godcod u)}{t, u \in \lamSet, t \BEeq u}\]
  qui est un problème sémantique.
\end{proof}

Dans le cas de la $\beta$-équivalence, on n'a pas tout à fait ce résultat,
puisque l'ensemble n'est pas sémantique. On définit donc des notions analogues à
celles de calculabilité pour le cas de la $\beta$-réduction. Tout d'abord,
on donne une version du théorème de point fixe utilisant le codage de Gödel des
$\lambda$-termes.

\begin{theorem}[Point fixe]
  Soit $t\in\lamSet$, alors il existe $\LtFt$ tel que
  \[t\;\encode{\godcod \LtFt} \Beq \LtFt\]
\end{theorem}

\begin{proof}
  On se donne un terme $\Ltd$ tel que
  \[\forall u\in \lamSet, \Ltd\;\encode{\godcod{u}} \rtClot{\bred}
  \encode{\godcod{u\;\encode{\godcod{u}}}}\]
  En posant $v \defeq \lambda x.t\;(\Ltd\;x)$ (où $x$ n'apparaît ni dans $t$,
  ni dans $\Ltd$), on obtient
  \[\LtFt \defeq v\;\encode{\godcod v}\]
  En effet, en faisant la réduction~:
  \begin{align*}
    \LtFt &\bred t\;(\Ltd\;\encode{\godcod{v}})\\
    &\rtClot{\bred} t\;\encode{\godcod{v\;\encode{\godcod{v}}}}\\
    &= t\;\encode{\godcod{\LtFt}}\qedhere
  \end{align*}
\end{proof}

\begin{definition}[Ensembles récursivement séparables]
  On dit que $\varPt,\varPtB\subseteq \lamSet$ sont récursivement séparables
  s'il existe un terme $t$ tel que
  \[\forall u \in \lamSet, t\;\encode{\godcod u} \rtClot{\bred}
  \begin{cases}
    \encode \bot\text{ si } u \in X\\
    \encode \top\text{ si } u \in Y
  \end{cases}\]
  $\varPt$ et $\varPtB$ sont dits récursivement inséparables sinon.
  \index[notion]{Ensemble!Récursivement séparables}
\end{definition}

\noindent
Un ensemble sémantique, mais aux sens de $\bred$, sera dit saturé ici.

\begin{definition}[Ensemble saturé]
  Une partie $\varPt\subseteq \lamSet$ est dite saturée si
  \[\forall t,u \in \lamSet,
  t \Beq u \implies t \in \varPt \implies u \in \varPt\]
  \index[notion]{Ensemble!Saturé}
\end{definition}

\begin{theorem}
  Deux ensembles disjoints saturés sont récursivement inséparables.
\end{theorem}

\begin{proof}
  Supposons que deux ensembles $\varPt,\varPtB$ vérifiant les hypothèses sont
  récursivement séparables, et soit $t$ un séparateur tel que donné dans la
  définition. Soient $t_{\varPt} \in \varPt$ et $t_{\varPtB} \in \varPtB$, et
  \[u \defeq \lambda n.\eqZ\;(t\;n)\;t_{\varPt}\;t_{\varPtB}\]
  On utilise maintenant le théorème de point fixe pour trouver $\LtF{u}$ tel que
  $u\;\encode{\godcod{\LtF{u}}}\Beq \LtF{u}$. On en déduit~:
  \begin{itemize}
  \item si $t\;\encode{\godcod{\LtF{u}}}\rtClot{\bred} \encode{\bot}$, alors
    $\LtF{u}\rtClot{\bred} t_{\varPtB}$
    donc $\LtF{u} \Beq t_{\varPtB} \in \varPtB$, mais alors
    $t\;\encode{\godcod{\LtF{u}}}\rtClot{\bred} \encode{\top}$
  \item de même, si $t\;\encode{\godcod{\LtF{u}}}\rtClot{\bred} \encode{\top}$,
    alors $\LtF{u} \Beq t_{\varPt} \in \varPt$, donc
    $t\;\encode{\godcod{\LtF{u}}}\rtClot{\bred} \encode{\bot}$.\qedhere
  \end{itemize}
\end{proof}

\begin{remark}
  La fonction construite est parfaitement analogue à la réduction construite
  pour le théorème de Rice, suivie de la preuve de la non calculabilité du
  problème de l'arrêt avec un point fixe.
\end{remark}

On peut alors prouver les résultats pour la réduction $\bred$.

\begin{theorem}
  La relation $\Beq$ est indécidable, et l'ensemble
  \[\compr{\godcod t}{t \in \lamSet, t \textit{ faiblement normalisant}}\]
  est indécidable.
\end{theorem}

On en déduit aussi le théorème de l'image.

\begin{theorem}[Image]
  Soit $\overline t \in \quot{\lamSet}{\Beq}$~; $t$ définit une fonction
  \[\makeFun{f_{\overline t}}{\quot{\lamSet}{\Beq}}{\quot{\lamSet}{\Beq}}
            {\clas{u}}{\clas{t\;u}}\]
  qui est une fonction constante ou une fonction dont l'image est infinie.
  \index[notion]{Théorème!de l'Image}
\end{theorem}

\begin{proof}
  En effet, supposons qu'il existe deux valeurs $t_1$ et $t_2$ telles que
  $\im(f_{\clas{t}}) = \{\clas{t_1},\clas{t_2}\}$ (dans le cas de plus de valeurs,
  l'argument s'adapte facilement). Les ensembles
  \[\varPt_{t_1} \defeq \compr{u \in \lamSet}{t\;u \Beq t_1}\qquad
  \varPt_{t_2}\defeq \compr{u \in \lamSet}{t\;u \Beq t_2}\]
  sont alors des ensembles disjoints et saturés~: ils sont donc inséparables.
  Pourtant, puisqu'on sait à la fois que
  $\varPt_{t_1}\cup \varPt_{t_2} = \lamSet$, que les deux ensembles sont disjoints
  et qu'ils sont c.e., on en déduit qu'ils sont tous deux décidables, ce qui est
  une contradiction.
\end{proof}

\section{Propriétés de la réduction}

On s'attarde maintenant sur $\bred$. L'objectif de cette section est de montrer
certaines propriétés de cette réduction, en particulier~:
\begin{itemize}
\item que $\bred$ a la propriété de Church-Rosser, ce qui permet de définir de
  façon univoque la forme normale d'un terme si elle existe, et d'avoir un
  algorithme qui, sous réserve de convergence, teste la $\beta$-équivalence de
  deux termes (avec ce qu'on a vu avant, on sait déjà que la non convergence
  rend l'égalité indécidable)~;
\item que toute réduction pour $\bred$ peut se factoriser suivant deux
  sous-réductions qui sont la réduction de tête et la réduction en profondeur~;
\item qu'il existe un choix de réduction permettant systématiquement d'atteindre
  une forme normale, si elle existe.
\end{itemize}

\subsection{Propriété de Church-Rosser}

Pour prouver la propriété de Church-Rosser, on utilise une méthode développée
par William Walker \name{Tait} et Per \name{Martin}-\name{Löf} (de ce qui est
dit dans \cite{Barendregt1984}), et affinée dans \cite{Takahashi1995},
impliquant la réduction parallèle.

Pour prouver la confluence du $\lambda$-calcul, la difficulté principale est,
étant donnés deux redexes dans une expression, de montrer que les deux ordres
de réduction mènent au même résultat. Cependant, une première réduction peut
multiplier des redexes, dont la redex de la deuxième réduction. La réduction
parallèle permet de réduire en une seule étape ces deux redexes, de sorte
que la propriété du carreau est vérifiée (et donc, par extension, la
confluence).

On définit donc la réduction parallèle.

\begin{definition}[Réduction parallèle]
  La réduction parallèle $\redPar$ est définie par les règles suivantes~:
  \begin{center}
    \begin{tabular}{cm{2cm}c}
      \bottomAlignProof
      \AxiomC{}
      \UnaryInfC{$x \redPar x$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$t\redPar t'$}
      \AxiomC{$u\redPar u'$}
      \BinaryInfC{$t\;u\redPar t'\;u'$}
      \DisplayProof
      \\
      \\
      \bottomAlignProof
      \AxiomC{$t\redPar t'$}
      \UnaryInfC{$\lambda x.t\redPar \lambda x.t'$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$t\redPar t'$}
      \AxiomC{$u\redPar u'$}
      \BinaryInfC{$(\lambda x.t)u \redPar t'[u'/x]$}
      \DisplayProof
    \end{tabular}
  \end{center}
  \index[notion]{Réduction!Parallèle}
  \index[notation]{$\redPar$}
\end{definition}

L'objectif est donc d'utiliser le \cref{lem.carreau.confluence.clot} en montrant
que $\mathord{\bred} \subseteq \mathord{\redPar} \subseteq
\mathord{\rtClot{\bred}}$ et que $\redPar$ a la propriété du carreau.

\begin{property}\label{proper.redPar}
  Les propriétés suivantes sont vérifiées pour $\redPar$~:
  \begin{itemize}
  \item $\redPar$ est réflexive~;
  \item $\mathord{\bred}\subseteq \mathord{\redPar}$~;
  \item si $t\redPar t'$ et $u \redPar u'$, alors $t[u/x] \redPar t'[u'/x]$~;
  \item $\mathord{\redPar} \subseteq\mathord{\rtClot{\bred}}$.
  \end{itemize}
\end{property}

\begin{proof}
  Montrons les différentes propriétés~:
  \begin{itemize}
  \item on montre par induction sur $t$ que $t\redPar t$. Si $t = x$, le
    résultat est évident, si on suppose que $t\redPar t$ et $u\redPar u$
    alors $t\;u\redPar t\;u$ et $\lambda x.t\redPar \lambda x.t$.
  \item puisque $\redPar$ est réflexive, et vu la forme des règles, on en
    déduit que $\redPar$ est une relation compatible. La dernière règle,
    en utilisant la réflexivité, nous indique que
    $(\lambda x.t)u\redPar t[u/x]$, donc
    $\mathord{\bred}\subseteq \mathord{\redPar}$.
  \item on raisonne par induction sur $t\redPar t'$~:
    \begin{itemize}
    \item si $t = t' = x$, on doit montrer que $u \redPar u'$, ce qui est vrai
      par hypothèse.
    \item si $t = t' = y$ où $y \neq x$, on doit montrer que $t \redPar t$,
      ce qui est vrai par réflexivité de $\redPar$.
    \item si $t = t_0\;t_1$, $t' = t'_0\;t'_1$, que $t_0 \redPar t'_0$ et
      $t_1\redPar t'_1$, alors on sait par hypothèse d'induction que
      $t_0[u/x] \redPar t'_0[u'/x]$ et $t_1[u/x] \redPar t'_1[u'/x]$, ce qui
      nous donne $t[u/x] \redPar t'[u'/x]$.
    \item si $t = \lambda y.t_0$ et $t' = \lambda y.t'_0$, où
      $t_0[u/x]\redPar t'_0[u'/x]$, alors on a bien la réduction
      $(\lambda y.t_0)[u/x] \redPar (\lambda y.t'_0)[u'/x]$.
    \item si $t = (\lambda y.t_0)\;t_1$ et $t' = t'_0[t'_1/y]$, avec comme
      hypothèse d'induction que $t_0[u/x]\redPar t'_0[u'/x]$ et
      $t_1[u/x]\redPar t'_1[u'/x]$, alors on sait qu'on a la réduction
      $(\lambda y.t_0[u/x]) \redPar t'_0[u'/x][t'_1[u'/x]/y]$, mais on a
      l'égalité
      \[t'_0[t'_1/y][u'/x] = t'_0[u'/x][t'_1[u'/x]/y]\]
      d'après l'\cref{exo.subst.subst}, d'où le résultat.
    \end{itemize}
    On en déduit que $t[u/x] \redPar t'[u'/x]$.
  \item on montre par induction sur $\redPar$ que si $t\redPar u$ alors
    $t\rtClot{\bred} u$~:
    \begin{itemize}
    \item puisque $\rtClot{\bred}$ est réflexive, on sait que
      $x\rtClot{\bred} x$.
    \item si $t\rtClot{\bred} t'$ et $u\rtClot{\bred} u'$, alors
      $t\;u\rtClot{\bred} t'\;u\rtClot{\bred} t'\;u'$ par compatibilité.
    \item si $t\rtClot{\bred} t'$, alors
      $\lambda x.t\rtClot{\bred} \lambda x.t'$ par compatibilité.
    \item si $t\rtClot{\bred} t'$ et $u\rtClot{\bred} u'$, alors
      $(\lambda y.t)\;u \bred t[u/x] \rtClot{\bred}
      t'[u/x]\rtClot{\bred} t'[u'/x]$
      d'après l'\cref{exo.subst.red}.
    \end{itemize}
    Donc, par induction sur $\redPar$, on en déduit que
    $\redPar\subseteq\bred^\star$.\qedhere
  \end{itemize}
\end{proof}

Il nous reste maintenant à prouver que la réduction parallèle a la propriété du
carreau. Pour cela, depuis un terme $t$ ayant des redexes $r_1,\ldots,r_p$,
on pourrait considérer que, lorsque $t \redPar u$, on choisit des redexes
$\{r_i\}_{i \in \indSet}$, et lorsque $t \redPar v$, on choisit des redexes
$\{r_i\}_{i \in \indSetB}$. Un réduit commun à $u$ et $v$ est alors le terme obtenu
par la réduction contractant les redexes $\{r_i\}_{i \in \indSet \cup \indSetB}$.

Cette méthode fonctionne, mais n'est pas nécessaire pour le résultat, que l'on
souhaite montrer~: il est possible de prendre comme réduit commun, peu importe
$u$ et $v$, le terme obtenu depuis $t$ en effectuant tous les redexes dans $t$.
On définit pour cela le développement en un pas.

\begin{definition}[Développement en un pas]
  On définit par induction sur $t\in \lamSet$ le terme $t^* \in \lamSet$~:
  \begin{itemize}
  \item si $t = x$, alors $t^* \defeq t$.
  \item si $t = \lambda x.u$, alors $t^* \defeq \lambda x. u^*$.
  \item si $t = u\;v$ où $u$ n'est pas une abstraction, alors
    $t^* \defeq u^*\;v^*$.
  \item si $t = (\lambda x.u)\;v$, alors $t^* \defeq u^*[v^*/x]$.
  \end{itemize}
  \index[notion]{Développement!en un Pas}
  \index[notation]{$t^*$}
\end{definition}

\begin{proposition}
  Soit $t \in \lamSet$. Pour tout $u \in \lamSet$ tel que $t\redPar u$, on a
  $u\redPar t^*$.
\end{proposition}

\begin{proof}
  On procède par induction sur $t\redPar u$~:
  \begin{itemize}
  \item le cas où $t = u = x$ est automatique.
  \item si $t = t_0\;t_1$, $u = u_0\;u_1$, $t_0 \redPar u_0$ et
    $t_1\redPar u_1$, on sait par hypothèse d'induction que
    $u_0\redPar t_0^*$ et que $u_1\redPar t_1^*$, donc
    $u_0\;u_1\redPar t_0^*\;t_1^*$. Si $t_0$ est une abstraction, alors
    on utilise à la place la dernière règle. Dans les deux cas, on
    en déduit que $u\redPar t^*$.
  \item si $t = \lambda x.t_0$, $u = \lambda x.u_0$ et $t_0\redPar u_0$, alors
    par hypothèse d'induction $u_0\redPar t_0^*$ donc
    $\lambda x.u_0\redPar \lambda x.t^*_0$, d'où le résultat.
  \item si $t = (\lambda x.t_0)\;t_1$, $u = u_0[u_1/x]$ et, par hypothèse
    d'induction, $u_0\redPar t_0^*$ et $u_1\redPar t_1^*$, alors
    par la propriété précédente, $u_0[u_1/x]\redPar t_0^*[t_1^*/x]$, donc
    $u\redPar t^*$.
  \end{itemize}
  D'où, par induction, $u\redPar t^*$.
\end{proof}

\begin{corollary}
  La relation $\bred$ est confluente. De façon équivalente, elle vérifie la
  propriété de Church-Rosser. Il y a donc unicité de la forme normale vers
  laquelle se réduit un terme $t$.
\end{corollary}

\begin{proof}
  On a vérifié les hypothèses du \cref{lem.carreau.confluence.clot} pour
  $\redPar$ et $\bred$, la propriété du carreau pour $\redPar$ étant obtenue
  en considérant, pour $t \redPar u,v$, par $u,v\redPar t^*$. Comme la
  propriété de Church-Rosser et la confluence sont équivalentes, on en déduit
  que $\bred$ vérifie aussi la propriété de Church-Rosser et, en conséquent,
  on a l'unicité de la forme normale.
\end{proof}

\begin{remark}
  On déduit aussi de la propriété de Church-Rosser que le $\lambda$-calcul est
  cohérent, c'est-à-dire qu'il existe deux termes $t,u$ tels que
  $t \not\Beq u$. En effet, par unicité de la forme normale, deux formes
  normales ne peuvent se réduire l'une en l'autre.
\end{remark}

\subsection{Lemme de standardisation}

Puisque nous avons prouvé que $\bred$ a la propriété de Church-Rosser, la donnée
importante d'un terme devient sa forme normale (si elle existe). On ne peut
cependant pas essayer de réduire un terme en en prenant des réductions
arbitraires~: si l'on considère par exemple $(\lambda x\,y.y)\;\LtOm$,
une réduction retourne $\id$ mais l'autre réduction ne fait rien.

Il est donc nécessaire de trouver une stratégie efficace pour normaliser les
termes.

\begin{definition}[Stratégie de normalisation]
  On appelle stratégie de normalisation une sous-relation $\varRR$ de $\bred$
  qui est fonctionnelle (partielle).

  On dit qu'une telle stratégie est normalisante si, pour tout $t$ tel que
  $t\toMax{\bred} u$, on a $t\toMax{\varRR} u$.
  \index[notion]{Stratégie!de Normalisation}
\end{definition}

On cherche donc une stratégie normalisante. Pour obtenir une telle stratégie
normalisante, on commence par étudier la forme des $\lambda$-termes.

\`A cause de la forme de $\bred$, et des cas inductifs d'un terme, on sait qu'un
terme $t$ peut toujours se mettre sous la forme
\[t = \lambda x_1\cdots x_n.u\;t_1\;t_2\;\cdots\;t_k\]
où $u,t_1,\ldots,t_k$ sont des termes, et $x_1,\ldots,x_n$ sont des variables.
Deux choix sont alors possibles~:
\begin{itemize}
\item $u$ est une variable
\item $u$ est une abstraction
\end{itemize}
dans le premier cas, on dit que $t$ est une forme normale de tête
(\foreignexpr{head normal form}). Dans le second, on dit que $u\;t_1$ est une
redex de tête. La redex de tête est, en quelque sorte, la redex essentielle du
terme $t$.

\begin{definition}[Forme normale de tête, réduction de tête]
  Le sous-ensemble $\hnf\subseteq \lamSet$ est défini inductivement par la
  grammaire
  \begin{align*}
    t_0 &\inddefeq \lambda x.t_0 | t \\
    t &\inddefeq t\;u | x \\
  \end{align*}
  où $x \in \lamVar$ et $u \in \lamSet$. On définit de plus la réduction
  $\Hred$ par les règles
  \begin{center}
    \begin{tabular}{cm{2cm}c}
      \bottomAlignProof
      \AxiomC{}
      \UnaryInfC{$(\lambda x.t)u \Hred t[u/x]$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$t\Hred t'$}
      \UnaryInfC{$\lambda x.t\Hred \lambda x.t'$}
      \DisplayProof
      \\
      \\
      \multicolumn{3}{c}{
        \bottomAlignProof
        \AxiomC{$t \Hred t'$}
        \AxiomC{$t\text{ n'est pas une abstraction}$}
        \BinaryInfC{$t\;u\Hred t'\;u$}
        \DisplayProof
      }
    \end{tabular}
  \end{center}
  \index[notion]{Forme normale!de Tête}
  \index[notion]{Réduction!de Tête}
  \index[notation]{$\Hred$}
\end{definition}

\begin{exercise}
  Montrer que $t \in \hnf$ si et seulement si $t$ est une forme normale pour
  $\Hred$.
\end{exercise}

\begin{exercise}
  Montrer que $\Hred$ est une relation fonctionnelle.
\end{exercise}

On peut donc séparer les réductions depuis un terme en une réduction de tête
et d'autres potentielles réductions. On appelle ces réductions des réductions
internes.

\begin{definition}[Réduction interne]
  On définit la relation de réduction interne $\Ired$ par les règles
  \begin{center}
    \begin{tabular}{cm{2cm}c}
      \multicolumn{3}{c}{
        \bottomAlignProof
        \AxiomC{$u\bred u'$}
        \UnaryInfC{$t\;u'\Ired t\;u'$}
        \DisplayProof}
      \\
      \\
      \bottomAlignProof
      \AxiomC{$t \Ired t'$}
      \UnaryInfC{$\lambda x.t \Ired \lambda x.t'$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$t \Ired t'$}
      \UnaryInfC{$t\;u\Ired t'\;u$}
      \DisplayProof
    \end{tabular}
  \end{center}
  \index[notion]{Réduction!Interne}
  \index[notation]{$\Ired$}
\end{definition}

\begin{proposition}
  Si $t \bred u$, alors $t \Hred u$ ou $t \Ired u$.
\end{proposition}

\begin{proof}
  On raisonne par induction sur $t \bred u$~:
  \begin{itemize}
  \item si $t = (\lambda x.t_0)\;t_1$ et $u = t_0[t_1/x]$, alors
    $t \Hred u$.
  \item si $t = \lambda x.t'$, $u = \lambda x.u'$ et $t' \bred u'$, alors par
    hypothèse d'induction $t' \Hred u'$ ou $t' \Ired u'$, et dans ce cas on a
    respectivement $t \Hred u$ ou $t \Ired u$.
  \item si $t = t_0\;t_1$, $u = u_0\;t_1$ et $t_0 \bred u_0$, alors par
    hypothèse d'induction~:
    \begin{itemize}
    \item si $t_0 \Hred u_0$, alors $t \Hred u$.
    \item si $t_0 \Ired u_0$, alors $t \Ired u$.
    \end{itemize}
    donc $t\Hred u$ ou $t \Ired u$.
  \item si $t = t_0\;t_1$, $u = t_0\;u_1$ et $t_1\bred u_1$, alors
    $t \Ired u$.\qedhere
  \end{itemize}
\end{proof}

Le théorème de standardisation faible établit que ces deux réductions permettent
de décomposer les suites de réduction avec une forme remarquable~: toute
réduction $t \rtClot{\bred} u$ se décompose en
$t \rtClot{\Hred} v \rtClot{\Ired} u$.

Pour démontrer ce théorème, on cherche donc à construire la commutation
suivante~: si $t \Ired u \Hred v$, alors on peut trouver $u'$ tel que
$t \rtClot{\Hred} u' \rtClot{\Ired} v$. La difficulté est alors de trouver les
bonnes réductions internes à effectuer pour passer de $u'$ à $v$, sachant que la
réduction $t \Hred u'$ a pu multiplier la redex réduite dans $t \Ired u$.

Comme pour la propriété de Church-Rosser, on introduit une réduction parallèle,
ici une réduction parallèle interne.

\begin{definition}[Réduction parallèle interne]
  On définit la réduction parallèle interne $\IredPar$ par les règles
  \begin{center}
    \begin{tabular}{cm{2cm}c}
      \bottomAlignProof
      \AxiomC{}
      \UnaryInfC{$x \IredPar x$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$t \IredPar t'$}
      \UnaryInfC{$\lambda x.t\IredPar \lambda x.t'$}
      \DisplayProof
      \\
      \\
      \bottomAlignProof
      \AxiomC{$t \Ired t'$}
      \AxiomC{$u \redPar u'$}
      \BinaryInfC{$t\;u \IredPar t'\;u'$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$t \redPar t'$}
      \AxiomC{$u \redPar u'$}
      \BinaryInfC{$(\lambda x.t)\;u\IredPar (\lambda x.t')\;u'$}
      \DisplayProof
    \end{tabular}
  \end{center}
  \index[notion]{Réduction!Interne!Parallèle}
  \index[notation]{$\IredPar$}
\end{definition}

\begin{property}
  Les propriétés suivantes sont vérifiées pour $\IredPar$~:
  \begin{itemize}
  \item $\IredPar$ est réflexive~;
  \item $\IredPar\subseteq \redPar$~;
  \item $\IredPar\subseteq \Ired^\star$~;
  \item pour tous $t,u,v \in \lamSet$,
    $t \IredPar u \Hred v \implies t \redPar v$.
  \end{itemize}
\end{property}

\begin{proof}
  Les trois premières propriétés se prouvent de façon parfaitement analogue à la
  \cref{proper.redPar}, on se contente donc de prouver la dernière propriété.

  Soient $t,u,v \in \lamSet$ tels que $t \IredPar u \Hred v$. Remarquons d'abord
  que, si on peut décomposer $t$ en
  $t = \lambda x_1\;\cdots\;x_n.t'\;u_1\;\cdots\;u_k$, alors
  $u$ se décompose en
  $u = \lambda x_1\;\cdots\;x_n.u'\;u'_1\;\cdots\;u'_k$, car les réductions
  parallèles internes ne peuvent pas réduire la redex potentielle $t'\;u_1$ et
  peuvent donc uniquement réduire les redexes à l'intérieur des $u_i$ et de
  $t'$.

  De plus, si $t'$ est une variable, alors $u'$ est aussi une variable, donc
  $u\in\hnf$ et $u \Hred v$ n'est pas possible. On en déduit que
  \begin{itemize}
  \item $t'$ et $u'$ sont des abstractions, donc $t' = \lambda y.t''$ et
    $u' = \lambda y.u''$, avec $t'' \Ired u''$, et
  \item $v = \lambda x_1\;\cdots\;x_n.u''[u'_1/y]\;u_2\;\cdots u'_k$.
  \end{itemize}

  On en déduit, en utilisant le fait que $u_i \Ired u'_i$ et que
  $t''\Ired u''$, que $t \redPar v$.
\end{proof}

Ainsi, on peut décomposer $t \rtClot{\bred} u$ en $t\rtClot{\redPar} u$. Il
reste donc à prouver que $\redPar$ peut se décomposer en
$\rtClot{\Hred}\IredPar$.

\begin{lemma}
  Pour tous $t,u \in \lamSet$, si $t \redPar u$ alors il existe
  $t'$ tel que $t \rtClot{\Hred} t' \IredPar u$.
\end{lemma}

\begin{proof}
  On prouve par induction sur $\redPar$ qu'il existe une suite de réductions
  $t \Hred t_1 \Hred\cdots\Hred t'\IredPar u$ où pour chaque
  $i$, $t_i \redPar t'$~:
  \begin{itemize}
  \item si $t = u = x \in \lamVar$, alors la propriété est évidente en prenant
    $t' \defeq x$.
  \item cas de
    $t_0 \redPar u_0, t_1 \redPar u_1 \implies t_0\;t_1\redPar u_0\;u'_1$~:
    prouvons qu'il existe $t'$ tel que $t \Hred^\star t' \IredPar u$, et tel
    que la suite de réduction de $t$ à $t'$ est faite de termes qui se réduisent
    pour $\redPar$ vers $t'$.
    
    Par hypothèse d'induction sur $t_0 \redPar u_0$, on trouve $t'_0$ et une
    suite de termes $t_0 = v_1\Hred v_2\Hred \cdots \Hred v_k = t'_0$, où
    $v_i \redPar t'_0$. On raisonne par disjonction de cas~:
    \begin{itemize}
    \item supposons qu'il existe un $k_0$ (pris minimal) tel que $v_{k_0}$ est
      une abstraction. On sait alors que pour tout $k \geq k_0$, $v_k$ est
      aussi une abstraction, et (puisque les réductions internes ne changent pas
      la forme du terme) $u_0$ est aussi une abstraction.
      Posons $v_{k_0} = \lambda x.v'$ et $u_0 = \lambda x.u'$. On cherche
      à prouver que $t \Hred^\star v_{k_0}\;t_1 \IredPar u$.

      On sait que $t_1 \redPar u_1$. De plus, par hypothèse,
      $v_{k_0} \redPar u'$, donc
      $(\lambda x.v_{k_0})\IredPar (\lambda x.u')\;u_1$.
    \item s'il n'existe pas de tel $k_0$, alors on en déduit que
      $t_0\;t_1 \Hred v_1\;t_1\Hred\cdots \Hred v_k\;t_1$, et
      par hypothèse $t_1 \redPar u_1$ et $v_k = t'_0 \IredPar u_0$, donc
      $v_k\;t_1\IredPar u$.
    \end{itemize}
    D'où le résultat.
  \item cas de $t_0 \redPar u_0 \implies \lambda x.t_0\redPar \lambda x.u_0$~:
    par hypothèse d'induction, on trouve une suite de réductions, qui
    fonctionne pour donner le résultat en ajoutant une abstraction à chaque
    terme.
  \item cas de $t_0\redPar u_0, t_1\redPar u_1 \implies
    (\lambda x.t_0)t_1\redPar u_0[u_1/x]$~:
    par hypothèse, posons une suite de réductions
    $t_0 = v_1 \Hred v_2 \Hred\cdots \Hred v_k = t'_0$.
    On remarque que
    \[(\lambda x.t_0)t_1 \Hred t_0[t_1/x] \Hred v_1[t_1/x] \Hred\cdots
    \Hred v_k[t_1/x]\]
    On s'intéresse maintenant à $v_k$. En effet, on peut le mettre sous la
    forme $\lambda \vec x.v'\;\vec{w}$ où $v'$ est le redex de tête ou une
    variable de tête. Si $v'$ est un redex, toutes les réductions
    $t_1 \redPar u_1$ à l'intérieur de $\vec{w}$ peuvent s'effectuer en une
    réduction parallèle interne, donc il suffit de construire une suite dans
    le cas de $v'[t_1/x]$. On traite donc les cas d'une redex, d'une variable
    de tête $y \neq x$ et de la variable de tête $x$~:
    \begin{itemize}
    \item si $v' = x$, alors $v'[t_1/x] = t_1$. Par induction, on a
      une suite de $t_1$ à $u_1$. Si cette suite ne fait pas apparaître
      d'abstraction, alors on peut la concaténer à la suite déjà
      construite et obtenir le résultat. Sinon, on concatène toute la
      suite jusqu'à l'apparition d'une abstraction, puis on effectue une
      seule réduction interne parallèle.
    \item si $v' = y \neq x$, alors on effectue dans $\vec{w}$ des
      réductions de tête jusqu'à n'avoir que des réductions internes,
      qu'on peut effectuer en un pas.
    \item si $v'$ est une redex, alors $v'[t_1/x] \IredPar v'[u_1/x]$.
      \qedhere
    \end{itemize}
  \end{itemize}
\end{proof}

\noindent
On peut alors en déduire le théorème de standardisation faible.

\begin{theorem}[Standardisation faible]
  Pour tous $t,v \in \lamSet$, si $t \rtClot{\bred} v$ alors il existe $u$ tel
  que $t\rtClot{\Hred} u \rtClot{\Ired} v$.
  \index[notion]{Théorème!de Standardisation!Faible}
\end{theorem}

\begin{proof}
  Puisque toute réduction est de tête ou interne, il suffit de montrer que
  lorsque $t \Ired u \Hred v$, alors on peut trouver $u'$ tel que
  $t \rtClot{\Hred} u' \rtClot{\Ired} v$. Par propriété de $\IredPar$, on sait
  que si $t \Ired u \Hred v$, alors $t \IredPar u\Hred v$ puis
  $t \redPar v$. Mais alors, par le lemme précédent, on peut trouver
  $u'$ tel que $t \rtClot{\Hred} u' \IredPar v$, et par propriété de
  $\IredPar$, $u' \rtClot{\Ired} v$.

  On en déduit que
  $\mathord{\Ired} \circ\mathord{\Hred} \subseteq
  \mathord{\rtClot{\Hred}} \circ\mathord{\rtClot{\Ired}}$, d'où le
  résultat par induction sur la longueur d'une réduction $t \rtClot{\bred} v$.
\end{proof}

La standardisation faible est déjà un théorème puissant, mais (comme son nom
l'indique) on peut en donner une meilleure version.

En l'état, la décomposition qu'on obtient de $\bred$ se fait en deux parties~:
les réductions de tête, et les réductions internes. On peut chercher une
localisation plus précise des redexes, de sorte qu'une réduction de tête donne
un cas particulier de cette localisation.

Pour cela, on définit la notion de réduction marquée.

\begin{definition}[Réduction marquée]
  On définit la relation de réduction marquée $\bred^\varwd$, où
  $\varwd \in \toSt{\{0,1,2\}}$, par les règles suivantes~:
  \begin{center}
    \begin{tabular}{cm{2cm}c}
      \bottomAlignProof
      \AxiomC{}
      \UnaryInfC{$(\lambda x.t)v\bred^{\Ewd}t[v/x]$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$t \bred^{\varwd} t'$}
      \UnaryInfC{$\lambda x.t \bred^{0\varwd} \lambda x.t'$}
      \DisplayProof
      \\
      \\
      \bottomAlignProof
      \AxiomC{$t\bred^{\varwd} t'$}
      \UnaryInfC{$t\;v \bred^{1\varwd} t'\;v$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$v\bred^{\varwd} v'$}
      \UnaryInfC{$t\;v\bred^{2\varwd} t\;v'$}
      \DisplayProof
    \end{tabular}
  \end{center}

  On dit qu'une réduction $t\bred^{\varwd} a$ est à gauche d'une réduction
  $s\bred^{\varwv} b$ si $\varwd$ est plus petit que $\varwv$ pour l'ordre
  lexicographique.
  \index[notion]{Réduction!Marquée}
\end{definition}

\begin{example}
  Soit le terme
  \[t \defeq \lambda x\;y\;z.(\lambda x.x)\;y\;((\lambda a.a\;x)\;z)\]
  Tout redex est marqué par un certain mot, qui décrit comment on descend dans
  le terme pour arriver au redex souhaité. Par exemple, le redex
  $(\lambda x.x)\;y$ peut être réduit~: on doit alors passer sous les
  abstractions $\lambda x\;y\;z$, pour prendre le sous-terme gauche. On a ainsi
  \[t \bred^{0001} \lambda x\;y\;z.y\;((\lambda a.a\;x)\;z)\]
  De même, on peut vérifier que
  \[t \bred^{0002} \lambda x\;y\;z.(\lambda x.x)\;y\;(z\;x)\]
  On en déduit que l'une des deux réductions est à gauche de l'autre.
\end{example}

Remarquons qu'une réduction de tête est une réduction la plus à gauche possible.
Une réduction standard est une généralisation de ce fait~: plutôt que de
considérer des réduction de tête et internes, on considère des réductions
toujours plus à droite les unes des autres, ce qui donne un résultat plus
précis dans le cas des réductions internes.

\begin{definition}[Réduction standard]
  Soient deux termes $t,u \in \lamSet$. On appelle réduction standard de $t$
  vers $u$, que l'on note $t\Sred u$, la donnée d'une suite de réductions
  $t = v_0\bred^{\varwd_0} v_1 \bred^{\varwd_1}\cdots \bred^{\varwd_{k-1}}v_k$
  telle que $\varwd_i \leq \varwd_{i+1}$ pour l'ordre lexicographique.
  \index[notion]{Réduction!Standard}
\end{definition}

\begin{theorem}[Standardisation]
  Pour tous termes $t,u\in\lamSet$, si $t \rtClot{\bred} u$ alors $t\Sred u$.
  \index[notion]{Théorème!de Standardisation}
\end{theorem}

\begin{proof}
  D'après la standardisation faible, il est toujours possible de considérer
  que $t \rtClot{\Hred} t' \rtClot{\Ired} u$, et la dérivation est standard
  jusqu'à $t'$ puisque les réductions de tête sont les plus à gauche possible.
  On peut réécrire $t'$ sous la forme
  \[t' = \lambda x_1\ldots x_n. v_0\;v_1\;\cdots\;v_k\]
  si $n = k = 0$, alors $u = v_0 = t'$, et le résultat est direct.

  Sinon, on peut raisonner par induction (sur la relation bien fondée de
  sous-terme strict). Pour cela, on remarque d'abord que, puisque
  $t'\rtClot{\Ired} u$, $u$ peut aussi s'écrire sous la forme
  \[u = \lambda x_1\cdots x_n. v'_0\;v'_1\;\cdots\;v'_k\]
  (car les réductions internes ne changent pas la forme d'un terme) avec
  $v_i \rtClot{\bred} v'_i$ pour $i \in \{0,\ldots,k\}$.
  Mais alors, par induction, on trouve des dérivations standard
  $v_i \Sred v'_i$, donc
  \[\lambda x_1\ldots x_n.v_0\;v_1\;\cdots v_k \Sred
  \lambda x_1\ldots x_n.v'_0\;v'_1\;\cdots v'_k\]
  en appliquant les réductions standard de gauche à droite.

  On en déduit donc que toute réduction peut être transformée en une
  réduction standard équivalente.
\end{proof}

Ce résultat nous permet d'exhiber une stratégie normalisante, en réduisant
toujours le redex le plus à gauche. On donne une façon plus explicite de définir
la stratégie.

\begin{definition}[Réduction gauche]
  La réduction gauche est définie par
  \begin{center}
    \begin{tabular}{cm{1cm}c}
      \bottomAlignProof
      \AxiomC{}
      \UnaryInfC{$(\lambda x.t)u\Lred t[u/x]$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$t\Lred t'$}
      \UnaryInfC{$\lambda x.t\Lred \lambda x.t'$}
      \DisplayProof
      \\
      \\
      \bottomAlignProof
      \AxiomC{$t\Lred t'$}
      \AxiomC{$t\text{ n'est pas une abstraction}$}
      \BinaryInfC{$t\;u\Lred t'\;u$}
      \DisplayProof
      & &
      \bottomAlignProof
      \AxiomC{$u\Lred u'$}
      \AxiomC{$t \in \hnf$}
      \BinaryInfC{$t\;u\Lred t\;u'$}
      \DisplayProof
    \end{tabular}
  \end{center}
  \index[notion]{Réduction!Gauche}
  \index[notation]{$\Lred$}
\end{definition}

\begin{corollary}
  La relation $\Lred$ est une stratégie normalisante pour $\bred$.
\end{corollary}

\begin{remark}
  $\Lred$ ne décrit pas forcément toutes les réduction atteintes par $\bred$,
  car il est possible, étant donnés $3$ redexes $a,b,c$ listés de gauche à
  droite, de réduire $a$ puis $c$, sans réduire $b$, pour obtenir une
  réduction standard. La relation $\Lred$, elle, doit réduire $b$ avant de
  pouvoir réduire $c$.
\end{remark}

\clearpage

\section{Exercices et problèmes}

\subsection{Exercices}

\begin{exercise}[Appliquer des nombres]
  Calculer la forme normale de $\encode{n}\;\encode{m}$. En déduire une écriture
  de l'exponentiation d'entiers, et montrer qu'elle est correcte.
\end{exercise}

\begin{exercise}[Graphe de réduction]
  On appelle graphe de réduction d'un $\lambda$-terme $t$ la partie de
  $\lamSet$ contenant $t$, les termes vers lesquels se réduit $t$ et vue comme
  un graphe avec la relation binaire $\bred$. Pour chacun des graphes suivants,
  construire un terme $t$ dont le graphe de réduction de $t$ est le graphe
  attendu~:
  \begin{center}
    \begin{minipage}{0.35\textwidth}
      \begin{tikzpicture}
        \node (A) at (0,0) {$\bullet$};
        \node (B) at (2,0) {$\bullet$};
        \node (C) at (4,0) {$\bullet$};
        \path[->,>=latex] (A) edge[loop left] ();
        \draw[->,>=latex] (A) -- (B);
        \draw[->,>=latex] (B) -- (C);
      \end{tikzpicture}
    \end{minipage}
    \begin{minipage}{0.2\textwidth}
      \begin{tikzpicture}
        \node (A) at (0,0) {$\bullet$};
        \node (B) at (2,0) {$\bullet$};
        \path[->,>=latex] (A) edge[bend left] (B);
        \path[->,>=latex] (B) edge[bend left] (A);
      \end{tikzpicture}
    \end{minipage}
    \begin{minipage}{0.3\textwidth}
      \begin{tikzpicture}
        \node (A) at (0,0) {$\bullet$};
        \node (B) at (1.5,0.85) {$\bullet$};
        \node (C) at (1.5,-0.85) {$\bullet$};
        \path[->,>=latex] (A) edge[bend left] (B);
        \path[->,>=latex] (B) edge[bend left] (C);
        \path[->,>=latex] (C) edge[bend left] (A);
      \end{tikzpicture}
    \end{minipage}
  \end{center}
  Est-il possible de trouver un terme $t$ dont le graphe de réduction est
  $\bullet\leftarrow\bullet\rightarrow\bullet$~?
\end{exercise}

\begin{exercise}[\'Enigme interminable]
  Trouver un terme $t$ tel que $t$ est (faiblement) normalisant,
  $t\;(\lambda x.x)$ n'est pas normalisant, $t\;(\lambda x.x)\;(\lambda x.x)$
  est (faiblement) normalisant et
  $t\;(\lambda x.x)\;(\lambda x.x)\;(\lambda x.x)$ n'est pas normalisant.
\end{exercise}

\begin{exercise}[Le terme de Maurey]
  Donner explicitement le terme construit dans le cours pour encoder la
  soustraction. Donner un ordre de grandeur du nombre de réductions effectuées
  lors du calcul de $n - m$. Donner aussi le terme encodant l'opération de
  comparaison $n \leq m$, et donner un ordre de grandeur du nombre de réductions
  effectuées lors d'un tel calcul.

  On introduit maintenant le terme de Maurey, défini par~:
  \[M \defeq \lambda p\;q.\lambda x\;y\;z.
  (p\;(\lambda g\;f.f\;g)(\lambda d.x\;z))\;
  (q\;(\lambda g\;f.f\;g)(\lambda d.y\;z))\]
  Calculer la forme normale de $M\;\encode{2}\;\encode{3}$. Montrer que le
  terme de Maurey se réduit en ce qui est attendu, et que la longueur de la
  réduction de $M\;\encode{n}\;\encode{m}$ est de l'ordre de $\min(n,m)$.
\end{exercise}

\begin{exercise}[Les entiers de Scott]
  On définit un nouvel encodage d'entiers par
  \[Z \defeq \lambda z\;s.z\qquad S \defeq \lambda n.\lambda z\;s.s\;n\]
  Construire un terme $\rec'$ qui implémente la récursion pour les termes de
  la forme $S^n Z$. Montrer qu'avec cet encodage des entiers, on peut calculer
  la fonction prédécesseur en temps constant~; cela est-il possible avec les
  entiers de Church~?
\end{exercise}

\clearpage

\subsection{Problèmes}

\begin{problem}[Théories et théorème de Böhm]
  Dans ce problème, on s'intéresse aux théories équationnelles provenant du
  $\lambda$-calcul.
\end{problem}
