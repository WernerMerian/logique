...
des entiers, et on n'ajoute aucune variable liante aux abstractions. Le choix des
entiers n'est donc pas simplement une réécriture des variables en les énumérant
par $x_0,x_1,\ldots$ mais bien une façon alternative de lire des variables.
...
\begin{definition}[Représentation des paires]
  On définit les termes $\langle\tok,\tok\rangle$, $\projP{1}{}$ et $\proj{2}{}$ comme
  suit~:
  \begin{align*}
...
\begin{definition}[Ensembles récursivement séparables]
  On dit que $X,Y\subseteq \lamSet$ sont récursivement séparables s'il existe
  un terme $t$ tel que
  ...
  \end{cases}\]
  $X$ et $Y$ sont dit récursivement inséparables sinon.
\end{definition}
...
\begin{definition}[Ensemble saturé]
  Une partie $X\subseteq \lamSet$ est dite saturée si
  \[\forall t,u \in \lamSet, t \Beq u \implies t \in X \implies u \in X\]
\end{definition}
...
\begin{proof}
  Supposons que deux ensembles $X,Y$ vérifiant les hypothèses sont récursivement
  séparables, et soit $t$ un séparateur tel que donné dans la définition.
  Soient $t_X \in X$ et $t_Y \in Y$, et
  \[u \defeq \lambda n.\eqZ\;(t\;n)\;t_X\;t_Y\]
  ...
  \item si $t\;\encode{\godcod{\LtF{u}}}\rtClot{\bred} \encode{\bot}$, alors
    $\LtF{u}\rtClot{\bred} t_Y$
    donc $\LtF{u} \Beq t_Y \in Y$, mais alors
    $t\;\encode{\godcod{\LtF{u}}}\rtClot{\bred} \encode{\top}$
  \item de même, si $t\;\encode{\godcod{\LtF{u}}}\rtClot{\bred} \encode{\top}$, alors
    $\LtF{u} \Beq t_X \in X$, donc
...
\begin{proof}
  ...
  \[X_{t_1} \defeq \compr{u \in \lamSet}{t\;u \Beq t_1}\qquad
  X_{t_2}\defeq \compr{u \in \lamSet}{t\;u \Beq t_2}\]
  sont alors des ensembles disjoints et saturés~: ils sont donc inséparables.
  Pourtant, puisqu'on sait à la fois que $X_{t_1}\cup X_{t_2} = \lamSet$, que
  les deux ensembles sont disjoints et qu'ils sont c.e., on en déduit qu'ils
  sont tous deux décidables, ce qui est une contradiction.
\end{proof}
...
par Tait et Martin-Löf (de ce qui est dit dans \cite{Barendregt1984}), et 
affinée dans \cite{TAKAHASHI1995120}, impliquant la réduction parallèle.
...
Ainsi, on peut décomposer $t \rtClot{\bred} u$ en $t\rtClot{\redPar} u$. Il reste
donc à prouver que $\redPar$ peut se décomposer en $\rtClot{\Hred}\IredPar$.
...
\begin{theorem}[Standardisation faible]
  Pour tous $t,v \in \lamSet$, si $t \rtClot{\bred} v$ alors il existe $u$ tel que
  $t\rtClot{\Hred} u \rtClot{\Ired} v$.
\end{theorem}
