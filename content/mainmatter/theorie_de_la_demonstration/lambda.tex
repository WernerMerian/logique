...
arbitraires~: si l'on considère par exemple $(\lambda x.y.y)\;\Omega$,
...
\begin{definition}[Stratégie de normalisation]
  On appelle stratégie de normalisation une sous-relation $R$ de $\bred$ qui est
...
réduction $t \bred^\star u$ se décompose en $t \Hred^\star v \Ired^\star u$.
...
$t \Hred^\star u' \Ired^\star v$. La difficulté est alors de trouver les bonnes
réductions internes à effectuer pour passer de $u'$ à $v$, sachant que la
...
Ainsi, on peut décomposer $t \bred^\star u$ en $t\redPar^\star u$. Il reste donc
à prouver que $\redPar$ peut se décomposer en $\Hred^\star\IredPar$.

\begin{lemma}
  Pour tous $t,u \in \lamSet$, si $t \redPar u$ alors il existe
  $t'$ tel que $t \Hred^\star t' \IredPar u$.
\end{lemma}
...
\begin{theorem}[Standardisation faible]
  Pour tous $t,v \in \lamSet$, si $t \bred^\star v$ alors il existe $u$ tel que
  $t\Hred^\star u \Ired^\star v$.
\end{theorem}
...
\begin{proof}
  ...
  $t \Hred^\star u' \Ired^\star v$. Par propriété de $\IredPar$, on sait que
  si $t \Ired u \Hred v$, alors $t \IredPar u\Hred v$ puis
  $t \redPar v$. Mais alors, par le lemme précédent, on peut trouver
  $u'$ tel que $t \Hred^\star u' \IredPar v$, et par propriété de
  $\IredPar$, $u' \Ired^\star v$.

  On en déduit que $\Ired \Hred \subseteq \Hred^\star \Ired^\star$, d'où le
  résultat par induction sur la longueur d'une réduction $t \bred^\star v$.
\end{proof}
...
\begin{definition}[Réduction marquée]
  On définit la relation de réduction marquée $\bred^u$, où
  $u \in \{0,1,2\}^\star$, par les règles suivantes~:
  \begin{center}
    \begin{tabular}{cc}
      \bottomAlignProof
      \AxiomC{}
      \UnaryInfC{$(\lambda x.t)v\bred^{\varepsilon}t[v/x]$}
      \DisplayProof
      &
      \bottomAlignProof
      \AxiomC{$t \bred^u t'$}
      \UnaryInfC{$\lambda x.t \bred^{0u} \lambda x.t'$}
      \DisplayProof
      \\
      \\
      \bottomAlignProof
      \AxiomC{$t\bred^u t'$}
      \UnaryInfC{$t\;v \bred^{1u} t'\;v$}
      \DisplayProof
      &
      \bottomAlignProof
      \AxiomC{$v\bred^u v'$}
      \UnaryInfC{$t\;v\bred^{2u} t\;v'$}
      \DisplayProof
    \end{tabular}
  \end{center}

  On dit qu'une réduction $t\bred^u a$ est à gauche d'une réduction
  $s\bred^v b$ si $u$ est plus petit que $v$ pour l'ordre lexicographique.
\end{definition}
...
\begin{definition}[Réduction standard]
  ...
  vers $u$, que l'on note $t\Sred u$, une suite de réductions
  $t = v_0\bred^{u_0} v_1 \bred^{u_1}\cdots \bred^{u_{k-1}} v_k$
  telle que $u_i \leq u_{i+1}$ pour l'ordre lexicographique.
\end{definition}

\begin{theorem}[Standardisation]
  Pour tous termes $t,u\in\lamSet$, si $t \bred^\star u$ alors $t\Sred u$.
\end{theorem}

\begin{proof}
  D'après la standardisation faible, il est toujours possible de considérer
  que $t \Hred^\star t' \Ired^\star u$, et la dérivation est standard jusqu'à
  $t'$ puisque les réductions de tête sont les plus à gauche possible. On
  peut réécrire $t'$ sous la forme
